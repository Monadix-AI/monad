...
const session = await request<{ sessionId: string }>(server, '/v1/sessions', {
  ...
  body: JSON.stringify({
    title: `Workplace: managed external agent smoke (${agent})`,
    origin: {
      surface: 'web',
      client: 'external-agent-managed-smoke',
      transport: 'http',
      writableBy: ['http'],
      branchableBy: ['http']
    },
  ...
  `${JSON.stringify(
    {
      ...
      agent,
      cwd,
      note: 'Open the Workplace Project and the external agent diagnostics if provider login or approval is required.'
    },
    null,
    2
  )}\n`
