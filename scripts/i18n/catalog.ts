...
export async function writeCatalog() {
  ...
  if (!(await isCurrent(GENERATED_PATH, generated))) await Bun.write(GENERATED_PATH, generated);
  ...
  process.stdout.write('i18n catalog types generated\n');
}
...
export async function checkCatalog() {
  ...
  if (!en) throw new Error('canonical English locale is missing');
  if (!locales.find((locale) => locale.locale === 'zh')) throw new Error('builtin zh locale is missing');
  if (!(await isCurrent(GENERATED_PATH, renderGenerated(en)))) {
    diagnostics.push({ locale: 'types', message: 'catalog-types.ts is stale; run mise run i18n:types' });
  }
...
if (import.meta.main) {
  ...
  if (mode === '--write') await writeCatalog();
  else if (mode === '--check' || mode === undefined) await checkCatalog();
  else throw new Error('usage: bun scripts/i18n/catalog.ts [--check|--write]');
}
