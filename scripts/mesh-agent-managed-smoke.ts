...
function usage(): never {
  process.stderr.write(
    `${[
      'Usage:',
      '  mise run smoke:mesh-agent-managed -- --agent <name> --cwd <project-dir> [--server https://127.0.0.1:47749]',
      '',
...
const agent = values.agent ?? usage();
const cwd = values.cwd ?? usage();
const server = values.server ?? Bun.env.MONAD_SERVER_URL ?? (await resolveClientConn()).baseUrl;

const session = await request<{ sessionId: string }>(server, '/v1/sessions', {
  method: 'POST',
  headers: { 'content-type': 'application/json' },
  body: JSON.stringify({
    title: `Workplace: managed MeshAgent smoke (${agent})`,
    origin: {
      surface: 'web',
      client: 'mesh-agent-managed-smoke',
      transport: 'http',
      writableBy: ['http'],
      branchableBy: ['http']
    },
  ...
  `${JSON.stringify(
    {
      ...
      agent,
      cwd,
      note: 'Open the Workplace Project and the MeshAgent diagnostics if provider login or approval is required.'
    },
    null,
    2
  )}\n`
