...
export const mem0OptionalPeerExternals = ['better-sqlite3'] as const;

function isRecord(value: unknown): value is Record<string, unknown> {
  return typeof value === 'object' && value !== null && !Array.isArray(value);
}

export async function optionalPeerExternals(
  manifestPath: string,
  ...
  return Object.entries(peerDependenciesMeta)
    .filter(
      ([name, metadata]) =>
        isRecord(metadata) && metadata.optional === true && name in peerDependencies && !bundled.has(name)
    )
    .map(([name]) => name)
    .sort();
}
