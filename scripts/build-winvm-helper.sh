#!/usr/bin/env bash
# Build the Windows VM-sandbox host helper (native/winvm-helper, GOOS=windows both arches) and the
# guest-side gvforwarder (Linux, both arches — pinned gvisor-tap-vsock cmd/vm, the tap⇄vsock network
# forwarder the hyperv driver injects via Ignition), vendoring everything into packages/sandbox-vm.
# Run when native/winvm-helper changes or when bumping GVPROXY_VERSION; requires Go on PATH.
set -euo pipefail
cd "$(dirname "$0")/.."
...
for arch in arm64 amd64; do
  (cd "$TMP" &&
    GOOS=linux GOARCH="$arch" CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" \
      -o "$OLDPWD/$OUT/gvforwarder-$arch" github.com/containers/gvisor-tap-vsock/cmd/vm)
  echo "built $OUT/gvforwarder-$arch"
done
