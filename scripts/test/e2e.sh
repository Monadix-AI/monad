...
DPID=$!
WEB_PORT="$WPORT" MONAD_URL="http://127.0.0.1:${DPORT}" "$MONAD_BIN" web >/tmp/e2e-web.log 2>&1 &
WPID=$!
...
for _ in $(seq 1 60); do
  if curl -fsS "http://127.0.0.1:${DPORT}/health" >/dev/null 2>&1 \
     && curl -fsS "http://127.0.0.1:${WPORT}/" >/dev/null 2>&1; then ready=1; break; fi
  sleep 0.2
done
[ "$ready" = 1 ] || { echo "--- daemon.log ---"; cat /tmp/e2e-daemon.log; echo "--- web.log ---"; cat /tmp/e2e-web.log; fail "services did not become ready"; }

# ── assertions ─────────────────────────────────────────────────────────────────
curl -fsS "http://127.0.0.1:${DPORT}/health" | grep -q '"status":"ok"' && pass "daemon /health" || fail "daemon /health"

curl -fsS "http://127.0.0.1:${WPORT}/" | grep -q '<html' && pass "web serves embedded SPA" || fail "web /"

# The SPA's JS/CSS assets resolve from the embedded filesystem
asset=$(curl -fsS "http://127.0.0.1:${WPORT}/" | grep -oE '/_next/static/[^"]+\.js' | head -1 || true)
[ -n "$asset" ] && curl -fsS "http://127.0.0.1:${WPORT}${asset}" >/dev/null && pass "embedded asset ${asset##*/}" || fail "embedded static asset"

curl -fsS "http://127.0.0.1:${WPORT}/api/daemon/health" | grep -q '"status":"ok"' && pass "web → daemon proxy" || fail "proxy"

# ── full chat round through the WEB proxy (mock model is deterministic) ─────────
sid=$(curl -fsS -X POST "http://127.0.0.1:${WPORT}/api/daemon/v1/sessions" \
...
reply=$(curl -fsS -X POST "http://127.0.0.1:${WPORT}/api/daemon/v1/sessions/${sid}/messages/block" \
