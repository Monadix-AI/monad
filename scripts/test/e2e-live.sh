...
command -v jq >/dev/null 2>&1 || fail "jq is required (used to parse JSON/SSE)."
mkdir -p "$MONAD_HOME"
...
"$MONAD_BIN" credential add openrouter \
  "$(jq -nc --arg token "$OPENROUTER_API_KEY" '{label:"e2e-live",authType:"api_key",accessToken:$token}')" \
  >/dev/null && pass "credential added" || fail "credential add"
"$MONAD_BIN" model set \
  "$(jq -nc --arg model "$MODEL" '{alias:"default",provider:"openrouter",modelId:$model,params:{reasoningEffort:"low"},fallbacks:[]}')" \
...
text=$(jq -r '.message.text // empty' <<<"$reply")
...
tokstats=$(
  sed -n 's/^data:[[:space:]]*//p' /tmp/e2e-live-sse.txt |
    jq -sr '([.[] | select(.type == "agent.token")] | length) as $count
      | ([.[] | select(.type == "agent.message") | .payload.text] | last // "") as $final
      | "\($count)\t\($final)"'
)
ntok=${tokstats%%$'\t'*}
...
if grep -q '"type":"tool.called"' /tmp/e2e-live-tool.txt; then
  tool=$(
    sed -n 's/^data:[[:space:]]*//p' /tmp/e2e-live-tool.txt |
      jq -sr '[.[] | select(.type == "tool.called") | (.payload.name // .payload.toolName // .payload.tool // "?")]
        | first // empty'
  )
...
if jq -e 'tostring | test("in|tokens|total"; "i")' /tmp/e2e-live-usage.json >/dev/null 2>&1; then
