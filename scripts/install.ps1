...
.DESCRIPTION
  ...
  Usage (local dev — fully self-contained inside dist\):
    mise run release:build       # produces dist\monad-<ver>-windows-x64.tar.gz
...
try {
  ...
  if ($ForceInstall -and (Test-Path -LiteralPath $InstallDir)) {
    $resolvedInstall = [System.IO.Path]::GetFullPath($InstallDir).TrimEnd('\', '/')
    $resolvedHome = [System.IO.Path]::GetFullPath($HOME).TrimEnd('\', '/')
    $resolvedCwd = [System.IO.Path]::GetFullPath((Get-Location).Path).TrimEnd('\', '/')
    $resolvedRoot = [System.IO.Path]::GetPathRoot($resolvedInstall).TrimEnd('\', '/')
    if (
      $resolvedInstall.Equals($resolvedRoot, [System.StringComparison]::OrdinalIgnoreCase) -or
      $resolvedInstall.Equals($resolvedHome, [System.StringComparison]::OrdinalIgnoreCase) -or
      $resolvedInstall.Equals($resolvedCwd, [System.StringComparison]::OrdinalIgnoreCase)
    ) {
      Fatal "Refusing to clear unsafe install directory: $resolvedInstall"
    }
    Info "Clearing existing Monad installation at $resolvedInstall…"
