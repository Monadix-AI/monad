...
// daemon (acp-agent presets, obscura) and the built-in agent-adapter atoms, so the PATH-then-candidates
// rule — and any future Windows .exe/.cmd handling — lives in one place. Probes are injectable for
// deterministic tests.
...
export const defaultBinProbes: BinProbes = {
  which: (name) => {
    ...
    return resolved && isExecutable(resolved) ? resolved : searchPath(name);
  },
  exists: existsSync
};
