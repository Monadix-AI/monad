...
export interface CommandProjectInfo {
  projectId: string;
  title: string;
  current: boolean;
}

export interface CommandProjectSelection extends CommandProjectInfo {
  sessions?: CommandSessionInfo[];
  sessionId?: string;
}

export interface CommandProjectNavigation {
  listProjects(): Promise<CommandProjectInfo[]>;
  currentProject(): Promise<CommandProjectInfo | null>;
  switchProject(target: string): Promise<CommandProjectSelection | null>;
  leaveProject(): Promise<{ sessionId?: string }>;
}
...
export interface CommandRunContext {
  ...
  /** Optional Project navigation supplied by channel-like transports that own an active-session pointer. */
  readonly project?: CommandProjectNavigation;
   ...
   *  Agent policy): L1 fact dedup, then L2 graph, then L3 laws. */
...
export type CommandEffect =
  ...
  | { type: 'view-clear' }
  | { type: 'observation-render-mode-changed'; mode: 'detail' | 'summary' }
