...
export interface MeshAgentDiscoveredAgent {
  externalId: string;
  displayName: string;
  env?: Record<string, string>;
...
export interface MeshAgentDiscoveryProbe {
  launch: MeshAgentLaunchSpec;
  parse(output: string, exitCode: number | null): MeshAgentDiscoveredAgent[];
}
...
export const meshAgentOutputEventSchema = z.discriminatedUnion('type', [
  z.object({
    type: z.literal('session_usage_updated'),
    payload: meshAgentSessionUsageSchema
  }),
  z.object({
  ...
  // `callId` is the only link between a call and its result. An adapter that emits the two halves
  // as separate events must supply it on both; consumers pair strictly by this id and never infer
  // a pairing from arrival order. Omit it and the two halves render as unrelated cards.
  z.object({
...
export type MeshAgentModelOptionsProbe =
  | {
      launch: MeshAgentLaunchSpec;
      parse(output: string, exitCode: number | null): MeshAgentModelOption[];
    }
  | {
      resolve(): Promise<MeshAgentModelOption[]>;
    };
...
export interface MeshAgentProviderSessionUsageContext {
  providerSessionRef: string;
  workingPath: string;
  executable: string;
  env?: Record<string, string>;
...
export interface MeshAgentSessionUsageSource {
  read(context: MeshAgentProviderSessionUsageContext): Promise<MeshAgentSessionUsage | null>;
}
...
export interface MeshAgentManagedEnvContext {
  ...
  mcpServer?: NativeAgentManagedMcpServer;
}
...
export interface MeshAgentProviderAdapter {
  ...
  discoverAgents?(agent: MeshAgentView): MeshAgentDiscoveryProbe;
  provider: MeshAgentProvider;
  icon: ChannelIcon;
