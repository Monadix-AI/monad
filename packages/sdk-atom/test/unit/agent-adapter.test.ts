...
test('MeshAgent adapters can discover provider-owned agents', () => {
  const probe: MeshAgentDiscoveryProbe = {
    launch: {
    ...
    parse: () => [
      {
        externalId: 'test',
        displayName: 'test',
        env: { MONAD_OPENCLAW_AGENT_ID: 'test' }
      }
    ]
  };
  const adapter = {
    discoverAgents: (_agent: MeshAgentView) => probe
  } satisfies Pick<MeshAgentProviderAdapter, 'discoverAgents'>;

  expect(adapter.discoverAgents({} as MeshAgentView).parse('', 0)).toEqual([
    {
      externalId: 'test',
      displayName: 'test',
      env: { MONAD_OPENCLAW_AGENT_ID: 'test' }
