...
describe('createLogger', () => {
  test('NODE_ENV=test defaults to silent', async () => {
    const proc = Bun.spawn({
      cmd: [
        process.execPath,
        '-e',
        "import { createLogger } from './src/index.ts'; console.log(createLogger('probe').level)"
      ],
    ...
    expect({ exitCode, stdout: stdout.trim() }).toEqual({ exitCode: 0, stdout: 'silent' });
  });
  ...
  test('developer log subscribers receive structured records with session ids', async () => {
    ...
    expect(records.map(({ name, sessionId, event, msg, level }) => ({ name, sessionId, event, msg, level }))).toEqual([
      {
        name: 'subscriber-test',
        sessionId: 'ses_LOGTEST00000',
        event: 'test.event',
        msg: 'subscriber event',
        level: 20
      }
    ]);
  });
  ...
  test('custom destinations receive records at their own level', async () => {
    ...
    await waitFor(() => otelRecords.length === 2 && sentryRecords.length === 1, {
      message: 'destinations did not receive their filtered records'
    });
  ...
  test('configureLogger reconfigures already-created lazy loggers', async () => {
    ...
    await waitFor(() => firstRecords.length === 1, { message: 'first destination never received its record' });
    ...
    await waitFor(() => secondRecords.length === 1, { message: 'second destination never received its record' });
