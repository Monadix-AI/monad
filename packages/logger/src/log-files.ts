...
const DEBUG_LOG_PATTERN = /^monad-debug-\d{4}-\d{2}-\d{2}\.log$/;
...
export function isDebugLogFileName(name: string): boolean {
  return DEBUG_LOG_PATTERN.test(name);
}

const DEV_LOG_SUFFIX = '.jsonl';
const DEVELOPER_LOG_PATTERN = /^(?:channel|session)-[a-zA-Z0-9_.:-]+\.jsonl$/;
...
export function isDeveloperLogFileName(name: string): boolean {
  return DEVELOPER_LOG_PATTERN.test(name);
}
