...
const switchSessionCommandAtom = defineCommand({
  ...
  async run(ctx) {
    ...
    if (!target) {
      ...
      if (sessions.length === 0) return { message: ctx.t('cmd.sessions.empty') };
      const list = sessions
        .map((session, index) => `${session.active ? '➡️' : '  '} ${index + 1}. ${session.label ?? session.sessionId}`)
        .join('\n');
      return { message: ctx.t('cmd.sessions.list', { list }) };
    }
...
const projectCommandAtom = defineCommand({
  name: 'project',
  description: 'Choose the Project for this channel conversation',
  descriptionKey: 'cmd.project.desc',
  group: 'Conversation',
  argHint: '[list|current|use <number|project-id>|leave]',
  subcommands: [
    { id: 'list', name: 'List', description: 'List available Projects' },
    { id: 'current', name: 'Current', description: 'Show the current Project' },
    {
      id: 'use',
      name: 'Use',
      description: 'Switch to a Project',
      args: [{ name: 'target', type: 'string', required: true, repeated: true, placeholder: '<number|project-id>' }]
    },
    { id: 'leave', name: 'Leave', description: 'Return to the default Agent' }
  ],
  async run(ctx) {
    if (!ctx.project) return { message: ctx.t('cmd.project.unavailable') };
    const [action = 'list', ...rest] = ctx.args.trim().split(/\s+/).filter(Boolean);
    if (action === 'list') {
      const projects = await ctx.project.listProjects();
      if (projects.length === 0) return { message: ctx.t('cmd.project.empty') };
      const list = projects
        .map((project, index) => `${project.current ? '➡️' : '  '} ${index + 1}. ${project.title}`)
        .join('\n');
      return { message: ctx.t('cmd.project.list', { list }) };
    }
    if (action === 'current') {
      const project = await ctx.project.currentProject();
      return { message: project ? ctx.t('cmd.project.current', { title: project.title }) : ctx.t('cmd.project.none') };
    }
    if (action === 'use') {
      const target = rest.join(' ').trim();
      if (!target) return { message: ctx.t('cmd.project.use.usage') };
      const project = await ctx.project.switchProject(target);
      if (!project) return { message: ctx.t('cmd.project.notFound', { target }) };
      const sessions = project.sessions ?? [];
      const list = sessions
        .map((session, index) => `   ${index + 1}. ${session.label ?? session.sessionId}`)
        .join('\n');
      return {
        message: sessions.length
          ? ctx.t('cmd.project.use.sessions', { title: project.title, list })
          : ctx.t('cmd.project.use.empty', { title: project.title }),
        ...(project.sessionId ? { effect: { type: 'session-switched' as const, sessionId: project.sessionId } } : {})
      };
    }
    if (action === 'leave') {
      const { sessionId } = await ctx.project.leaveProject();
      return {
        message: ctx.t('cmd.project.leave.done'),
        ...(sessionId ? { effect: { type: 'session-switched' as const, sessionId } } : {})
      };
    }
    return { message: ctx.t('cmd.project.usage') };
  }
});
