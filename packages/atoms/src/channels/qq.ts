...
const qqHeartbeatIntervalSchema = z.number().int().min(1_000).max(300_000);
...
export function parseQQHeartbeatInterval(data: unknown): number | null {
  const result = z.object({ heartbeat_interval: qqHeartbeatIntervalSchema }).safeParse(data);
  return result.success ? result.data.heartbeat_interval : null;
}
...
export function createQQAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  function handle(raw: string): void {
    ...
    switch (p.op) {
      case 10: {
        const interval = parseQQHeartbeatInterval(p.d);
        if (interval === null) {
          ws?.close(4002, 'invalid hello payload');
          break;
        }
        stopHeartbeat();
...
export const qqChannelAtom = defineChannel({
  type: 'qq',
  name: 'QQ Bot',
  icon: channelIcons.qq,
  setup: channelSetupGuides.qq,
  capabilities: QQ_CAPABILITIES,
  envVars: [
    { name: 'QQ_APP_ID', description: 'Bot AppID', required: true, secret: true, credentialKey: 'appId' },
    { name: 'QQ_TOKEN', description: 'Bot token', required: true, secret: true, credentialKey: 'token' },
    {
      name: 'QQ_CLIENT_SECRET',
      description: 'AppSecret (for group/C2C AppAccessToken)',
      required: false,
      secret: true,
      credentialKey: 'clientSecret'
    },
    { name: 'QQ_SANDBOX', description: 'Use the sandbox gateway', required: false, credentialKey: 'sandbox' }
  ],
