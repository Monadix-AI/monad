...
// platform I/O.
//
// Connection fields include the AAD app credentials, tenant, inbound port, and path.
...
export function createTeamsAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const tenantId = ctx.secrets.tenantId || 'botframework.com';
  const port = Number(ctx.secrets.port) || 8806;
  const path = ctx.secrets.path || '/teams';
...
export const teamsChannelAtom = defineChannel({
  ...
  icon: channelIcons.teams,
  setup: channelSetupGuides.teams,
  capabilities: TEAMS_CAPABILITIES,
  envVars: [
    { name: 'TEAMS_APP_ID', description: 'Bot AAD app id', required: true, secret: true, credentialKey: 'appId' },
    {
      name: 'TEAMS_APP_PASSWORD',
      description: 'Bot AAD app password',
      required: true,
      secret: true,
      credentialKey: 'appPassword'
    },
    { name: 'TEAMS_TENANT_ID', description: 'AAD tenant id', required: false, credentialKey: 'tenantId' },
    { name: 'TEAMS_PORT', description: 'Inbound port (default 8806)', required: false, credentialKey: 'port' },
    { name: 'TEAMS_PATH', description: 'Inbound path (default /teams)', required: false, credentialKey: 'path' }
  ],
