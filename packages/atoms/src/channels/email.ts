...
//
// Mailbox, server, and password values arrive as explicit connection fields.
...
export function createEmailAdapter(ctx: ChannelContext): ChannelAdapter {
  const user = ctx.secrets.user ?? '';
  const password = ctx.secrets.token ?? '';
  const smtpPassword = ctx.secrets.smtpPassword ?? password;
  const imapHost = ctx.secrets.imapHost ?? '';
  const imapPort = Number(ctx.secrets.imapPort) || 993;
  const smtpHost = ctx.secrets.smtpHost ?? '';
  const smtpPort = Number(ctx.secrets.smtpPort) || 465;
  const from = ctx.secrets.from ?? user;
  const pollSec = Number(ctx.secrets.pollSec) || 30;
  ...
  return {
    ...
    async connect() {
      ...
      if (!user || !imapHost || !smtpHost) throw new Error('email: user, IMAP host, and SMTP host are required');
...
export const emailChannelAtom = defineChannel({
  ...
  icon: channelIcons.email,
  setup: channelSetupGuides.email,
  capabilities: EMAIL_CAPABILITIES,
  envVars: [
    { name: 'EMAIL_USER', description: 'Mailbox address', required: true, credentialKey: 'user' },
    { name: 'EMAIL_IMAP_HOST', description: 'IMAP host', required: true, credentialKey: 'imapHost' },
    { name: 'EMAIL_IMAP_PORT', description: 'IMAP port (default 993)', required: false, credentialKey: 'imapPort' },
    { name: 'EMAIL_SMTP_HOST', description: 'SMTP host', required: true, credentialKey: 'smtpHost' },
    { name: 'EMAIL_SMTP_PORT', description: 'SMTP port (default 465)', required: false, credentialKey: 'smtpPort' },
    { name: 'EMAIL_FROM', description: 'From address (defaults to mailbox)', required: false, credentialKey: 'from' },
    {
      name: 'EMAIL_POLL_SECONDS',
      description: 'Polling interval (default 30)',
      required: false,
      credentialKey: 'pollSec'
    },
    {
      name: 'EMAIL_PASSWORD',
      description: 'IMAP/SMTP password',
      required: true,
      secret: true,
      credentialKey: 'token'
    },
    {
      ...
      description: 'SMTP password when different from IMAP',
      required: false,
      secret: true,
