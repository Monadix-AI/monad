...
//
// Connection fields: host, port, TLS, nickname, and comma-separated channels.
...
export function createIrcAdapter(ctx: ChannelContext): ChannelAdapter {
  const password = ctx.secrets.serverPassword;
  const host = ctx.secrets.host ?? '';
  const port = Number(ctx.secrets.port) || 6697;
  const tls = ctx.secrets.tls !== 'false';
  const nick = ctx.secrets.nick || 'monad';
  const channels = (ctx.secrets.channels ?? '')
    .split(',')
...
export const ircChannelAtom = defineChannel({
  type: 'irc',
  name: 'IRC',
  icon: channelIcons.irc,
  setup: channelSetupGuides.irc,
  capabilities: IRC_CAPABILITIES,
  envVars: [
    { name: 'IRC_HOST', description: 'IRC server host', required: true, credentialKey: 'host' },
    { name: 'IRC_PORT', description: 'IRC server port (default 6697)', required: false, credentialKey: 'port' },
    { name: 'IRC_TLS', description: 'Use TLS (true unless set to false)', required: false, credentialKey: 'tls' },
    { name: 'IRC_NICK', description: 'Bot nickname (default monad)', required: false, credentialKey: 'nick' },
    {
      name: 'IRC_CHANNELS',
      description: 'Comma-separated channels to join',
      required: false,
      credentialKey: 'channels'
    },
    {
      name: 'IRC_PASSWORD',
