...
//
// Connection fields: registered account and optional signal-cli path.
...
export function createSignalAdapter(ctx: ChannelContext): ChannelAdapter {
  const account = ctx.secrets.account ?? '';
  const cliPath = ctx.secrets.cliPath || 'signal-cli';
  ...
  return {
    ...
    async connect() {
      if (!account) throw new Error('signal: registered account is required');
      try {
...
export const signalChannelAtom = defineChannel({
  ...
  icon: channelIcons.signal,
  setup: channelSetupGuides.signal,
  capabilities: SIGNAL_CAPABILITIES,
  envVars: [
    { name: 'SIGNAL_ACCOUNT', description: 'Registered phone number', required: true, credentialKey: 'account' },
    {
      name: 'SIGNAL_CLI_PATH',
      description: 'signal-cli executable path (default signal-cli)',
      required: false,
      credentialKey: 'cliPath'
    }
  ],
