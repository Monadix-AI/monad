...
// Inbound port and path are explicit connection fields.
...
export function createLineAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const port = Number(ctx.secrets.port) || 8801;
  const path = ctx.secrets.path || '/line';
...
export const lineChannelAtom = defineChannel({
  type: 'line',
  name: 'LINE',
  icon: channelIcons.line,
  setup: channelSetupGuides.line,
  capabilities: LINE_CAPABILITIES,
  envVars: [
    {
      name: 'LINE_CHANNEL_ACCESS_TOKEN',
      description: 'Channel access token (Bearer)',
      required: true,
      secret: true,
      credentialKey: 'token'
    },
    {
      ...
      description: 'Channel secret (webhook signature)',
      required: true,
      secret: true,
      credentialKey: 'channelSecret'
    },
    { name: 'LINE_PORT', description: 'Inbound port (default 8801)', required: false, credentialKey: 'port' },
    { name: 'LINE_PATH', description: 'Inbound path (default /line)', required: false, credentialKey: 'path' }
  ],
