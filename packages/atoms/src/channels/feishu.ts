...
//
// App credentials and optional endpoint overrides arrive as explicit connection fields.
...
export function createFeishuAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const baseUrl = (ctx.secrets.baseUrl || 'https://open.feishu.cn').replace(/\/+$/, '');
  const port = Number(ctx.secrets.port) || 8804;
  const path = ctx.secrets.path || '/feishu';
...
export const feishuChannelAtom = defineChannel({
  ...
  setup: channelSetupGuides.feishu,
  capabilities: FEISHU_CAPABILITIES,
  envVars: [
    { name: 'FEISHU_APP_ID', description: 'App ID', required: true, secret: true, credentialKey: 'appId' },
    {
    ...
    },
    {
      ...
      description: 'Event subscription encrypt key',
      required: false,
      secret: true,
      credentialKey: 'encryptKey'
    },
    { name: 'FEISHU_BASE_URL', description: 'API base URL', required: false, credentialKey: 'baseUrl' },
    { name: 'FEISHU_PORT', description: 'Inbound port (default 8804)', required: false, credentialKey: 'port' },
    { name: 'FEISHU_PATH', description: 'Inbound path (default /feishu)', required: false, credentialKey: 'path' }
  ],
