...
//
// Inbound port/path and optional outbound URL are explicit connection fields.
...
export function createWebhookAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const port = Number(ctx.secrets.port) || 8788;
  const path = ctx.secrets.path || '/inbound';
  const outboundUrl = ctx.secrets.outboundUrl || undefined;
  ...
  return {
    ...
    async send(chatId: string, content: string, opts?: SendOptions): Promise<SentMessage> {
      ...
      if (!outboundUrl) {
        ctx.log('warn', 'webhook send dropped: no outbound URL configured');
        return { ref, chatId };
      }
...
export const webhookChannelAtom = defineChannel({
  ...
  icon: channelIcons.webhook,
  setup: channelSetupGuides.webhook,
  capabilities: WEBHOOK_CAPABILITIES,
  envVars: [
    {
      name: 'WEBHOOK_SECRET',
      description: 'Shared signing secret',
      required: true,
      secret: true,
      credentialKey: 'token'
    },
    { name: 'WEBHOOK_OUTBOUND_URL', description: 'Reply delivery URL', required: false, credentialKey: 'outboundUrl' },
    { name: 'WEBHOOK_PORT', description: 'Inbound port (default 8788)', required: false, credentialKey: 'port' },
    { name: 'WEBHOOK_PATH', description: 'Inbound path (default /inbound)', required: false, credentialKey: 'path' }
  ],
