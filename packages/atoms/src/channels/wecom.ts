...
// Agent ID, inbound port, and path are explicit connection fields.
...
export function createWecomAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const agentId = ctx.secrets.agentId ?? '';
  const port = Number(ctx.secrets.port) || 8805;
  const path = ctx.secrets.path || '/wecom';
  ...
  return {
    ...
    async connect() {
      if (!corpId || !corpSecret || !aesKey || !agentId)
        throw new Error('wecom: corp ID, corp secret, AES key, and agent ID are required');
...
export const wecomChannelAtom = defineChannel({
  ...
  setup: channelSetupGuides.wecom,
  capabilities: WECOM_CAPABILITIES,
  envVars: [
    { name: 'WECOM_AGENT_ID', description: 'Agent ID', required: true, credentialKey: 'agentId' },
    { name: 'WECOM_CORP_ID', description: 'Corp ID', required: true, secret: true, credentialKey: 'corpId' },
    {
      name: 'WECOM_CORP_SECRET',
      description: 'App secret',
      required: true,
      secret: true,
      credentialKey: 'corpSecret'
    },
    {
      name: 'WECOM_CALLBACK_TOKEN',
      description: 'Callback signature token',
      required: true,
      secret: true,
      credentialKey: 'token'
    },
    {
      name: 'WECOM_AES_KEY',
      description: 'Callback EncodingAESKey',
      required: true,
      secret: true,
      credentialKey: 'aesKey'
    },
    { name: 'WECOM_PORT', description: 'Inbound port (default 8805)', required: false, credentialKey: 'port' },
    { name: 'WECOM_PATH', description: 'Inbound path (default /wecom)', required: false, credentialKey: 'path' }
  ],
