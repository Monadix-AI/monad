...
//
// BlueBubbles server and inbound endpoint settings arrive as explicit connection fields.
...
export function createImessageAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const serverUrl = (ctx.secrets.serverUrl ?? '').replace(/\/+$/, '');
  const inboundSecret = ctx.secrets.inboundSecret ?? '';
  const port = Number(ctx.secrets.port) || 8808;
  const path = ctx.secrets.path || '/imessage';
  ...
  return {
    ...
    async connect() {
      if (!serverUrl || !password) throw new Error('imessage: BlueBubbles server URL and password are required');
      try {
...
export const imessageChannelAtom = defineChannel({
  ...
  setup: channelSetupGuides.imessage,
  capabilities: IMESSAGE_CAPABILITIES,
  envVars: [
    {
    ...
    {
      name: 'BLUEBUBBLES_INBOUND_SECRET',
      description: 'Optional webhook secret',
      required: false,
      secret: true,
      credentialKey: 'inboundSecret'
    },
    { name: 'IMESSAGE_PORT', description: 'Inbound port (default 8808)', required: false, credentialKey: 'port' },
    { name: 'IMESSAGE_PATH', description: 'Inbound path (default /imessage)', required: false, credentialKey: 'path' }
  ],
