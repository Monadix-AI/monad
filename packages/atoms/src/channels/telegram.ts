...
export function createTelegramAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const optTimeout = Number(ctx.secrets.pollTimeoutSec) || POLL_TIMEOUT_SEC;
  // Override for a self-hosted Telegram Bot API server (or a test double).
  const baseUrl = telegramApiBaseUrl(ctx.secrets.apiBaseUrl || 'https://api.telegram.org');
...
export function telegramApiBaseUrl(url: string): string {
  let end = url.length;
...
export const telegramChannelAtom = defineChannel({
  ...
  setup: channelSetupGuides.telegram,
  capabilities: TELEGRAM_CAPABILITIES,
  envVars: [
    { name: 'TELEGRAM_TOKEN', description: 'Bot token', required: true, secret: true, credentialKey: 'token' },
    {
      name: 'TELEGRAM_API_BASE_URL',
      description: 'Bot API base URL',
      required: false,
      credentialKey: 'apiBaseUrl'
    },
    {
      name: 'TELEGRAM_POLL_TIMEOUT',
      description: 'Long-poll timeout in seconds',
      required: false,
      credentialKey: 'pollTimeoutSec'
    }
  ],
