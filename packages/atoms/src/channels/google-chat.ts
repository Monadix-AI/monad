...
//
// Service-account and optional endpoint settings arrive as explicit connection fields.
...
export function createGoogleChatAdapter(ctx: ChannelContext): ChannelAdapter {
  ...
  const port = Number(ctx.secrets.port) || 8807;
  const path = ctx.secrets.path || '/gchat';
  const audienceOpt = ctx.secrets.audience ?? '';
  ...
  return {
    ...
    async connect() {
      ...
      // The inbound JWT verifier needs an audience override or the key's client_email. If it
      ...
      if (!jwtAudience()) {
        throw new Error(
          'gchat: cannot derive JWT audience — set the audience field or provide key JSON with client_email'
        );
      }
      server.start();
    },
...
export const googleChatChannelAtom = defineChannel({
  type: 'gchat',
  name: 'Google Chat',
  icon: channelIcons.gchat,
  setup: channelSetupGuides.gchat,
  capabilities: GCHAT_CAPABILITIES,
  envVars: [
    {
      name: 'GCHAT_SERVICE_ACCOUNT',
      description: 'Service-account key JSON',
      required: true,
      secret: true,
      credentialKey: 'serviceAccount'
    },
    { name: 'GCHAT_AUDIENCE', description: 'JWT audience override', required: false, credentialKey: 'audience' },
    { name: 'GCHAT_PORT', description: 'Inbound port (default 8807)', required: false, credentialKey: 'port' },
    { name: 'GCHAT_PATH', description: 'Inbound path (default /gchat)', required: false, credentialKey: 'path' }
  ],
