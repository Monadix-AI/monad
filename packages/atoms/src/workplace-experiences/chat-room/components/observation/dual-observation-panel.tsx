...
import { observationSessionUsageMeter } from './session-usage-meter.ts';
...
export function DualObservationPanel(props: DualObservationPanelProps): React.ReactElement {
  ...
  const sessionUsage = hooks.useSessionUsage(
    { id: meshSessionId, transcriptTargetId },
    { skip: !meshSessionId || !transcriptTargetId }
  );
  ...
  return (
    <MeshAgentObservationPanel
      ...
      sessionUsageMeter={observationSessionUsageMeter(sessionUsage.currentData)}
      stream={stream}
    />
  );
}
