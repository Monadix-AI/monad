...
const DEFAULT_DUAL_OBSERVATION_HOOKS: ObservationPanelHooks = {
  ...
  useSessionUsage: useGetMeshSessionUsageQuery
};
...
function agentPresenceLabel(agent: Participant): string {
  ...
  if (agent.presence === 'sleeping') return 'Sleeping';
  if (agent.presence === 'waking') return 'Waking';
...
@keyframes workplace-agent-sleep {
  0%, 100% { transform: translateY(1px) scale(0.98); filter: saturate(0.72); }
  50% { transform: translateY(2px) scale(0.96); filter: saturate(0.62); }
}

@keyframes workplace-agent-sleep-z {
  0% { opacity: 0; transform: translate(-3px, 5px) scale(0.72); }
  35%, 70% { opacity: 0.72; }
  100% { opacity: 0; transform: translate(4px, -7px) scale(1); }
}

@keyframes workplace-agent-wake {
  0%, 100% { transform: scale(0.96); }
  45% { transform: scale(1.06); }
}

@keyframes workplace-agent-wake-spark {
  0%, 100% { opacity: 0.35; transform: rotate(-12deg) scale(0.82); }
...
.workplace-agent-status-avatar[data-active='true'] .workplace-agent-status-core {
  border-color: var(--agent-presence-color);
  animation: workplace-agent-status-breathe 1.8s ease-in-out infinite;
}

.workplace-agent-status-avatar[data-active='true'] .workplace-agent-status-core::after {
  position: absolute;
  inset: -3px;
...
.workplace-agent-status-avatar[data-presence='sleeping'] .workplace-agent-status-core {
  animation: workplace-agent-sleep 4.2s ease-in-out infinite;
}

.workplace-agent-status-avatar[data-presence='waking'] .workplace-agent-status-core {
  border-color: var(--agent-presence-color);
  animation: workplace-agent-wake 1.15s cubic-bezier(0.22, 1, 0.36, 1) infinite;
}
...
.workplace-agent-status-sleep span {
  position: absolute;
  animation: workplace-agent-sleep-z 2.8s ease-out infinite;
}

.workplace-agent-status-sleep span:first-child {
  left: 2px;
  bottom: 1px;
}

.workplace-agent-status-sleep span:last-child {
  right: 1px;
  top: 0;
  font-size: 12px;
...
.workplace-agent-status-wake {
  ...
  font-size: 18px;
  line-height: 1;
  animation: workplace-agent-wake-spark 1.15s ease-in-out infinite;
}
...
.workplace-agent-status-phase {
  ...
  animation:
    workplace-agent-phase-bubble-pop 220ms cubic-bezier(0.16, 1, 0.3, 1) both,
...
@media (prefers-reduced-motion: reduce) {
	  .workplace-agent-status-avatar,
	  .workplace-agent-status-core,
	  .workplace-agent-status-core::after,
	  .workplace-agent-status-sleep span,
	  .workplace-agent-status-wake,
	  .workplace-agent-status-phase,
...
export function AgentTasksRail({ room }: { room: AgentTasksRailRoom }): React.ReactElement {
  ...
  const renderAgent = (agent: Participant, observationEvents?: readonly AgentObservationEvent[]) => {
    ...
    const observedPresence = observationEvents
      ? active
        ? 'working'
        : agent.presence === 'sleeping' ||
            agent.presence === 'waking' ||
            agent.presence === 'needs-login' ||
            agent.presence === 'failed' ||
            agent.presence === 'stopped'
          ? agent.presence
          : 'idle'
      : agent.presence;
    const displayAgent =
    ...
    const statusLabel =
      ...
      (displayAgent.presence === 'sleeping'
        ? t('web.workplace.agentStatus.sleeping')
        : displayAgent.presence === 'waking'
          ? t('web.workplace.agentStatus.waking')
          : displayAgent.presence === 'idle'
            ? t('web.workplace.agentStatus.idleDescription')
            : agentPresenceLabel(displayAgent));
      ...
      >
        ...
        >
          ...
          {displayAgent.presence === 'sleeping' ? (
            <span
              aria-hidden="true"
              className="workplace-agent-status-sleep"
            >
              <span>z</span>
              <span>Z</span>
            </span>
          ...
          {displayAgent.presence === 'waking' ? (
            <span
              aria-hidden="true"
              className="workplace-agent-status-wake"
            >
              ✦
            </span>
          ) : null}
