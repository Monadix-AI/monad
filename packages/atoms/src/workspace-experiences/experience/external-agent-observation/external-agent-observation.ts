...
type ExternalAgentObservationAdapter = Pick<ExternalAgentProviderAdapter, 'observation' | 'provider'>;
type ExternalAgentObservationAdapterProjection = NonNullable<ExternalAgentProviderAdapter['observation']>;

type ParsedTimelineEntry =
  | { kind: 'events'; events: ExternalAgentObservationEvent[] }
  | { kind: 'message-group'; key: string };

type ExternalAgentObservationAdapterResolver = (
  provider: ExternalAgentProvider | string | undefined
) => ExternalAgentObservationAdapter | undefined;

let externalAgentObservationAdapterResolver: ExternalAgentObservationAdapterResolver = () => undefined;

export function configureExternalAgentObservationAdapterResolver(
  resolver: ExternalAgentObservationAdapterResolver
): void {
  externalAgentObservationAdapterResolver = resolver;
}
...
function observationAdapter(args: {
  ...
  return args.adapter ?? externalAgentObservationAdapterResolver(args.provider);
}

function observation(args: {
  id: string;
  role: ExternalAgentObservationEvent['role'];
  text?: string;
  source: ExternalAgentObservationEvent['source'];
...
function jsonRecordEntries(text: string): ExternalAgentObservationJsonRecordEntry[] {
...
function recordEvents(
  id: string,
  provider: ExternalAgentProvider | string | undefined,
  adapterObservation: ExternalAgentProviderAdapter['observation'] | undefined,
  record: Record<string, unknown>,
  recordIndex: number
): ExternalAgentObservationEvent[] {
  const out =
...
function parsedJsonEvents(args: {
  id: string;
  provider?: ExternalAgentProvider | string;
  adapterObservation?: ExternalAgentProviderAdapter['observation'];
  ...
  const messageGroups = new Map<
    string,
    { projector: NonNullable<ExternalAgentObservationAdapterProjection['messageGroup']>; state: unknown }
  >();
...
function externalAgentObservationEvents(args: {
  ...
  observedAt?: string;
  mode?: 'history' | 'live';
}): ExternalAgentObservationEvent[] | undefined {
...
export function externalAgentStreamItems(args: {
  id: string;
  provider?: ExternalAgentProvider | string;
  adapter?: ExternalAgentObservationAdapter;
  output?: string;
  ...
  const text = args.output?.trim();
  if (!text) return [];
  const structured = externalAgentObservationEvents(args);
...
export function externalAgentStructuredEvents(args: {
  id: string;
  provider?: ExternalAgentProvider | string;
  adapter?: ExternalAgentObservationAdapter;
  output?: string;
  ...
  return externalAgentObservationEvents(args);
}

function roleFallbackActivity(event: ExternalAgentObservationEvent): ExternalAgentObservationActivity | undefined {
...
export function classifyExternalAgentActivity(
  event: ExternalAgentObservationEvent,
  opts: { provider?: ExternalAgentProvider | string; adapter?: ExternalAgentObservationAdapter } = {}
...
export function externalAgentEventsAreGenerating(
  events: readonly ExternalAgentObservationEvent[],
  opts: { provider?: ExternalAgentProvider | string; adapter?: ExternalAgentObservationAdapter } = {}
): boolean {
  let active = false;
  for (const event of events) {
    const kind = classifyExternalAgentActivity(event, opts);
...
function mergeUsageRows(
  existing: ExternalAgentUsageLimitMeterRow[],
  next: ExternalAgentUsageLimitMeterRow[]
): ExternalAgentUsageLimitMeterRow[] {
...
export function externalAgentUsageLimitMeter(args: {
  output?: string;
  provider?: ExternalAgentProvider | string;
  adapter?: ExternalAgentObservationAdapter;
  ...
  if (!text) return null;
  const adapterObservation = observationAdapter(args)?.observation;
  const rows = jsonRecordEntries(text).reduce<ExternalAgentUsageLimitMeterRow[]>((acc, entry) => {
...
export function externalAgentUsageLimitMeterFromResponse(
  usage: ExternalAgentUsageResponse | undefined
): ExternalAgentUsageLimitMeter | null {
