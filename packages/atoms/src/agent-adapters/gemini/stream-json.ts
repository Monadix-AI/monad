...
// wedging the parse (schema-first at the runtime boundary; see docs/internal/development/conventions.md §3).
...
function accumulatorFor(context: object | undefined): StreamJsonAccumulator {
  if (context) {
    return {
      get: () => turnTextByHandle.get(context) ?? '',
      append: (text) => turnTextByHandle.set(context, (turnTextByHandle.get(context) ?? '') + text),
      reset: () => turnTextByHandle.delete(context)
    };
  }
...
export function parseGeminiStreamJson(chunk: string, context?: object): MeshAgentOutputEvent[] {
  const acc = accumulatorFor(context);
...
export function createGeminiStreamJsonParser(): (chunk: string) => MeshAgentOutputEvent[] {
  const context = {};
  return (chunk) => parseGeminiStreamJson(chunk, context);
}
