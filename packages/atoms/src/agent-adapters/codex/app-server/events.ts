...
export interface CodexAppServerEventContext {
  appServer?: {
...
type CodexNotificationHandler = (
  ...
  handle?: CodexAppServerEventContext
...
const CODEX_SERVER_NOTIFICATION_HANDLERS: Record<string, CodexNotificationHandler> = {
  ...
  'item/agentMessage/delta': (_record, p) =>
    typeof p.delta === 'string' ? [{ type: 'agent_message', payload: { text: p.delta } }] : [],
  'thread/tokenUsage/updated': (record) => {
    const usage = codexSessionUsageFromNotification(record);
    return usage ? [{ type: 'session_usage_updated', payload: usage }] : [];
  },
  'turn/completed': (_record, p) => {
    const text = codexTurnFinalText(p.turn) || stringValue(p.text, p.result, p.outputText);
    return [{ type: 'agent_message', payload: { text, final: true } }];
  },
...
export function parseCodexSessionJsonl(chunk: string, handle?: CodexAppServerEventContext): MeshAgentOutputEvent[] {
