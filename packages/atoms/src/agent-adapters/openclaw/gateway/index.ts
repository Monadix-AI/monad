...
function sendConnect(handle: MeshAgentRuntimeHandle, state: OpenClawConnectState, nonce: string): void {
  if (!handle.gateway) return;
  ...
  try {
    void Promise.resolve(handle.gateway.send(frame)).catch(() => undefined);
  } catch {
    return;
  }
}
...
function handleConnectChallenge(
  ...
  state.nonce = nonce;
  sendConnect(handle, state, nonce);
  return [];
}
...
function responseEvents(frame: OpenClawEnvelope, handle?: MeshAgentRuntimeHandle): MeshAgentOutputEvent[] {
  ...
  if (frame.ok === false) {
    ...
    if (kind === 'initialize' && errorInfo?.retryable === true && handle?.gateway) {
      const state = connectStates.get(handle);
      if (state?.nonce && state.retryCount < 5) {
        state.retryCount += 1;
        state.connectId = frameId(handle);
        handle.pendingRequests?.set(state.connectId, 'initialize');
        const retryAfterMs =
          typeof errorInfo.retryAfterMs === 'number' && Number.isFinite(errorInfo.retryAfterMs)
            ? Math.max(0, Math.min(errorInfo.retryAfterMs, 2_000))
            : 500;
        setTimeout(() => {
          if (connectStates.get(handle) === state && state.nonce) sendConnect(handle, state, state.nonce);
        }, retryAfterMs);
        return [];
      }
    }
...
export function parseOpenClawFrame(frame: OpenClawEnvelope, handle?: MeshAgentRuntimeHandle): MeshAgentOutputEvent[] {
  ...
  if (frame.type === 'event' && typeof frame.event === 'string') {
    ...
    if (handle?.providerSessionRef && typeof sessionKey === 'string' && sessionKey !== handle.providerSessionRef)
      return [];
    return eventFrameEvents(frame.event, payload);
  }
  return [];
}
...
export function steerOpenClawInput(handle: MeshAgentRuntimeHandle, input: string): void {
  ...
  handle.gateway.send(req('sessions.steer', id, { key: handle.providerSessionRef, message: input }));
}

export function interruptOpenClaw(handle: MeshAgentRuntimeHandle): void {
  if (!handle.gateway) throw new Error('MeshAgent session has no OpenClaw gateway input bridge');
  if (!handle.providerSessionRef) throw new Error('OpenClaw gateway session is not ready');
  const id = frameId(handle);
  handle.pendingRequests?.set(id, 'interrupt');
  handle.gateway.send(req('sessions.abort', id, { key: handle.providerSessionRef }));
}
