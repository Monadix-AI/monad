import type { Message, Participant } from '../../src/workplace-experiences/experience/types.ts';

import { expect, test } from 'bun:test';

import { shouldJumpToOwnMessage } from '../../src/workplace-experiences/chat-room/components/message-list.tsx';
import {
...
test('server echoes match optimistic messages only when the reply relation matches', () => {
  const optimistic = createOptimisticUserMessage({
  ...
  const wrongTargetEcho: Message = {
    ...optimistic,
    id: 'msg_wrong00000000',
    localStatus: undefined,
    orderKey: '2026-07-21T00:00:01.000Z',
    replyToMessageId: 'msg_other00000000',
    retrySend: undefined
  };
  const matchingEcho: Message = {
    ...wrongTargetEcho,
    id: 'msg_match00000000',
    replyToMessageId: 'msg_target0000000'
  };

  expect(mergeOptimisticMessages([wrongTargetEcho], [optimistic]).map((message) => message.id)).toEqual([
    'optimistic-reply',
    'msg_wrong00000000'
  ]);
  expect(mergeOptimisticMessages([matchingEcho], [optimistic])).toEqual([
    { ...matchingEcho, renderKey: 'optimistic-reply' }
  ]);
});
