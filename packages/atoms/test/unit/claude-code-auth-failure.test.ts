...
import { parseClaudeStreamJson } from '../../src/agent-adapters/claude-code/index.ts';
...
test('maps a top-level authentication_failed stream event to connection_required', () => {
  const events = parseClaudeStreamJson(`${authFailureLine}\n`);
...
test('maps a non-authentication error result to a terminal provider error', () => {
  const line = JSON.stringify({
    type: 'result',
    subtype: 'error_during_execution',
    is_error: true,
    result: 'The provider rejected this turn',
    permission_denials: []
  });
  const events = parseClaudeStreamJson(`${line}\n`);
  expect(events).toEqual([
    {
      type: 'provider_error',
      payload: { code: 'error_during_execution', message: 'The provider rejected this turn' }
    }
  ]);
});
