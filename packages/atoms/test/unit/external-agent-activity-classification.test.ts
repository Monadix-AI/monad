...
test('generating is derived generically from the adapter classification (codex + claude)', () => {
  const codexInFlight = externalAgentStreamItems({
    id: 'c',
    provider: 'codex',
    output: [
  ...
  const codexSettled = externalAgentStreamItems({
    id: 'c',
    provider: 'codex',
    output: [
      '{"method":"turn/started","params":{"turn":{}}}',
  ...
  expect(externalAgentEventsAreGenerating(codexInFlight, { provider: 'codex' })).toBe(true);
  expect(externalAgentEventsAreGenerating(codexSettled, { provider: 'codex' })).toBe(false);

  const claudeSettled = externalAgentStreamItems({
    id: 'c',
  ...
  expect(externalAgentEventsAreGenerating(claudeSettled, { provider: 'claude-code' })).toBe(false);
});

test('a lone user input is not generating, but does not clear an in-flight turn', () => {
  const loneUser = externalAgentStreamItems({
    id: 'c',
    provider: 'codex',
    output: '{"items":[{"id":"u1","text":"hi","type":"userMessage","createdAtMs":1783332000000}]}'
  });
  expect(externalAgentEventsAreGenerating(loneUser, { provider: 'codex' })).toBe(false);

  const toolResultMidTurn = externalAgentStreamItems({
    id: 'c',
    provider: 'claude-code',
    output: [
  ...
  expect(externalAgentEventsAreGenerating(toolResultMidTurn, { provider: 'claude-code' })).toBe(true);
});
