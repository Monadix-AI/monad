import type { ExternalAgentProvider, ExternalAgentRawHistoryPage } from '@monad/protocol';

import { expect, test } from 'bun:test';
import { externalAgentRawHistoryPageSchema } from '@monad/protocol';

import { builtinAgentAdapters } from '../../src/agent-adapters/index.ts';

interface RealObservationFixture {
  provider: ExternalAgentProvider;
  page: ExternalAgentRawHistoryPage;
}
...
async function fixture(provider: 'codex' | 'claude-code'): Promise<RealObservationFixture> {
  const raw = (await Bun.file(
    new URL(`../fixtures/external-agent-observation/${provider}.raw.json`, import.meta.url)
  ).json()) as Record<string, unknown>;
  return {
    provider: raw.provider as ExternalAgentProvider,
    page: externalAgentRawHistoryPageSchema.parse(raw.page)
  };
}
