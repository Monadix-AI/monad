...
import { meshAgentStreamItems } from '../../src/workplace-experiences/experience/mesh-agent-observation/mesh-agent-observation.ts';
...
test('registry resolves codex/gemini/qwen provider output structurally', () => {
  ...
  expect(codex).toEqual([
    {
      id: 'mesh_c00000000000:json:0:reasoning-delta',
      role: 'agent',
      text: 'Inspecting.',
      source: 'codex-app-server',
      providerEventType: 'item/reasoning/textDelta',
      provenance: {
        rawEvents: [{ method: 'item/reasoning/textDelta', params: { delta: 'Inspecting.' } }]
      }
    }
  ]);
});
