...
import { externalAgentNeutralStreamItems } from '../../src/workspace-experiences/experience/external-agent-observation/external-agent-observation.ts';

test('projects a codex turn into neutral events: assistant-message + a completed turn-end', () => {
  const events = externalAgentNeutralStreamItems({
    id: 'c',
    provider: 'codex',
    output: [
...
test('plain-text output with no adapter projects to assistant-message', () => {
  const events = externalAgentNeutralStreamItems({ id: 'c', output: 'just some text' });
