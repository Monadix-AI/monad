...
import { createTelegramAdapter, normalizeTelegramMessage, telegramApiBaseUrl } from '../../src/channels/telegram.ts';

test('Telegram API base normalization removes arbitrarily long trailing slash runs in linear time', () => {
  expect(telegramApiBaseUrl(`https://api.telegram.org${'/'.repeat(100_000)}`)).toBe('https://api.telegram.org');
});

function fakeContext(): ChannelContext {
  return {
    config: { type: 'telegram' } as ChannelContext['config'],
    log: () => {},
    onMessage: () => {},
