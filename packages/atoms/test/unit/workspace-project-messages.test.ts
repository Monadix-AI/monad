...
} from '../../src/workplace-experiences/chat-room/utils/optimistic-messages.ts';
import {
  __workplaceProjectMessageTest,
  messageToView
} from '../../src/workplace-experiences/chat-room/utils/projection.ts';
import { configureMeshAgentObservationAdapterResolver } from '../../src/workplace-experiences/experience/mesh-agent-observation/mesh-agent-observation.ts';
import { projectMemberParticipants } from '../../src/workplace-experiences/experience/project-projection.ts';
...
test('Monad messages generate an avatar with the active style like other built-in agents', () => {
  const projected = messageToView(
    {
      kind: 'message',
      id: 'msg_MONAD_AVATAR0',
      role: 'assistant',
      parts: [{ type: 'text', text: 'Ready.' }],
      replyable: true,
      status: 'done',
      seq: '2026-07-22T00:00:00.000Z'
    },
    '',
    new Map(),
    new Map(),
  ...
  expect(projected).toMatchObject({
    authorName: 'Monad',
    avatarUrl: __workplaceProjectMessageTest.entityAvatarUrl('agent:Monad', 'bottts-neutral'),
    icon: 'monad',
    tag: 'AI'
  });
});

test('project command directives keep user input in a bubble and collapse the system reply into details', () => {
  const command = messageToView({
    kind: 'message',
    id: 'msg_COMMAND_USER0',
    role: 'user',
    parts: [{ type: 'artifact', messageType: 'directive', text: '/sessions' }],
    replyable: false,
    status: 'done',
    seq: '2026-08-06T04:27:34.283Z'
  });
  const reply = messageToView({
    kind: 'message',
    ...
    replyable: false,
    status: 'done',
    seq: '2026-08-06T04:27:34.289Z'
  });

  expect([command, reply]).toEqual([
    {
      id: 'msg_COMMAND_USER0',
      authorId: 'me',
      authorName: 'Operator',
      av: 'ME',
      avatarUrl: __workplaceProjectMessageTest.entityAvatarUrl('user:Operator'),
      icon: undefined,
      kind: 'human',
      tag: 'User',
      time: '',
      text: '/sessions',
      replyable: false,
      orderKey: '2026-08-06T04:27:34.283Z',
      streaming: false
    },
    {
      id: 'msg_COMMAND_REPLY',
      ...
      time: '',
      text: '',
      orderKey: '2026-08-06T04:27:34.289Z',
...
test('project message projection preserves structured question presentation', () => {
  const question = { question: 'Which path?', options: ['Ship', 'Revise'] };
...
test('project message projection preserves reply eligibility and the selected target', () => {
  expect(
    messageToView({
      kind: 'message',
      id: 'msg_REPLY00000000',
      role: 'assistant',
      parts: [{ type: 'text', text: 'The current answer' }],
      replyToMessageId: 'msg_TARGET0000000',
      replyable: true,
      status: 'done',
      seq: '2026-07-21T00:00:01.000Z'
    })
  ).toMatchObject({
    id: 'msg_REPLY00000000',
    replyToMessageId: 'msg_TARGET0000000',
...
test('direct-message artifacts project to system events with current display names and complete text detail', () => {
  const projected = messageToView(
    {
      kind: 'message',
      id: 'msg_DM_EVENT0000',
...
test('MeshAgent structured result output is projected as readable observation items', () => {
      ...
      [
        {
          ...
          output: JSON.stringify({
            type: 'result',
            subtype: 'success',
            result: '仍被拦截，无法继续。卡点未变：shell 命令需要你批准。',
            permission_denials: [
              {
                tool_name: 'Bash',
                tool_input: {
  ...
  expect(observationFields(stream.items)).toEqual([
    { id: 'mesh_structuryOpn:result', text: '仍被拦截，无法继续。卡点未变：shell 命令需要你批准。' },
    { id: 'mesh_structuryOpn:denial:0', text: 'Permission blocked Bash: git status' }
  ]);
});
...
test('MeshAgent stream-json events are projected as readable observation items', () => {
  ...
  expect(observationFields(stream.items)).toEqual([
    ...
    // `tool_result` carries the `toolu_1` it answers, so it folds into the call above instead of
    // rendering as an orphan item.
...
test('MeshAgent projection ignores startup prose before stream-json objects', () => {
  const output = [
    'started claude-code in /Users/test/.monad/workplace/project/claude-code',
    ...
    JSON.stringify({
      ...
      params: {
        thread: {
          ...
          cwd: '/Users/test/project/.dev/.monad/workplace/project/test'
        }
      }
    }),
    JSON.stringify({
...
test('optimistic user messages stay after their transcript anchor while newer agent messages arrive', () => {
  const before: Message = {
    ...
    time: '',
    text: 'before',
    orderKey: '2026-07-23T01:20:39.900Z'
  };
  const after: Message = {
    ...before,
    id: 'after',
    text: 'after',
    orderKey: '2026-07-23T01:20:39.969Z'
  };
  const optimistic = createOptimisticUserMessage({
    createdAt: '2026-07-23T01:20:40.000Z',
    human: { id: 'human', name: 'Human', av: 'H', kind: 'human', presence: 'online', tag: 'YOU' },
    id: 'optimistic:user',
    precedingMessageId: before.id,
    retry: () => {},
    status: 'sending',
    text: 'user message'
  });

  expect(mergeOptimisticMessages([before, after], [optimistic]).map((message) => message.id)).toEqual([
    'before',
    'optimistic:user',
    'after'
  ]);
...
test('server echoes replace anchored optimistic messages without comparing client and daemon clocks', () => {
  const before: Message = {
    id: 'before',
    authorId: 'agent',
    authorName: 'Agent',
  ...
  const serverEcho: Message = {
    ...
    time: '',
    text: 'user message',
    orderKey: '2026-07-23T01:20:39.945Z'
  };
  const optimistic = createOptimisticUserMessage({
    createdAt: '2026-07-23T01:20:40.000Z',
    human: { id: 'human', name: 'Human', av: 'H', kind: 'human', presence: 'online', tag: 'YOU' },
  ...
  expect(mergeOptimisticMessages([before, serverEcho], [optimistic])).toEqual([
    before,
    { ...serverEcho, renderKey: 'optimistic:user' }
  ]);
});
