...
test('managed Codex pre-approves every Monad communication tool', () => {
  ...
  expect(args.filter((arg) => arg.startsWith('mcp_servers.monad.tools.'))).toEqual([
    ...
    'mcp_servers.monad.tools.project_plan_list.approval_mode="approve"',
    'mcp_servers.monad.tools.project_plan_add.approval_mode="approve"',
    'mcp_servers.monad.tools.project_plan_update.approval_mode="approve"',
    'mcp_servers.monad.tools.project_plan_delete.approval_mode="approve"'
  ]);
});
