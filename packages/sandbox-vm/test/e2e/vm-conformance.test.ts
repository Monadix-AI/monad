...
//
// This is the launcher's admission gate: a backend
...
describe.skipIf(!ENABLED)('net:none confinement (fs escape, credential, privilege)', () => {
  ...
  test('guest overlays hide denied paths', async () => {
    const r = await sh(`cat ${guestArg(join(deniedDir, 'id_ed25519'))} 2>/dev/null || echo DENIED`, NET, AGENT);
  ...
  test('symlink escape: a link in the writable mount → host path does NOT grant host write', async () => {
    // Symlink inside an allowed dir pointing outside, then write through it.
  ...
  test('overlapping writable parent and readable child do not re-expose deny targets', async () => {
    const child = join(ws, 'overlap-child');
    const denied = join(child, '.ssh');
    await mkdir(denied, { recursive: true });
    await writeFile(join(denied, 'id_ed25519'), 'OVERLAP_PRIVATE_KEY');
    const policy: Policy = {
      writableRoots: [ws],
      readableRoots: [child],
    ...
    const result = await sh(`cat ${guestArg(join(denied, 'id_ed25519'))} 2>/dev/null || echo DENIED`, policy, AGENT);
