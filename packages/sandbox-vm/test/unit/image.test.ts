      ...
      artifacts: {
        applehv: {
          formats: {
            'raw.gz': {
              disk: {
                location: 'https://builds.coreos.fedoraproject.org/prod/fedora-coreos-applehv.aarch64.raw.gz',
                sha256: 'deadbeef',
      ...
      artifacts: {
        qemu: {
          formats: {
            'qcow2.xz': {
              disk: {
                location: 'https://builds.coreos.fedoraproject.org/prod/fcos-qemu.x86_64.qcow2.xz',
                sha256: 'beef'
              }
            }
          }
...
test('resolveImageArtifact pulls the hyperv vhdx.zip disk for a Windows target', async () => {
        ...
        artifacts: {
          hyperv: {
            formats: {
              'vhdx.zip': {
                disk: {
                  location: 'https://builds.coreos.fedoraproject.org/prod/fcos-hyperv.x86_64.vhdx.zip',
                  sha256: 'f00d'
                }
              }
            }
  ...
  expect(a.location).toBe('https://builds.coreos.fedoraproject.org/prod/fcos-hyperv.x86_64.vhdx.zip');
...
test('resolveImageArtifact pulls the qemu x86_64 qcow2.xz disk for a Linux target', async () => {
  ...
  expect(a.location).toBe('https://builds.coreos.fedoraproject.org/prod/fcos-qemu.x86_64.qcow2.xz');
...
test('resolveImageArtifact pulls the applehv aarch64 raw.gz disk from stream metadata', async () => {
  ...
  expect(a.location).toBe('https://builds.coreos.fedoraproject.org/prod/fedora-coreos-applehv.aarch64.raw.gz');
...
test('CoreOS artifact URLs reject alternate origins and credential confusion', () => {
  expect({
    official: trustedCoreOsArtifactUrl('https://builds.coreos.fedoraproject.org/prod/image.raw.gz'),
    alternate: trustedCoreOsArtifactUrl('https://attacker.example/image.raw.gz'),
    suffix: trustedCoreOsArtifactUrl('https://builds.coreos.fedoraproject.org.attacker.example/image.raw.gz'),
    credentials: trustedCoreOsArtifactUrl('https://builds.coreos.fedoraproject.org@attacker.example/image.raw.gz'),
    insecure: trustedCoreOsArtifactUrl('http://builds.coreos.fedoraproject.org/prod/image.raw.gz')
  }).toEqual({
    official: 'https://builds.coreos.fedoraproject.org/prod/image.raw.gz',
    alternate: null,
    suffix: null,
    credentials: null,
    insecure: null
...
test('ensureBaseImage uses fully verified cached metadata without refetching the stream', async () => {
  ...
  const destination = join(root, 'images', `${digest.slice(0, 16)}${target.ext}`);
