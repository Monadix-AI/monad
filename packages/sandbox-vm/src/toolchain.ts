...
const WINVM_HELPER_ASSET = `winvm-helper-${WINVM_HELPER_ARCH}.exe` as const;

async function resolveWinvmHelper(): Promise<string> {
  if (config.winvmHelperPath) return config.winvmHelperPath;
  try {
    return await resolveVendorAsset(WINVM_HELPER_ASSET);
  } catch (cause) {
    throw new Error(
      `vm toolchain: winvm-helper (${WINVM_HELPER_ASSET}) unavailable — run native/winvm-helper/build.sh (requires Go) or set sandbox.vm.winvmHelperPath: ${cause instanceof Error ? cause.message : String(cause)}`
    );
  }
}

async function resolveWindows(): Promise<ResolvedToolchain> {
  const helper = await resolveWinvmHelper();
...
export function vmToolchainMaybeAvailable(): boolean {
  ...
  if (process.platform === 'win32') {
    // Whether Hyper-V is enabled is the helper's own probe in prepare(). The helper itself is
    // obtainable on every Windows host — already on disk, or downloaded by prepare() from its
    // pinned release — so its absence is not a reason to report the backend unavailable. This
    // matches the behaviour from when the helper was committed to the repo and always existed.
    if (config.winvmHelperPath) return existsSync(config.winvmHelperPath);
    return true;
  }
  return false;
}
