...
const COREOS_ARTIFACT_HOST = 'builds.coreos.fedoraproject.org';
...
export function trustedCoreOsArtifactUrl(value: string): string | null {
  try {
    const url = new URL(value);
    if (
      url.protocol !== 'https:' ||
      url.hostname !== COREOS_ARTIFACT_HOST ||
      url.port !== '' ||
      url.username !== '' ||
      url.password !== ''
    ) {
...
export async function resolveImageArtifact(
  ...
  const location = trustedCoreOsArtifactUrl(disk.location);
  if (!location) throw new Error('vm image: CoreOS metadata returned an untrusted artifact URL');
  return { location, sha256: disk.sha256, uncompressedSha256: disk['uncompressed-sha256'] };
}
...
function validArtifact(value: unknown): value is ImageArtifact {
  ...
  return (
    ...
    trustedCoreOsArtifactUrl(artifact.location) !== null &&
