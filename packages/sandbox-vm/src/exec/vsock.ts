...
export interface VsockReadinessOptions {
  ...
  vmmExited?: Promise<number>;
}

export async function waitForVsock(spec: VsockExecSpec, options: VsockReadinessOptions = {}): Promise<void> {
  if (!options.vmmExited) return waitForVsockReadiness(spec, options);
  const controller = new AbortController();
  try {
    return await Promise.race([
      waitForVsockReadiness(spec, options, controller.signal),
      options.vmmExited.then((code) => {
        throw new Error(`virtual machine exited with code ${code} before the guest vsock agent was ready`);
      })
    ]);
  } finally {
    controller.abort();
...
async function waitForVsockReadiness(
  spec: VsockExecSpec,
  options: VsockReadinessOptions,
  signal?: AbortSignal
): Promise<void> {
