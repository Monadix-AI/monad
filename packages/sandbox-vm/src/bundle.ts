...
export function bundleReuseMarker(reuse: string): string {
  const kind = reuse.startsWith('agt:') ? 'agt' : 'ses';
  const digest = new Bun.CryptoHasher('sha256').update(reuse).digest('hex').slice(0, 12);
  return `${kind}_${digest}_`;
}

export function safeKey(key: string): string {
  const separator = key.lastIndexOf('#');
  const reuse = separator >= 0 ? key.slice(0, separator) : key;
  const fingerprint = separator >= 0 ? key.slice(separator + 1) : '';
  const suffix = /^[a-f0-9]{16}$/.test(fingerprint)
    ? fingerprint
    : new Bun.CryptoHasher('sha256').update(key).digest('hex').slice(0, 16);
  return `${bundleReuseMarker(reuse)}${suffix}`;
}
