...
test('workplace experience agent identity resolver advances the host API major version', () => {
  expect(WORKPLACE_EXPERIENCE_API_VERSION).toBe(3);
  expect(isWorkplaceExperienceApiCompatible(2)).toBe(false);
  expect(isWorkplaceExperienceApiCompatible(3)).toBe(true);
});
