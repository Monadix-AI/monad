...
const HERMES_ICON_URL = new URL('../assets/product-icons/hermes.png', import.meta.url).href;
...
function ProductIconSvg({
  ...
  if (product === 'antigravity' || product === 'gemini' || product === 'gemini-cli') {
    return (
      <GeminiCliIcon
        background={background}
  ...
  if (product === 'monad') return <MonadIcon />;
  return (
    <CodexIcon
      gradientId={gradientId}
      title={title}
...
function MonadIcon(): React.ReactElement {
  return (
    <span
      style={{
...
function OpenClawIcon({ gradientId, title }: { gradientId: string; title: string }): React.ReactElement {
  const lobsterGradientId = `lobster-gradient-${gradientId}`;
  return (
    <svg
      height="100%"
      viewBox="0 0 120 120"
