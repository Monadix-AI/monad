...
test('UnifiedComposer preserves the editor minimum height when its parent contracts', () => {
  const markup = renderToStaticMarkup(<UnifiedComposer editor={<textarea aria-label="Message" />} />);
  const contentStyle = /<div class="chat-input-content" style="([^"]+)"/.exec(markup);

  expect(contentStyle?.slice(1)).toEqual(['min-height:56px;opacity:1']);
});
...
test('context usage remains informative and clickable before usage data is available', () => {
  ...
  expect(markup).toBe(
    '<fieldset aria-label="Message composer" style="border:0;flex-shrink:0;margin:0;min-block-size:var(--shared-composer-min-height, 104px);min-inline-size:0;padding:0"><div class="chat-input-chrome shared-composer-panel"><div class="chat-input-frame"><div class="chat-input-surface-frame"><div aria-hidden="true" class="chat-input-aurora"><div class="chat-input-aurora-root"><div class="chat-input-aurora-inner-glow"><div class="chat-input-aurora-glow-pulse"><div class="chat-input-aurora-edge-mask"><div class="chat-input-aurora-blur-field"><div class="chat-input-aurora-gradient"></div></div></div></div></div><div class="chat-input-aurora-border-pulse"><div class="chat-input-aurora-border-mask"><div class="chat-input-aurora-gradient"></div></div></div></div></div><div class="chat-input-surface composer-live-dense" role="presentation"><div class="chat-input-content" style="min-height:56px;opacity:1"><textarea aria-label="Message"></textarea></div></div></div><div class="shared-composer-accessory-rail" style="align-items:center;display:flex;gap:8px;justify-content:space-between;min-width:0"><div class="shared-composer-accessory shared-composer-accessory-left" style="align-items:center;display:inline-flex;gap:4px;min-width:0"></div><div class="shared-composer-accessory shared-composer-accessory-right" style="align-items:center;display:inline-flex;gap:4px;margin-left:auto;min-width:0"><button data-state="closed" data-slot="popover-trigger" type="button" aria-haspopup="dialog" aria-expanded="false" aria-label="Context usage" class="workplace-action" style="flex:none;width:32px;height:32px;border:none;border-radius:50%;background:transparent;color:var(--shared-composer-control-fg, var(--muted-foreground));cursor:pointer;display:flex;align-items:center;justify-content:center" title="Usage data is not available yet"><svg aria-hidden="true" height="18" viewBox="0 0 24 24" width="18"><circle cx="12" cy="12" fill="none" opacity="0.25" r="10" stroke="currentColor" stroke-width="2"></circle><circle cx="12" cy="12" fill="none" opacity="0.78" r="10" stroke="currentColor" stroke-dasharray="62.83185307179586 62.83185307179586" stroke-dashoffset="62.83185307179586" stroke-linecap="round" stroke-width="2" style="transform:rotate(-90deg);transform-origin:center"></circle></svg></button></div></div></div></div></fieldset>'
  );
});
