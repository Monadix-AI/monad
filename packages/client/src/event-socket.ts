...
// (see docs/internals/infra/realtime-channels.md), and all other request/response goes over REST.
...
export interface ControlSubscriptionOptions {
  onOpen?: () => void;
}
...
export class EventSocket {
  ...
  private readonly controlHandlers = new Map<EventHandler, ControlSubscriptionOptions>();
  ...
  subscribeControl(onEvent: EventHandler, options: ControlSubscriptionOptions = {}): () => void {
    ...
    this.controlHandlers.set(onEvent, options);
    if (first) this.openAndSubscribe();
    else if (this.ws?.readyState === WebSocket.OPEN) options.onOpen?.();
    return () => {
  ...
  private ensureOpen(): void {
    ...
    ws.addEventListener('open', () => {
      ...
      this.resubscribeAll();
      for (const { onOpen } of this.controlHandlers.values()) onOpen?.();
    });
  ...
  private onMessage(ev: MessageEvent): void {
    ...
    if (msg.method === 'sessions.event') {
      for (const handler of this.controlHandlers.keys()) handler(msg.params.event);
      return;
    }
