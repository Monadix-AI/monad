...
type ObservationHandler = (access: ExternalAgentObservationAccessResponse) => void;
 ...
 *  `externalAgentObservationAccessResponseSchema`); this accumulates them so every consumer reads a full
...
export function createExternalAgentObservationFolder(onObservation: ObservationHandler): ObservationHandler {
  let accumulated = '';
  let cursor = 0;
  return (access) => {
    ...
    if (typeof access.output === 'string') {
      ...
      if (seq > cursor) {
        ...
        if (accumulated.length > EXTERNAL_AGENT_OUTPUT_SNAPSHOT_MAX) {
          accumulated = accumulated.slice(accumulated.length - EXTERNAL_AGENT_OUTPUT_SNAPSHOT_MAX);
        }
        cursor = seq;
      }
    }
