...
export async function checkDaemonVersion(baseUrl: string, token?: string): Promise<VersionCheckResult> {
  const url = daemonHealthUrl(baseUrl);
  if (!url) {
    return {
      compatible: false,
      daemonVersion: 'unknown',
      clientVersion: CLIENT_VERSION,
      reason: 'daemon URL must use HTTP or HTTPS'
    };
  }
  ...
  if (isLoopbackHttpsUrl(url.toString())) (fetchOpts as BunFetchRequestInit).tls = { rejectUnauthorized: false };
