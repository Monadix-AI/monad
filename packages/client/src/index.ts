...
import { type ControlSubscriptionOptions, EventSocket } from './event-socket.ts';
import { createMonadTreaty, makeLoopbackHttpsFetcher, makeUnixFetcher } from './treaty.ts';

export type { ControlSubscriptionOptions } from './event-socket.ts';
...
export type MeshAgentStateFrameHandler = (frame: MeshAgentStateFrame) => void;
export type MeshAgentSessionUsageHandler = (usage: MeshAgentSessionUsage) => void;
...
export class MonadClient {
  ...
  subscribeControl(onEvent: EventHandler, options?: ControlSubscriptionOptions): () => void {
    return this.eventSocket().subscribeControl(onEvent, options);
  }
  ...
  async sendStreamable(
    ...
    const result = await this.treaty.v1
      ...
      .messages.post(
        ...
        { headers: { accept: 'text/event-stream' }, fetch: signal ? { signal } : undefined }
      );
  ...
  streamMeshAgentState(
    sessionId: SessionId,
    onFrame: MeshAgentStateFrameHandler,
    opts?: { afterEventId?: EventId; onOpen?: () => void; onError?: (err: StreamError) => void }
  ): () => void {
    return this.stream(
      `/${CONTROL_API_VERSION}/sessions/${encodeURIComponent(sessionId)}/mesh-state/stream`,
      meshAgentStateFrameSchema,
      onFrame,
      {
        ...opts,
        ...
        fatalStatuses: new Set([400, 401, 403, 404, 409, 410]),
        onInvalid: () => opts?.onError?.({ kind: 'fatal', cause: new Error('invalid mesh agent state frame') })
      }
    );
  }
  ...
  streamMeshAgentSessionUsage(
    id: string,
    transcriptTargetId: SessionId,
    onUsage: MeshAgentSessionUsageHandler,
    opts?: { onOpen?: () => void; onError?: (err: StreamError) => void }
  ): () => void {
    return this.stream(
      meshAgentObservationPath(id, 'usage/events', transcriptTargetId),
      meshAgentSessionUsageSchema,
      onUsage,
      opts
    );
   ...
   * generation streams (docs/internals/infra/realtime-channels.md). A frame for which `isTerminal` returns true ends
  ...
  stream<T>(
    ...
    opts?: {
      ...
      eventIdOf?: (value: T) => string | undefined;
      fatalStatuses?: ReadonlySet<number>;
    ...
    void (async () => {
      ...
      while (!controller.signal.aborted) {
        ...
        try {
          ...
          if ((opts?.fatalStatuses ?? FATAL_SSE_STATUS).has(response.status)) {
            ...
            if (reader) {
              ...
              const lastId = await readTypedSseStream(
                ...
                (value: T) => {
                  ...
                  if (resume) afterEventId = opts?.eventIdOf?.(value) ?? afterEventId;
                  onEvent(value);
                },
                {
