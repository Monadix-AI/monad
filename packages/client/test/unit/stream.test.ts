...
test('session usage stream uses the scoped runtime route and validates adapter fields', async () => {
  let capturedUrl = '';
  const c = clientWith(async (url, init) => {
  ...
  const usage = await new Promise<unknown>((resolve) => {
    const dispose = c.streamMeshAgentSessionUsage('mesh-1', 'ses_sessionusage01', (next) => {
      dispose();
      resolve(next);
    });
  });

  expect({ capturedUrl, usage }).toEqual({
    capturedUrl: 'http://127.0.0.1:1/v1/mesh/sessions/mesh-1/usage/events?transcriptTargetId=ses_sessionusage01',
    usage: { total: 30, input: 20, output: 10, context: { used: 12, window: 100 } }
  });
});
