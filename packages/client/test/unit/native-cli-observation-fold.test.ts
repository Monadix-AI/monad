...
import { createNativeCliObservationFolder } from '../../src/native-cli-observation-fold.ts';

function live(fields: { output?: string; append?: string; seq?: number }): NativeCliObservationAccessResponse {
  return {
    state: 'live',
...
function collect(): {
  push: (a: NativeCliObservationAccessResponse) => void;
  frames: NativeCliObservationAccessResponse[];
} {
  const frames: NativeCliObservationAccessResponse[] = [];
  return { push: (a) => frames.push(a), frames };
}

test('full output snapshot passes through and sets the cursor', () => {
  const { push, frames } = collect();
  const fold = createNativeCliObservationFolder(push);
...
test('the accumulator is bounded but the cursor keeps the true cumulative seq', () => {
  const { push, frames } = collect();
  const fold = createNativeCliObservationFolder(push);
  fold(live({ output: 'x'.repeat(NATIVE_CLI_OUTPUT_SNAPSHOT_MAX), seq: NATIVE_CLI_OUTPUT_SNAPSHOT_MAX }));
  const nextSeq = NATIVE_CLI_OUTPUT_SNAPSHOT_MAX + 3;
  fold(live({ append: 'yyy', seq: nextSeq }));
  const last = frames.at(-1);
  const output = last?.state === 'live' && typeof last.output === 'string' ? last.output : '';
  expect(output.length).toBe(NATIVE_CLI_OUTPUT_SNAPSHOT_MAX);
...
test('non-live frames pass through untouched', () => {
  const { push, frames } = collect();
  const fold = createNativeCliObservationFolder(push);
  const unavailable: NativeCliObservationAccessResponse = {
    state: 'unavailable',
    nativeCliSessionId: 'ncli_test',
    reason: 'exited'
  };
