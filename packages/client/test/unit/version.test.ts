...
import { CLIENT_VERSION, daemonHealthUrl, isLoopbackHttpsUrl, isVersionCompatible } from '../../src/version.ts';
...
test('daemon health URLs accept only HTTP transports and discard attacker-controlled query state', () => {
  expect({
    https: daemonHealthUrl('https://daemon.example/api?token=secret#fragment')?.toString(),
    http: daemonHealthUrl('http://127.0.0.1:52749')?.toString(),
    file: daemonHealthUrl('file:///etc/passwd'),
    javascript: daemonHealthUrl('javascript:alert(1)'),
    malformed: daemonHealthUrl('not a URL')
  }).toEqual({
    https: 'https://daemon.example/api/health',
    http: 'http://127.0.0.1:52749/health',
    file: null,
    javascript: null,
    malformed: null
  });
