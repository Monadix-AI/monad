...
export const UNIVERSAL_METHODS = {
  ...
  'sessions.attention.list': {
    http: { verb: 'GET', template: '/v1/sessions/attention' },
    query: listSessionAttentionQuerySchema,
    result: listSessionAttentionResponseSchema
  },
  'sessions.attention.consume': {
    http: { verb: 'POST', template: '/v1/sessions/:id/attention/consume' },
    path: idPath,
    body: consumeSessionAttentionRequestSchema,
    result: consumeSessionAttentionResponseSchema
  },
  // Durable session plan (P0-C), human/operator-facing. `sessionId`/`todoId` travel in the URL;
  // the body is the full logical request schema with those path fields omitted (never
  // redeclared) plus an optional client provenance `origin` hint — the wire never accepts an
  // `actor` or `projectMemberId` field, the daemon always derives the actor server-side.
  'sessions.plan.list': {
    http: { verb: 'GET', template: '/v1/sessions/:id/plan' },
    path: idPath,
    result: listSessionPlanResponseSchema
  },
  'sessions.plan.todo.add': {
    http: { verb: 'POST', template: '/v1/sessions/:id/plan/todos' },
    path: idPath,
    body: addSessionPlanTodoRequestSchema
      .omit({ sessionId: true })
      .extend({ origin: createOperationSourceHintSchema.optional() }),
    result: sessionPlanTodoResponseSchema
  },
  'sessions.plan.todo.update': {
    http: { verb: 'PATCH', template: '/v1/sessions/:id/plan/todos/:todoId' },
    path: { id: sessionIdSchema, todoId: sessionPlanTodoIdSchema },
    body: updateSessionPlanTodoRequestSchema
      .omit({ sessionId: true, todoId: true })
      .extend({ origin: createOperationSourceHintSchema.optional() }),
    result: sessionPlanTodoResponseSchema
  },
  'sessions.plan.todo.delete': {
    http: { verb: 'DELETE', template: '/v1/sessions/:id/plan/todos/:todoId' },
    path: { id: sessionIdSchema, todoId: sessionPlanTodoIdSchema },
    body: deleteSessionPlanTodoRequestSchema
      .omit({ sessionId: true, todoId: true })
      .extend({ origin: createOperationSourceHintSchema.optional() }),
    result: deleteSessionPlanTodoResponseSchema
  },
  'sessions.get': {
  ...
  'sessions.resolveUiMessages': {
    http: { verb: 'POST', template: '/v1/sessions/:id/ui-messages/resolve' },
    path: idPath,
    body: resolveUiMessagesRequestSchema,
    result: resolveUiMessagesResponseSchema
  },
  'sessions.branch': {
  ...
  'approvals.pending': {
    http: { verb: 'GET', template: '/v1/approvals/pending' },
    query: listPendingApprovalsQuerySchema,
    result: listPendingApprovalsResponseSchema
  },
  'approvals.list': {
  ...
  'graph.get': {
    http: { verb: 'GET', template: '/v1/graph' },
    query: optionalMemoryScopeQuerySchema,
    result: getGraphResponseSchema
  },
  'memory.laws.get': {
    http: { verb: 'GET', template: '/v1/memory/laws' },
    query: optionalMemoryScopeQuerySchema,
    result: getLawsResponseSchema
  },
  ...
  'mesh.session.usage': {
    http: { verb: 'GET', template: '/v1/mesh/sessions/:id/usage' },
    path: meshSessionPath,
    query: meshSessionScopeQuery,
    result: meshAgentSessionUsageResponseSchema
  },
  'mesh.session.input': {
  ...
  'mesh.usage.overview': {
    http: { verb: 'GET', template: '/v1/mesh/usage' },
    result: meshUsageOverviewResponseSchema
  },
...
// it streams over the SSE endpoint GET /v1/sessions/:id/events (see docs/internals/infra/realtime-channels.md).
