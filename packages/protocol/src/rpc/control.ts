...
export type { ResolveUiMessagesRequest, ResolveUiMessagesResponse } from '../ui.ts';
export type {
  ClarifyRespondRequest,
  ClarifyRespondResponse,
  ToolApproveRequest,
  ToolApproveResponse
} from './interaction-control.ts';
...
export const MESSAGE_ATTACHMENT_BINARY_MAX_BYTES = Math.floor(MESSAGE_ATTACHMENT_DATA_MAX / 4) * 3;
 ...
 * accepted from the body; the daemon fills `transport` server-side (never trusting the client for
 * it). No environment snapshot, open extension bag, or writableBy hint is accepted.
 */
export const createOperationSourceHintSchema = z.object({
  surface: sessionSurfaceSchema.optional(),
  client: z.string().max(200).optional(),
  clientVersion: z.string().max(100).optional()
});
export type CreateOperationSourceHint = z.infer<typeof createOperationSourceHintSchema>;
...
export const createSessionRequestSchema = z.object({
  ...
  /** Optional client-declared provenance hints (see createOperationSourceHintSchema). */
...
export const createAgentRequestSchema = z.object({
  ...
  memory: agentMemorySettingsSchema.default({
    enabled: true,
    advanced: true,
...
export const agentPromptSlotsSchema = z.object({
  agent: z.string(),
  user: z.string()
});
export type AgentPromptSlots = z.infer<typeof agentPromptSlotsSchema>;

export const getAgentPromptResponseSchema = z.object({
  prompt: z.string(),
  slots: agentPromptSlotsSchema
});
export type GetAgentPromptResponse = z.infer<typeof getAgentPromptResponseSchema>;

export const setAgentPromptRequestSchema = z
  .object({
    prompt: z.string().optional(),
    slots: agentPromptSlotsSchema.partial().optional()
  })
  .refine((value) => value.prompt !== undefined || value.slots !== undefined, {
    message: 'prompt or slots is required'
  });
...
export const sendMessageAttachmentSchema = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    name: z.string().min(1).max(500),
    mediaType: z.string().min(1).max(100),
    size: z.number().int().nonnegative().max(MESSAGE_ATTACHMENT_BINARY_MAX_BYTES),
    dataBase64: z.string().max(MESSAGE_ATTACHMENT_DATA_MAX)
  }),
  z.object({
...
export const agentSessionKindSchema = z.enum(['chat', 'project', 'monadix']);
export type AgentSessionKind = z.infer<typeof agentSessionKindSchema>;

export const listSessionsQuerySchema = offsetPaginationQuerySchema
  .extend({
  ...
  .refine((query) => query.kind === undefined || query.agentId !== undefined, {
    message: 'agentId is required when kind is provided',
    path: ['agentId']
  });
...
export const undoDeleteSessionResponseSchema = z.object({ undone: z.boolean() });
export type UndoDeleteSessionResponse = z.infer<typeof undoDeleteSessionResponseSchema>;
...
export const skillListItemSchema = z.object({
  ...
  modelTier: modelTierSchema.optional(), // capability tier of a `context: fork` skill
