...
// that user). See docs/internals/agent-runtime/memory.md.
...
export const optionalMemoryScopeQuerySchema = z
  .object({
...
export type OptionalMemoryScopeQuery = z.infer<typeof optionalMemoryScopeQuerySchema>;
...
// docs/internal/development/conventions.md §5 — the HTTP body schema derives from it via `.omit()`.
...
export const qdrantPreparationPhaseSchema = z.enum(['idle', 'downloading', 'ready', 'failed']);
export type QdrantPreparationPhase = z.infer<typeof qdrantPreparationPhaseSchema>;

export const qdrantPreparationStatusSchema = z.object({
  phase: qdrantPreparationPhaseSchema,
  assetName: z.string().nullable(),
  loadedBytes: z.number().int().nonnegative(),
  totalBytes: z.number().int().nonnegative().nullable(),
  error: z.string().nullable()
});
export type QdrantPreparationStatus = z.infer<typeof qdrantPreparationStatusSchema>;
...
export const memoryStatusResponseSchema = z.object({
  ...
  qdrant: z
    .object({
      phase: qdrantPhaseSchema,
      error: z.string().nullable(),
      installed: z.boolean(),
      preparation: qdrantPreparationStatusSchema
    })
    .optional(),
