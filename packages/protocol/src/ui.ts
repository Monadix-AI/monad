...
export const uiQuestionPresentationSchema = z.object({
  question: z.string().min(1),
  options: z.array(z.string()),
  answer: z.string().min(1).optional()
});
export type UIQuestionPresentation = z.infer<typeof uiQuestionPresentationSchema>;
...
export const resolveUiMessagesRequestSchema = z.object({
  messageIds: z.array(messageIdSchema).max(100)
});
export type ResolveUiMessagesRequest = z.infer<typeof resolveUiMessagesRequestSchema>;

export const resolveUiMessagesResponseSchema = z.object({
  items: z.array(uiMessageItemSchema)
});
export type ResolveUiMessagesResponse = z.infer<typeof resolveUiMessagesResponseSchema>;
...
const uiReplayCursorSchema = z.union([eventCursorSchema, eventIdSchema]);

export const uiSnapshotEventSchema = z.object({
  kind: z.literal('snapshot'),
  cursor: uiReplayCursorSchema.optional(),
  items: z.array(uiItemSchema),
  messageOutline: z.array(uiMessageOutlineItemSchema).optional(),
