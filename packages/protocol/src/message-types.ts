...
import { clarifyChoiceModeSchema, clarifyFormSchema, urlElicitationSchema } from './clarify.ts';
...
export interface MessageTypeDescriptor<D = unknown> {
  ...
  /** Whether a terminal, active message of this type can be replied to. */
  replyable?: boolean;
...
export const clarifyMessageDataSchema = z.object({
  requestId: requestIdSchema,
  question: z.string().min(1),
  options: z.array(z.string()),
  mode: clarifyChoiceModeSchema,
  allowOther: z.boolean(),
  form: clarifyFormSchema.optional(),
  urlElicitation: urlElicitationSchema.optional(),
  status: z.enum(['pending', 'answered', 'timed-out', 'cancelled'])
});
export type ClarifyMessageData = z.infer<typeof clarifyMessageDataSchema>;

export const BUILTIN_MESSAGE_TYPES: Record<string, MessageTypeDescriptor> = Object.freeze({
  text: { type: 'text', dataSchema: z.unknown(), fallbacks: ['text'], includeInContext: true, replyable: true },
  markdown: {
    type: 'markdown',
  ...
  clarify: {
    type: 'clarify',
    dataSchema: clarifyMessageDataSchema,
    fallbacks: ['data', 'text'],
    includeInContext: true,
    replyable: true
  },
  ...
  error: { type: 'error', dataSchema: z.unknown(), fallbacks: ['text'], includeInContext: false, replyable: false },
...
export function isReplyableMessage(message: Pick<ChatMessage, 'active' | 'stream' | 'type'>): boolean {
  return (
    message.active &&
    message.stream.status !== 'pending' &&
    message.stream.status !== 'streaming' &&
    resolveMessageType(message.type).replyable === true
  );
}
