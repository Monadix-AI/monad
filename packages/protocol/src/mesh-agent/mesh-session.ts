...
export const meshAgentSessionUsageSchema = z
  .object({
  ...
  .catchall(z.unknown());
export type MeshAgentSessionUsage = z.infer<typeof meshAgentSessionUsageSchema>;

export const meshAgentSessionUsageResponseSchema = z.object({
  usage: meshAgentSessionUsageSchema.nullable()
});
export type MeshAgentSessionUsageResponse = z.infer<typeof meshAgentSessionUsageResponseSchema>;
...
export type MeshSessionUsageSnapshot = z.infer<typeof meshSessionUsageSnapshotSchema>;

export const meshUsageOverviewResponseSchema = z.object({
  checkedAt: z.string(),
  providerUsage: z.array(meshAgentUsageResponseSchema),
  sessionUsage: z.array(meshSessionUsageSnapshotSchema)
});
export type MeshUsageOverviewResponse = z.infer<typeof meshUsageOverviewResponseSchema>;
...
export const meshSessionViewSchema = z.object({
  ...
  sessionId: sessionIdSchema,
  agentName: z.string(),
  // Canonical id, not a generic string, so malformed ids never enter the view. Optional because this is a
  // field added after MeshSessionView was already a multi-consumer wire type: `toView` always emits it, but
  // keeping it optional avoids breaking every other view constructor across packages that predates it.
  projectMemberId: projectMemberIdSchema.nullable().optional(),
  provider: meshAgentProviderSchema,
  productIcon: meshAgentProductIconSchema.optional(),
