...
import { agentIdSchema, iso8601Schema, meshSessionIdSchema, messageIdSchema, transcriptTargetIdSchema } from './ids.ts';
import { mcpServerStatusSchema } from './mcp-server.ts';
import { memoryScopeSchema } from './memory.ts';
import { agentSessionChangedPayloadSchema } from './mesh-agent/agent-session-state.ts';
...
export const sessionAttentionConsumedPayloadSchema = transcriptEventIdentitySchema.extend({
  itemKeys: z.array(z.string().min(1)),
  cause: z.enum(['open', 'visible'])
});

export const sessionAttentionUpdatedPayloadSchema = transcriptEventIdentitySchema;

export const workplaceProjectOrderUpdatedPayloadSchema = z.object({
...
export const meshCatalogUpdatedPayloadSchema = z.object({
  resources: z.array(z.enum(['agents', 'presets', 'invitable-agents'])).min(1),
...
export const clarifyRequestedPayloadSchema = z.object({
  ...
  urlElicitation: urlElicitationSchema.optional(),
...
export const clarifyResolvedPayloadSchema = z.object({
  ...
  status: z.enum(['answered', 'skipped', 'timed_out', 'cancelled']).optional(),
  questionMessageId: messageIdSchema,
  answerMessageId: messageIdSchema.optional(),
  reason: z.string().optional()
});

const legacyClarifyRequestedPayloadSchema = clarifyRequestedPayloadSchema.omit({ questionMessageId: true });
const legacyClarifyResolvedPayloadSchema = clarifyResolvedPayloadSchema.omit({ questionMessageId: true });
...
export const meshAgentConnectionRequiredPayloadSchema = z.object({
  ...
  authAgentName: z.string().min(1).optional(),
  provider: meshAgentProviderSchema,
...
// Canonical size bounds for every variable-length MeshAgent field that can enter a mesh-state frame.
// Bounding at the source event payload (and mirrored on the view schemas) makes a schema-valid frame's
// worst-case encoded size provably bounded, so no single oversized field or flood can balloon the frame
// past the SSE byte-bounded queue cap. Kept small because the conservative frame estimator assumes each
// UTF-16 unit can escape to up to 6 JSON bytes.
export const MESH_APPROVAL_TEXT_MAX = 1024;
export const MESH_APPROVAL_DATA_MAX_BYTES = 2048;
export const MESH_LOGIN_REASON_MAX = 1024;
export const MESH_AGENT_NAME_MAX = 256;
export const MESH_WORKING_PATH_MAX = 1024;
export const MESH_REF_MAX = 256;

export function meshApprovalDataWithinBudget(data: unknown): boolean {
  if (data === undefined) return true;
  try {
    return new TextEncoder().encode(JSON.stringify(data)).length <= MESH_APPROVAL_DATA_MAX_BYTES;
  } catch {
    return false; // unserializable (e.g. a cycle) — reject rather than risk an unbounded encode
  }
}
...
export const meshAgentLoginRequiredPayloadSchema = z.object({
  ...
  authAgentName: z.string().min(1).max(MESH_AGENT_NAME_MAX).optional(),
  provider: meshAgentProviderSchema,
...
export type SessionAttentionConsumedPayload = z.infer<typeof sessionAttentionConsumedPayloadSchema>;
export type SessionAttentionUpdatedPayload = z.infer<typeof sessionAttentionUpdatedPayloadSchema>;
export type WorkplaceProjectOrderUpdatedPayload = z.infer<typeof workplaceProjectOrderUpdatedPayloadSchema>;
...
export const EVENT_DEFINITIONS = {
  ...
  'agent.session.changed': defineEvent(agentSessionChangedPayloadSchema, 'control', 'transient'),
  ...
  'session.attention.consumed': defineEvent(sessionAttentionConsumedPayloadSchema, 'control', 'durable'),
  'session.attention.updated': defineEvent(sessionAttentionUpdatedPayloadSchema, 'control', 'transient'),
  'workplace.project.order_updated': defineEvent(workplaceProjectOrderUpdatedPayloadSchema, 'control', 'durable'),
  ...
  'mesh.catalog.updated': defineEvent(meshCatalogUpdatedPayloadSchema, 'control', 'transient'),
  ...
  'tool.approval_requested': defineEvent(toolApprovalRequestedPayloadSchema, 'both', 'durable'),
  'tool.approval_resolved': defineEvent(toolApprovalResolvedPayloadSchema, 'both', 'durable'),
  'clarify.requested': defineEvent(clarifyRequestedPayloadSchema, 'both', 'durable'),
  'clarify.resolved': defineEvent(clarifyResolvedPayloadSchema, 'both', 'durable'),
  ...
  'mesh.approval_requested': defineEvent(meshAgentApprovalRequestedPayloadSchema, 'both', 'durable'),
  'mesh.approval_resolved': defineEvent(meshAgentApprovalResolvedPayloadSchema, 'both', 'durable'),
  ...
  // Session-scoped plan state (P0-C). Control-plane only — applying a plan event must never wake or
  // schedule an agent; it is presentation/audit state for clients watching the session.
  'session.plan.todo_upserted': defineEvent(sessionPlanTodoUpsertedPayloadSchema, 'control', 'durable'),
  'session.plan.todo_removed': defineEvent(sessionPlanTodoRemovedPayloadSchema, 'control', 'durable')
...
/**
 * Read adapter for durable rows written before clarifications had canonical message links. Current
 * writes stay strict through parseEvent; only the two recognized legacy lifecycle shapes are
 * omitted from replay because they cannot be represented by today's wire contract.
 */
export function parsePersistedEvent(input: unknown): Event | null {
  const parsed = eventSchema.safeParse(input);
  if (parsed.success) return parsed.data;
  const envelope = eventEnvelopeSchema.parse(input);
  const missingQuestionMessageId = !Object.hasOwn(envelope.payload, 'questionMessageId');
  const legacy =
    missingQuestionMessageId && envelope.type === 'clarify.requested'
      ? legacyClarifyRequestedPayloadSchema.safeParse(envelope.payload)
      : missingQuestionMessageId && envelope.type === 'clarify.resolved'
        ? legacyClarifyResolvedPayloadSchema.safeParse(envelope.payload)
        : null;
