...
/** One high-risk tool call currently blocked on a human decision. `requestId` is what
 *  `tools.approve` resolves; it also rides on the `tool.approval_requested` event. */
...
export const listPendingApprovalsQuerySchema = z.object({ sessionId: z.string().optional() });
export type ListPendingApprovalsQuery = z.infer<typeof listPendingApprovalsQuerySchema>;

export const listPendingApprovalsResponseSchema = z.object({ pending: z.array(pendingApprovalSchema) });
