...
import {
  ...
  experienceFanoutRequestSchema,
  experienceProjectionEventSchema,
  updateWorkplaceProjectRequestSchema
} from '../src/workplace-project.ts';

test('project cwd belongs to the creation contract only', () => {
  expect(createWorkplaceProjectRequestSchema.parse({ title: 'Project', cwd: '/tmp/project' })).toEqual({
    title: 'Project',
    cwd: '/tmp/project'
  });
  expect(updateWorkplaceProjectRequestSchema.keyof().options).toEqual([
    'title',
    'state',
    'archived',
    'model',
...
test('experience fanout request keeps project membership separate from transport details', () => {
  const parsed = experienceFanoutRequestSchema.parse({
    ...
    recipients: [
      { participantId: 'mesh-agent:monad--agt_main', displayName: 'Monad', transport: 'mesh-agent' },
      {
  ...
  expect(parsed.recipients.map((recipient) => recipient.transport)).toEqual(['mesh-agent', 'mesh-agent', 'acp']);
  expect(experienceFanoutRequestSchema.safeParse({ ...parsed, recipients: [] }).success).toBe(false);
  expect(
    experienceFanoutRequestSchema.safeParse({
      ...parsed,
      recipients: [{ participantId: 'monad', transport: 'monad' }]
    }).success
  ).toBe(false);
});
