...
test('inbox read-control contracts parse unread batches and global read results', () => {
  const readControls = protocol as unknown as {
    markInboxUnreadRequestSchema?: { parse(input: unknown): unknown };
    markInboxUnreadResponseSchema?: { parse(input: unknown): unknown };
    markAllInboxReadResponseSchema?: { parse(input: unknown): unknown };
  };
  expect(readControls.markInboxUnreadRequestSchema?.parse({ itemKeys: ['approval:req_ABCDEF123456'] })).toEqual({
    itemKeys: ['approval:req_ABCDEF123456']
  });
  expect(readControls.markInboxUnreadResponseSchema?.parse({ itemKeys: ['approval:req_ABCDEF123456'] })).toEqual({
    itemKeys: ['approval:req_ABCDEF123456']
  });
  expect(readControls.markAllInboxReadResponseSchema?.parse({ readAt: '2026-07-22T00:00:00.000Z', count: 3 })).toEqual({
