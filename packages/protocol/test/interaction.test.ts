...
import * as interactionProtocol from '../src/interaction.ts';
import {
...
test('parses the exact InteractionProducer attribution contract', () => {
  const protocol = interactionProtocol as typeof interactionProtocol & {
    interactionProducerSchema?: { parse(value: unknown): unknown };
  };

  expect(
    protocol.interactionProducerSchema?.parse({
      kind: 'atom-pack',
      packId: 'vendor.pack',
      atomId: 'configure'
    })
  ).toEqual({ kind: 'atom-pack', packId: 'vendor.pack', atomId: 'configure' });
});
