...
test('reply policy is registry-owned and only terminal active messages are replyable', () => {
  expect(resolveMessageType('tool_call').replyable).toBe(false);
  expect(resolveMessageType('text').replyable).toBe(true);
  expect(resolveMessageType('clarify').replyable).toBe(true);
  expect(resolveMessageType('something_custom').replyable).toBe(false);
  expect(isReplyableMessage({ active: true, stream: { status: 'complete' }, type: 'text' })).toBe(true);
  expect(isReplyableMessage({ active: true, stream: { status: 'streaming' }, type: 'text' })).toBe(false);
  expect(isReplyableMessage({ active: false, stream: { status: 'complete' }, type: 'text' })).toBe(false);
  expect(isReplyableMessage({ active: true, stream: { status: 'complete' }, type: 'tool_call' })).toBe(false);
});

test('clarify message data requires its complete durable state', () => {
  expect(
    clarifyMessageDataSchema.parse({
  ...
  expect(() => clarifyMessageDataSchema.parse({ requestId: 'clarify_TEST00000' })).toThrow();
});

test('URL elicitation accepts secure completion metadata and rejects unsafe links', () => {
  const base = {
    requestId: 'clarify_TEST00000',
  ...
  expect(clarifyMessageDataSchema.parse(base)).toEqual(base);
  expect(() =>
    clarifyMessageDataSchema.parse({
      ...base,
      urlElicitation: { ...base.urlElicitation, url: 'javascript:alert(1)' }
    })
  ).toThrow();
});
