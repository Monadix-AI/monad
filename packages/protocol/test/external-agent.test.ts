...
} from '../src/external-agent/index.ts';
import { daemonHttpContract, httpErrorSchema } from '../src/http.ts';

test('external agent view requires provider-owned full-capability defaults', () => {
  const parsed = externalAgentViewSchema.parse({
    name: 'codex',
    provider: 'codex',
    command: 'codex',
...
test('external agent view accepts Gemini as a provider-owned external agent provider', () => {
  const parsed = externalAgentViewSchema.parse({
    name: 'gemini',
    provider: 'gemini',
    command: 'gemini',
...
test('external agent view accepts Qwen as a provider-owned external agent provider', () => {
  const parsed = externalAgentViewSchema.parse({
    name: 'qwen',
    provider: 'qwen',
    command: 'qwen',
...
test('external agent view carries project member templates defined on the CLI agent page', () => {
  const parsed = externalAgentViewSchema.parse({
    name: 'codex',
    provider: 'codex',
    command: 'codex',
...
test('external agent view rejects duplicate project member template ids', () => {
  expect(
    externalAgentViewSchema.safeParse({
      name: 'codex',
      provider: 'codex',
      command: 'codex',
      enabled: true,
...
test('external agent names are safe single path segments', () => {
  const validAgent = {
    name: 'Claude Code',
  ...
  expect(externalAgentViewSchema.safeParse(validAgent).success).toBe(true);
  expect(
    startExternalAgentRequestSchema.safeParse({ agentName: 'Claude Code', workingPath: '/tmp/project' }).success
  ).toBe(true);
  expect(workplaceProjectMembersExtSchema.safeParse([{ type: 'external-agent', name: 'Claude Code' }]).success).toBe(
    true
  );

  for (const unsafeName of ['../codex', '..\\codex', '.', '..', 'C:codex', 'codex/child', 'codex\\child', 'codex\0x']) {
    expect(externalAgentViewSchema.safeParse({ ...validAgent, name: unsafeName }).success).toBe(false);
    expect(
      startExternalAgentRequestSchema.safeParse({ agentName: unsafeName, workingPath: '/tmp/project' }).success
    ).toBe(false);
    expect(workplaceProjectMembersExtSchema.safeParse([{ type: 'external-agent', name: unsafeName }]).success).toBe(
      false
    );
  }
});

test('external agent preset view includes a provider install page', () => {
  const parsed = externalAgentPresetSchema.parse({
    id: 'codex',
    label: 'Codex',
    provider: 'codex',
    productIcon: 'codex',
...
test('external agent adapter settings are declared as text, switch, or closed select controls', () => {
  expect(
    externalAgentSettingSchema.parse({
      key: 'command',
      label: 'Command',
      kind: 'text',
      placeholder: 'codex'
  ...
  const preset = externalAgentPresetSchema.parse({
    id: 'codex',
    label: 'Codex',
    provider: 'codex',
    productIcon: 'codex',
  ...
  expect(
    externalAgentSettingSchema.safeParse({
      key: 'defaultLaunchMode',
      label: 'Launch mode',
      kind: 'select',
      options: []
...
test('external agent view carries adapter setting values separately from declarations', () => {
  const parsed = externalAgentViewSchema.parse({
    name: 'codex',
    provider: 'codex',
    command: 'codex',
...
test('external agent approval resolution request carries provider request id and decision', () => {
  expect(
    externalAgentApprovalResolutionRequestSchema.parse({ requestId: 'req_1', allow: true, reason: 'approved by user' })
  ).toEqual({ requestId: 'req_1', allow: true, reason: 'approved by user' });
});

test('start request requires an absolute working path', () => {
  expect(startExternalAgentRequestSchema.safeParse({ agentName: 'codex', workingPath: 'relative/path' }).success).toBe(
    false
  );
  expect(startExternalAgentRequestSchema.safeParse({ agentName: 'codex', workingPath: '/tmp/project' }).success).toBe(
    true
  );
  expect(
    startExternalAgentRequestSchema.parse({
      agentName: 'codex',
...
test('external agent usage response carries optional quota-style records', () => {
  const parsed = externalAgentUsageResponseSchema.parse({
    agentName: 'codex',
    provider: 'codex',
  ...
  expect(externalAgentUsageResponseSchema.safeParse({ ...parsed, records: [{ name: '', current: 1 }] }).success).toBe(
    false
  );
});
...
test('workplace project members ext schema is shared by web and daemon', () => {
  ...
  expect(workplaceProjectMembersExtSchema.safeParse([{ type: 'external-agent', name: '' }]).success).toBe(false);
  expect(
    workplaceProjectMembersExtSchema.safeParse([
      { type: 'external-agent', name: 'codex', settings: { launchMode: 'bad' } }
    ]).success
  ).toBe(false);
});

test('workplace external agent members can be instantiated multiple times from one template', () => {
  const parsed = workplaceProjectMembersExtSchema.parse([
    {
      type: 'external-agent',
      name: 'codex-reviewer',
...
test('external agent auth views model provider-owned login relay without project session fields', () => {
  ...
  expect(
    externalAgentAuthStatusResponseSchema.parse({
      agentName: 'claude-code',
      provider: 'claude-code',
...
const startReq = (workingPath: string) =>
  startExternalAgentRequestSchema.safeParse({ agentName: 'codex', workingPath, launchMode: 'pty' });

test('startExternalAgentRequest accepts POSIX absolute working paths', () => {
  expect(startReq('/home/user/project').success).toBe(true);
  expect(startReq('/').success).toBe(true);
});

test('startExternalAgentRequest accepts Windows absolute working paths', () => {
  expect(startReq('C:\\Users\\me\\project').success).toBe(true);
  expect(startReq('C:/Users/me/project').success).toBe(true);
  expect(startReq('\\\\server\\share\\project').success).toBe(true);
});

test('startExternalAgentRequest rejects relative working paths', () => {
