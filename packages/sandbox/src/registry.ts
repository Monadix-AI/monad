...
export function disposeSandboxSession(sessionId: string): void {
  for (const launcher of hostSandboxPlatform.launchers) {
    if (supportsPlatform(launcher, process.platform)) void launcher.disposeSession?.(sessionId);
  }
  for (const entry of entries.values()) {
    if (supportsPlatform(entry.launcher, process.platform)) void entry.launcher.disposeSession?.(sessionId);
  }
}
...
export function disposeSandboxAgent(agentId: string): void {
  for (const launcher of hostSandboxPlatform.launchers) {
    if (supportsPlatform(launcher, process.platform)) void launcher.disposeAgent?.(agentId);
  }
  for (const entry of entries.values()) {
    if (supportsPlatform(entry.launcher, process.platform)) void entry.launcher.disposeAgent?.(agentId);
  }
}

function supportsPlatform(launcher: SandboxLauncher, platform: NodeJS.Platform): boolean {
  return launcher.platforms === undefined || launcher.platforms.includes(platform);
}

function isCandidate(launcher: SandboxLauncher, platform: NodeJS.Platform): boolean {
  if (!supportsPlatform(launcher, platform)) return false;
...
export function selectSandboxLauncher(
  ...
  logger.warn(
    `monad: agent.sandbox.backend="${backend}" but no "${backend}" sandbox launcher is registered ` +
      "(are the providing Atom Pack's Atoms available?) — falling back to the light OS sandbox."
  );
  return selectAuto(platform);
}
