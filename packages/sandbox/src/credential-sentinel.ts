// Per-execution sentinel registry for credential masking (env-based).
//
...
import type { CredentialRedaction } from './credential-redaction-stream.ts';

import { randomUUID } from 'node:crypto';

import { normalizeHost } from './egress-policy.ts';
 ...
 * destination exactly matches THAT credential's host. This prevents laundering credential A through
...
export class SentinelRegistry {
   ...
   * whose `injectHosts` exactly match the normalized canonical `host`. A
  ...
  substitute(host: string, text: string): string {
    ...
    for (const c of this.byName.values()) {
      for (const substitution of c.substitutions) {
        ...
        if (!substitution.injectHosts.some((allowedHost) => normalizeHost(host) === normalizeHost(allowedHost)))
          continue;
  ...
  redactionsForHost(host: string): CredentialRedaction[] {
    const normalizedHost = normalizeHost(host);
    const redactions: CredentialRedaction[] = [];
    for (const credential of this.byName.values()) {
      for (const substitution of credential.substitutions) {
        if (!substitution.injectHosts.some((allowedHost) => normalizeHost(allowedHost) === normalizedHost)) continue;
        redactions.push({ secret: substitution.real, sentinel: substitution.fake });
      }
    }
    return redactions;
  }

  redactionsForResponse(): CredentialRedaction[] {
    const redactions: CredentialRedaction[] = [];
    for (const credential of this.byName.values()) {
      for (const substitution of credential.substitutions) {
        redactions.push({ secret: substitution.real, sentinel: substitution.fake });
