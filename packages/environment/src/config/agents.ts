...
export const sandboxConfigSchema = z
  .object({
    ...
    // Secret fields are stored raw beside the backend that owns them and redacted in settings views.
...
// credentials are stored directly beside their provider in agents.json.
...
export const providerSchema = z.object({
  ...
  credentials: z.array(providerCredentialSchema).default([])
});
...
// Stdio environment values may use environment references. HTTP credentials are stored directly
// beside the server that owns them and redacted in settings views.
...
const directMcpSecretSchema = z.string().refine((value) => !value.startsWith('${secret:'), {
  message: 'MCP credentials must be stored directly'
});

export const storedOAuthTokenSchema = z
  .object({
...
const mcpHttpAuthSchema = z
  .discriminatedUnion('mode', [
    ...
    z.object({ mode: z.literal('bearer'), token: directMcpSecretSchema }),
    z.object({ mode: z.literal('headers'), headers: z.record(z.string(), directMcpSecretSchema) }),
    z.object({
...
const mcpHttpServerSchema = z.object({
  ...
  headers: z.record(z.string(), directMcpSecretSchema).optional(),
  oauth: storedOAuthTokenSchema.optional(),
  requestTimeoutMs: z.number().int().positive().optional(),
  enabled: z.boolean().default(true),
  trust: mcpTrustSchema
});
...
// path and the provider (publish) path; the token lives in mesh.json. An operator-defined
// mcpServers entry named "monadix" takes precedence and this preset
...
// Fractions are of the active model's context limit. Full reference: docs/internals/agent-runtime/context-management.md.
...
export const monadAgentsConfigSchema = z.object({
  ...
  model: z.object({
    ...
    tierOverrides: z.record(z.string(), modelTierSchema).default({}),
