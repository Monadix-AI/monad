...
export const logAutoCleanupSchema = z.object({
  enabled: z.boolean().default(true),
  retentionDays: z.number().int().min(1).max(30).default(14)
});
export type LogAutoCleanup = z.infer<typeof logAutoCleanupSchema>;

const logsConfigSchema = z
  .object({
    autoCleanup: logAutoCleanupSchema.default({ enabled: true, retentionDays: 14 })
  })
  .default({ autoCleanup: { enabled: true, retentionDays: 14 } });

const directChannelCredentialSchema = z
  .object({
    token: z.string().min(1),
    extra: z.record(z.string(), z.string()).optional()
  })
  .strict()
  .refine((value) => !value.token.startsWith('${secret:'), {
    message: 'channel credential token must be stored directly'
  });

function legacyChannelOptions(options: Record<string, unknown>): Record<string, string> {
...
export const channelInstanceSchema = z
  .object({
    id: channelIdSchema,
    ...
    mapping: z
      .object({
        ...
        reset: z
          .object({ idleMinutes: z.number().int().positive().optional(), daily: z.boolean().optional() })
          .optional()
      })
      .default({ granularity: 'per-conversation' }),
    credential: directChannelCredentialSchema.optional(),
    rateLimitPerMin: z.number().int().positive().default(20)
  })
  .strict()
  .transform(({ options = {}, ...channel }) => {
    const extra = { ...legacyChannelOptions(options), ...channel.credential?.extra };
    return {
      ...channel,
      type:
        channel.type === 'whatsapp' && channel.credential !== undefined && typeof options.phoneNumberId === 'string'
          ? 'whatsapp-business'
          : channel.type,
      ...(channel.credential
        ? {
...
export const monadSystemConfigSchema = z.object({
  ...
  // Operator review of which packs may run a workplace experience in the web host. `evidence` (the
  // default) accepts a pack on its recorded install evidence; `allowlist` accepts only the packs
  // named here, whatever their evidence. `allow` also waives missing evidence under `evidence`, so
  // an operator who reviewed a pack by hand can admit it; `deny` always wins.
  atomExperienceReview: z
    .object({
      policy: z.enum(['evidence', 'allowlist']).default('evidence'),
      allow: z.array(z.string().min(1)).default([]),
      deny: z.array(z.string().min(1)).default([])
    })
    .strict()
    .default({ policy: 'evidence', allow: [], deny: [] }),
  atomRegistries: z
    .object({
      github: z
        ...
        .optional(),
      npm: z
        .object({ token: z.string().min(1).optional(), registry: httpUrlSchema.optional() })
        .strict()
        .optional()
    })
    .strict()
...
export function createDefaultConfig(displayName: string): MonadConfig {
  return {
    ...
    locale: 'en',
    atomPins: {},
    atomExperienceReview: { policy: 'evidence', allow: [], deny: [] },
    atomRegistries: {},
