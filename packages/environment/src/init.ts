...
export interface InitOptions {
  /** Human-readable local profile name. Defaults to $USER or "". */
  displayName?: string;
  /**
...
export async function initMonadHome(paths: MonadPaths, opts: InitOptions = {}): Promise<InitResult> {
  ...
  if (Bun.env.NODE_ENV !== 'development') {
    await Promise.all([
      Bun.write(join(paths.runtime, 'config.schema.json'), CONFIG_SCHEMA_CONTENT),
      Bun.write(join(paths.runtime, 'agents.schema.json'), AGENTS_SCHEMA_CONTENT),
      Bun.write(join(paths.runtime, 'mesh.schema.json'), MESH_SCHEMA_CONTENT),
  ...
  if (created) {
    await saveAll(paths, createDefaultConfig(displayName));
  }
