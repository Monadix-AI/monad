...
export function agentCredentialVaultDenyRoots(paths: Pick<MonadPaths, 'auth' | 'credentials'>): readonly string[] {
  return Object.freeze(Array.from(new Set([paths.auth, paths.credentials])));
}

export function mcpTaskJournalPath(paths: Pick<MonadPaths, 'dbDir'>, serverName: string): string {
  const safeName = serverName.replace(/[^a-zA-Z0-9_-]/g, '_').slice(0, 128) || 'server';
  return join(paths.dbDir, 'mcp-tasks', `${safeName}.json`);
}
