...
import { disableRemoteAccess, generateRemoteToken, getLanIp, getTailscaleIp } from '../../src/remote-access.ts';
...
describe('disableRemoteAccess', () => {
  test('sets enabled=false and clears the token', async () => {
    ...
    await saveAll(paths, {
      ...cfg,
      network: { ...cfg.network, remoteAccess: { enabled: true, token: generateRemoteToken() } }
    });
    await disableRemoteAccess(paths);

    const updated = await loadConfig(paths);
    expect(updated?.network.remoteAccess).toEqual({ enabled: false, token: null });
  });
