import type { AgentId, InstallSkillResponse } from '@monad/protocol';
...
export type SkillMutationTarget = { kind: 'workspace' } | { kind: 'agent'; agentId: AgentId };
export type InstallSkillArg = {
  source: string;
  consent?: boolean;
  overwrite?: boolean;
  ...
  endpoints: (builder) => ({
    installSkill: builder.mutation<InstallSkillResponse, InstallSkillArg>({
      ...
      invalidatesTags: (result) =>
        result?.needsConsent ? [] : ['InstalledSkills', 'Skills', 'SlashCommands', 'ImportInventory']
    })
  })
});
