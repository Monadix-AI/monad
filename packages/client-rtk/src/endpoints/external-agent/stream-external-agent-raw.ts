import type { ExternalAgentRawFrame, SessionId } from '@monad/protocol';
...
interface ExternalAgentRawStreamState {
  fatalError: boolean;
  frames: ExternalAgentRawFrame[];
  frameOffset: number;
}

const streamExternalAgentRawApi = sessionsApi.injectEndpoints({
  overrideExisting: true,
  endpoints: (builder) => ({
    streamExternalAgentRaw: builder.query<
      ExternalAgentRawStreamState,
    ...
    >({
      ...
      async onCacheEntryAdded(
        { id, transcriptTargetId, afterCursor },
        {
          ...
          updateCachedData: (fn: (draft: ExternalAgentRawStreamState) => void) => void;
          extra: unknown;
        }
      ) {
        ...
        try {
          ...
          dispose = clientOf({ extra }).streamExternalAgentRaw(
            id,
            transcriptTargetId,
            (frame) => {
...
export const { useStreamExternalAgentRawQuery } = streamExternalAgentRawApi;
