    ...
    treaty: {
      v1: {
        settings: {
          ...
          developer: {
            get: () =>
              resolve('getDeveloper', '$raw', {
                developerMode: false,
                logsDir: 'logs',
                logs: { autoCleanup: { enabled: true, retentionDays: 14 } }
              }),
            put: (body: unknown) => resolve('setDeveloper', '$raw', body, body),
            logs: {
              preview: {
                post: (body: unknown) => resolve('previewLogCleanup', '$raw', { files: 0, bytes: 0 }, body)
              },
              delete: () => resolve('clearLogs', '$raw', { filesCleared: 0, filesFailed: 0, bytesFreed: 0 })
            }
          },
          model: {
            default: {
...
test('workplace projects: list/create/update/delete use the typed treaty project routes', async () => {
  ...
  const client = fakeClient({
    ...
    listWorkplaceProjects: handler('$raw', async (arg) => {
      calls.push(['list', arg]);
      return { projects: deleted ? [] : [project], total: deleted ? 0 : 1, limit: 50, offset: 0, orderRevision: 0 };
    }),
  ...
  const listData = list.data as { orderRevision?: number; total?: number } | undefined;
  expect(listData && { orderRevision: listData.orderRevision, total: listData.total }).toEqual({
    orderRevision: 0,
    total: 1
  });
...
test('previewLogCleanup: posts the proposed policy and returns the exact impact', async () => {
  let written: unknown;
  const client = fakeClient({
    previewLogCleanup: handler('$raw', async (body) => {
      written = body;
      return { files: 4, bytes: 8192 };
    })
  });
  const store = createMonadStore({ client });

  const res = await dispatchEndpoint(store, 'previewLogCleanup', { enabled: true, retentionDays: 7 });

  expect(res.data).toEqual({ files: 4, bytes: 8192 });
  expect(written).toEqual({ enabled: true, retentionDays: 7 });
});

test('clearLogs: returns the exact cleanup result', async () => {
  const client = fakeClient({
    clearLogs: handler('$raw', async () => ({ filesCleared: 3, filesFailed: 1, bytesFreed: 4096 }))
  });
  const store = createMonadStore({ client });

  const res = await dispatchEndpoint(store, 'clearLogs');

  expect(res.data).toEqual({ filesCleared: 3, filesFailed: 1, bytesFreed: 4096 });
});

test('setDeveloper: refreshes cached developer settings after a policy update', async () => {
  let getCalls = 0;
  let written: unknown;
  let settings = {
    developerMode: false,
  ...
  const client = fakeClient({
    getDeveloper: handler('$raw', async () => {
      getCalls += 1;
      return settings;
    }),
    setDeveloper: handler('$raw', async (body) => {
      written = body;
      settings = {
        ...settings,
        logs: (body as { logs: typeof settings.logs }).logs
      };
      return settings;
    })
  });
  ...
  await dispatchEndpoint(store, 'getDeveloper');
  expect(getCalls).toBe(1);

  const res = await dispatchEndpoint(store, 'setDeveloper', {
  ...
  expect(written).toEqual({ logs: { autoCleanup: { enabled: true, retentionDays: 21 } } });
  expect(getCalls).toBe(2);
});
