...
import { prepareMemoryBackendApi } from '../../src/endpoints/memory/prepare-memory-backend.ts';
...
test('toError: preserves additive public error metadata', () => {
  const err = toError({
    status: 502,
    value: {
      ...
      code: 'BAD_GATEWAY',
      retryable: true,
      requestId: 'req_1234567890ab',
      details: { provider: 'openrouter', retryAfterMs: 250 }
    }
  });
  expect(err).toEqual({
    status: 502,
...
test('toError: rejects malformed additive metadata without losing the legacy error contract', () => {
  const err = toError({
    status: 500,
    value: {
...
test('createAgent: posts new agent and invalidates Agents cache', async () => {
  ...
  await store.dispatch(
    createAgentApi.endpoints.createAgent.initiate({
      name: 'New',
      capabilities: [],
      credentialIds: [],
...
function fakeMemoryClient(overrides: {
  ...
  prepareBackend?: (body: unknown) => Promise<void>;
      ...
      v1: {
        memory: {
          ...
          backend: {
            prepare: {
              post: async (body: unknown) => {
                if (overrides.prepareBackend) await overrides.prepareBackend(body);
                return ok({ ok: true });
              }
            },
...
test('prepareMemoryBackend: posts the backend and invalidates Memory cache', async () => {
  let statusCalls = 0;
  ...
  await store.dispatch(memoryApi.endpoints.getMemoryStatus.initiate(undefined));
  await store.dispatch(prepareMemoryBackendApi.endpoints.prepareMemoryBackend.initiate({ backend: 'mem0' }));
  await new Promise((resolve) => setTimeout(resolve, 0));

  expect(prepared).toEqual({ backend: 'mem0' });
