...
test('streamMeshAgentRaw: disposes immediately when its final subscriber leaves', async () => {
  let disposals = 0;
  const client = baseClient({
    streamMeshAgentRaw: () => () => {
      disposals++;
  ...
  const store = createMonadStore({ client });
  const subscription = dispatchEndpoint(store, 'streamMeshAgentRaw', {
    id: 'mesh_100000000000',
    transcriptTargetId: SESSION_ID
  });
  await subscription;
...
test('streamMeshAgentConvenience: disposes immediately when its final subscriber leaves', async () => {
  let disposals = 0;
  const client = baseClient({
  ...
  const store = createMonadStore({ client });
  const subscription = dispatchEndpoint(store, 'streamMeshAgentConvenience', {
    id: 'mesh_100000000000',
    transcriptTargetId: SESSION_ID
  });
  await subscription;
