...
test('import rejects an unknown subcommand without contacting the daemon', async () => {
  const registered = commands.find((c) => c.name === 'import');
  let calls = 0;
  const client = {
    treaty: {
      v1: {
  ...
  await expect(
    registered?.run({
      positionals: ['nonsense'],
      flags: { from: 'codex', path: '/tmp/x' },
      globals: { json: false, quiet: false, verbose: 0, yes: false, color: false },
