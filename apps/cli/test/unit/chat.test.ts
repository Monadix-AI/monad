...
import { isReplExitCommand } from '../../src/commands/chat.ts';
...
test('/exit and /quit leave the REPL; anything else is still a message', () => {
  for (const line of ['/exit', '/quit', ' /EXIT ', '/Quit']) {
    expect(isReplExitCommand(line)).toBe(true);
  }
  // A bare `exit` is text the user may want the agent to receive, and `/compact` belongs to the
  // daemon's slash-command namespace — neither may be swallowed by the client.
  for (const line of ['exit', 'quit', '/exiting', '/compact', 'please /exit', '']) {
    expect(isReplExitCommand(line)).toBe(false);
  }
});
