...
import { EXIT } from '../../src/commands/types.ts';
...
function context(flags: Record<string, unknown>, client: unknown): CommandContext {
  return {
    positionals: [sessionId],
    flags,
...
test('--until ends the stream on the named event and disposes the subscription', async () => {
  let disposals = 0;
  const client = {
    subscribeControl(onEvent: (event: Event) => void) {
      queueMicrotask(() => {
        onEvent(event('evt_200000000001', sessionId, 'session.run.started'));
        onEvent(event('evt_200000000002', sessionId, 'session.run.completed'));
      });
      return () => {
        disposals++;
      };
  ...
  await silenced(() => watch.run(context({ until: 'session.run.completed' }, client)));

  // Without this the command only ends on SIGINT, so `send --detach && watch` hangs in a script.
  expect(disposals).toBe(1);
});

test('--timeout elapsing with no matching event exits non-zero and still disposes', async () => {
  let disposals = 0;
  const client = {
    subscribeControl() {
      return () => {
  ...
  await expect(
    silenced(() => watch.run(context({ until: 'session.run.completed', timeout: 0.05 }, client)))
...
test('a non-positive --timeout fails before anything is subscribed', async () => {
  let subscribed = false;
  const client = {
    subscribeControl() {
      subscribed = true;
  ...
  await expect(watch.run(context({ timeout: 0 }, client))).rejects.toMatchObject({ code: EXIT.USAGE });
