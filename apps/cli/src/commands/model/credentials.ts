...
import { resolvePayload } from '../../lib/chat.ts';
import { t } from '../../lib/i18n.ts';
import { bold, cyan, dim, green, json, out, red } from '../../lib/output.ts';
import { requireTreatyData } from '../../lib/treaty.ts';
import { usageError } from '../types.ts';

// Secrets never leave the daemon: list shows only configured state, and add echoes just the new
// credential id — never the raw token the user passed in.
export const command: CommandDef = {
  name: 'credentials',
  aliases: ['creds'],
  synopsis: 'credentials <list|add|rm|test> <providerId> [json|-]',
  ...
  async run({ positionals: args, client }) {
        ...
        const status =
          c.lastStatus === 'ok'
            ...
            : c.lastStatus === 'error'
              ? red(c.lastStatus)
              : dim(c.lastStatus);
        out(cyan(c.id) + dim('  ') + bold(c.label) + dim(`  ${c.authType}  ${c.configured ? '••••' : '—'}  `) + status);
      }
      return;
    }
    ...
    switch (action) {
      case 'add': {
        const payload = providerId ? await resolvePayload(arg) : undefined;
        if (!providerId || !payload) throw usageError('usage: monad credential add <providerId> <json|->');
        const body = addCredentialBodySchema.parse(JSON.parse(payload));
