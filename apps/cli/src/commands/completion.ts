...
type Subcommands = Record<string, string[]>;

/** Second-level tokens, derived from each command's own `subcommands` declaration so the script and
 *  the dispatcher cannot drift. Lazy import avoids the index.ts ⇄ completion.ts cycle. */
async function subcommandMap(): Promise<Subcommands> {
  const { commands } = await import('./index.ts');
  return Object.fromEntries(
    commands.filter((c) => c.subcommands?.length).map((c) => [c.name, c.subcommands as string[]])
  );
}
...
function bash(words: string, SUBCOMMANDS: Subcommands): string {
...
function zsh(words: string, SUBCOMMANDS: Subcommands): string {
  const subCases = Object.entries(SUBCOMMANDS)
...
async function fish(): Promise<string> {
  const [tokens, SUBCOMMANDS] = await Promise.all([commandTokens(), subcommandMap()]);
...
export const command: CommandDef = {
  ...
  async run({ positionals }) {
    ...
    const [tokens, subs] = await Promise.all([commandTokens(), subcommandMap()]);
