...
export const command: CommandDef = {
  name: '__complete',
  hidden: true,
  synopsis: '__complete <sessions|agents|mesh-sessions|providers|config-keys>',
  ...
  async run({ positionals, client }) {
    ...
    try {
      if (type === 'sessions') {
        ...
        process.stdout.write(`${agents.map((a) => a.name).join('\n')}\n`);
      } else if (type === 'mesh-sessions') {
        const { sessions } = requireTreatyData(await client.treaty.v1.mesh.runtimes.get({ query: {} }));
        process.stdout.write(`${sessions.map((m) => m.id).join('\n')}\n`);
