...
/** Two independent switches per skill, both operator-owned config (`settings.skills`):
 *  `disabled` removes it entirely, `autoloadDisabled` keeps it `/name`-invocable but out of the
 *  model's context. `--autoload-only` picks the second, narrower one. */
async function setEnabled(
  ...
  if (!name) throw usageError('usage: monad skill enable|disable <name> [--autoload-only]');
  const current = requireTreatyData(await client.treaty.v1.settings.skills.get());
  const field = flags['autoload-only'] === true ? 'autoloadDisabled' : 'disabled';
  const listed = new Set(current[field]);
  if (enable) listed.delete(name);
  else listed.add(name);
  const updated = requireTreatyData(await client.treaty.v1.settings.skills.put({ [field]: [...listed] }));
  json(updated);
  out((enable ? green(t('cli.enabled')) : yellow(t('cli.disabled'))) + dim(`  ${name}`));
  if (!enable && field === 'autoloadDisabled') out(dim(t('cli.skills.stillInvocable', { name })));
}

export const command: CommandDef = {
  name: 'skill',
  group: 'configure',
  synopsis: 'skill <list|search|install|update|remove|enable|disable|new|validate> [arg]',
  subcommands: ['list', 'search', 'install', 'update', 'remove', 'enable', 'disable', 'new', 'validate'],
  description: 'manage skills (list, search, install, update, remove, enable, disable, new, validate)',
  descriptionKey: 'cli.cmd.skill.desc',
  flags: {
    scope: {
      type: 'string',
      description: 'list scope: runtime | global | atom-pack | agent',
      descriptionKey: 'cli.skills.flag.scope'
    },
    'autoload-only': {
      type: 'boolean',
      description: 'enable/disable only automatic model-context loading; keep /name invocation',
      descriptionKey: 'cli.skills.flag.autoloadOnly'
    }
  },
  async run({ positionals: args, flags, globals, client }) {
    ...
    if (sub === 'remove' || sub === 'rm') return remove(target, client);
    if (sub === 'enable' || sub === 'disable') return setEnabled(sub === 'enable', target, flags, client);
    if (sub && sub !== 'list') {
      out(t('cli.skills.usage'));
      return;
    }
