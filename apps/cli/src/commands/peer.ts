...
// the next daemon start. The token lands in mesh.json via `peer token`.
export const command: CommandDef = {
  name: 'peer',
  group: 'configure',
  synopsis: 'peer <list|add|token|enable|disable|rm> [arg]',
  subcommands: ['list', 'add', 'token', 'enable', 'disable', 'rm'],
  description: 'manage peer daemons for task delegation',
  descriptionKey: 'cli.cmd.peer.desc',
  flags: {
    label: { type: 'string', description: 'display label (peer add)', descriptionKey: 'cli.peer.flag.label' },
    agent: {
      type: 'string',
      description: 'default target agent on the peer (peer add)',
      descriptionKey: 'cli.peer.flag.agent'
    },
    id: {
      type: 'string',
      description: 'explicit peer id (peer add; default peer_<12-char-nanoid>)',
      descriptionKey: 'cli.peer.flag.id'
    }
  },
  async run({ positionals: args, flags, client }) {
    ...
    switch (action) {
      ...
      case 'token': {
        ...
        requireTreatyData(await peers({ id }).credential.put({ action: 'replace', value: token }));
