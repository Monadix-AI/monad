...
import type { AgentConfig, ModelRouter } from '#/agent/index.ts';
...
export function buildHandlers(
  ...
  opts?: {
    ...
    /** Override bounded history assembly for integration cases that exercise compacted prompts. */
    history?: AgentConfig['history'];
    /** Share an external store (so a memory service + the agent see the same messages/sessions). */
    store?: ReturnType<typeof createStore>;
    /** Share the daemon host-interaction service with tests that must claim and resolve a request. */
    interactions?: InteractionService;
    /** Override the agent-loop message repository for production-ingress boundary tests. */
    messageRepo?: AgentConfig['messageRepo'];
    ...
    /** Override L3 reads so transport tests can exercise scoped laws. */
    ...
    /** Dynamic workplace experience API route resolver for atom HTTP tests. */
    ...
    /** Dynamic workplace experience descriptors for atom HTTP tests. */
    getWorkplaceExperiences?: () => RegisteredWorkplaceExperience[];
    configManager?: ConfigAccess;
    logMaintenance?: LogMaintenanceService;
  }
) {
  ...
  return Object.assign(
    createDaemonHandlers({
      ...
      logMaintenance:
        opts?.logMaintenance ??
        new LogMaintenanceService({
          logsDir: modelDeps.paths.logs,
          captureDir: meshFixtureCaptureDirectory(modelDeps.paths)
        }),
      skills: opts?.skills ?? [],
      ...
      getLaws: opts?.getLaws ?? (async () => ({ laws: [] })),
      getUpgradeInfo: opts?.getUpgradeInfo,
      getWorkplaceExperienceApiHandler: opts?.getWorkplaceExperienceApiHandler,
      getWorkplaceExperienceApiRoute: opts?.getWorkplaceExperienceApiRoute,
      getExperienceWorkers: opts?.getExperienceWorkers,
      getWorkplaceExperiences: opts?.getWorkplaceExperiences,
      memoryPrepareBackend: opts?.memoryPrepareBackend ?? (async () => {}),
      memorySetBackend: opts?.memorySetBackend ?? (async () => {}),
...
// serves (docs/internals/infra/runtime.md). Looping a suite over TRANSPORTS and driving it through the uniform
