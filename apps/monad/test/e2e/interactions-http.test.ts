...
import { httpErrorSchema, type InteractionRequest, type InteractionResult } from '@monad/protocol';
...
test('atom interaction can be claimed and completed once over HTTP without echoing secrets', async () => {
  ...
  const invalidSubmissionBody = httpErrorSchema.parse(await invalidSubmission.json());
  if (!invalidSubmissionBody.requestId) throw new Error('canonical HTTP error omitted requestId');
  expect(invalidSubmission.headers.get('x-monad-request-id')).toBe(invalidSubmissionBody.requestId);
  expect({ ...invalidSubmissionBody, requestId: '<request-id>' }).toEqual({
  ...
  const duplicateBody = httpErrorSchema.parse(await duplicate.json());
  if (!duplicateBody.requestId) throw new Error('canonical HTTP error omitted requestId');
  expect(duplicate.headers.get('x-monad-request-id')).toBe(duplicateBody.requestId);
  expect({ ...duplicateBody, requestId: '<request-id>' }).toEqual({
