...
// (TCP loopback + Unix socket). Asserts CRUD works, persists to agents.json
...
function httpView() {
  return {
    ...
    auth: { mode: 'bearer' as const, token: { action: 'replace' as const, value: 'mcp-canary-token' } },
    enabled: true,
...
interface ServersBody {
  servers: {
    ...
    url?: string;
    auth?: { mode: string; token?: { configured: boolean } };
  }[];
}
...
async function runMcpServerCrud(call: Call, paths: MonadPaths): Promise<void> {
  ...
  expect(remote?.auth?.token).toEqual({ configured: true });
  expect(JSON.stringify(remote)).not.toContain('mcp-canary-token');

  // 4. persisted to agents.json
  expect((await loadConfig(paths))?.mcpServers.find((s) => s.name === 'fs')).toMatchObject({
    transport: 'stdio',
    command: 'npx'
  });
  const stored = await loadAll(paths);
  expect(stored?.mcpServers.length).toBe(2);
  expect(stored?.mcpServers.find((candidate) => candidate.name === 'remote')).toMatchObject({
    auth: { mode: 'bearer', token: 'mcp-canary-token' }
  });

  // 4a. metadata-only bearer update preserves the token, replacement overwrites it, and removal
  // retains the server with unauthenticated mode.
  expect(
    (
      await call('PUT', '/v1/settings/mcp-servers/remote', {
        server: {
          ...httpView(),
  ...
  expect((await loadAll(paths))?.mcpServers.find((candidate) => candidate.name === 'remote')).toMatchObject({
    url: 'https://mcp2.example.com/sse',
    auth: { mode: 'bearer', token: 'mcp-canary-token' }
  });
  ...
  expect(
    (
      await call('PUT', '/v1/settings/mcp-servers/remote', {
        server: {
          ...httpView(),
          auth: { mode: 'bearer', token: { action: 'replace', value: 'mcp-next-canary-token' } }
        }
      })
    ).status
  ).toBe(200);
  expect((await loadAll(paths))?.mcpServers.find((candidate) => candidate.name === 'remote')).toMatchObject({
    auth: { mode: 'bearer', token: 'mcp-next-canary-token' }
  });

  const removedCredential = await call('PUT', '/v1/settings/mcp-servers/remote', {
    server: {
      ...httpView(),
      auth: { mode: 'bearer', token: { action: 'remove' } }
    }
  });
  expect(removedCredential.status).toBe(200);
  res = await call('GET', '/v1/settings/mcp-servers/remote');
  expect(((await res.json()) as { server: { auth: unknown } }).server.auth).toEqual({ mode: 'none' });
  expect((await loadAll(paths))?.mcpServers.find((candidate) => candidate.name === 'remote')).toMatchObject({
    name: 'remote',
    auth: { mode: 'none' }
  });
  ...
  expect(
    (
      await call('PUT', '/v1/settings/mcp-servers/new-bearer', {
        server: {
          ...httpView(),
          name: 'new-bearer',
          auth: { mode: 'bearer' }
