...
test('a local development directory installs through the consent flow and records its revision', async () => {
  const preview = await mod.installAtomPack({ source: `local:${stagedDir}`, consent: false });
  expect(preview.needsConsent).toBe(true);
  expect(preview.atoms).toEqual(['channel', 'skill']);

  const installed = await mod.installAtomPack({ source: `local:${stagedDir}`, consent: true });
  expect(installed.name).toBe('wa');
  expect((await mod.listAtomPacks()).atomPacks.find((pack) => pack.name === 'wa')).toMatchObject({
    atoms: ['channel', 'skill'],
    sourceKind: 'local',
