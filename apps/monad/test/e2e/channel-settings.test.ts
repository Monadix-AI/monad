...
// that the bot token is persisted beside its channel but NEVER returned by list()/status().
...
async function runChannelCrud(call: Call, paths: MonadPaths): Promise<void> {
  ...
  res = await call('GET', `/v1/settings/channels/${CHANNEL_ID}/pairings`);
  expect(res.status).toBe(404);
  res = await call('POST', `/v1/settings/channels/${CHANNEL_ID}/pair`, { code: 'ABC123' });
  expect(res.status).toBe(404);
  res = await call('POST', `/v1/settings/channels/${CHANNEL_ID}/login`);
  expect(res.status).toBe(400);

  // 3. it lists back without any raw credential field
  ...
  expect(JSON.stringify(channels[0])).not.toContain('tok-initial');
  ...
  // 4. set the credential → persisted directly on the channel, not echoed
  res = await call('PUT', `/v1/settings/channels/${CHANNEL_ID}/credential`, {
    action: 'replace',
    value: { token: SECRET }
  });
  expect(res.status).toBe(200);
  ...
  expect(cfg?.channels.find((channel) => channel.id === CHANNEL_ID)?.credential?.token).toBe(SECRET);
  ...
  expect(cfg?.channels.find((channel) => channel.id === CHANNEL_ID)?.credential?.token).toBe('replacement-token');

  res = await call('PUT', `/v1/settings/channels/${CHANNEL_ID}/credential`, { action: 'remove' });
  expect(res.status).toBe(200);
  cfg = await loadConfig(paths);
  expect(cfg?.channels.find((channel) => channel.id === CHANNEL_ID)?.credential).toBeUndefined();
  res = await call('GET', '/v1/settings/channels');
  expect(((await res.json()) as ChannelsBody).channels[0]?.credentialConfigured).toBe(false);
  ...
  // 7. remove → gone, and auth.json remains unchanged
