...
import { CONTROL_API_VERSION, type InteractionProducer, type InteractionRequest } from '@monad/protocol';
...
test('a control subscriber sees host interaction events over the websocket', async () => {
  const interactions = new InteractionService({ createId: () => 'interaction-ws-1' });
  const app = createHttpTransport(buildHandlers(mockModel(), undefined, { interactions })).listen({
    hostname: '127.0.0.1',
    port: 0
