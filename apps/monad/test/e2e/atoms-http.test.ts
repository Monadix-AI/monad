...
import { emptyAuth, initMonadHome, loadAuth, loadConfig, saveAll, saveAuth } from '@monad/environment';
import { type AtomPackUpdateCheck, ModelProviderType } from '@monad/protocol';
...
test('GET checks a local update and confirmed POST installs it while preserving enabled state', async () => {
  await post('/v1/atoms/install', { source: `local:${stagedDir}`, consent: true });
  await post('/v1/atoms/wa/disable');
  await writeFile(
    join(stagedDir, 'atom-pack.json'),
    JSON.stringify({ name: 'wa', version: '2.0.0', sdkVersion: '0', atoms: ['channel'], entry: 'dist/atom-pack.js' })
  );

  const unconfirmed = await post('/v1/atoms/wa/update');
  const beforeConfirmation = (await (await fetch(`${base}/v1/atoms`)).json()) as {
    atomPacks: Array<{ name: string; version: string }>;
  };
  const checkResponse = await fetch(`${base}/v1/atoms/wa/update`);
  const check = (await checkResponse.json()) as AtomPackUpdateCheck;
  const response = await post('/v1/atoms/wa/update', { confirm: true, revision: check.latestRevision });
  const body = await response.json();
  const listed = (await (await fetch(`${base}/v1/atoms`)).json()) as {
    atomPacks: Array<{ name: string; version: string; enabled: boolean; canUpdate: boolean }>;
  };
  const updated = listed.atomPacks.find((pack) => pack.name === 'wa');

  expect({
    status: response.status,
    unconfirmed: {
      status: unconfirmed.status,
      version: beforeConfirmation.atomPacks.find((pack) => pack.name === 'wa')?.version
    },
    check: {
      name: check.name,
    ...
    status: 200,
    unconfirmed: { status: 400, version: '1.0.0' },
    check: {
      name: 'wa',
      sourceKind: 'local',
    ...
    body: { name: 'wa', atoms: ['channel'], warnings: [] },
    pack: { name: 'wa', version: '2.0.0', enabled: false, canUpdate: true }
  });
});
...
test('POST /v1/atoms/skills/install runs model review before remote install consent', async () => {
  ...
  await writeFile(archive, tarGzWithName('reviewed/SKILL.md', skill));
...
test('POST /v1/atoms/skills/install: review model failure still requires consent and supports retry', async () => {
  ...
  await writeFile(join(skillDir, 'SKILL.md'), skill);
  await writeFile(archive, tarGzWithName('failed/SKILL.md', skill));
