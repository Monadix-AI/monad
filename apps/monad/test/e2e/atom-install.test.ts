...
test('parseAtomPackSource handles github / npm / local', () => {
  ...
  expect(() => parseAtomPackSource('npm:@scope/n@1.2.3')).toThrow('unrecognized Atom Pack source');
  expect(parseAtomPackSource('/abs/p')).toMatchObject({ kind: 'local', path: resolve('/abs/p') });
});
...
function githubRepositoryFetch(archive: Uint8Array, commit = GITHUB_COMMIT) {
  return async (url: string): Promise<Response> => {
    if (url.includes('/commits/')) return new Response(commit);
    if (url.includes('/tarball/')) return new Response(archive, { headers: { 'content-type': 'application/gzip' } });
    return new Response('not found', { status: 404 });
  };
}

function repositoryArchive(prefix = ''): Uint8Array {
  const encoder = new TextEncoder();
  const files = [
    {
      name: `${prefix}atom-pack.json`,
      body: `${JSON.stringify({ name: 'power', version: '1.0.0', sdkVersion: '0', atoms: ['skill'], entry: 'dist/atom-pack.js' })}\n`
    },
    {
      name: `${prefix}dist/atom-pack.js`,
      body: 'export default {manifest:{name:"power",version:"1.0.0",sdkVersion:"0",atoms:["skill"]},register(){}};'
    },
    { name: `${prefix}skills/repo-skill/SKILL.md`, body: '# Repository skill' }
  ];
  const chunks: Uint8Array[] = [];
  for (const file of files) {
    const content = encoder.encode(file.body);
    ...
    put(`owner-power-${GITHUB_COMMIT}/${file.name}`, 0, 100);
  ...
  const tar = new Uint8Array(chunks.reduce((size, chunk) => size + chunk.length, 1024));
  let offset = 0;
...
test('github fetch resolves a repository ref and stages its prebuilt Atom Pack files', async () => {
  const fetcher = createAtomFetcher({ fetch: githubRepositoryFetch(repositoryArchive()) });
  const stagedPack = await fetcher(parseAtomPackSource('github:owner/power@main'));

  expect({
    manifest: stagedPack.manifestRaw,
    revision: stagedPack.revision,
    fileAtoms: stagedPack.fileAtoms,
    files: [...(stagedPack.files?.keys() ?? [])].sort()
  }).toEqual({
    manifest: { name: 'power', version: '1.0.0', sdkVersion: '0', atoms: ['skill'], entry: 'dist/atom-pack.js' },
    revision: GITHUB_COMMIT,
    fileAtoms: { skills: ['repo-skill'], mcpServers: [], locales: [] },
    files: ['atom-pack.json', 'dist/atom-pack.js', 'skills/repo-skill/SKILL.md']
  });
});

test('github fetch supports an Atom Pack in a repository subdirectory', async () => {
  const fetcher = createAtomFetcher({ fetch: githubRepositoryFetch(repositoryArchive('packs/power/')) });
  const stagedPack = await fetcher(parseAtomPackSource('github:owner/power@main/packs/power'));

  expect({ revision: stagedPack.revision, skills: stagedPack.fileAtoms?.skills }).toEqual({
    revision: GITHUB_COMMIT,
    skills: ['repo-skill']
  });
});

test('github fetch rejects an invalid resolved commit', async () => {
  const fetcher = createAtomFetcher({
    fetch: githubRepositoryFetch(repositoryArchive(), 'not-a-commit')
  });

  await expect(fetcher(parseAtomPackSource('github:owner/power@main'))).rejects.toThrow(/invalid commit revision/i);
});
...
test('remote source without manifest integrity warns at consent', async () => {
  let seen: string[] = [];
  ...
  expect(seen).toEqual(['no integrity hash — the bundle cannot be verified against tampering in transit']);
});
