...
const CODEX_SESSION = process.env.MONAD_MESH_AGENT_REAL_CODEX_SESSION;
const CODEX_CWD = process.env.MONAD_MESH_AGENT_REAL_CODEX_CWD ?? process.cwd();
const CLAUDE_SESSION = process.env.MONAD_MESH_AGENT_REAL_CLAUDE_SESSION;
const CLAUDE_CWD = process.env.MONAD_MESH_AGENT_REAL_CLAUDE_CWD ?? process.cwd();
const AUTH_STATUS_SMOKE = process.env.MONAD_MESH_AGENT_REAL_AUTH_STATUS === '1';

function validEvents(events: MeshAgentOutputEvent[]): MeshAgentOutputEvent[] {
  return events.filter((event) => meshAgentOutputEventSchema.safeParse(event).success);
}
...
function requirePipeStdin(stdin: ReturnType<typeof Bun.spawn>['stdin']): PipeStdin {
  if (!stdin || typeof stdin === 'number') throw new Error('MeshAgent test process has no pipe stdin');
  return stdin;
}
...
async function readUntil(
  ...
  parse: (stdout: string) => MeshAgentOutputEvent | undefined,
  timeoutMs: number
): Promise<MeshAgentOutputEvent | undefined> {
...
describe.skipIf(!CODEX_SESSION)('MeshAgent real Codex resume contract', () => {
  test('resumes a local Codex app-server session through the adapter', async () => {
    const session = CODEX_SESSION;
    if (!session) throw new Error('MONAD_MESH_AGENT_REAL_CODEX_SESSION is required');
    ...
    let requestSeq = 0;
    codexMeshAgentAdapter.initialize?.(
      {
    ...
    try {
      const sessionRef = await readUntil(
        proc.stdout,
        (stdout) =>
          validEvents(codexMeshAgentAdapter.parseOutput(stdout)).find((event) => event.type === 'session_ref'),
        60_000
      );
...
describe.skipIf(!CLAUDE_SESSION)('MeshAgent real Claude Code resume contract', () => {
  test('resumes a local Claude Code stream-json session through the adapter', async () => {
    const session = CLAUDE_SESSION;
    if (!session) throw new Error('MONAD_MESH_AGENT_REAL_CLAUDE_SESSION is required');
    ...
    claudeCodeMeshAgentAdapter.sendInput(
      {
    ...
    try {
      const sessionRef = await readUntil(
        proc.stdout,
        (stdout) =>
          validEvents(claudeCodeMeshAgentAdapter.parseOutput(stdout)).find((event) => event.type === 'session_ref'),
        45_000
      );
      expect(sessionRef?.payload.providerSessionRef).toBe(session);
    } finally {
...
describe.skipIf(!AUTH_STATUS_SMOKE)('MeshAgent real provider auth status smoke', () => {
  test('checks Codex auth status through the adapter status probe', async () => {
    const launch = codexMeshAgentAdapter.buildAuthStatusLaunch({
      name: 'codex',
      provider: 'codex',
      command: 'codex',
      enabled: true,
    ...
    expect(['authenticated', 'unauthenticated', 'unknown']).toContain(
      codexMeshAgentAdapter.parseAuthStatus(result.output, result.exitCode)
    );
  }, 15_000);

  test('checks Claude Code auth status through the adapter status probe', async () => {
    const launch = claudeCodeMeshAgentAdapter.buildAuthStatusLaunch({
      name: 'claude-code',
    ...
    const result = await collectProcessOutput(proc, 10_000);
    expect(['authenticated', 'unauthenticated', 'unknown']).toContain(
      claudeCodeMeshAgentAdapter.parseAuthStatus(result.output, result.exitCode)
    );
  }, 15_000);
});
