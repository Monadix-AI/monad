...
async function configureSessionEventAgent(call: Call, dir: string): Promise<void> {
  ...
  await writeFile(
    ...
    [
      ...
      '    if (!line) continue;',
      '    process.stdout.write(JSON.stringify({ type: "assistant", session_id: "mock-session-1", message: { role: "assistant", content: [{ type: "text", text: "echo:" + line }] } }) + "\\n");',
  ...
  describe(`mesh-agent session-event runtime over ${kind}`, () => {
    ...
    test('normalizes and contains MeshAgent working paths', async () => {
      ...
      try {
        ...
        expect(httpErrorSchema.parse(await rejected.json())).toEqual({
