...
test('agent_acp_delegate spawns an external ACP agent and returns its answer', async () => {
  const tool = createAcpDelegateTool({
    agents: [
      { name: 'mock', command: bunExecutable(), args: [fixture], enabled: true, osSandbox: false, forwardMcp: false }
    ]
  });
...
test('agent_acp_delegate rejects an unknown / disabled agent name', async () => {
  const tool = createAcpDelegateTool({
    agents: [
      { name: 'mock', command: bunExecutable(), args: [fixture], enabled: true, osSandbox: false, forwardMcp: false },
      { name: 'off', command: bunExecutable(), args: [fixture], enabled: false, osSandbox: false, forwardMcp: false }
    ]
  });
...
test("monad's configured MCP servers are forwarded to the sub-agent's newSession", async () => {
  const tool = createAcpDelegateTool({
    agents: [
      { name: 'mock', command: bunExecutable(), args: [fixture], enabled: true, osSandbox: false, forwardMcp: true }
    ],
    mcpServers: [
