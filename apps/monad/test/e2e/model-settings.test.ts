...
// responses while the full token is persisted beside its provider. Runs over both transports.
...
for (const kind of TRANSPORTS) {
  describe(`model-settings over ${kind}`, () => {
    ...
    test('provider + credential + profile + default CRUD round-trips and redacts secrets', async () => {
      ...
      // 3. list is redacted: configured state only, never a raw token or stable fragment
      ...
      // ...but agents.json holds the full secret and auth.json remains the Agent Credential vault.
      const cfgWithCredential = await loadConfig(paths);
      const auth = await loadAuth(paths.auth);
      expect(
        cfgWithCredential?.model.providers.find((provider) => provider.id === 'oai')?.credentials[0]?.accessToken
      ).toBe('sk-supersecret-1234');
