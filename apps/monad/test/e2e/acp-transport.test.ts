...
test('session/cancel stops the turn with stopReason cancelled', async () => {
  ...
  await makeClientApp(updates).connectWith(clientStream, async (ctx) => {
    ...
    // Cancel must land mid-stream: wait for the first chunk rather than guessing at the token pacing.
    await waitFor(() => updates.some((u) => u.update.sessionUpdate.endsWith('message_chunk')), {
      message: 'agent never streamed a chunk to cancel mid-turn'
    });
