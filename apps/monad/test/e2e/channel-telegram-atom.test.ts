...
// atom-pack-sourced adapter returns host routing guidance end-to-end against a mock Bot API.
...
// ── end-to-end: the atom-pack-sourced adapter returns a host reply through a mock Bot API ──
...
test('atom-pack-loaded telegram guides an unbound channel to connect a Project Session', async () => {
      ...
      channels: [
        {
          ...
          credential: { token: 'dogfood-token', extra: { apiBaseUrl: tg.url, pollTimeoutSec: '1' } },
          rateLimitPerMin: 100
        }
      ]
    },
    (await import('@monad/environment')).emptyAuth()
  );

  try {
    await service.start();
    const deadline = Date.now() + 5000;
    while (Date.now() < deadline && !tg.outbound.some((text) => text.includes('/project use'))) await Bun.sleep(25);
    expect(tg.outbound).toContain(
