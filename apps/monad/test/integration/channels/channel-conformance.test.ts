...
// loop (conversation mapping, rate-limit, render chunking, echo guard, etc.) that can't be
// tested against real third-party platforms. Documented stances live in docs/internals/infra/channel-conformance.md.
...
// ───────────────────────── C. Core routing / keying (ChannelService) ────────────────────────────
// Reference matrix B9 (session keying), A3/A8 (self/dedup).
...
test('C: inbound users are dispatched without channel authorization configuration', async () => {
  const h = await coreHarness(chConfig());
  h.push(inbound({ chatId: 'c', userId: 'sender', text: 'hello' }));
  await h.flush();
