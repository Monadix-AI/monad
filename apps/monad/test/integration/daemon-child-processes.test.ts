...
const daemonExecutableName = process.platform === 'win32' ? 'monad.exe' : 'monad';
const watchdogExecutableName = process.platform === 'win32' ? 'monad-watchdog.exe' : 'monad-watchdog';
beforeEach(async () => {
  watchdogDir = await mkdtemp(join(tmpdir(), 'monad-watchdog-'));
  await writeFile(join(watchdogDir, daemonExecutableName), '', { mode: 0o755 });
  await symlink(daemonExecutableName, join(watchdogDir, watchdogExecutableName));
});
afterEach(async () => {
...
test('daemonChildSupervisorArgv self-execs the role-named sibling even when entryPath is a compiled binary virtual path (always != execPath)', () => {
  const execPath = join(watchdogDir, daemonExecutableName);
  expect(
    daemonChildSupervisorArgv({
      execPath,
    ...
    process.platform === 'win32' ? execPath : join(watchdogDir, watchdogExecutableName),
    '/$bunfs/root/monad',
    '--daemon-child-supervisor',
    '42',
    '/tmp/daemon-child-processes.json'
