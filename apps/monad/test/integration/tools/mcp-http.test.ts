...
test('http transport failure marks the connection unavailable and notifies subscribers once', async () => {
  const reasons: string[] = [];
  conn.onDisconnect?.((reason) => reasons.push(reason));
  server.stop(true);

  await conn.callTool('echo', { text: 'after-stop' }).catch(() => undefined);
  for (let attempt = 0; attempt < 50 && reasons.length === 0; attempt += 1) await Bun.sleep(10);
  const reason = reasons[0];
  if (!reason) throw new Error('HTTP disconnect notification was not delivered');

  expect({ failure: conn.failure, count: reasons.length, reason: reasons[0] }).toEqual({
    failure: reason,
    count: 1,
    reason
  });
