...
const cwdCommand = process.platform === 'win32' ? 'pwd -W' : 'pwd';
...
test('shell_exec fires shared path gate when cwd escapes sandbox', async () => {
  ...
  expect(calls[0]?.key).toBe(`cwd:${comparablePath(await realpath(outside))}`);
});
...
test('shell_exec path gate resolves relative escaped cwd from sandbox root', async () => {
  ...
  try {
    ...
    const res = await shellExecTool.run({ command: cwdCommand, cwd: '../outside' }, c);
    expect(comparablePath(res.metadata.stdout.trim())).toBe(comparablePath(outside));
    expect(calls).toEqual([{ tool: 'path_access', key: `cwd:${comparablePath(outside)}` }]);
  } finally {
