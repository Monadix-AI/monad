...
beforeEach(async () => {
  ...
  if (process.platform === 'win32') server.bindTcpFallback();
...
test('WebSocket streams command events live', async () => {
  ...
  await waitFor(() => events.length === 1, { message: 'command events were not streamed over the socket' });

  expect(events.map((event) => event.args)).toEqual([['SET', 'live', 'go']]);
  ws.close();
});
