...
async function connect(): Promise<InstanceType<typeof Bun.RedisClient>> {
  const client = new Bun.RedisClient(server.clientUrl);
...
test('PEXPIRE and key expires', async () => {
  await redis.set('k', 'v');
  await redis.pexpire('k', 20);
  expect(await redis.get('k')).toBe('v');
  await waitFor(async () => (await redis.get('k')) === null, { message: 'key never expired' });
...
describe('pub/sub', () => {
  test('subscribe and receive published messages', async () => {
    ...
    await waitFor(() => received.length === 2, { message: 'published messages were not delivered' });
  ...
  test('10 messages all received', async () => {
    ...
    await waitFor(() => received.length === 10, { message: 'not every published message was delivered' });
  ...
  test('unsubscribe stops messages', async () => {
    ...
    await waitFor(() => received.length === 1, { message: 'pre-unsubscribe message was not delivered' });

    await subClient.unsubscribe('ch2');
    await pub.publish('ch2', 'after');
    // A delivery after unsubscribe would arrive on the same connection as 'before' did, so one
    // round-trip on that connection is enough to rule it out without a fixed settle wait.
...
describe('streams', () => {
  ...
  test('XREAD BLOCK wakes when a later XADD lands', async () => {
    ...
    // The waiter must be registered before the write lands, or the entry is published to nobody and
    // the read blocks forever. onCommand fires in the same tick as the handler that registers it,
    // so observing the XREAD is a sufficient signal.
    let blockedRead = false;
    const off = server.onCommand((e) => {
      if (e.args[0] === 'XREAD') blockedRead = true;
    });
    const pending = redis.send('XREAD', ['BLOCK', '0', 'STREAMS', 'blk', '$']);
    await waitFor(() => blockedRead, { message: 'blocking XREAD never reached the server' });
    off();
