...
import { agentAutoConsolidationDue, consolidateGraph, type GraphMessage } from '#/services/memory/graph/service.ts';
...
test('agentAutoConsolidationDue gates background consolidation by Agent settings and interval', () => {
  const MIN = 60_000;
  const memory = {
    enabled: true,
    advanced: true,
  ...
  expect(agentAutoConsolidationDue(memory, 0, 10 * MIN)).toBe(false);
  expect(agentAutoConsolidationDue(memory, 0, 30 * MIN)).toBe(true);
  expect(agentAutoConsolidationDue({ ...memory, enabled: false }, 0, 30 * MIN)).toBe(false);
  expect(agentAutoConsolidationDue({ ...memory, autoConsolidate: false }, 0, 30 * MIN)).toBe(false);
});
