...
function adapter(options: { directModels?: boolean; throwInModelParser?: boolean } = {}): MeshAgentProviderAdapter {
  return {
    ...
    modelOptions: () =>
      options.directModels
        ? {
            resolve: async () => [{ value: 'live-model', displayName: 'Live Model 2.0', speeds: ['fast'] }]
          }
        : {
            launch: probeLaunch,
            parse: (output) => {
              ...
              return output.includes('valid')
                ? [{ value: 'live-model', displayName: 'Live Model', speeds: ['fast'] }]
                : [];
            }
          },
    argumentSupport: () => ({
...
function expectedPreset(options: { modelsLive: boolean; supportLive: boolean }): ProjectedPreset {
  return {
    ...
    settings: undefined,
    modelOptions: options.modelsLive ? ['live-model'] : [],
    ...(options.modelsLive ? { modelOptionDisplayNames: { 'live-model': 'Live Model' } } : {}),
    ...(options.modelsLive ? { speedsByModel: { 'live-model': ['fast'] } } : {}),
    reasoningEfforts: options.supportLive ? ['high'] : [],
    reasoningEffortsByModel: options.supportLive ? { 'live-model': ['high'] } : undefined
  };
}

test('uses direct provider model discovery without sending it through the process runner', async () => {
  registerAgentAdapterImpl(adapter({ directModels: true }));
  const launches: string[][] = [];
  try {
    const presets = await listMeshAgentPresets(
      probes,
      async (launch) => {
    ...
    const [preset] = testPresets(presets);

    expect({
      modelOptions: preset?.modelOptions,
      modelOptionDisplayNames: preset?.modelOptionDisplayNames,
      speedsByModel: preset?.speedsByModel,
      launches
    }).toEqual({
      modelOptions: ['live-model'],
      modelOptionDisplayNames: { 'live-model': 'Live Model 2.0' },
      speedsByModel: { 'live-model': ['fast'] },
      launches: [['/bin/probe-tool', '--help']]
    });
  } finally {
...
test('deduplicates equal probes per request and executes a fresh batch on the next request', async () => {
  ...
  try {
    ...
    const first = await listMeshAgentPresets(probes, runner, [provider]);
    const second = await listMeshAgentPresets(probes, runner, [provider]);
...
test('does not present static aliases as discovered models when execution or parsing fails', async () => {
  const cases = [
    {
      configured: adapter(),
  ...
  for (const item of cases) {
    ...
    try {
      const presets = await listMeshAgentPresets(probes, item.runner, [provider]);
...
test('does not present static aliases for configured agents when model discovery fails', async () => {
  const configured = adapter();
  registerAgentAdapterImpl(configured);
  try {
    const capabilities = await resolveMeshAgentCapabilities(
      [
        {
          ...configured.detect(probes),
          name: provider,
          enabled: true,
      ...
      async () => Promise.reject(new Error('launch failed')),
      probes
    );

    expect(capabilities).toEqual([{ modelOptions: [], reasoningEfforts: [], reasoningEffortsByModel: undefined }]);
  } finally {
    unregisterAgentAdapterImpl(provider);
  }
});
