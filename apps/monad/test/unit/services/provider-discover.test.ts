...
const goodProvider = `export default {
  type: 'scanned',
  descriptor: { type: 'scanned', label: 'Scanned', icon: { title: 'Scanned', path: 'M4 4h16v16H4z' }, strategy: 'native' },
...
test('discover() accepts a non-text ModelProvider without stream()', async () => {
  await withDir(async (dir) => {
    await writeFile(
      ...
      `export default {
        type: 'image-only',
        descriptor: { type: 'image-only', label: 'Image Only', icon: { title: 'Image Only', path: 'M4 4h16v16H4z' }, strategy: 'native' },
...
test('discover() accepts an array default export and registers each', async () => {
  await withDir(async (dir) => {
    await writeFile(
      join(dir, 'multi.js'),
      `const mk = (t) => ({ type: t, descriptor: { type: t, label: t, icon: { title: t, path: 'M4 4h16v16H4z' }, strategy: 'native' }, async *stream() {} });
