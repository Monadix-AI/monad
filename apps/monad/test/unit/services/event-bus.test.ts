...
const ev = (type: EventType, sessionId: string): Event => {
  ...
  switch (type) {
    ...
    case 'tool.approval_resolved':
      return makeEvent(target, type, { requestId: 'request-1', tool: 'write', allow: true }, { at });
    case 'mesh.started':
      return makeEvent(
        target,
        type,
...
test('makeEvent preserves project routing metadata for post-delete delivery', () => {
  const event = makeEvent(
    'ses_100000000000' as SessionId,
    'session.deleted',
    {},
    {
...
test('generic runtime subscriber sees approval and lifecycle events across sessions', () => {
  ...
  const disposeControl = bus.subscribeControl((event) => control.push(event.type));

  bus.publish(ev('tool.approval_requested', 'ses_a00000000000'));
  bus.publish(ev('tool.approval_resolved', 'ses_a00000000000'));
  bus.publish(ev('session.run.completed', 'ses_b00000000000'));
  dispose();
  disposeControl();
  bus.publish(ev('session.updated', 'ses_c00000000000'));

  expect(seen).toEqual(['tool.approval_requested', 'tool.approval_resolved', 'session.run.completed']);
  expect(control).toEqual(['tool.approval_requested', 'tool.approval_resolved', 'session.run.completed']);
});
