...
test('warns when pinned to a mutable ref (tag/branch)', async () => {
  ...
  expect(info?.warnings).toEqual([
    'pinned to mutable ref "v1.2.3" — content can change under you; pin to a commit SHA'
  ]);
});
  ...
  const fetcher = createSkillFetcher({
    runGit: async (args) => {
      if (args[1] === 'clone') {
        ...
        if (!checkoutDir) throw new Error('missing checkout directory');
        await mkdir(join(checkoutDir, 'pixel2motion'), { recursive: true });
        await Bun.write(join(checkoutDir, 'pixel2motion', 'SKILL.md'), md('pixel2motion'));
      }
      return args[3] === 'rev-parse' ? SHA : '';
    }
  });
