...
test('parseSkillMd parses context: fork and a capability tier', () => {
  const { frontmatter } = parseSkillMd(
    ['---', 'name: research', 'description: Deep research.', 'context: fork', 'modelTier: fast', '---', 'body'].join(
      '\n'
    )
  );
  expect(frontmatter.context).toBe('fork');
  expect(frontmatter.modelTier).toBe('fast');
});
...
test('parseSkillMd rejects an unknown tier value', () => {
  expect(() => parseSkillMd(['---', 'name: x', 'description: y', 'modelTier: turbo', '---', 'b'].join('\n'))).toThrow(
    /frontmatter/
  );
});
