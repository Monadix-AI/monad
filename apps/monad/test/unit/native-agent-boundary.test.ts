...
test('native agent HTTP controller delegates chatroom and direct capability behavior', async () => {
...
test('native agent base registry has no chatroom import', async () => {
...
test('native CLI host substrate does not import chatroom capability modules', async () => {
  const files = [
...
test('native CLI lifecycle does not own chatroom Q&A wall projection', async () => {
  const files = [
