...
test('shell_exec background secondary gate allow expands roots and starts outside cwd', async () => {
  ...
  try {
    ...
    const command = process.platform === 'win32' ? 'pwd -W' : 'pwd';
    const comparablePath = (path: string): string =>
      process.platform === 'win32' ? path.replaceAll('/', '\\').toLowerCase() : path;
    const { id } = await startProcess({ command, cwd: outside, terminalMode: 'pipe' }, c);
    const r = await waitForExit(id);
    expect(r.exitCode).toBe(0);
    expect(comparablePath(r.stdout.trim())).toBe(comparablePath(outside));
