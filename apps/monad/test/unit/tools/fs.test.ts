...
test('file_grep omits files under denied Credential vault roots', async () => {
  const credentials = join(root, 'credentials');
  const visible = join(root, 'visible-canary.txt');
  await mkdir(credentials, { recursive: true });
  await writeFile(join(credentials, 'auth.json'), 'vault-scan-canary');
  await writeFile(visible, 'vault-scan-canary');
  configureSandboxReadDeny([credentials]);
  try {
    const out = await fileGrepTool.run({ pattern: 'vault-scan-canary', path: root }, ctx([root]));
    expect(out.metadata).toEqual([{ file: 'visible-canary.txt', line: 1, text: 'vault-scan-canary' }]);
  } finally {
    configureSandboxReadDeny([]);
    await Promise.all([rm(credentials, { recursive: true, force: true }), rm(visible, { force: true })]);
  }
});
...
*** End Patch`
  ...
  expect(basename(out.metadata.touchedFiles[0] ?? '')).toBe('patch-add.txt');
...
*** End Patch`
  ...
  expect(out.metadata.touchedFiles.map((path) => basename(path))).toEqual([
...
*** End Patch`
  ...
  expect(basename(out.metadata.files[0]?.path ?? '')).toBe('patch-move-from.txt');
  expect(basename(out.metadata.files[0]?.newPath ?? '')).toBe('patch-move-to.txt');
...
test('file_write outside sandbox: allow gate succeeds and uses path_access key', async () => {
  ...
  expect(calls[0]?.key).toBe(
    `write:${process.platform === 'win32' ? canonicalOutside.toLowerCase() : canonicalOutside}`
  );
