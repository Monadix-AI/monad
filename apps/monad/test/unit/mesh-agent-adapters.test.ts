import type { ImportSettingsCategory, MeshAgentView } from '@monad/protocol';
...
import { parseMeshAgentArgumentSupport } from '@monad/atoms/agent-adapters/argument-support';
...
import { meshAgentStreamItems } from '@monad/atoms/mesh-agent-observation';

import {
  buildMeshAgentArgumentSupportProbe,
  buildMeshAgentAuthLaunch,
  buildMeshAgentAuthStatusLaunch,
  buildMeshAgentLaunch,
  listMeshAgentModelOptions,
...
function builtinAdapter(provider: MeshAgentView['provider']) {
...
const codexMeshAgentAdapter = builtinAdapter('codex');
const claudeCodeMeshAgentAdapter = builtinAdapter('claude-code');
const geminiMeshAgentAdapter = builtinAdapter('gemini');
const qwenMeshAgentAdapter = builtinAdapter('qwen');
const openClawMeshAgentAdapter = builtinAdapter('openclaw');
const hermesMeshAgentAdapter = builtinAdapter('hermes');

// The MeshAgent registry is populated at daemon boot via the gated atom-pack path; unit tests drive
...
const claudeAgent: MeshAgentView = {
  name: 'claude-code',
...
const geminiAgent: MeshAgentView = {
  name: 'gemini',
  provider: 'gemini',
  command: 'gemini',
  enabled: true,
...
const openClawAgent: MeshAgentView = {
  name: 'openclaw',
  provider: 'openclaw',
  command: 'openclaw',
  enabled: true,
...
const hermesAgent: MeshAgentView = {
  name: 'hermes',
  provider: 'hermes',
  command: 'hermes',
  enabled: true,
...
function expectMeshAgentOutputContract(events: unknown[]): void {
  for (const event of events) {
    expect(meshAgentOutputEventSchema.safeParse(event).success).toBe(true);
  }
}

test('Codex adapter launches an interactive CLI rooted at the requested working path', () => {
  const launch = buildMeshAgentLaunch(codexAgent, { workingPath: '/tmp/project', launchMode: 'pty' });
...
test('Codex adapter exposes MeshAgent settings import candidates and preview', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-codex-import-'));
  ...
  const candidates = codexMeshAgentAdapter.settingsImport?.detect({
    which: () => undefined,
    exists: (path) => path === join(homedir(), '.codex')
  });
  expect(candidates?.map((candidate) => candidate.path)).toContain(join(homedir(), '.codex'));

  const preview = await codexMeshAgentAdapter.settingsImport?.preview({ path: codexHome, replace: false });
  ...
  const mergedPreview = await codexMeshAgentAdapter.settingsImport?.preview({
    sources: [
  ...
  expect(
    mergedPreview?.items
      .filter((item) => item.category === 'meshAgents')
      .map((item) => item.target)
      .sort()
  ).toEqual(['codex', 'codex-workspace']);
  expect(mergedPreview?.items.find((item) => item.target === 'codex-workspace')?.summary).toContain(
    'gpt-5.6-workspace'
  );
});

test('Claude Code adapter exposes MeshAgent settings import preview without raw secret values', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-claude-import-'));
  ...
  const preview = await claudeCodeMeshAgentAdapter.settingsImport?.preview({ path: claudeHome, replace: false });
  const item = preview?.items.find((entry) => entry.target === 'claude-code');
  expect(item?.category).toBe('meshAgents');
  expect(item?.action).toBe('add');
});

test('adapter migration previews skills and MCP for all supported provider settings', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-adapter-migration-'));
  const codexHome = join(dir, '.codex');
  const claudeHome = join(dir, '.claude');
  ...
  const codexPreview = await codexMeshAgentAdapter.settingsImport?.preview({ path: codexHome, replace: false });
  const claudePreview = await claudeCodeMeshAgentAdapter.settingsImport?.preview({
    path: claudeHome,
    replace: false
  });
  const geminiPreview = await geminiMeshAgentAdapter.settingsImport?.preview({ path: geminiHome, replace: false });
  const qwenPreview = await qwenMeshAgentAdapter.settingsImport?.preview({ path: qwenHome, replace: false });
...
test('Hermes and OpenClaw adapter migration previews provider, channel, and mapped Monad agent settings', async () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-framework-migration-'));
  ...
  const hermesPreview = await hermesMeshAgentAdapter.settingsImport?.preview({ path: hermesHome, replace: false });
  const openClawPreview = await openClawMeshAgentAdapter.settingsImport?.preview({
    path: openClawHome,
    replace: false
  });
...
test('MeshAgent adapters require their binary before marking the preset installed', () => {
  const cases = [
    { adapter: codexMeshAgentAdapter, homeDir: '/.codex' },
    { adapter: claudeCodeMeshAgentAdapter, homeDir: '/.claude' },
    { adapter: geminiMeshAgentAdapter, homeDir: '/.gemini' },
    { adapter: qwenMeshAgentAdapter, homeDir: '/.qwen' },
    { adapter: openClawMeshAgentAdapter, homeDir: '/.openclaw' },
    { adapter: hermesMeshAgentAdapter, homeDir: '/.hermes' }
  ];
...
test('MeshAgent adapters pass managed agent workspace as an additional accessible directory', () => {
  const codex = buildMeshAgentLaunch(codexAgent, {
    workingPath: '/tmp/project',
    extraWorkingPaths: ['/tmp/agent-workspace'],
    launchMode: 'pty'
  });
  const claude = buildMeshAgentLaunch(claudeAgent, {
    workingPath: '/tmp/project',
    extraWorkingPaths: ['/tmp/agent-workspace'],
    launchMode: 'json-stream'
  });
  const gemini = buildMeshAgentLaunch(geminiAgent, {
    workingPath: '/tmp/project',
    extraWorkingPaths: ['/tmp/agent-workspace'],
    launchMode: 'json-stream'
  });
  const qwen = buildMeshAgentLaunch(qwenAgent, {
    workingPath: '/tmp/project',
    extraWorkingPaths: ['/tmp/agent-workspace'],
    launchMode: 'json-stream'
  });
...
test('Codex adapter passes requested model id and reasoning effort to provider launch', () => {
  const pty = buildMeshAgentLaunch(codexAgent, {
    workingPath: '/tmp/project',
    launchMode: 'pty',
    modelId: 'gpt-5.5',
...
test('managed MeshAgent launches force provider approvals to be skipped', () => {
  const codex = buildMeshAgentLaunch(codexAgent, {
    workingPath: '/tmp/project',
...
test('delegated managed launches omit the skip-approval flag so the provider projects approvals', () => {
  ...
  const openclaw = buildMeshAgentLaunch(openClawAgent, {
    workingPath: '/tmp/project',
    launchMode: 'app-server',
    skipProviderApprovals: false
  });
...
test('adapters advertise which launch modes can proxy provider approvals to the human', () => {
  expect(codexMeshAgentAdapter.supportsApprovalResolution?.('app-server')).toBe(true);
  expect(codexMeshAgentAdapter.supportsApprovalResolution?.('pty')).toBe(false);
  expect(qwenMeshAgentAdapter.supportsApprovalResolution?.('json-stream')).toBe(true);
  expect(qwenMeshAgentAdapter.supportsApprovalResolution?.('pty')).toBe(false);
  expect(openClawMeshAgentAdapter.supportsApprovalResolution?.('app-server')).toBe(true);
  expect(claudeCodeMeshAgentAdapter.supportsApprovalResolution?.('json-stream') ?? false).toBe(false);
  expect(geminiMeshAgentAdapter.supportsApprovalResolution?.('json-stream') ?? false).toBe(false);
  ...
  expect(hermesMeshAgentAdapter.supportsApprovalResolution?.('app-server')).toBe(true);
  expect(hermesMeshAgentAdapter.supportsApprovalResolution?.('cli-oneshot') ?? false).toBe(false);
});

test('only proxy-capable adapters advertise the approvalProxy capability', () => {
  expect(codexMeshAgentAdapter.detect().capabilities?.approvalProxy).toBe(true);
  expect(qwenMeshAgentAdapter.detect().capabilities?.approvalProxy).toBe(true);
  expect(openClawMeshAgentAdapter.detect().capabilities?.approvalProxy).toBe(true);
  expect(claudeCodeMeshAgentAdapter.detect().capabilities?.approvalProxy ?? false).toBe(false);
  expect(geminiMeshAgentAdapter.detect().capabilities?.approvalProxy ?? false).toBe(false);
  expect(hermesMeshAgentAdapter.detect().capabilities?.approvalProxy ?? false).toBe(false);
});
...
test('Claude adapter passes requested model id and reasoning effort to the provider', () => {
  const launch = buildMeshAgentLaunch(claudeAgent, {
    workingPath: '/tmp/project',
    launchMode: 'json-stream',
...
test('Claude adapter maps the thinking summary setting to the provider display flag', () => {
  const defaultLaunch = buildMeshAgentLaunch(claudeAgent, {
    workingPath: '/tmp/project',
    launchMode: 'json-stream'
  });
  const omittedLaunch = buildMeshAgentLaunch(
...
test('Claude adapter merges ultracode into existing inline session settings', () => {
  const launch = buildMeshAgentLaunch(
...
test('MeshAgent argument support parser extracts flags and enumerated values from help output', () => {
  const support = parseMeshAgentArgumentSupport(`
Usage: cli [options]
  --model <model>
...
test('Codex argument support probe extracts reasoning efforts from model catalog output', () => {
  const support = codexMeshAgentAdapter.argumentSupport?.(codexAgent).parse(
    JSON.stringify({
      models: [
        {
          slug: 'gpt-5.5',
...
test('Codex adapter retries one transient thread-not-found response before requiring reconnect', async () => {
  ...
  codexMeshAgentAdapter.initialize?.(handle, {
    workingPath: '/Users/test/Projects/monad',
    providerSessionRef: '019f6b1f-a60e-7d82-9dd0-3f6dfbc46e5a'
  });
  codexMeshAgentAdapter.parseOutput(JSON.stringify({ id: 5, result: {} }), handle);
  sent.length = 0;

  expect(
    codexMeshAgentAdapter.parseOutput(
...
test('MeshAgent adapters expose provider argument support probes', () => {
  expect(buildMeshAgentArgumentSupportProbe(codexAgent)?.launch.argv).toEqual([
    'codex',
    'debug',
    'models',
    '--bundled'
  ]);
  expect(buildMeshAgentArgumentSupportProbe(claudeAgent)?.launch.argv).toEqual(['claude', '--help']);
  expect(buildMeshAgentArgumentSupportProbe(geminiAgent)?.launch.argv).toEqual(['gemini', '--help']);
  expect(buildMeshAgentArgumentSupportProbe(qwenAgent)?.launch.argv).toEqual(['qwen', '--help']);
});

test('Codex adapter launches app-server stdio with initialization messages', () => {
  const launch = buildMeshAgentLaunch(codexAgent, { workingPath: '/tmp/project', launchMode: 'app-server' });
...
test('Codex adapter launches app-server over a ws listener when transport is ws', () => {
  const launch = buildMeshAgentLaunch(codexAgent, {
    workingPath: '/tmp/project',
    launchMode: 'app-server',
...
test('Codex adapter requires a socket path for the unix transport', () => {
  expect(() =>
    buildMeshAgentLaunch(codexAgent, {
      workingPath: '/tmp/project',
      launchMode: 'app-server',
      appServerTransport: 'unix'
    })
...
test('MeshAgent auth launches provider-owned login and status commands', () => {
  expect(buildMeshAgentAuthLaunch(codexAgent).argv).toEqual(['codex', 'login']);
  expect(buildMeshAgentAuthStatusLaunch(codexAgent).argv).toEqual(['codex', 'login', 'status']);
  expect(buildMeshAgentAuthStatusLaunch(codexAgent).env?.CODEX_NON_INTERACTIVE).toBe('1');
  expect(buildMeshAgentAuthLaunch(claudeAgent).argv).toEqual(['claude', 'auth', 'login']);
  expect(buildMeshAgentAuthStatusLaunch(claudeAgent).argv).toEqual(['claude', 'auth', 'status', '--json']);
  expect(buildMeshAgentAuthLaunch(geminiAgent).argv).toEqual(['gemini']);
  expect(buildMeshAgentAuthLaunch(geminiAgent).env).toMatchObject({
    NO_BROWSER: 'true',
    TERM: 'xterm-256color'
  });
  expect(buildMeshAgentAuthStatusLaunch(geminiAgent).argv).toEqual([process.execPath, '--eval', expect.any(String)]);
  expect(buildMeshAgentAuthLaunch(qwenAgent).argv).toEqual(['qwen']);
  expect(buildMeshAgentAuthStatusLaunch(qwenAgent).argv).toEqual(['qwen', '--list-sessions']);
  expect(codexMeshAgentAdapter.detect({ which: () => undefined, exists: () => true }).capabilities).toEqual({
    auth: 'pty',
    events: 'paged',
    resume: 'structured',
  ...
  expect(claudeCodeMeshAgentAdapter.detect({ which: () => undefined, exists: () => true }).capabilities).toEqual({
    auth: 'pty',
  ...
  expect(geminiMeshAgentAdapter.detect({ which: () => undefined, exists: () => true }).capabilities).toEqual({
    auth: 'pty',
    events: 'provider-owned',
    resume: 'pty',
    approval: 'provider-owned',
  ...
  expect(qwenMeshAgentAdapter.detect({ which: () => undefined, exists: () => true }).capabilities).toEqual({
    auth: 'pty',
    events: 'provider-owned',
    resume: 'pty',
    approval: 'provider-owned',
...
test('MeshAgent auth status parsers use structured output or documented status exit codes', () => {
  expect(codexMeshAgentAdapter.parseAuthStatus(JSON.stringify({ authenticated: true }), 0)).toBe('authenticated');
  expect(codexMeshAgentAdapter.parseAuthStatus(JSON.stringify({ authenticated: false }), 0)).toBe('unauthenticated');
  expect(codexMeshAgentAdapter.parseAuthStatus('logged in as test', 0)).toBe('authenticated');
  expect(codexMeshAgentAdapter.parseAuthStatus('not logged in; run codex login', 1)).toBe('unauthenticated');
  expect(claudeCodeMeshAgentAdapter.parseAuthStatus(JSON.stringify({ state: 'authenticated' }), 0)).toBe(
    'authenticated'
  );
  expect(claudeCodeMeshAgentAdapter.parseAuthStatus('Authenticated', 0)).toBe('authenticated');
  expect(claudeCodeMeshAgentAdapter.parseAuthStatus('Please login', 1)).toBe('unauthenticated');
  expect(claudeCodeMeshAgentAdapter.parseAuthStatus('unexpected provider error', 2)).toBe('unknown');
  expect(geminiMeshAgentAdapter.parseAuthStatus(JSON.stringify({ authenticated: true }), 0)).toBe('authenticated');
  expect(
    geminiMeshAgentAdapter.parseAuthStatus(
      'Please set an Auth method in your /Users/test/.gemini/settings.json or specify GEMINI_API_KEY',
      0
    )
  ).toBe('unknown');
  expect(geminiMeshAgentAdapter.parseAuthStatus('Waiting for authentication...', 0)).toBe('unknown');
  expect(geminiMeshAgentAdapter.parseAuthStatus('command completed', 0)).toBe('unknown');
  expect(qwenMeshAgentAdapter.parseAuthStatus(JSON.stringify({ authenticated: true }), 0)).toBe('authenticated');
  expect(qwenMeshAgentAdapter.parseAuthStatus('Waiting for authentication...', 0)).toBe('unknown');
});
...
test('Codex adapter defers thread/start until the initialize response when a request ledger is present', () => {
  ...
  codexMeshAgentAdapter.initialize?.(handle, { workingPath: '/tmp/project' });
  // presence-ok: provider protocol state must stay adapter-local rather than widening the shared handle.
  expect(Object.keys(handle).sort()).toEqual(['appServer', 'kill', 'launchMode', 'nextRequestId', 'pendingRequests']);
  ...
  codexMeshAgentAdapter.parseOutput(JSON.stringify({ id: 0, result: { userAgent: 'codex' } }), handle);
...
test('Codex adapter tracks the in-flight turn and addresses interrupt/steer at it', () => {
  ...
  codexMeshAgentAdapter.interrupt?.(handle);
  codexMeshAgentAdapter.steer?.(handle, 'also check the tests');
...
test('Codex adapter auto-compacts and re-runs the turn on context overflow', () => {
  ...
  codexMeshAgentAdapter.sendInput(handle, 'summarize the repo');
  writes.length = 0;
  ...
  codexMeshAgentAdapter.parseOutput(overflow, handle);
  ...
  writes.length = 0;
  expect(codexMeshAgentAdapter.parseOutput(overflow, handle)).toEqual([
...
test('MeshAgent launch enforces unsafe arguments declared by a third-party adapter', () => {
  const provider = 'policy-test';
  registerAgentAdapterImpl({
    ...codexMeshAgentAdapter,
    provider,
    unsafeArgument: (args: string[]) => (args.includes('--provider-root') ? '--provider-root' : undefined)
  } as typeof codexMeshAgentAdapter & { unsafeArgument(args: string[]): string | undefined });
  try {
    expect(() =>
      buildMeshAgentLaunch(
        { ...codexAgent, provider, args: ['--provider-root'] },
        { workingPath: '/tmp/project', launchMode: 'pty' }
      )
    ).toThrow('dangerous MeshAgent arg "--provider-root" requires allowAutopilot');
  } finally {
    unregisterAgentAdapterImpl(provider);
  }
});

test('MeshAgent launch rejects shell command strings in command fields', () => {
  expect(() =>
    buildMeshAgentLaunch({ ...codexAgent, command: 'codex --cd /tmp/project' }, { workingPath: '/tmp/project' })
  ).toThrow(/command/i);
});

test('Claude Code adapter launches in the requested cwd and advertises stream-json capability', () => {
  const launch = buildMeshAgentLaunch(claudeAgent, { workingPath: '/tmp/project', launchMode: 'pty' });
...
test('Claude Code adapter launches structured stream-json mode with print protocol flags', () => {
  const launch = buildMeshAgentLaunch(claudeAgent, { workingPath: '/tmp/project', launchMode: 'json-stream' });
...
test('Qwen and Gemini managed project launches inject the managed prompt as system instructions', () => {
  ...
  expect(qwenMeshAgentAdapter.managedRuntime?.usesSystemPromptFile).toBe(true);
  expect(geminiMeshAgentAdapter.managedRuntime?.usesSystemPromptFile).toBe(true);
...
test('Claude Code adapter resumes with the provider session ref in PTY and stream-json modes', () => {
  const pty = buildMeshAgentLaunch(claudeAgent, {
    workingPath: '/tmp/project',
    launchMode: 'pty',
    providerSessionRef: 'claude-session-1'
  });
  const stream = buildMeshAgentLaunch(claudeAgent, {
    workingPath: '/tmp/project',
    launchMode: 'json-stream',
    providerSessionRef: 'claude-session-1'
  });
...
test('Gemini adapter launches in the requested cwd and advertises stream-json capability', () => {
  const launch = buildMeshAgentLaunch(geminiAgent, { workingPath: '/tmp/project', launchMode: 'pty' });
...
test('Gemini adapter launches structured stream-json mode with official output-format flag', () => {
  const launch = buildMeshAgentLaunch(geminiAgent, { workingPath: '/tmp/project', launchMode: 'json-stream' });
...
test('Gemini adapter resumes with the provider session ref in PTY and stream-json modes', () => {
  const pty = buildMeshAgentLaunch(geminiAgent, {
    workingPath: '/tmp/project',
    launchMode: 'pty',
    providerSessionRef: 'gemini-session-1'
  });
  const stream = buildMeshAgentLaunch(geminiAgent, {
    workingPath: '/tmp/project',
    launchMode: 'json-stream',
    providerSessionRef: 'gemini-session-1'
  });
...
test('Qwen adapter launches in the requested cwd and advertises stream-json capability', () => {
  const launch = buildMeshAgentLaunch(qwenAgent, { workingPath: '/tmp/project', launchMode: 'pty' });
...
test('Qwen adapter launches the SDK bidirectional stream-json session (input + output)', () => {
  const launch = buildMeshAgentLaunch(qwenAgent, { workingPath: '/tmp/project', launchMode: 'json-stream' });
...
test('Qwen adapter resumes with the provider session ref in PTY and stream-json modes', () => {
  const pty = buildMeshAgentLaunch(qwenAgent, {
    workingPath: '/tmp/project',
    launchMode: 'pty',
    providerSessionRef: 'qwen-session-1'
  });
  const stream = buildMeshAgentLaunch(qwenAgent, {
    workingPath: '/tmp/project',
    launchMode: 'json-stream',
    providerSessionRef: 'qwen-session-1'
  });
...
test('Gemini adapter keeps yolo approval mode behind dangerous mode opt-in', () => {
  expect(() =>
    buildMeshAgentLaunch({ ...geminiAgent, args: ['--approval-mode=yolo'] }, { workingPath: '/tmp/project' })
  ).toThrow(/dangerous/i);
  expect(() =>
    buildMeshAgentLaunch({ ...geminiAgent, args: ['--approval-mode', 'yolo'] }, { workingPath: '/tmp/project' })
  ).toThrow(/dangerous/i);
  expect(() => buildMeshAgentLaunch({ ...geminiAgent, args: ['--yolo'] }, { workingPath: '/tmp/project' })).toThrow(
    /dangerous/i
  );
...
test('MeshAgent presets detect Codex, Claude Code, Gemini, and Qwen as direct client commands', async () => {
  const presets = await listMeshAgentPresets({ which: (name) => `/bin/${name}`, exists: () => false });
...
test('MeshAgent adapters expose supported model options with agent override', () => {
  expect(codexMeshAgentAdapter.listSupportedModels()).toEqual([
    'gpt-5.5',
    'gpt-5.4',
    'gpt-5.4-mini',
    'gpt-5.3-codex',
  ...
  expect(claudeCodeMeshAgentAdapter.listSupportedModels()).toEqual(['fable', 'opus', 'sonnet', 'haiku']);
  expect(geminiMeshAgentAdapter.listSupportedModels()).toEqual(['gemini-2.5-pro', 'gemini-2.5-flash']);
  expect(qwenMeshAgentAdapter.listSupportedModels()).toEqual(['qwen3-coder-plus', 'qwen3-coder-flash']);
  expect(codexMeshAgentAdapter.listSupportedModels({ ...codexAgent, modelOptions: ['custom-codex'] })).toEqual([
    'custom-codex'
  ]);
});

test('MeshAgent model option probes parse command output with display names', () => {
  expect(
    codexMeshAgentAdapter.modelOptions?.(codexAgent).parse(
      JSON.stringify({
        models: [
...
test('Claude Code model option probe parses model aliases from help output', () => {
  expect(
    claudeCodeMeshAgentAdapter.modelOptions?.(claudeAgent).parse(
      `
...
test('MeshAgent model options prefer command probe output before adapter fallback', () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-model-options-'));
...
test('MeshAgent model options prefer command probe output before configured options', () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-configured-model-options-'));
  const command = join(dir, process.platform === 'win32' ? 'fake-codex.cmd' : 'fake-codex');
  const catalog = JSON.stringify({
    models: [
...
test('MeshAgent model option display names come from command probe output', () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-model-option-labels-'));
  const command = join(dir, process.platform === 'win32' ? 'fake-codex.cmd' : 'fake-codex');
  const catalog = JSON.stringify({
    models: [
...
test('MeshAgent model options fall back to configured options when command probe is empty', () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-empty-model-options-'));
  const command = join(dir, process.platform === 'win32' ? 'fake-codex.cmd' : 'fake-codex');
...
test('Codex model options ignore single o-series help examples and use adapter fallback', () => {
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-codex-help-'));
  const command = join(dir, process.platform === 'win32' ? 'fake-codex.cmd' : 'fake-codex');
  writeFileSync(
    command,
    process.platform === 'win32'
...
test('MeshAgent launch resolves provider commands before spawn', () => {
  const codexLaunch = resolveMeshAgentLaunchCommand(
    codexMeshAgentAdapter,
    buildMeshAgentLaunch(codexAgent, { workingPath: '/tmp/project', launchMode: 'pty' }),
    { which: () => undefined, exists: (path) => path === '/Applications/Codex.app/Contents/Resources/codex' }
  );
  const claudeLaunch = resolveMeshAgentLaunchCommand(
    claudeCodeMeshAgentAdapter,
    buildMeshAgentLaunch(claudeAgent, { workingPath: '/tmp/project', launchMode: 'pty' }),
    { which: (name) => (name === 'claude' ? '/Users/test/bin/claude' : undefined), exists: () => false }
  );
  const geminiLaunch = resolveMeshAgentLaunchCommand(
    geminiMeshAgentAdapter,
    buildMeshAgentLaunch(geminiAgent, { workingPath: '/tmp/project', launchMode: 'pty' }),
    { which: (name) => (name === 'gemini' ? '/Users/test/bin/gemini' : undefined), exists: () => false }
  );
  const qwenLaunch = resolveMeshAgentLaunchCommand(
    qwenMeshAgentAdapter,
    buildMeshAgentLaunch(qwenAgent, { workingPath: '/tmp/project', launchMode: 'pty' }),
...
test('MeshAgent launch fails before spawn when provider command cannot be resolved', () => {
  expect(() =>
    resolveMeshAgentLaunchCommand(
      codexMeshAgentAdapter,
      buildMeshAgentLaunch(codexAgent, { workingPath: '/tmp/project', launchMode: 'pty' }),
...
test('Codex adapter parses app-server raw response item notifications into structured events', () => {
  ...
  const events = codexMeshAgentAdapter.parseOutput(chunk);
  expectMeshAgentOutputContract(events);
...
test('MeshAgent adapters ignore malformed and unknown provider output outside the Monad contract', () => {
  const invalidApproval = codexMeshAgentAdapter.parseOutput(
    JSON.stringify({
      method: 'item/commandExecution/requestApproval',
      params: { command: 'echo missing request id' }
    })
  );
  expect(invalidApproval.every((event) => meshAgentOutputEventSchema.safeParse(event).success)).toBe(false);
});

test('Codex app-server turn completion extracts the final agent message from turn items', () => {
  const events = codexMeshAgentAdapter.parseOutput(
    JSON.stringify({
      method: 'turn/completed',
      params: {
        threadId: 'thr_123',
...
test('Codex adapter dispatches app-server responses by request id, not result shape', () => {
  ...
  const threadEvents = codexMeshAgentAdapter.parseOutput(
    JSON.stringify({
      id: 3,
...
test('Gemini adapter translates stream-json events into the Monad MeshAgent contract', () => {
  const chunk = [
  ...
  const events = geminiMeshAgentAdapter.parseOutput(chunk);
  expectMeshAgentOutputContract(events);
  expect(events).toEqual([
...
test('Gemini adapter surfaces error results and drops non-fatal warnings', () => {
  const warned = geminiMeshAgentAdapter.parseOutput(
    [
  ...
  const failed = geminiMeshAgentAdapter.parseOutput(
    JSON.stringify({
      type: 'result',
      status: 'error',
  ...
  expectMeshAgentOutputContract(failed);
...
test('Gemini history observation folds assistant stream-json message deltas by result boundary', () => {
  ...
  const liveEvents = meshAgentStreamItems({ id: 'mesh_gemini000000', adapter: geminiMeshAgentAdapter, output });
  const pagedEvents = meshAgentStreamItems({
    id: 'mesh_gemini000000',
    adapter: geminiMeshAgentAdapter,
    output,
    mode: 'events'
  });
  ...
  expect(pagedEvents.map((event) => [event.providerEventType, event.text])).toEqual([['message', 'I will inspect.']]);
});
...
test('Qwen adapter translates SDK stream-json messages into the Monad MeshAgent contract', () => {
  const chunk = [
    JSON.stringify({
      type: 'system',
  ...
  const events = qwenMeshAgentAdapter.parseOutput(chunk);
  expectMeshAgentOutputContract(events);
  expect(events).toEqual([
    {
      type: 'session_ref',
...
test('Qwen adapter surfaces error results as provider errors', () => {
  const events = qwenMeshAgentAdapter.parseOutput(
    JSON.stringify({
      type: 'result',
...
test('Qwen history observation drops partial stream events and keeps complete SDK messages', () => {
  ...
  const liveEvents = meshAgentStreamItems({ id: 'mesh_qwen00000000', adapter: qwenMeshAgentAdapter, output });
  const pagedEvents = meshAgentStreamItems({
    id: 'mesh_qwen00000000',
    adapter: qwenMeshAgentAdapter,
    output,
    mode: 'events'
  });

  expect(liveEvents.some((event) => event.providerEventType === 'content_block_delta')).toBe(true);
  expect(pagedEvents.map((event) => [event.providerEventType, event.text])).toEqual([
...
test('Qwen adapter surfaces can_use_tool control requests and resolves them over the control plane', () => {
  ...
  qwenMeshAgentAdapter.resolveApproval(handle, {
    requestId: 'req-1',
    allow: true,
...
test('Qwen adapter initializes and frames turns through the SDK stream-json bridge', () => {
  ...
  qwenMeshAgentAdapter.initialize?.(handle, { workingPath: '/tmp/project' });
  qwenMeshAgentAdapter.sendInput(handle, 'summarize');
...
test('Gemini adapter leaves PTY prompt resolution to the raw terminal input bridge', () => {
  ...
  geminiMeshAgentAdapter.resolveApproval(handle, {
...
test('Codex adapter echoes a numeric approval request id back to the app server', () => {
  ...
  codexMeshAgentAdapter.resolveApproval(handle, {
    requestId: '17',
    allow: true,
...
test('Codex event source requests and projects paged app-server history without rollout files', async () => {
  ...
  const page = await codexMeshAgentAdapter.events.readPage?.(
    {
      providerSessionRef: 'codex-thread-1',
      workingPath: '/tmp/project',
      limitBytes: 8192,
    ...
    { limit: 3, before: 'cursor-1', view: 'convenience' }
  );
...
test('Codex history observation folds realtime item lifecycle into final events', () => {
  ...
  const liveEvents = meshAgentStreamItems({ id: 'mesh_test00000000', adapter: codexMeshAgentAdapter, output });
  const pagedEvents = meshAgentStreamItems({
    id: 'mesh_test00000000',
    adapter: codexMeshAgentAdapter,
    output,
    mode: 'events'
  });

  expect(liveEvents.some((event) => event.providerEventType === 'item/mcpToolCall/progress')).toBe(true);
  expect(pagedEvents.map((event) => event.providerEventType)).toEqual([
    'function_call',
    'function_call_output',
    'item/agentMessage'
  ]);
  expect(pagedEvents.map((event) => event.text)).toEqual([
...
test('Codex adapter keeps child status notifications from replacing the managed root thread', () => {
  ...
  const childEvents = codexMeshAgentAdapter.parseOutput(
    JSON.stringify({
      method: 'thread/status/changed',
  ...
  const rootEvents = codexMeshAgentAdapter.parseOutput(
    JSON.stringify({
      method: 'thread/status/changed',
...
test('Codex adapter accepts Monad input and approval decisions through its app-server bridge', () => {
  ...
  codexMeshAgentAdapter.sendInput(handle, 'summarize');
  codexMeshAgentAdapter.resolveApproval(handle, {
...
test('Claude Code adapter parses stream-json messages into structured events', () => {
  ...
  const events = claudeCodeMeshAgentAdapter.parseOutput(chunk);
  expectMeshAgentOutputContract(events);
  expect(events).toEqual([
    {
      type: 'session_ref',
...
test('MeshAgent process killer targets the process group on Unix and falls back to pid kill', () => {
...
test('MeshAgent process killer kills the whole tree on Windows', () => {
  const calls: Array<[number, NodeJS.Signals]> = [];
...
test('MeshAgent process killer falls back to direct pid kill when Windows tree-kill fails', () => {
  const calls: Array<[number, NodeJS.Signals]> = [];
  killMeshAgentProcess(
    123,
    'SIGTERM',
...
test('MeshAgent process killer ignores already-dead POSIX process groups', () => {
  const calls: Array<[number, NodeJS.Signals]> = [];
  expect(() =>
    killMeshAgentProcess(
      123,
...
test('MeshAgent process killer ignores already-dead direct fallback pids', () => {
  const calls: Array<[number, NodeJS.Signals]> = [];
  expect(() =>
    killMeshAgentProcess(
      123,
...
test('pty fallback picks json-stream for adapters that support it', () => {
  for (const adapter of [claudeCodeMeshAgentAdapter, geminiMeshAgentAdapter, qwenMeshAgentAdapter]) {
...
test('pty fallback picks app-server for codex, which has no json-stream mode but does support stdio', () => {
  const preset = codexMeshAgentAdapter.detect({ which: () => undefined, exists: () => false });
...
test('pty fallback is undefined for OpenClaw/Hermes: app-server-only but ws-only transport (no stdio)', () => {
  ...
  for (const adapter of [openClawMeshAgentAdapter, hermesMeshAgentAdapter]) {
    const preset = adapter.detect({ which: () => undefined, exists: () => false });
...
test('OpenClaw adapter launches the interactive CLI in pty mode rooted at the working path', () => {
  const launch = buildMeshAgentLaunch(openClawAgent, { workingPath: '/tmp/project', launchMode: 'pty' });
...
test('OpenClaw adapter launches the gateway over a ws app-server transport', () => {
  const launch = buildMeshAgentLaunch(openClawAgent, {
    workingPath: '/tmp/project',
    launchMode: 'app-server'
  });
...
test('OpenClaw adapter rejects an unsupported app-server transport', () => {
  expect(() =>
    buildMeshAgentLaunch(openClawAgent, {
      workingPath: '/tmp/project',
      launchMode: 'app-server',
...
test('OpenClaw preset advertises pty + app-server and a ws transport', () => {
  const preset = openClawMeshAgentAdapter.detect({ which: () => '/bin/openclaw', exists: () => false });
...
test('OpenClaw adapter surfaces pty terminal output as plain agent messages', () => {
  const events = openClawMeshAgentAdapter.parseOutput('working on it...\n', {
    launchMode: 'pty',
    kill() {}
  });
  expectMeshAgentOutputContract(events);
...
test('OpenClaw adapter maps the real gateway envelope to the MeshAgent contract', () => {
  ...
  const events = openClawMeshAgentAdapter.parseOutput(chunk, { launchMode: 'app-server', kill() {} });
  expectMeshAgentOutputContract(events);
  expect(events).toEqual([
...
test('OpenClaw adapter signs the connect challenge, defers session start, and resolves the session ref', () => {
  ...
  openClawMeshAgentAdapter.initialize?.(handle, {
    workingPath: '/tmp/project',
    modelId: 'openclaw-default',
  ...
  openClawMeshAgentAdapter.parseOutput(
  ...
  openClawMeshAgentAdapter.parseOutput(JSON.stringify({ type: 'res', id: '0', ok: true, payload: {} }), handle);
  ...
  const refEvents = openClawMeshAgentAdapter.parseOutput(
...
test('OpenClaw connect omits auth and signs an empty token when no gateway token is configured', () => {
  ...
  openClawMeshAgentAdapter.initialize?.(handle, { workingPath: '/tmp/project' });
  openClawMeshAgentAdapter.parseOutput(
...
test('OpenClaw adapter sends a turn and resolves an approval over the app-server bridge', () => {
  ...
  openClawMeshAgentAdapter.sendInput(handle, 'summarize the repo');
  expect(JSON.parse(writes[0] ?? '')).toEqual({
    type: 'req',
  ...
  openClawMeshAgentAdapter.resolveApproval(handle, { requestId: 'req-1', allow: true });
...
test('OpenClaw event source requests provider-owned chat history over the gateway', async () => {
  ...
  let responseId: string | number | undefined;
  await openClawMeshAgentAdapter.events.readPage?.(
    {
      providerSessionRef: 'agent:dev:oc-9',
      workingPath: '/tmp/project',
      limitBytes: 8192,
  ...
  const events = openClawMeshAgentAdapter.parseOutput(
    JSON.stringify({
      type: 'res',
      id: '8',
      ok: true,
...
test('OpenClaw history observation projects provider chat messages without guessing live deltas', () => {
  ...
  const pagedEvents = meshAgentStreamItems({
    id: 'mesh_openclaw0000',
    adapter: openClawMeshAgentAdapter,
    output,
    mode: 'events'
  });

  expect(pagedEvents.map((event) => [event.role, event.providerEventType, event.text])).toEqual([
...
test('OpenClaw and Hermes auth launches use provider-owned login and status commands', () => {
  expect(buildMeshAgentAuthLaunch(openClawAgent).argv).toEqual(['openclaw', 'models', 'auth', 'login']);
  expect(buildMeshAgentAuthStatusLaunch(openClawAgent).argv).toEqual(['openclaw', 'models', 'status', '--check']);
  expect(buildMeshAgentAuthLaunch(hermesAgent).argv).toEqual(['hermes', 'auth']);
  ...
  expect(buildMeshAgentAuthStatusLaunch(hermesAgent).argv).toEqual(['hermes', 'auth', 'list']);
});

test('OpenClaw and Hermes auth status parsers use structured output or status exit codes', () => {
  expect(openClawMeshAgentAdapter.parseAuthStatus(JSON.stringify({ authenticated: true }), 0)).toBe('authenticated');
  expect(openClawMeshAgentAdapter.parseAuthStatus('logged in', 0)).toBe('authenticated');
  expect(openClawMeshAgentAdapter.parseAuthStatus('not signed in', 1)).toBe('unauthenticated');
  expect(openClawMeshAgentAdapter.parseAuthStatus('auth expiring soon', 2)).toBe('authenticated');
  expect(openClawMeshAgentAdapter.parseAuthStatus('', null)).toBe('unknown');
  expect(hermesMeshAgentAdapter.parseAuthStatus(JSON.stringify({ state: 'authenticated' }), 0)).toBe('authenticated');
  expect(hermesMeshAgentAdapter.parseAuthStatus('signed in as test', 0)).toBe('authenticated');
  expect(hermesMeshAgentAdapter.parseAuthStatus('', 0)).toBe('unknown');
  expect(hermesMeshAgentAdapter.parseAuthStatus('no accounts', 0)).toBe('unauthenticated');
  expect(hermesMeshAgentAdapter.parseAuthStatus('no accounts', 1)).toBe('unauthenticated');
});

test('Hermes adapter launches interactive pty, and a real app-server gateway with its ws hints', () => {
  const pty = buildMeshAgentLaunch(hermesAgent, { workingPath: '/tmp/project', launchMode: 'pty' });
  ...
  const appServer = buildMeshAgentLaunch(hermesAgentWithToken, {
    workingPath: '/tmp/project',
    launchMode: 'app-server'
  });
  ...
  const withPort = buildMeshAgentLaunch(hermesAgentWithToken, {
    workingPath: '/tmp/project',
    launchMode: 'app-server',
    appServerPort: 19124
  });
...
test('Hermes adapter appends --yolo (not the wrong generic --auto-approve) when skipping approvals', () => {
  ...
  const pty = buildMeshAgentLaunch(hermesAgent, {
    workingPath: '/tmp/project',
    launchMode: 'pty',
    skipProviderApprovals: true
  });
  ...
  const noSkip = buildMeshAgentLaunch(hermesAgent, {
    workingPath: '/tmp/project',
    launchMode: 'pty',
    skipProviderApprovals: false
  });
...
test('Hermes app-server launch fails fast without a configured token instead of hanging on a doomed dial', () => {
  ...
  expect(() => buildMeshAgentLaunch(hermesAgent, { workingPath: '/tmp/project', launchMode: 'app-server' })).toThrow(
    /HERMES_DASHBOARD_SESSION_TOKEN/
  );
});

test('Hermes preset advertises pty + app-server + cli-oneshot (real gateway as of v0.18.0)', () => {
  const preset = hermesMeshAgentAdapter.detect({ which: () => '/bin/hermes', exists: () => false });
  ...
  expect(preset.supportedAppServerTransports).toEqual(['ws']);
  expect(preset.capabilities?.events).toBe('provider-owned');
});

test('Hermes adapter prefers the provider HTTP messages API for earlier events', async () => {
  ...
  try {
    const page = await hermesMeshAgentAdapter.events.readPage?.(
      {
        providerSessionRef: 'api-session',
        workingPath: '/tmp/project',
        limitBytes: 8192
      },
      { limit: 10, view: 'convenience' }
    );
    expect(
      page?.state === 'available' && page.view === 'convenience'
        ? page.events.map((event) => [event.role, event.text])
        : []
    ).toEqual([
      ['user', 'api user'],
      ['agent', 'api answer']
...
test('Hermes adapter falls back to provider CLI export before reading the SQLite store', async () => {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-hermes-export-'));
  ...
  try {
    const page = await hermesMeshAgentAdapter.events.readPage?.(
      ...
      { limit: 1, view: 'convenience' }
    );
    expect(
      page?.state === 'available' && page.view === 'convenience' ? page.events.map((event) => event.text) : []
    ).toEqual(['export answer']);
    expect(page?.state === 'available' && page.view === 'convenience' ? page.nextCursor : undefined).toBe('1');
  } finally {
    if (previousApi === undefined) delete Bun.env[apiEnv];
    else Bun.env[apiEnv] = previousApi;
...
test('Hermes adapter uses the local SQLite session store only as a final history fallback', async () => {
  ...
  const previousPath = Bun.env[pathEnv];
  const dir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-hermes-history-'));
  ...
  try {
    ...
    const firstPage = await hermesMeshAgentAdapter.events.readPage?.(
      {
        providerSessionRef: 'agent:dev:hermes:key-1',
        workingPath: '/tmp/project',
        limitBytes: 8192
      },
      { limit: 3, view: 'convenience' }
    );
    expect(
      firstPage?.state === 'available' && firstPage.view === 'convenience' ? firstPage.nextCursor : undefined
    ).toBe('3');
    expect(
      firstPage?.state === 'available' && firstPage.view === 'convenience'
        ? firstPage.events.map((event) => event.text)
        : []
    ).toEqual(['Tool call terminal {"command":"ls"}', 'tool output', 'done']);

    const secondPage = await hermesMeshAgentAdapter.events.readPage?.(
      {
        providerSessionRef: 'parent-session',
        workingPath: '/tmp/project',
        limitBytes: 8192
      },
      { limit: 3, before: '3', view: 'convenience' }
    );
    expect(
      secondPage?.state === 'available' && secondPage.view === 'convenience'
        ? secondPage.events.map((event) => event.text)
        : []
    ).toEqual(['inspect']);
    expect(
      secondPage?.state === 'available' && secondPage.view === 'convenience' ? secondPage.nextCursor : undefined
    ).toBeUndefined();

    const fullPage = await hermesMeshAgentAdapter.events.readPage?.(
      {
        providerSessionRef: 'parent-session',
        workingPath: '/tmp/project',
        limitBytes: 8192
    ...
    const events = fullPage?.state === 'available' && fullPage.view === 'convenience' ? fullPage.events : [];
...
test('Hermes adapter surfaces pty plain-text output as agent messages', () => {
  const events = hermesMeshAgentAdapter.parseOutput('the answer is 42', { launchMode: 'pty', kill() {} });
  expectMeshAgentOutputContract(events);
...
test('Hermes adapter opens a session and resolves the session ref from real session.create result fields', () => {
  ...
  hermesMeshAgentAdapter.initialize?.(handle, { workingPath: '/tmp/project', modelId: 'hermes-4' });
  ...
  const refEvents = hermesMeshAgentAdapter.parseOutput(
...
test('Hermes adapter resumes with the persistent key and sends a turn keyed on the ephemeral session_id', () => {
  ...
  hermesMeshAgentAdapter.initialize?.(handle, {
    workingPath: '/tmp/project',
...
test('Hermes adapter unwraps the event-wrapper envelope and resolves an approval by session id', () => {
  ...
  hermesMeshAgentAdapter.initialize?.(handle, { workingPath: '/tmp/project' });
  hermesMeshAgentAdapter.parseOutput(JSON.stringify({ id: 5, result: { session_id: 'sid-1' } }), handle);
  ...
  const events = hermesMeshAgentAdapter.parseOutput(chunk, handle);
  expectMeshAgentOutputContract(events);
  expect(events).toEqual([
  ...
  hermesMeshAgentAdapter.resolveApproval(handle, { requestId: 'sid-1:1', allow: true });
  expect(JSON.parse(writes[1] ?? '')).toEqual({
    method: 'approval.respond',
    id: 5,
    params: { session_id: 'sid-1', choice: 'once' }
  });
  const resolvedEvents = hermesMeshAgentAdapter.parseOutput(
...
test('Hermes adapter assigns distinct requestIds to overlapping approval requests in the same session', () => {
  ...
  hermesMeshAgentAdapter.initialize?.(handle, { workingPath: '/tmp/project' });
  hermesMeshAgentAdapter.parseOutput(JSON.stringify({ id: 9, result: { session_id: 'sid-9' } }), handle);
...
test('hermes managedRuntime runs the project callback via cli-oneshot (no persistent backend)', () => {
  ...
  expect(hermesMeshAgentAdapter.oneshotTurnArgs?.('do the thing', {})).toEqual(['--yolo', '-z', 'do the thing']);
});

test('openclaw managedRuntime enables the project callback via app-server + developer instructions', () => {
  const managed = openClawMeshAgentAdapter.managedRuntime;
...
test('OpenClaw sessions.create response with an empty key emits nothing (no invalid session_ref)', () => {
  ...
  const empty = openClawMeshAgentAdapter.parseOutput(
