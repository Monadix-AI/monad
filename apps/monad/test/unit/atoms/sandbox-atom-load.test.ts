...
import type { SandboxLauncher, WorkplaceExperienceDefinition } from '@monad/sdk-atom';
...
test('the power pack registers only contributed heavy launchers through the gated loader', async () => {
  ...
  await loadChannelAtomPacks([monadPowerPack], {
    ...
    onWorkplaceExperience: (experience) => experiences.push(experience)
  });
...
test('built-in pack registers workplace experiences through the gated loader', async () => {
  const experiences: WorkplaceExperienceDefinition[] = [];
  await loadChannelAtomPacks([builtinAtomPack], {
    onWorkplaceExperience: (experience) => experiences.push(experience)
  });
