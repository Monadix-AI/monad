...
import type { ModelProvider, WorkplaceExperienceDefinition } from '@monad/sdk-atom';

import { afterEach, expect, test } from 'bun:test';
import { unregisterMessageType, workplaceExperiencePermissionSchema } from '@monad/protocol';
...
function channel(type: string) {
  return defineChannel({
    type,
    name: type,
    icon: { title: type, path: 'M4 4h16v16H4z' },
    capabilities: caps,
    create: () => ({
      type,
      capabilities: caps,
...
function provider(type: string): ModelProvider {
  return defineProvider({
    type,
    descriptor: { type, label: type, icon: { title: type, path: 'M4 4h16v16H4z' }, strategy: 'native' },
...
const projectExperience: WorkplaceExperienceDefinition = {
  id: 'custom-workspace',
  title: 'Custom workspace',
  entry: { type: 'web-component', module: './workplace-experience.js', tagName: 'custom-workspace' }
};
...
test('workplace-experience atoms are forwarded to the daemon sink', async () => {
  const registered: WorkplaceExperienceDefinition[] = [];
  const experiencePack = defineAtomPack({
    manifest: { name: 'px', version: '1.0.0', sdkVersion: SDK_VERSION, atoms: ['workplace-experience'] },
    workplaceExperiences: [projectExperience]
  });

  await loadChannelAtomPacks([experiencePack], { onWorkplaceExperience: (experience) => registered.push(experience) });

  expect(registered).toEqual([projectExperience]);
});

test('workplace-experience duplicate ids are rejected by the daemon registry', () => {
  const registry = new AtomPackRegistry();
  registry.registerWorkplaceExperience(projectExperience, 'first-pack');

  expect(() =>
    registry.registerWorkplaceExperience({ ...projectExperience, title: 'Other custom workspace' }, 'second-pack')
  ).toThrow(/duplicate workplace experience id "custom-workspace"/);
  expect([...registry.workplaceExperiences.values()]).toEqual([
    { ...projectExperience, atomPackId: 'first-pack', permissions: [] }
  ]);
});

test('workplace-experience permissions come from the manifest, not from the definition an atom pack ships', () => {
  const registry = new AtomPackRegistry();

  registry.registerWorkplaceExperience(
    { ...projectExperience, permissions: ['project.approvals.resolve', 'project.members.remove'] },
    'greedy-pack',
    ['project.sessions.read']
  );

  expect([...registry.workplaceExperiences.values()]).toEqual([
    { ...projectExperience, atomPackId: 'greedy-pack', permissions: ['project.sessions.read'] }
  ]);
});

test('first-party workplace experiences are stamped with the full permission set', () => {
  const registry = new AtomPackRegistry();

  registry.registerWorkplaceExperience(
    { id: 'chat-room', title: 'Chat room', entry: { type: 'host-component', component: 'chat-room' } },
    'monad-builtins'
  );

  expect(registry.workplaceExperiences.get('chat-room')?.permissions).toEqual([
    ...workplaceExperiencePermissionSchema.options
  ]);
});

test('workplace-experience host component entries are first-party only', () => {
  const registry = new AtomPackRegistry();
...
test('workplace-experience API routes must be registered by the experience owner', () => {
  const registry = new AtomPackRegistry();
  registry.registerWorkplaceExperience(projectExperience, 'owner-pack');
  ...
  ).toThrow(/workplace experience API route "POST \/search" for "custom-workspace" from "attacker-pack" is not owned/);
  ...
  expect(registry.getWorkplaceExperienceApiHandler('custom-workspace', 'POST', '/search')).toBe(ownerHandler);
});

test('workplace-experience API routes cannot be preempted before the experience owner registers', () => {
  const registry = new AtomPackRegistry();
  ...
  ).toThrow(/unknown workplace experience id "custom-workspace"/);
...
test('undeclared workplace-experience atoms are rejected during daemon atom loading', async () => {
  const registered: WorkplaceExperienceDefinition[] = [];
  const errors: { atomPack: string; error: unknown }[] = [];
  const experiencePack = defineAtomPack({
  ...
  await loadChannelAtomPacks([experiencePack], {
    onError: (atomPack, error) => errors.push({ atomPack, error }),
    onWorkplaceExperience: (experience) => registered.push(experience)
  });
