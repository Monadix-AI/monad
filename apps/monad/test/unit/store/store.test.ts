...
describe('TTL', () => {
  ...
  test('expires key after TTL', async () => {
    store.set('k', Buffer.from('v'), { px: 20 });
    expect(store.get('k')?.toString()).toBe('v');
    await waitFor(() => store.get('k') === null, { message: 'key never expired' });
  ...
  test('expire sets TTL in seconds', () => {
    store.set('k', Buffer.from('v'));
    expect(store.expire('k', 10)).toBe(true);
    const remaining = store.pttl('k');
    expect(remaining).toBeGreaterThan(9_000);
    expect(remaining).toBeLessThanOrEqual(10_000);
  });
  test('expire with 0 seconds drops the key immediately', () => {
    store.set('k', Buffer.from('v'));
    store.expire('k', 0);
  ...
  test('sweep removes expired keys', async () => {
    store.set('a', Buffer.from('1'), { px: 10 });
    store.set('b', Buffer.from('2'));
    await waitFor(() => store.pttl('a') === -2, { message: 'key a never expired' });
    store.sweep();
    expect(store.dbsize()).toBe(1);
    expect(store.get('b')?.toString()).toBe('2');
  });
});
...
describe('inspect', () => {
  ...
  test('omits expired keys', async () => {
    store.set('gone', Buffer.from('v'), { px: 10 });
    store.set('stays', Buffer.from('v'));
    expect(store.inspect(10).strings.map((entry) => entry.key)).toEqual(['gone', 'stays']);
    await waitFor(() => store.pttl('gone') === -2, { message: 'key never expired' });
    expect(store.inspect(10).strings.map((entry) => entry.key)).toEqual(['stays']);
  });
});
