...
function insertRawEvent(
  sqlite: Database,
  input: { id?: string; sessionId: SessionId; type: Event['type']; payload: Record<string, unknown> }
): string {
  ...
  sqlite
    ...
    .run(id, input.sessionId, input.type, null, JSON.stringify(input.payload), '2026-07-20T00:00:00.000Z');
  return id;
}
...
test('legacy clarification rows are omitted from history and unresolved requests are reconciled before restore', () => {
  const store = createStore();
  const sqlite = (store as unknown as { sqlite: Database }).sqlite;
  const sessionId = newId('ses') as SessionId;
  const legacyPendingId = insertRawEvent(sqlite, {
    sessionId,
    type: 'clarify.requested',
    payload: { requestId: 'clarify_LEGACY_PENDING', question: 'Old pending question' }
  });
  insertRawEvent(sqlite, {
    sessionId,
    type: 'clarify.requested',
    payload: { requestId: 'clarify_LEGACY_DONE', question: 'Old answered question' }
  });
  insertRawEvent(sqlite, {
    sessionId,
    type: 'clarify.resolved',
    payload: { requestId: 'clarify_LEGACY_DONE', answer: 'Old answer' }
  });
  const current = completedEvent(sessionId, 'current');
  store.appendEvents([current]);

  expect(store.reconcileLegacyClarificationEvents()).toBe(1);
  expect(store.reconcileLegacyClarificationEvents()).toBe(0);
  expect(store.findDanglingInterrupts()).toEqual([]);
  expect(store.listEvents(sessionId).map((event) => event.id)).toEqual([current.id]);
  const reconciledPayload = sqlite.query('SELECT payload FROM events WHERE id = ?').get(legacyPendingId) as {
    payload: string;
  };
  expect(JSON.parse(reconciledPayload.payload)).toEqual({
    requestId: 'clarify_LEGACY_PENDING',
    question: 'Old pending question',
    legacyResolution: 'cancelled'
  });
  store.close();
});
