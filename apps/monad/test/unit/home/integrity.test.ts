...
describe('checkAndRepair', () => {
  ...
  test('preserves corrupt auth.json and requires manual version 2 migration', async () => {
    await initMonadHome(paths);
    await Bun.write(paths.auth, '{not valid json}');

    const store = createStore({ path: paths.db });
    await expect(checkAndRepair(paths, store)).rejects.toThrow(/auth\.json requires manual migration to version 2/);
    store.close();

    expect(await Bun.file(paths.auth).text()).toBe('{not valid json}');
  });
