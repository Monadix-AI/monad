...
test('MeshAgent structured line buffer joins split lines and keeps partial tails', () => {
...
test('MeshAgent structured line buffer discards oversized lines and resumes after newline', () => {
  const state = { text: '', discarding: false };
...
test('MeshAgent structured line buffer filters oversized complete lines without dropping following lines', () => {
  const state = { text: '', discarding: false };
