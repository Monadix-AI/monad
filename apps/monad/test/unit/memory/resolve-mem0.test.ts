...
const cfg = {
  model: {
    default: '',
    providers: [
      { id: 'oa', type: 'openai', label: 'OpenAI', credentials: [{ accessToken: 'sk-oa' }] },
      { id: 'ol', type: 'ollama', label: 'Ollama', baseUrl: 'http://localhost:11434/v1', credentials: [] },
      { id: 'an', type: 'anthropic', label: 'Anthropic', credentials: [] }
    ],
    profiles: [
      {
        alias: 'default',
...
const auth = emptyAuth() as MonadAuth;
...
test('openrouter LLM maps to openai provider with default base URL', () => {
  const orCfg = {
    model: {
      ...
      providers: [
        ...cfg.model.providers,
        { id: 'or', type: 'openrouter', label: 'OpenRouter', credentials: [{ accessToken: 'sk-or' }] }
      ],
      profiles: [
        ...cfg.model.profiles,
        {
  ...
  const r = resolveMem0Models(orCfg, auth, { llm: 'or-chat', embedder: 'oa:text-embedding-3-small' });
