...
import { resolveEnvironmentMap, resolveEnvironmentRef } from '#/config/environment-refs.ts';
import {
...
describe('environment resolution', () => {
  test('resolveEnvironmentRef resolves an environment reference', () => {
    Bun.env.MONAD_TEST_SECRET = 'sek';
    // biome-ignore lint/suspicious/noTemplateCurlyInString: testing literal secret-ref syntax
    expect(resolveEnvironmentRef('${env:MONAD_TEST_SECRET}')).toBe('sek');
  ...
  test('resolveEnvironmentRef passes plain values through', () => {
    expect(resolveEnvironmentRef('plain-token')).toBe('plain-token');
  });

  test('resolveEnvironmentRef throws on an unset env reference', () => {
    // biome-ignore lint/suspicious/noTemplateCurlyInString: testing literal secret-ref syntax
    expect(() => resolveEnvironmentRef('${env:MONAD_DEFINITELY_UNSET}')).toThrow(/unset/);
  });

  test('resolveEnvironmentRef rejects legacy secret references', () => {
    // biome-ignore lint/suspicious/noTemplateCurlyInString: testing literal secret-ref syntax
    expect(() => resolveEnvironmentRef('${secret:github}')).toThrow(/unsupported/);
  });

  test('resolveEnvironmentMap resolves every value', () => {
    Bun.env.MONAD_TEST_SECRET = 'sek';
    // biome-ignore lint/suspicious/noTemplateCurlyInString: testing literal secret-ref syntax
    expect(resolveEnvironmentMap({ A: '${env:MONAD_TEST_SECRET}', B: 'plain' })).toEqual({
      A: 'sek',
      B: 'plain'
    });
    // biome-ignore lint/suspicious/noUndeclaredEnvVars: test-only env var set/deleted in same block
...
describe('resolveConfigMcpSpecs — monadix preset synthesis', () => {
  ...
  test('synthesizes the monadix server when enabled', () => {
    ...
    expect(resolveConfigMcpSpecs(cfg)).toEqual([buildMonadixMcpServer(cfg.monadix)]);
  });

  test('omits the monadix server when disabled', () => {
    const cfg = baseCfg();
    cfg.monadix = { enabled: false };
    expect(resolveConfigMcpSpecs(cfg)).toEqual([]);
  });
