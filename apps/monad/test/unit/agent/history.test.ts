...
const messageContent = (id: string, text: string, replyToMessageId?: string): ModelContentPart[] => [
  {
    type: 'text',
    text: `[Monad message: id=${id}${replyToMessageId ? ` reply_to=${replyToMessageId}` : ''}]`
  },
  { type: 'text', text }
];
...
test('replayHistory coalesces adjacent user messages for model input only', () => {
  ...
  expect(replayed).toEqual([
    {
      role: 'user',
      content: [...messageContent('m1', 'A'), { type: 'text', text: '\n\n' }, ...messageContent('m2', 'B')]
    }
  ]);
});
...
test('replayHistory does not coalesce user messages across an assistant turn', () => {
  ...
  expect(replayed).toEqual([
    { role: 'user', content: messageContent('m1', 'A') },
    { role: 'assistant', content: messageContent('m2', 'answer') },
    { role: 'user', content: messageContent('m3', 'B') }
  ]);
});

test('replayHistory prefixes retained message content with id-only daemon metadata', () => {
  const answer: ChatMessage = {
    ...msg('msg_answer', 'user', 'answer body'),
    replyToMessageId: 'msg_question'
  };

  const replayed = replayHistory([answer]);

  expect(replayed).toEqual([
    {
      role: 'user',
      content: [
        { type: 'text', text: '[Monad message: id=msg_answer reply_to=msg_question]' },
        { type: 'text', text: 'answer body' }
      ]
    }
  ]);
  expect(JSON.stringify(replayed)).not.toContain('compacted target body');
});

test('replayHistory pairs a non-adjacent clarify lifecycle and includes question and answer once', () => {
  const question: ChatMessage = {
    ...msg('msg_question', 'assistant', 'Which branch?'),
    type: 'clarify',
    data: {
      requestId: 'clarify_request',
  ...
  const answer: ChatMessage = {
    ...msg('msg_answer', 'user', 'main'),
    replyToMessageId: 'msg_question'
  };
  const call: ChatMessage = {
    ...msg('msg_call', 'assistant', 'clarify call'),
    type: 'tool_call',
    data: {
  ...
  const result: ChatMessage = {
    ...msg('msg_result', 'tool', 'clarify result'),
    type: 'tool_result',
    data: {
      toolCallId: 'tool_call_1',
      toolName: 'clarify_ask',
      output: '{"answerMessageId":"msg_answer"}',
      result: {
        modelContent: '{"answerMessageId":"msg_answer"}',
        metadata: { answerMessageId: 'msg_answer' }
      },
      ok: true
    }
  };
  ...
  for (const history of [
    [question, answer, call, result],
    [call, question, answer, result]
  ]) {
    const replayed = replayHistory(history);
    const serialized = JSON.stringify(replayed);
    expect(replayed).toEqual(expected);
    expect(serialized.match(/Which branch\?/g)).toHaveLength(1);
    expect(serialized.match(/main/g)).toHaveLength(1);
  }
});

test('replayHistory anchors clarify lifecycle before a retained answer whose question was compacted away', () => {
  const answer: ChatMessage = {
    ...msg('msg_answer', 'user', 'main'),
    replyToMessageId: 'msg_compacted_question'
  };
  const call: ChatMessage = {
    ...msg('msg_call', 'assistant', 'clarify call'),
    type: 'tool_call',
  ...
  const replayed = replayHistory([answer, call, result]);
  ...
  expect(replayed).toEqual([
    ...
    },
    {
      role: 'user',
      content: [
        { type: 'text', text: '[Monad message: id=msg_answer reply_to=msg_compacted_question]' },
        { type: 'text', text: 'main' }
      ]
    }
  ]);
  expect(JSON.stringify(replayed)).not.toContain('compacted question body');
});
...
test('manual compact() folds the full loaded window even below the soft threshold', async () => {
  ...
  expect(lastPrompt()).toContain('user: [Monad message: id=m1] a');
  expect(lastPrompt()).toContain('assistant: [Monad message: id=m4] recent');
...
test('over threshold: compacts older rows, advances the durable boundary, keeps recent tail', async () => {
  ...
  expect(out.messages[0]?.content).toEqual(messageContent('m4', 'recent'));
...
test('next turn loads only since the boundary (bounded), folding the prior summary', async () => {
  ...
  expect(out.messages.map((m) => m.content)).toEqual([
    messageContent('m4', 'recent'),
    messageContent('m5', 'next question')
  ]);
});
...
test('background: soft threshold compacts WITHOUT blocking; the result lands on a later turn', async () => {
  ...
  expect(out2.messages.map((m) => m.content)).toEqual([messageContent('m3', 'recent')]);
});
...
test('background: a hard-threshold turn with no in-flight compaction compacts synchronously', async () => {
  ...
  expect(out.messages.map((m) => m.content)).toEqual([messageContent('m3', 'recent')]);
