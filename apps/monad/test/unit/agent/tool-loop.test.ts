import type { ChatMessage, Event, HookDecision, HookInput, Hooks, SessionId } from '@monad/protocol';
...
test('message_retrieve returns one active current-transcript message without resolving its reply target', async () => {
  const store = createStore();
  const transcriptTargetId = sid();
  const otherTranscriptTargetId = sid();
  const createdAt = new Date().toISOString();
  const target: ChatMessage = {
    id: newId('msg'),
    sessionId: transcriptTargetId,
    role: 'assistant',
    text: 'ancestor content must stay out',
    type: 'text',
  ...
  const crossTranscript: ChatMessage = {
    ...reply,
    id: newId('msg'),
    sessionId: otherTranscriptTargetId,
    replyToMessageId: undefined
  };
  for (const [message, fingerprint] of [
    [target, 'retrieve:target:v1'],
    [reply, 'retrieve:reply:v1'],
    [crossTranscript, 'retrieve:cross:v1']
  ] as const) {
    store.createMessage({ message, idempotencyKey: newId('idem'), fingerprint });
  }
  const lookup = new MessageLookup(
    store,
    ({ transcriptTargetId: requestedTranscript, actor }) =>
      actor.kind === 'daemon-agent' && actor.sessionId === requestedTranscript
  );
  const tool = createMessageRetrieveTool(lookup);
  const context = { sessionId: transcriptTargetId, log: () => {} };
  ...
  expect(tool.inputSchema?.safeParse({ messageId: 'not-a-message-id' }).success).toBe(false);
  store.close();
});
...
function canonicalClarifyTool(
  messages: InMemoryMessageRepo,
  questionMessageId: `msg_${string}`,
  answerMessageId: `msg_${string}`
) {
  return createClarifyTool(async (sessionId, request) => {
    messages.append({
      ...
      type: 'clarify',
      data: { requestId: 'clarify_TEST00000' },
    ...
    messages.append({
      ...
      sessionId: sessionId as SessionId,
      role: 'user',
      text: 'Use the safe path',
      type: 'text',
      replyToMessageId: questionMessageId,
      createdAt: '2026-07-21T00:00:01.000Z'
    });
    return { requestId: 'clarify_TEST00000', answer: 'Use the safe path', status: 'answered', answerMessageId };
  });
}

function expectCanonicalClarifyContinuation(
  prompt: ModelMessage[],
  ...
  expect(prompt.at(-1)).toEqual({
    role: 'user',
    content: [
      { type: 'text', text: `[Monad message: id=${answerMessageId} reply_to=${questionMessageId}]` },
      { type: 'text', text: 'Use the safe path' }
    ]
  });
  expect(
    prompt.filter(
      (message) =>
        message.role === 'user' &&
        ((typeof message.content === 'string' && message.content === 'Choose the path') ||
          (Array.isArray(message.content) &&
            message.content.some((part) => part.type === 'text' && part.text === 'Choose the path')))
    )
  ).toEqual([]);
}

test('block tool continuation receives the committed clarify answer once after the pointer result', async () => {
  const questionMessageId = newId('msg');
  const answerMessageId = newId('msg');
  const messages = new InMemoryMessageRepo();
  const { model, prompts } = capturingModel([
    { tool: 'clarify_ask', input: { question: 'Choose the path' } },
    'continued'
  ]);
  const loop = new AgentLoop({
    model,
    tools: [canonicalClarifyTool(messages, questionMessageId, answerMessageId)],
    messages,
    defaultModel: 'mock',
    emit: () => {}
  });

  await loop.runBlock(sid(), 'start');

  expect(prompts).toHaveLength(2);
  expectCanonicalClarifyContinuation(prompts[1] as ModelMessage[], questionMessageId, answerMessageId);
});

test('streaming tool continuation receives the committed clarify answer once after the pointer result', async () => {
  const questionMessageId = newId('msg');
  const answerMessageId = newId('msg');
  const messages = new InMemoryMessageRepo();
  const prompts: ModelMessage[][] = [];
      ...
      if (step++ === 0) {
        yield {
          ...
          call: {
            ...
            toolName: 'clarify_ask',
            input: { question: 'Choose the path' }
          }
        };
        return;
      }
      yield { type: 'text' as const, token: 'continued' };
    },
    async complete(): Promise<ModelResult> {
      return { text: 'unused', finishReason: 'stop' };
    }
    ...
    prompt.some(
      (m) =>
        ...
        (typeof m.content === 'string'
          ...
          : m.content.some((part) => part.type === 'text' && part.text.includes('Observation')))
    )
  ).toBe(true);
});
