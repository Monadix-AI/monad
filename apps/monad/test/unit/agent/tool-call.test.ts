...
test('rejects a registered tool hidden from the current session', async () => {
  const tool = createToolCallTool(
    () => [echoTool('private_tool')],
    (sessionId, name) => sessionId === 'owner_session' && name === 'private_tool'
  );

  await expect(
    (tool.run as (...args: unknown[]) => Promise<unknown>)({ name: 'private_tool', args: { msg: 'secret' } }, baseCtx)
