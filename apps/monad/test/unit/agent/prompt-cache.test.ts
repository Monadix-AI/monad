...
test('replayHistory excludes directive (slash-command) turns from the model prompt', () => {
  ...
  expect(replayed.map((m) => m.content)).toEqual([content('m1', 'hello'), content('m4', 'after the command')]);
});
...
test('replayHistory restores attachment model context without exposing it as visible message text', () => {
  ...
  expect(replayHistory(history)).toEqual([{ role: 'user', content: content('m1', modelText) }]);
});
...
test('replayHistory skips orphaned user message when generation failed with no assistant output', () => {
  ...
  expect(out.map((m) => m.content)).toEqual([
    content('m1', 'prior question'),
    content('m2', 'prior answer'),
    content('m5', 'retry question')
  ]);
});
...
test('replayHistory keeps user message when tool steps were persisted before the error', () => {
  ...
  expect(out[0]?.content).toEqual(content('m1', 'do complex task'));
  expect(out[3]?.content).toEqual(content('m6', 'retry'));
});
...
test('replayHistory coalesces a per-segment turn (preamble text + tool_call) into one assistant message', () => {
  ...
  expect(blocks[0]).toEqual({ type: 'text', text: '[Monad message: id=m2]' });
  expect(blocks[1]).toEqual({ type: 'text', text: 'Let me check.' });
  expect(blocks[2]).toMatchObject({ type: 'tool-call', toolName: 'search' });
  expect(out[3]?.content).toEqual(content('m5', 'The answer.'));
});
