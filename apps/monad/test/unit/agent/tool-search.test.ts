...
test('filters tools hidden from the current session out of deferred search', async () => {
  const model = makeModel('private_tool\nshared_tool');
  const tool = createToolSearchTool({
    model,
    searchModelId: 'fast',
    getTools: () => [makeTool('private_tool'), makeTool('shared_tool')],
    getToolRevision: () => 8,
    builtinToolNames: new Set(),
    filterForSession: (_sessionId, name) => name !== 'private_tool'
  });

  const result = await runSearch(tool, { query: 'find a tool' });
  expect(result).toContain('## shared_tool');
  expect(result).not.toContain('private_tool');
});
