...
test('plain error-like object: extracts message + numeric code', () => {
  const { code, message } = extractError({ type: 'Object', message: 'Internal Server Error', code: 500 });
  expect({ code, message }).toEqual({ code: '500', message: 'Internal Server Error' });
});
