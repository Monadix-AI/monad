// Real POSIX process-group kill on Linux. The unit-injected tests in external-agent-adapters.test.ts
// cover the platform branching with a stubbed kill fn; this one exercises the actual syscall path —
// killExternalAgentProcess must reap the whole group (the CLI leader AND anything it forked), not just
...
test('killExternalAgentProcess reaps the whole process group, not just the leader', async () => {
  ...
  // mirroring how an external agent forks subprocesses under one group.
  ...
  try {
    ...
    killExternalAgentProcess(parentPid, 'SIGTERM');
