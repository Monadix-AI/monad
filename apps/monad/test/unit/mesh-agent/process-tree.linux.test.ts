// Real POSIX process-group kill on Linux. The unit-injected tests in mesh-agent-adapters.test.ts
// cover the platform branching with a stubbed kill fn; this one exercises the actual syscall path —
// killMeshAgentProcess must reap the whole group (the CLI leader AND anything it forked), not just
...
test('killMeshAgentProcess reaps the whole process group, not just the leader', async () => {
  ...
  // mirroring how an MeshAgent forks subprocesses under one group.
  ...
  try {
    ...
    killMeshAgentProcess(parentPid, 'SIGTERM');
