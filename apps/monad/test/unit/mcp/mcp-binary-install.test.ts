...
test('falls back to a direct release asset when the GitHub API is rate limited', async () => {
  const bytes = new TextEncoder().encode('release binary');
  const requests: string[] = [];
  const progress: Array<{ assetName: string; loadedBytes: number; totalBytes?: number; percent?: number }> = [];
  const fetcher = createReleaseAssetFetcher({
    fetch: async (url) => {
      requests.push(url);
    ...
    resolveAssetName: (platform, arch) =>
      platform === 'darwin' && arch === 'arm64' ? 'widget-aarch64-apple-darwin.tar.gz' : null,
    onDownloadProgress: (value) => progress.push(value)
  });

  const asset = await fetcher(source, 'darwin', 'arm64');

  expect({
    requests,
    asset: { name: asset.name, bytes: [...asset.bytes], checksums: asset.checksums },
    progress
  }).toEqual({
    requests: [
      'https://api.github.com/repos/acme/widget-mcp/releases/tags/v1.0.0',
      'https://github.com/acme/widget-mcp/releases/download/v1.0.0/widget-aarch64-apple-darwin.tar.gz'
    ],
    asset: { name: 'widget-aarch64-apple-darwin.tar.gz', bytes: [...bytes], checksums: undefined },
    progress: [
      {
        assetName: 'widget-aarch64-apple-darwin.tar.gz',
...
test('keeps release metadata asset selection and checksum loading when the GitHub API succeeds', async () => {
  const bytes = new TextEncoder().encode('release binary');
    ...
    fetch: async (url) => {
      ...
      if (url.startsWith('https://api.github.com/')) {
        return Response.json({
          assets: [
            {
              name: 'widget-aarch64-apple-darwin.tar.gz',
              browser_download_url: 'https://downloads.example/widget.tar.gz'
            },
      ...
      if (url.endsWith('checksums.txt')) {
        return new Response(`${checksum}  widget-aarch64-apple-darwin.tar.gz\n`);
      }
  ...
  expect({
    ...
    requests: [
      'https://api.github.com/repos/acme/widget-mcp/releases/tags/v1.0.0',
      'https://downloads.example/widget.tar.gz',
      'https://downloads.example/checksums.txt'
    ],
    asset: {
      name: 'widget-aarch64-apple-darwin.tar.gz',
      bytes: [...bytes],
      checksums: [['widget-aarch64-apple-darwin.tar.gz', checksum]]
    }
  });
});
