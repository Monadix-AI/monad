...
test('unzip enforces entry-count and expanded-size limits before extraction', () => {
  const archive = makeZip('large', new TextEncoder().encode('expanded'));

  expect(() => unzip(archive, { maxEntries: 0 })).toThrow(/too many entries/i);
  expect(() => unzip(archive, { maxTotalBytes: 1 })).toThrow(/expands beyond limit/i);
});
