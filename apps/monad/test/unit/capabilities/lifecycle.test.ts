...
import { createAgentCapabilityRuntime, createCapabilitiesLifecycleModule } from '#/capabilities/lifecycle.ts';
...
test('registers static tools behind sandbox and credential protections', async () => {
  const runtime = createAgentCapabilityRuntime({
    paths,
...
test('creates capabilities after platform sandbox readiness', async () => {
  ...
  const expected = createAgentCapabilityRuntime({ paths, sandboxRoots: setup.sandboxRoots, tools: [] });
