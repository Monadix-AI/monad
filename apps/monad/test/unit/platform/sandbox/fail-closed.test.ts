...
test('createSandbox installs the exact XDG auth target in every child read-deny policy', async () => {
  const home = await mkdtemp(join(tmpdir(), 'monad-sandbox-vault-policy-'));
  const base = pathsForHome(home);
  const paths = {
    ...base,
    configs: join(home, 'xdg-config', 'monad'),
    credentials: join(home, 'xdg-config', 'monad', 'credentials'),
    auth: join(home, 'xdg-config', 'monad', 'auth.json')
  };
  const cfg = createDefaultConfig('Test');
  cfg.sandbox.confine = false;
  ...
  try {
    ...
    expect(sandboxReadDenyRoots()).toEqual(expect.arrayContaining([paths.auth, paths.credentials]));
    expect(sandboxReadDenyRoots()).not.toContain(paths.configs);
  } finally {
    await rm(home, { recursive: true, force: true });
  }
});
