...
test('legacy startup housekeeping schedules only its two database maintenance jobs', () => {
  const intervals: number[] = [];
  const events: string[] = [];
  const timer = { unref: () => void events.push('timer:unref') };
  const store = {
    reconcileOrphanedDelegates: () => void events.push('delegates:reconcile'),
    pruneOldAcpDelegates: () => {
      events.push('delegates:prune');
      return 0;
    },
    pruneExitedMeshSessions: () => {
      events.push('mesh:prune');
      return 0;
    }
  } as unknown as Store;
  ...
  expect({ events, intervals }).toEqual({
    events: ['delegates:reconcile', 'delegates:prune', 'timer:unref', 'mesh:prune', 'timer:unref'],
    intervals: [24 * 60 * 60 * 1_000, 24 * 60 * 60 * 1_000]
  });
});
