...
interface ModuleOptions<Snapshot> {
  after?: string[];
  afterReady?: RuntimeModule<Snapshot>['afterReady'];
...
test('invokes typed post-ready hooks only after the kernel reaches ready', async () => {
  const events: string[] = [];
  let kernel!: RuntimeKernel;
  kernel = new RuntimeKernel([
    runtimeModule('service', async () => 'service-output', {
      afterReady: (current, _context, signal) => {
        events.push(`${kernel.state.getState().phase}:${current}:${signal.aborted}`);
      }
    })
  ]);
  ...
  expect(events).toEqual(['ready:service-output:false']);
});
