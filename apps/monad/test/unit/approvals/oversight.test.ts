...
import { createOversightHandlers } from '#/handlers/daemon-handlers/handlers-oversight.ts';
...
test('the pending handler exposes each blocked call by its resolvable requestId', async () => {
  const { events, oversight } = capture();
  const handlers = createOversightHandlers(oversight);
  const gated = oversight.gate(gateReq);
  const requestId = requested(events)[0]?.payload.requestId as string;

  expect(await handlers.pending()).toEqual({
    pending: [{ requestId, sessionId: 'ses_TEST00000000', tool: 'email_send' }]
  });

  await handlers.approve({ requestId, allow: true });
  expect(await gated).toEqual({ allow: true });
  expect(await handlers.pending()).toEqual({ pending: [] });
});

test('the pending handler filters to one session', async () => {
  const { events, oversight } = capture();
  const handlers = createOversightHandlers(oversight);
  const gated = oversight.gate(gateReq);
  const requestId = requested(events)[0]?.payload.requestId as string;

  expect(await handlers.pending({ sessionId: 'ses_OTHER000000' })).toEqual({ pending: [] });

  await handlers.approve({ requestId, allow: false });
  await gated;
});
