...
import { applyMemoryCommandDiscovery, memoryCommandDecision } from '#/handlers/commands/memory-policy.ts';
...
describe('Agent Memory command policy', () => {
  const policy = (effectiveLevel: 0 | 1 | 2 | 3, advanced = effectiveLevel > 1) => ({
    agentId: effectiveLevel === 0 ? null : ('agt_000000000001' as never),
    effectiveLevel,
    enabled: effectiveLevel > 0,
    advanced
  });
  ...
  test.each([
    ...
    [3, ['check-memory', 'consolidate', 'memory', 'why']]
  ] as const)('L%i exposes only the allowed Memory commands', (level, expected) => {
    const memoryCommands = applyMemoryCommandDiscovery(
      seededCommandRegistry().list([], undefined, { filter: 'all' }),
      policy(level)
    )
      .filter((command) => command.group === 'Memory' && command.enabled)
      .map((command) => command.id)
      .sort();
    expect(memoryCommands).toEqual([...expected]);
  });

  test('L1 discovery keeps only consolidate under /memory', () => {
    const memory = applyMemoryCommandDiscovery(
      seededCommandRegistry().list([], undefined, { filter: 'all' }),
      policy(1, false)
    ).find((command) => command.id === 'memory');
    expect(memory?.subcommands?.map((subcommand) => subcommand.id)).toEqual(['consolidate']);
  });

  test('direct execution policy distinguishes disabled, Advanced, and global-level gates', () => {
    expect({
      disabled: memoryCommandDecision('memory', 'consolidate', policy(0)),
      advanced: memoryCommandDecision('consolidate', '2', policy(1, false)),
      global: memoryCommandDecision('why', 'belief', policy(2, true)),
      allowed: memoryCommandDecision('check-memory', '', policy(3, true))
    }).toEqual({
      disabled: { allowed: false, messageKey: 'cmd.memoryDisabledForAgent' },
      advanced: { allowed: false, messageKey: 'cmd.memoryAdvancedRequired' },
      global: { allowed: false, messageKey: 'cmd.memoryLevelRequired' },
      allowed: { allowed: true }
    });
  });

  test('dispatch passes parsed arguments to the host policy before running', async () => {
    let received = '';
    const result = await dispatchCommand(seededCommandRegistry(), '/consolidate 2', (args) => fakeCtx(args), {
      denyCommand: (_def, args) => {
        received = args;
        return { message: 'denied' };
      }
    });
    expect({ received, result }).toEqual({ received: '2', result: { message: 'denied' } });
  });
});
