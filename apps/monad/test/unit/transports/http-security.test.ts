...
test('resolveAllowedOrigin: rejects cross-site origins — does not reflect them', () => {
  expect(resolveAllowedOrigin(req({ host: 'dashboard.example.com', origin: 'https://attacker.example' }))).toBeNull();
});

test('resolveAllowedOrigin: returns null when no Origin header', () => {
  expect(resolveAllowedOrigin(req({ host: 'dashboard.example.com' }))).toBeNull();
});
...
test('corsHeaders exposes request correlation only to an allowed browser origin', () => {
  expect(corsHeaders(req({ host: 'localhost:52749', origin: 'http://localhost:52749' }))).toEqual({
    'access-control-allow-origin': 'http://localhost:52749',
    'access-control-allow-methods': 'GET, POST, PUT, PATCH, DELETE, OPTIONS',
    'access-control-allow-headers':
      'content-type, authorization, idempotency-key, x-monad-agent-id, x-monad-session-id',
    'access-control-allow-credentials': 'true',
    'access-control-expose-headers': 'x-monad-request-id'
  });
  expect(corsHeaders(req({ host: 'localhost:52749', origin: 'https://attacker.example' }))).toEqual({});
});
