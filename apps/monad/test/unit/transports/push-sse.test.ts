...
test('closes the stream and disposes the subscription when the request aborts', async () => {
  ...
  request.abort();
  const result = await Promise.race([reader.read(), Bun.sleep(25).then(() => null)]);
  const disposedOnAbort = disposed;
  if (result === null) await reader.cancel();

  expect({ disposed: disposedOnAbort, result }).toEqual({
    disposed: 1,
