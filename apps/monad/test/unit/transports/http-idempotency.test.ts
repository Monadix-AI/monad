...
const DIRECT_HANDLER_KEY = 'idem_89abcdef0123';
const DIRECT_UNKNOWN_KEY = 'idem_9abcdef01234';
const REPLAYED_ERROR_KEY = 'idem_abcdef012345';
...
async function canonicalError(response: Response): Promise<CanonicalError> {
  const body = (await response.json()) as CanonicalError;
  expect(body.requestId).toMatch(/^req_[0-9a-zA-Z]{12}$/);
  expect(response.headers.get('x-monad-request-id')).toBe(body.requestId);
  return body;
}

function normalizedError(body: CanonicalError): CanonicalError {
...
describe('HTTP Idempotency-Key', () => {
  ...
  test('rejects a duplicate key reused with a different body', async () => {
    ...
    expect(second.headers.get('x-monad-request-id')).toBe((second.body as CanonicalError).requestId);
    expect(normalizedError(second.body as CanonicalError)).toEqual({
  ...
  test('replays handler errors instead of leaving the key in progress', async () => {
    ...
    expect(normalizedError(first.body as CanonicalError)).toEqual({
      error: 'session not found: ses_missing00000',
      code: 'NOT_FOUND',
      retryable: false,
      requestId: '<request-id>'
    });
    expect(normalizedError(second.body as CanonicalError)).toEqual(normalizedError(first.body as CanonicalError));
    expect((second.body as CanonicalError).requestId).not.toBe((first.body as CanonicalError).requestId);
    expect(second.headers.get('x-monad-request-id')).toBe((second.body as CanonicalError).requestId);
    expect(second.headers.get('idempotent-replayed')).toBe('true');
  });
  ...
  test('rejects non-nanoid idempotency keys', async () => {
    ...
    expect(res.headers.get('x-monad-request-id')).toBe((res.body as CanonicalError).requestId);
    expect(normalizedError(res.body as CanonicalError)).toEqual({
  ...
  test('handles concurrent duplicate keys without running the write twice', async () => {
    ...
    expect(inProgress.headers.get('x-monad-request-id')).toBe((inProgress.body as CanonicalError).requestId);
    expect(normalizedError(inProgress.body as CanonicalError)).toEqual({
  ...
  test('redacts a caught HandlerError through the canonical public mapper', async () => {
    const response = await withHttpIdempotency({
      body: { secret: true },
      handler: () => {
        throw new HandlerError('invalid', 'rejected /Users/private token=secret');
      },
      method: 'POST',
      path: '/v1/direct',
      request: new Request('http://localhost/v1/direct', {
        method: 'POST',
        headers: { 'idempotency-key': DIRECT_HANDLER_KEY }
      }),
      scope: 'POST:/v1/direct',
      store: createInMemoryHttpIdempotencyStore()
    });
    expect(response.status).toBe(400);
    expect(normalizedError(await canonicalError(response))).toEqual({
  ...
  test('redacts a caught unknown error into the canonical internal envelope', async () => {
    const response = await withHttpIdempotency({
      body: {},
      handler: () => {
      ...
      request: new Request('http://localhost/v1/direct', {
        method: 'POST',
        headers: { 'idempotency-key': DIRECT_UNKNOWN_KEY }
      }),
      scope: 'POST:/v1/direct',
      store: createInMemoryHttpIdempotencyStore()
    });
  ...
  test('replayed stored errors use the current attempt correlation ID', async () => {
    const store = createInMemoryHttpIdempotencyStore();
    let attempts = 0;
    const run = () =>
      withHttpIdempotency({
        ...
        handler: () => {
          attempts += 1;
          throw new HandlerError('conflict', 'same safe conflict', 'CONFLICT');
        },
        method: 'POST',
        path: '/v1/direct',
        request: new Request('http://localhost/v1/direct', {
          method: 'POST',
          headers: { 'idempotency-key': REPLAYED_ERROR_KEY }
        }),
        scope: 'POST:/v1/direct',
        store
      });
    const first = await run();
    const replayed = await run();
    const firstBody = await canonicalError(first);
    const replayedBody = await canonicalError(replayed);
    expect(attempts).toBe(1);
    expect(first.status).toBe(409);
    expect(replayed.status).toBe(409);
    expect(normalizedError(firstBody)).toEqual({
      error: 'same safe conflict',
      code: 'CONFLICT',
      retryable: false,
      requestId: '<request-id>'
    });
    expect(normalizedError(replayedBody)).toEqual(normalizedError(firstBody));
    expect(replayedBody.requestId).not.toBe(firstBody.requestId);
    expect(replayed.headers.get('idempotent-replayed')).toBe('true');
  });
});
