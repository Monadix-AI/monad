...
test('POST /v1/sessions/:id/messages forwards attachments to the session handler', async () => {
  const app = buildApp();
  const sessionId = await createSession(app, 'attachment turn');
  const res = await app.handle(
    new Request(`http://localhost/v1/sessions/${sessionId}/messages`, {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
  ...
  const body = (await listed.json()) as {
    messages: Array<{ data?: { attachments?: Array<{ name: string; mime: string; bytes: number }> } }>;
  };
  expect(body.messages[0]?.data?.attachments).toEqual([
    expect.objectContaining({ name: 'preview.png', mime: 'image/png', bytes: 3 })
  ]);
});
