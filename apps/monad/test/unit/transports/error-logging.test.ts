...
test('JSON-RPC exposes the stable reply-target code without leaking an internal error', async () => {
  const replies: unknown[] = [];
  const handlers = buildHandlers(mockModel());
  const { sessionId } = await handlers.session.create({ title: 'rpc reply error' });
  ...
  await handleRpcMessage(
    JSON.stringify({
      ...
      params: { id: sessionId, text: 'reply', generate: false, replyToMessageId: newId('msg') }
    }),
    createConnectionState(),
    handlers,
    (message) => replies.push(message),
