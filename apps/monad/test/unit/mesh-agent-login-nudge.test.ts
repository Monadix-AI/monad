...
test('keeps a stable neutral login requirement across reprobes and resolves it by the same identity', async () => {
  const bus = new EventBus();
  const nudge = new MeshAgentLoginNudge({
    bus,
    authStatus: async () => authStatusResponse('unauthenticated')
  ...
  const dispose = nudge.start();
  const loginEvents: Array<{ at: string; payload: unknown }> = [];
  const resolved: unknown[] = [];
  bus.subscribe(sessionId, (event) => {
    if (event.type === 'mesh.login_required') loginEvents.push({ at: event.at, payload: event.payload });
    if (event.type === 'mesh.login_resolved') resolved.push(event.payload);
  });

  connectionRequired(bus);
  await settled();
  const first = nudge.pendingLoginRequirements(sessionId);
  const firstLiveAt = loginEvents[0]?.at;
  if (!firstLiveAt) throw new Error('login_required event was not published');

  connectionRequired(bus);
  await settled();
  const refreshed = nudge.pendingLoginRequirements(sessionId);

  expect({ first, refreshed, firstLiveAt }).toEqual({
    first: [
      {
        id: 'mesh-login:claude-code:claude-code',
        observedAt: firstLiveAt,
        meshSessionId: 'mesh_login0000000',
        agentName: 'claude-code',
        authAgentName: 'claude-code',
        provider: 'claude-code',
  ...
  expect({ pending: nudge.pendingLoginRequirements(sessionId), resolved }).toEqual({
    pending: [],
    resolved: [{ agentName: 'claude-code', authAgentName: 'claude-code', provider: 'claude-code' }]
  });
  dispose();
});

test('recreates a login card only after communication fails again following restart', async () => {
  const bus = new EventBus();
  const beforeRestart = new MeshAgentLoginNudge({
    bus,
    authStatus: async () => authStatusResponse('unauthenticated')
  });
  const disposeBeforeRestart = beforeRestart.start();

  connectionRequired(bus);
  await settled();
  expect(beforeRestart.pendingLoginRequiredEvents(sessionId).map((event) => event.type)).toEqual([
    'mesh.login_required'
  ]);
  disposeBeforeRestart();

  const afterRestart = new MeshAgentLoginNudge({
    bus,
    authStatus: async () => authStatusResponse('unauthenticated')
  });
  const disposeAfterRestart = afterRestart.start();
  expect(afterRestart.pendingLoginRequiredEvents(sessionId)).toEqual([]);

  connectionRequired(bus);
  await settled();

  expect(afterRestart.pendingLoginRequiredEvents(sessionId).map((event) => event.type)).toEqual([
    'mesh.login_required'
  ]);
  disposeAfterRestart();
});
...
test('refresh reprobe clears a pending login card after the agent authenticated elsewhere', async () => {
  ...
  expect(nudge.pendingLoginRequiredEvents(sessionId)).toEqual([]);
  expect(resolved).toEqual([{ agentName: 'pmem_claude_opus', authAgentName: 'claude-code', provider: 'claude-code' }]);
  dispose();
});
...
test('resolveAuthenticated publishes login_resolved to every session that got a nudge, once', async () => {
  ...
  expect(resolved).toEqual([{ agentName: 'claude-code', authAgentName: 'claude-code', provider: 'claude-code' }]);
  dispose();
});
...
test('resolveAuthenticated removes project member login cards for the authenticated config agent', async () => {
  ...
  expect(resolved).toEqual([
    { agentName: 'pmem_claude_opus', authAgentName: 'claude-code', provider: 'claude-code' },
    { agentName: 'pmem_claude_sonnet', authAgentName: 'claude-code', provider: 'claude-code' }
  ]);
  dispose();
});
