...
test('provider event pages receive the resolved agent environment', async () => {
  ...
  const eventAdapter = {
    ...
    events: {
      ...
      readPage: async (context: unknown) => {
        ...
        return { state: 'available' as const, view: 'raw' as const, records: [], coverage: 'settled' as const };
      }
    }
  ...
  const pages = new MeshAgentEventPages({
    live: new Map([[live.id, live]]),
    resolveAgentEnv: async () => ({ OPENCLAW_STATE_DIR: '/tmp/openclaw-state' }),
    store: { getMeshSession: () => undefined }
  } as never);

  await pages.rawEventsPage(live.id, { limit: 20 });
