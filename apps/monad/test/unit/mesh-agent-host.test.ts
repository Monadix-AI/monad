import type { Event, MeshAgentView, MeshSessionState } from '@monad/protocol';
...
if (!testEvents) throw new Error('built-in MeshAgent event source is required');
...
async function waitForMeshSession(
  store: ReturnType<typeof createStore>,
  id: string,
  predicate: (session: NonNullable<ReturnType<typeof store.getMeshSession>>) => boolean,
  timeoutMs = 10_000
) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const session = store.getMeshSession(id);
    if (session && predicate(session)) return session;
    await Bun.sleep(25);
  }
  throw new Error(`timed out waiting for MeshAgent session ${id}`);
}

test('MeshAgent auth status probes use a global 20 second timeout', () => {
  expect(AUTH_STATUS_TIMEOUT_MS).toBe(20_000);
});

test('MeshAgent auth status probes can use a host-specific timeout', async () => {
  ...
  const adapter = {
    ...
    authStatus: (agent: MeshAgentView) => ({
      launch: {
...
test('managed MeshAgent default server URL follows the daemon HTTPS switch', () => {
  expect(resolveMeshAgentManagedServerUrl({ networkHttps: { enabled: true }, port: 53210 })).toBe(
    'https://127.0.0.1:53210'
  );
  expect(resolveMeshAgentManagedServerUrl({ networkHttps: { enabled: false }, port: 53210 })).toBe(
...
test('managed MeshAgent explicit daemon port overrides ambient worktree port', () => {
...
test('MeshAgent host can stop every live session during daemon shutdown', () => {
  ...
  for (const id of ['mesh_shutdownone0', 'mesh_shutdowntwo0']) {
    store.upsertMeshSession({
      ...
      lastVisibleSeq: 0,
      state: 'running',
      pid: id === 'mesh_shutdownone0' ? 111 : 222,
  ...
  expect(killed).toEqual(['mesh_shutdownone0:SIGTERM', 'mesh_shutdowntwo0:SIGTERM']);
  expect(store.getMeshSession('mesh_shutdownone0')?.state).toBe('stopped');
  expect(store.getMeshSession('mesh_shutdowntwo0')?.state).toBe('stopped');
});

test('MeshAgent host stops live sessions for a disconnected provider adapter', () => {
  const store = createStore();
  const bus = new EventBus();
  const projectId = 'ses_01KWHOST4eTF';
  const meshSessionId = 'mesh_providerYAIx';
  ...
  const host = new MeshAgentHost({ store, bus, agents: async () => [] });
  const adapter = {
    provider: 'codex',
    productIcon: 'openai',
    parseOutput: () => [],
  ...
  expect(store.getMeshSession(meshSessionId)?.state).toBe('stopped');
});
...
test('managed provider final can retire a consumed inbox turn without auto-posting', async () => {
  ...
  store.upsertMeshSession({
    ...
    launchMode: 'app-server',
    runtimeRole: 'managed-project-agent',
    agentRuntimeId: 'mesh_hosttest0000',
    agentRuntimeTokenHash: null,
    lastDeliveredSeq: 1,
    lastVisibleSeq: 1,
    state: 'running',
  ...
  store.insertMessage('msg_THINKING0000', 'ses_01KWHOSTmAx6', '', '2026-07-02T00:00:02.000Z', 'assistant', {
    data: {
      agentName: 'codex',
      meshSessionId: 'mesh_hosttest0000',
      reasoning: 'Thinking',
  ...
  store.enqueueMeshAgentInboxItem('mesh_hosttest0000', 1);
  store.markMeshAgentInboxDelivered('mesh_hosttest0000', 1);
  store.markMeshAgentInboxConsumed('mesh_hosttest0000', 1);
  ...
  ).outputPipeline.emitManagedProjectOutput('ses_01KWHOSTmAx6', 'mesh_hosttest0000', 'No action needed.', false, false);
  await Bun.sleep(0);
...
test('managed MeshAgent output is observable only while its live raw store exists', async () => {
  const store = createStore();
  const host = new MeshAgentHost({
    store,
    bus: new EventBus(),
  ...
  const meshSessionId = 'mesh_hostsnap2f5a';
  store.upsertMeshSession({
    id: meshSessionId,
    transcriptTargetId: projectId,
    agentName: 'codex',
  ...
  ).outputPipeline.output(projectId, meshSessionId, '{"type":"result","result":"secret"}\n', 'stdout', adapter);
  await Bun.sleep(250);

  expect(host.observeRaw(meshSessionId)).toMatchObject({ state: 'live', observationEpoch: 'oep_test' });
  expect(store.getMeshSession(meshSessionId)?.outputSnapshot).toBe('');
  (
    host as unknown as {
      live: Map<string, unknown>;
    }
  ).live.delete(meshSessionId);
  expect(host.observeRaw(meshSessionId)).toEqual({
    state: 'unavailable',
    reason: 'provider events unavailable'
  });
});

test('MeshAgent observation stream pushes incremental deltas the client can reconstruct', async () => {
  const store = createStore();
  const host = new MeshAgentHost({ store, bus: new EventBus(), agents: async () => [] });
  const projectId = 'ses_01KWHOSTmAxf';
  const meshSessionId = 'mesh_hostdeltp9ri';
  store.upsertMeshSession({
    id: meshSessionId,
    transcriptTargetId: projectId,
    agentName: 'codex',
  ...
  const internal = host as unknown as {
    live: Map<string, unknown>;
    outputPipeline: {
      output(t: string, id: string, chunk: string, stream: string, adapter: MeshAgentProviderAdapter): void;
    };
  };
  internal.live.set(meshSessionId, live);

  const received: Array<{ cursor: string; data: unknown }> = [];
  const sub = host.subscribeRawObservation(meshSessionId, {
    onFrame: (frame) => received.push({ cursor: frame.cursor, data: frame.data }),
    onDone: () => {}
  });
  expect(sub.frames).toEqual([]);

  internal.outputPipeline.output(projectId, meshSessionId, 'Hello, ', 'stdout', adapter);
  await Bun.sleep(250);
  internal.outputPipeline.output(projectId, meshSessionId, 'world!', 'stdout', adapter);
  await Bun.sleep(250);

  expect(received).toEqual([
    { cursor: 'live:oep_test:1', data: 'Hello, ' },
    { cursor: 'live:oep_test:2', data: 'world!' }
  ]);
  sub.dispose();
});

test('MeshAgent observation resume returns only the delta past the client cursor', async () => {
  const host = new MeshAgentHost({ store: createStore(), bus: new EventBus(), agents: async () => [] });
  const id = 'mesh_resumeseED9Q';
  const adapter = {
    events: testEvents,
    provider: 'codex',
    productIcon: 'openai',
  ...
  const full = host.observeRaw(id);
  if (full.state !== 'live') throw new Error('expected live raw events');
  expect(full.frames.map(({ cursor, data }) => ({ cursor, data }))).toEqual([
    { cursor: 'live:oep_test:1', data: 'Hello, ' },
    { cursor: 'live:oep_test:2', data: 'world!' }
  ]);
  const resumed = host.observeRaw(id, 1);
  if (resumed.state !== 'live') throw new Error('expected resumable raw events');
  expect(resumed.frames.map(({ cursor, data }) => ({ cursor, data }))).toEqual([
    { cursor: 'live:oep_test:2', data: 'world!' }
  ]);
  const caughtUp = host.observeRaw(id, 2);
  if (caughtUp.state !== 'live') throw new Error('expected caught-up raw events');
  expect(caughtUp.frames).toEqual([]);
});

test('MeshAgent app-server reconnect re-dials the socket and resumes the thread', async () => {
  const host = new MeshAgentHost({
    store: createStore(),
    bus: new EventBus(),
    agents: async () => [],
  ...
  const meshSessionId = 'mesh_reconnec1Vci';
  ...
  await internal.appServerConnections.reconnect(meshSessionId);
...
test('MeshAgent app-server gives up instead of reconnecting forever when the transport keeps reopening but the handshake keeps failing', async () => {
  ...
  const meshSessionId = 'mesh_reconnecDa55';
  const adapter = {
    provider: 'openclaw',
  ...
  const live = {
    ...
    appServerRedial: async () => {
      ...
      if (redials <= MAX_DRIVER_ITERATIONS) {
        setTimeout(() => internal.appServerConnections.handleDisconnect(meshSessionId), 20);
      }
      return { send: () => {}, close: () => {} };
    },
    initializeContext: { workingPath: '/tmp/project' },
  ...
  internal.appServerConnections.handleDisconnect(meshSessionId);

  const deadline = Date.now() + 15_000;
  while (Date.now() < deadline && internal.live.has(meshSessionId)) {
    await Bun.sleep(100);
  }

  expect(internal.live.has(meshSessionId)).toBe(false); // gave up — not stuck reconnecting forever
  ...
  expect(events).toContain('mesh.connection_required'); // user-visible signal, not a silent hang
}, 20_000);

test('MeshAgent app-server disconnect during initial startup redials before failing, and exhaustion still rejects the pending startup', async () => {
...
test('MeshAgent input throws instead of silently vanishing into a stale connection while the app-server is reconnecting', async () => {
  ...
  const host = new MeshAgentHost({
    ...
    bus: new EventBus(),
    agents: async () => {
      throw new Error('MeshAgent input should not read agent config');
    }
  });
  const id = 'mesh_reconnecLZKf';
  let sent = 0;
  const adapter = {
    provider: 'openclaw',
    productIcon: 'openclaw',
...
test('MeshAgent idle suspend releases a resumable json-stream process and reconnects on input', async () => {
  const store = createStore();
  const workdir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-idle-'));
...
`
  ...
  const adapter = {
    ...
    listSupportedModels: () => [],
    resolveCommand: (command: string) => command,
    buildLaunch: (agent: MeshAgentView, opts: { providerSessionRef?: string }) => ({
      argv: [
        agent.command,
        ...(agent.args ?? []),
  ...
  bus.subscribe(projectId, (event) => {
    if (event.type === 'mesh.idle_suspended' || event.type === 'mesh.idle_resumed') {
  ...
  try {
    ...
    const suspended = await waitForMeshSession(
      store,
      view.id,
    ...
    expect(lifecycleEvents.map(({ type, payload }) => ({ type, payload }))).toEqual([
      {
        type: 'mesh.idle_suspended',
        payload: {
          agentId: 'pmem_idle_reviewer',
          agentName: 'Idle Reviewer',
          type: 'idle_suspended',
          payload: { meshSessionId: view.id, idleTimeoutMs: 300 }
        }
      }
    ]);
    const suspendedConnection = host.connectionSnapshot(view.id);
    expect(suspendedConnection).toMatchObject({ state: 'disconnected', meshSessionId: view.id });
    ...
    expect(store.getMeshSession(view.id)?.state).toBe('running');
    expect(store.getMeshSession(view.id)?.pid).toBeNumber();
    const resumedConnection = host.connectionSnapshot(view.id);
    if (resumedConnection.state !== 'connected') throw new Error('resumed MeshSession must be connected');
    expect(resumedConnection.observationEpoch).not.toBe(suspendedEpoch);
    expect(lifecycleEvents.map(({ type, payload }) => ({ type, payload }))).toEqual([
      {
        type: 'mesh.idle_suspended',
        payload: {
      ...
      {
        type: 'mesh.idle_resumed',
        payload: {
          agentId: 'pmem_idle_reviewer',
          agentName: 'Idle Reviewer',
          type: 'idle_resumed',
          payload: { meshSessionId: view.id }
        }
      }
    ]);
  } finally {
...
test('MeshAgent idle resume waits for app-server thread readiness before sending input', async () => {
  const store = createStore();
  const workdir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-app-server-idle-'));
  const logPath = join(workdir, 'runtime.log');
...
`
  ...
  const adapter = {
    ...
    listSupportedModels: () => [],
    resolveCommand: (command: string) => command,
    buildLaunch: (agent: MeshAgentView) => ({
      argv: [agent.command, ...(agent.args ?? [])],
      cwd: workdir,
      launchMode: 'app-server',
  ...
  try {
    ...
    for (let i = 0; i < 40 && store.getMeshSession(view.id)?.pid !== null; i++) {
      await Bun.sleep(25);
    }
...
test('MeshAgent idle suspend unlinks an app-server unix socket before later resume', async () => {
  const host = new MeshAgentHost({ store: createStore(), bus: new EventBus(), agents: async () => [] });
  const workdir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-socket-idle-'));
  const socketPath = join(workdir, 'provider.sock');
  writeFileSync(socketPath, '');
  const id = 'mesh_idlesockHcft';
  const adapter = {
    provider: 'codex',
    productIcon: 'codex',
    parseOutput: () => [],
...
test('MeshAgent idle resume keeps the pty fallback launch mode instead of rebuilding the original pty mode', async () => {
  const store = createStore();
  const workdir = mkdtempSync(join(tmpdir(), 'monad-mesh-agent-pty-fallback-idle-'));
  const logPath = join(workdir, 'runtime.log');
...
`
  ...
  const adapter = {
    ...
    listSupportedModels: () => [],
    resolveCommand: (command: string) => command,
    buildLaunch: (agent: MeshAgentView, opts: { launchMode?: string; providerSessionRef?: string }) => ({
      argv: [
        agent.command,
        ...(agent.args ?? []),
...
test('managed MeshAgent observation restores Codex provider history from persisted pointers', async () => {
  const store = createStore();
  const host = new MeshAgentHost({
    store,
    bus: new EventBus(),
  ...
  const meshSessionId = 'mesh_hostunavarpU';
  const providerSessionRef = crypto.randomUUID();
  const testRun = `monad-mesh-agent-host-${Date.now()}`;
  ...
  try {
    const page = await host.projectedEventsPage(meshSessionId, { view: 'convenience', limit: 100 });
    expect(page.events.filter((event) => event.role === 'agent').map((event) => event.text)).toEqual([
      'restored from provider history'
    ]);
  } finally {
...
test('managed MeshAgent observation ignores legacy snapshots and requires provider history', async () => {
  const store = createStore();
  ...
  const meshSessionId = 'mesh_hostsnapK8uP';
  ...
  await expect(host.projectedEventsPage(meshSessionId, { view: 'convenience', limit: 100 })).rejects.toMatchObject({
    code: 'unsupported_capability',
    message: `provider events unavailable for stopped session: ${meshSessionId}`
  });
  expect(providerAttempts).toBe(1);
});

test('managed MeshAgent events prefer the adapter local source over a CLI fallback', async () => {
  const root = join(homedir(), '.codex', 'sessions', '2099', '01', `monad-mesh-agent-host-cli-${Date.now()}`);
  ...
  const meshSessionId = 'mesh_hostclihRLrJ';
  const agent: MeshAgentView = {
    name: 'codex',
    provider: 'codex',
    command: script,
...
test('managed MeshAgent observation restores Claude Code provider history', async () => {
  const root = join(homedir(), '.claude', 'projects', `monad-mesh-agent-host-claude-${Date.now()}`);
  ...
  const meshSessionId = 'mesh_hostclauvTb9';
  const host = new MeshAgentHost({ store, bus: new EventBus(), agents: async () => [] });
  store.upsertMeshSession({
    id: meshSessionId,
    transcriptTargetId: projectId,
...
test('managed MeshAgent observation restores Gemini checkpoint history', async () => {
  const root = join(homedir(), '.gemini', 'tmp', `monad-mesh-agent-host-gemini-${Date.now()}`, 'chats');
  ...
  const meshSessionId = 'mesh_hostgemiqd5i';
  const host = new MeshAgentHost({ store, bus: new EventBus(), agents: async () => [] });
  store.upsertMeshSession({
    id: meshSessionId,
    transcriptTargetId: projectId,
...
test('managed MeshAgent observation restores Qwen stream-json history', async () => {
  const root = join(homedir(), '.qwen', 'monad-mesh-agent-host', String(Date.now()));
  ...
  const meshSessionId = 'mesh_hostqwen7L9e';
  const host = new MeshAgentHost({ store, bus: new EventBus(), agents: async () => [] });
  store.upsertMeshSession({
    id: meshSessionId,
    transcriptTargetId: projectId,
...
test('MeshAgent usage returns empty records when the adapter has no usage probe', async () => {
  const host = new MeshAgentHost({
    store: createStore(),
    bus: new EventBus(),
    agents: async () => [
...
test('listLive returns only starting/running runtimes across all projects', () => {
  ...
  insertSession('mesh_startb000000', 'ses_01KWLIVEhNu6', 'hermes', 'starting', '2026-07-02T00:00:02.000Z');
  insertSession('mesh_stopa0000000', 'ses_01KWLIVEhNu5', 'codex', 'stopped', '2026-07-02T00:00:03.000Z');
  insertSession('mesh_exitb0000000', 'ses_01KWLIVEhNu6', 'qwen', 'exited', '2026-07-02T00:00:04.000Z');

  const live = host.listLive().sessions;
  expect(live.map((s) => s.id)).toEqual(['mesh_runa00000000', 'mesh_startb000000']);
  expect(live.every((s) => s.state === 'running' || s.state === 'starting')).toBe(true);
  // spans multiple projects and surfaces framework (openclaw/hermes) providers, not just MeshAgents
...
test('listLive/listAllSummaries paginate with a cursor and expose nextCursor', () => {
  ...
  for (let i = 0; i < 5; i++) {
    store.upsertMeshSession({
      ...
      launchMode: 'app-server',
      runtimeRole: 'managed-project-agent',
      agentRuntimeId: `mesh_page${i}0000000`,
      agentRuntimeTokenHash: null,
      lastDeliveredSeq: 0,
      lastVisibleSeq: 0,
      state: 'running',
  ...
  expect(firstPage.sessions.map((s) => s.id)).toEqual(['mesh_page30000000', 'mesh_page40000000']);

  const secondPage = host.listLive({ limit: 2, before: firstPage.nextCursor });
  expect(secondPage.sessions.map((s) => s.id)).toEqual(['mesh_page10000000', 'mesh_page20000000']);

  const lastPage = host.listLive({ limit: 2, before: secondPage.nextCursor });
  expect(lastPage.sessions.map((s) => s.id)).toEqual(['mesh_page00000000']);
...
test('cli-oneshot session has no persistent process and runs a fresh CLI per turn', async () => {
  ...
  const host = new MeshAgentHost({ store, bus: new EventBus(), agents: async () => [agent] });
  const projectId = 'ses_01KWHOSTmAxf';
  try {
    const view = await host.start({
      transcriptTargetId: projectId,
    ...
    const observedOutput = (): string => {
      const observation = host.observeRaw(view.id);
...
test('an autopilot managed session auto-denies a leaked provider approval', () => {
  ...
  const { resolveCalls, live } = seedApprovalLiveSession(host, store, {
    ...
    id: 'mesh_approval0dVJ',
    proxyApprovals: false
  });

  expect(resolveCalls).toEqual([{ allow: false, reason: 'managed project MeshAgent provider approvals are disabled' }]);
  expect(live.pendingApprovals.has('req-1')).toBe(false);
});
...
test('a real spawned managed process actually receives the autopilot skip flag on its argv', async () => {
  ...
  const testAdapter: MeshAgentProviderAdapter = {
  ...
  const host = new MeshAgentHost({ store, bus: new EventBus(), agents: async () => agents, monadHome });
...
async function runRealAdapterArgvCapture(opts: {
  ...
  const host = new MeshAgentHost({ store, bus: new EventBus(), agents: async () => [agent], monadHome });
