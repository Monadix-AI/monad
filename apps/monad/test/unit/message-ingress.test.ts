...
test('durable commands commit once and publish the canonical lifecycle event once', async () => {
  ...
  const deliveredOutcome = await service.deliverWithOutcome({
  ...
  const delivered = deliveredOutcome.message;
  expect(deliveredOutcome.changed).toBe(true);
  ...
  expect(
    await service.deliverWithOutcome({
      transcriptTargetId: target,
      idempotencyKey: keys.ingressDeliver,
      producer,
      role: 'user',
  ...
  ).toEqual({ message: delivered, changed: false });
  expect(scoped).toHaveLength(1);
...
test('delivery preserves a reply edge in the committed message and created event', async () => {
  const { store, bus, service } = ingress();
  const target = newId('ses');
  const events: Event[] = [];
  bus.subscribe(target, (event) => events.push(event));
  const question = await service.deliver({
    transcriptTargetId: target,
  ...
  const answer = await service.deliver({
    transcriptTargetId: target,
    idempotencyKey,
    producer,
    role: 'assistant',
  ...
  expect(store.getMessage(target, answer.id)).toEqual(expectedAnswer);
...
test('command turns persist two directives and fan out the same canonical events', async () => {
  ...
  const scoped: Event[] = [];
  const inline: Event[] = [];
  const replyTarget = await service.deliver({
    transcriptTargetId: target,
    idempotencyKey: newId('idem'),
    producer,
    role: 'assistant',
...
test('streaming commands publish ordered deltas and reuse one terminal event across planes', async () => {
  ...
  const settledOutcome = await service.settleWithOutcome(
    {
      transcriptTargetId: target,
      messageId: pending.id,
  ...
  const settled = settledOutcome.message;
  expect(settledOutcome.changed).toBe(true);
  ...
  expect(
    await service.settleWithOutcome({
      transcriptTargetId: target,
      messageId: pending.id,
      idempotencyKey: keys.ingressSettle,
      producer,
  ...
  ).toEqual({ message: settled, changed: false });
...
test('a durable event append failure rolls back the message mutation atomically', async () => {
  const store = createStore();
  const bus = new EventBus();
  const target = newId('ses');
  const service = createMessageIngress({ store, bus, targetExists: () => true });
  (store as { appendEvents: (batch: Event[]) => void }).appendEvents = () => {
  ...
  // presence-ok: the durable event append fails inside the mutation transaction, so the whole
  // transaction rolls back — the message row and its revision bump must never have been persisted.
  expect(store.listMessages(target)).toEqual([]);
