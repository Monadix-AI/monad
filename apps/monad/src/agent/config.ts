...
// slots (AGENT.md / USER.md), and per-agent persona resolution. Everything here is a
...
export async function createConfigWatchers(deps: {
  ...
  // User-editable prompt slots from the workspace root — AGENT.md/AGENTS.md, USER.md.
  ...
  watchService.register({
    ...
    onChange: async () => {
      ...
      logger.info('monad: hot-reloaded workspace prompt slots (AGENT.md / USER.md)');
    }
  });
