import type { Cost, MessageId, SessionId } from '@monad/protocol';
...
import type { AgentLoopDeps, ChatMessage, ImageAttachment, TurnRunOptions } from './types.ts';
...
export class AgentLoop {
  ...
  async runStream(
    ...
    if (ex?.skill.fork && this.deps.runFork) {
      ...
      try {
        ...
        const segment = this.writer.beginSegment(sessionId, turn.assistantMessageId, turn.userMessageId, 0);
        segment.emitToken(result);
        await segment.settle(result);
        await this.writer.finishBookkeeping(sessionId, turn.assistantMessageId, result);
      } catch (err) {
        await this.writer.emitError(sessionId, turn.assistantMessageId, err);
        throw err;
      }
      return;
    }
  ...
  async runStreamFromHistory(
    sessionId: SessionId,
    signal?: AbortSignal,
    options?: { onHistoryPrepared?: () => void }
  ): Promise<void> {
    ...
    if (submit.blocked) {
      await this.writer.finishTurn(sessionId, messageId, trailingUser.id as MessageId, submit.reason);
      return;
    }
    this.prompt.setUserTextOverride(submit.text);
    await this.runAssistantStream(sessionId, messageId, trailingUser.id as MessageId, signal);
  }
  ...
  private async runAssistantStream(
    ...
    try {
      if (this.availableTools.length > 0 || this.deps.steers) {
        await this.runStreamWithTools(sessionId, messageId, triggerMessageId, signal);
        return;
      }

      const messages = await this.prompt.prepare(sessionId, await this.prompt.buildPrompt(sessionId));
      const seg = this.writer.beginSegment(sessionId, messageId, triggerMessageId, 0);
      let text = '';
      let reasoning = '';
      ...
      if (!(await seg.settle(stop.text, undefined, terminalData))) {
        await this.writer.appendEmptyAnswer(sessionId, messageId, triggerMessageId, terminalData);
      }
  ...
  async runBlock(
    ...
    if (ex?.skill.fork && this.deps.runFork) {
      ...
      try {
        ...
        return this.writer.finishTurn(sessionId, turn.assistantMessageId, turn.userMessageId, result);
      } catch (err) {
        await this.writer.emitError(sessionId, turn.assistantMessageId, err);
        throw err;
      }
    ...
    try {
      if (this.availableTools.length > 0) {
        ...
        return this.writer.finishTurn(sessionId, turn.assistantMessageId, turn.userMessageId, text, usage);
      }
  ...
  private async runStreamWithTools(
    ...
    let activeTriggerMessageId = triggerMessageId;
    let reasonBase = 0;
    ...
    for (; step < stepLimit && !budgetExceeded(); step++) {
      messages = await this.prompt.prepare(sessionId, messages); // re-bound each step: tool round-trips grow it
      const seg = this.writer.beginSegment(sessionId, segmentId, activeTriggerMessageId, reasonBase);
      ...
      if (isFinal) {
        const steeredTurn = await this.consumeSteers(sessionId, messages, true, {
          role: 'assistant',
          content: text || '(continuing)'
        });
        if (steeredTurn) {
          if (!wrote) await seg.settle(text, reasoning);
          segmentId = steeredTurn.assistantMessageId;
          activeTriggerMessageId = steeredTurn.userMessageId;
          continue;
        }
        ...
        if (!wrote) await this.writer.appendEmptyAnswer(sessionId, segmentId, activeTriggerMessageId, terminalData);
      ...
      const steeredTurn = await this.consumeSteers(sessionId, messages, false);
      if (steeredTurn) {
        segmentId = steeredTurn.assistantMessageId;
        activeTriggerMessageId = steeredTurn.userMessageId;
      } else {
    ...
    for (;;) {
      ...
      if (!(await seg.settle(stop.text, undefined, terminalData))) {
        await this.writer.appendEmptyAnswer(sessionId, segmentId, activeTriggerMessageId, terminalData);
      }
  ...
  ): Promise<{ assistantMessageId: `msg_${string}`; userMessageId: MessageId } | undefined> {
