...
import type { FileObservationStore, Tool, ToolBackends, ToolContext, ToolGate } from '#/capabilities/tools/types.ts';
...
export interface AgentLoopDeps {
  ...
  ask?: (
    sessionId: string,
    request: Parameters<NonNullable<ToolContext['ask']>>[0]
  ) => ReturnType<NonNullable<ToolContext['ask']>>;
  resolveClarification?: (
    requestId: string,
    action: 'complete' | 'cancel'
  ) => ReturnType<NonNullable<ToolContext['resolveClarification']>>;
