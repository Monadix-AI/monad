...
function messageMetadataPart(message: ChatMessage): ModelContentPart {
  return {
    type: 'text',
    text: `[Monad message: id=${message.id}${message.replyToMessageId ? ` reply_to=${message.replyToMessageId}` : ''}]`
  };
}

function messageOwnedContent(message: ChatMessage): ModelContentPart[] {
  const text =
    message.role === 'user' ? (asModelInputOverride(message.data)?.modelInput.text ?? message.text) : message.text;
  return [messageMetadataPart(message), { type: 'text', text }];
}

interface ReplayedToolPair {
  callIndex: number;
  resultIndex: number;
...
function toolPairs(history: ChatMessage[]): ReplayedToolPair[] {
  const results = new Map<string, Array<{ index: number; result: PersistedToolResult }>>();
  for (let index = 0; index < history.length; index++) {
    const message = history[index];
    if (message?.type !== 'tool_result') continue;
    const result = asPersistedToolResult(message.data);
    if (!result) continue;
    const candidates = results.get(result.toolCallId) ?? [];
    candidates.push({ index, result });
    results.set(result.toolCallId, candidates);
  }

  const pairs: ReplayedToolPair[] = [];
  const usedResults = new Set<number>();
  for (let index = 0; index < history.length; index++) {
    const message = history[index];
    if (message?.type !== 'tool_call') continue;
    const call = asPersistedToolCall(message.data);
    if (!call) continue;
    const candidate = results
      .get(call.toolCallId)
      ?.find(({ index: resultIndex }) => resultIndex > index && !usedResults.has(resultIndex));
    if (!candidate) continue;
    usedResults.add(candidate.index);
    pairs.push({ callIndex: index, resultIndex: candidate.index, call, result: candidate.result });
  }
  return pairs;
}

function clarifyLifecycleAnchors(
  history: ChatMessage[],
  pair: ReplayedToolPair
): { answerIndex: number; questionIndex?: number } | undefined {
  if (pair.call.toolName !== 'clarify_ask') return undefined;
  const metadata = pair.result.result?.metadata;
  const answerId = metadata && typeof metadata === 'object' ? Reflect.get(metadata, 'answerMessageId') : undefined;
  if (typeof answerId !== 'string') return undefined;
  const answerIndex = history.findIndex((message) => message.id === answerId);
  const answer = history[answerIndex];
  if (!answer?.replyToMessageId) return undefined;
  const questionIndex = history.findIndex((message) => message.id === answer.replyToMessageId);
  const question = history[questionIndex];
  const requestId =
    question?.data && typeof question.data === 'object' ? Reflect.get(question.data, 'requestId') : undefined;
  return {
    answerIndex,
    ...(question?.type === 'clarify' && typeof requestId === 'string' ? { questionIndex } : {})
  };
}

function replayToolPair(pair: ReplayedToolPair): ModelMessage[] {
  if (pair.call.providerExecuted) {
    return [
      {
        role: 'user',
        content: `${OBSERVATION_PREFIX}[${pair.call.toolName}]: ${modelOutputForToolResult(pair.result)}`
      }
    ];
  }
  return [
    {
      ...
      content: [
        {
          type: 'tool-call',
          toolCallId: pair.call.toolCallId,
    ...
    },
    {
      ...
      content: [
        {
          type: 'tool-result',
          toolCallId: pair.result.toolCallId,
          toolName: pair.result.toolName,
          output: modelOutputForToolResult(pair.result)
        }
      ]
    }
  ];
 ...
 * pairs (by toolCallId) with a later `tool_result` row is replayed
...
export function replayHistory(history: ChatMessage[]): ModelMessage[] {
  const out: ModelMessage[] = [];
  const pairs = toolPairs(history);
  const pairAt = new Map<number, ReplayedToolPair>();
  const pairedCallIndices = new Set<number>();
  const pairedResultIndices = new Set<number>();
  const linkedClarifyQuestionIndices = new Set<number>();
  for (const pair of pairs) {
    const anchors = clarifyLifecycleAnchors(history, pair);
    const insertionIndex = anchors
      ? Math.min(pair.callIndex, anchors.answerIndex, anchors.questionIndex ?? Number.POSITIVE_INFINITY)
      : pair.callIndex;
    pairAt.set(insertionIndex, pair);
    pairedCallIndices.add(pair.callIndex);
    pairedResultIndices.add(pair.resultIndex);
    if (anchors?.questionIndex !== undefined) linkedClarifyQuestionIndices.add(anchors.questionIndex);
  }
  for (let i = 0; i < history.length; i++) {
    ...
    if (pair) out.push(...replayToolPair(pair));
    if (pairedCallIndices.has(i) || pairedResultIndices.has(i) || linkedClarifyQuestionIndices.has(i)) continue;
    ...
    out.push({
      role: m.role,
      content: m.role === 'user' || m.role === 'assistant' ? messageOwnedContent(m) : m.text
    });
  }
