...
export class ToolExecutor {
  ...
  async runToolCalls(
    ...
    for (let i = 0; i < calls.length; i++) {
      ...
      if (call.toolName === 'clarify_ask') {
        const metadata = result?.metadata;
        const answerMessageId =
          metadata && typeof metadata === 'object' ? Reflect.get(metadata, 'answerMessageId') : undefined;
        const parsedAnswerMessageId = messageIdSchema.safeParse(answerMessageId);
        if (parsedAnswerMessageId.success) {
          const history = await this.deps.messages.list(sessionId);
          const answer = history.find(
            (message) => message.id === parsedAnswerMessageId.data && message.role === 'user'
          );
          if (answer) followups.push(...replayHistory([answer]));
        }
      }
  ...
  private async executeToolCall(sessionId: SessionId, call: ToolCall, signal?: AbortSignal): Promise<ExecuteOutcome> {
    ...
    try {
      const output = await invokeTool(tool, toolInput, {
        ...
        ask: ask ? (request) => ask(sessionId, request) : undefined,
        resolveClarification: this.deps.resolveClarification,
        log: () => {},
