...
import agentCredentialsPath from './prompts/agent-credentials.prompt.md' with { type: 'file' };
...
const AGENT_CREDENTIALS_TEMPLATE = await definePrompt<{
  credentials: readonly AgentCredentialManifestItem[];
}>({ id: 'agent.credentials', sourcePath: agentCredentialsPath });
...
export interface AgentCredentialManifestItem {
  label: string;
  description?: string;
...
export function renderAgentCredentialManifest(credentials: readonly AgentCredentialManifestItem[]): string {
  return credentials.length ? AGENT_CREDENTIALS_TEMPLATE.render({ credentials }) : '';
}
...
export type UserPromptSlots = Pick<SystemPromptSlots, 'agent' | 'user'>;
...
export function renderAgentSystemPrompt(input: {
  ...
  const values: Record<string, string | undefined> = {
    ...
    CREDENTIALS: input.slots.credentials,
  ...
  return CUSTOM_SYSTEM_TEMPLATE.render({
    ...
    slots: {
      ...
      credentials: used.has('CREDENTIALS') ? undefined : input.slots.credentials,
