import type { Event, GenerationParams, Hooks, ModelTier, SessionId } from '@monad/protocol';
...
import type { AgentLoopDeps, LoadedSkill, MessageRepo, ToolSearchConfig } from './loop/index.ts';
...
export interface AgentConfig {
  ...
  /** Non-secret Agent Credential manifest, resolved per turn from the live daemon snapshot. */
  credentialManifest?: string | ((sessionId?: SessionId) => string | undefined);
  /** Ambient run context (date/cwd/os/sandbox…) rendered into the system prompt. */
  environment?: AgentEnvironment;
  /** Absent → high-risk tools are denied. */
  gate?: ToolGate;
  /** Human-input bridge for interactive tools such as MCP input_required. */
  ask?: AgentLoopDeps['ask'];
  resolveClarification?: AgentLoopDeps['resolveClarification'];
   ...
   */
  resolveTier?: (modelTier: ModelTier) => string | undefined;
...
export function createAgent(config: AgentConfig): Agent {
  ...
  const runFork = async (
    ...
    const forkModel = (modelTier && config.resolveTier?.(modelTier)) || defaultModel;
  ...
  return {
    ...
    loop: (emit, opts) => {
      ...
      return new AgentLoop({
        ...
        linkAssistantReplies: opts?.linkAssistantReplies,
        steers: opts?.steers,
        ...
        gate: config.gate,
        ask: config.ask,
        resolveClarification: config.resolveClarification,
        hooks: config.hooks,
