...
import { register as messageRetrieveRegister } from '#/capabilities/tools/registry/message-retrieve.ts';
...
export interface AgentDeps {
  ...
  toolAllowedForSession?: (sessionId: string, toolName: string) => boolean;
  ...
  credentialManifest: (sessionId?: string) => string | undefined;
}
...
export function createAgentExecutionService(deps: AgentDeps): AgentExecutionService {
  ...
  const staticTools: ToolList = [
    ...
    ...messageRetrieveRegister({ lookup: messageLookup }),
    ...extraTools
  ];
  ...
  const toolSearchTool = only(
    toolSearchRegister({
      ...
      filterForSession: toolAllowedForSession,
      builtinToolNames,
      topK: 5
    })
  );
  const toolCallTool = only(toolCallRegister({ getTools: getAllTools, filterForSession: toolAllowedForSession }));

  const agent = createAgent({
    model: agentModel,
    ask: async (sessionId, request) => clarify.ask(sessionId, request),
    resolveClarification: async (requestId, action) => {
      const result = await clarify.respond(requestId, undefined, action);
    ...
    resolveTier: (modelTier) =>
      modelCatalog.pickProfileForTier(modelTier, modelService.profiles, modelService.tierOverrides),
    sessionRepo: {
