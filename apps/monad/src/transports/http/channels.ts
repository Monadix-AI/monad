...
const sendChannelMessageBodySchema = sendMessageRequestSchema.pick({
  attachments: true,
...
export function createChannelsController(
  ...
  return new Elysia({ tags: ['http-only'] })
    ...
    .post('/workplace/projects/reorder', async ({ body }) => handlers.session.reorderProject(body), {
      body: contracts.reorder.body,
      response: contracts.reorder.response,
      detail: { summary: 'Reorder workplace project', description: 'Moves a project to a persisted breakpoint.' }
    })
    .get(
