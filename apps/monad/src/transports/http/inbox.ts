...
export function createInboxController(handlers: ReturnType<typeof createDaemonHandlers>) {
  ...
  return new Elysia({ tags: ['http-only'] })
    ...
    .post('/inbox/read-all', () => handlers._nativeAgentStore.markAllOperatorInboxRead(), {
      response: contracts.readAll.response,
      detail: { summary: 'Mark every current Inbox item as read' }
    })
    .post('/inbox/unread', ({ body }) => handlers._nativeAgentStore.markOperatorInboxUnread(body.itemKeys), {
      body: contracts.unread.body,
      response: contracts.unread.response,
      detail: { summary: 'Mark Inbox items as unread' }
    })
    .get(
      '/inbox/mentions',
      ({ query }) => ({
