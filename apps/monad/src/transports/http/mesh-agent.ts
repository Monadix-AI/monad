...
function createMeshAgentSessionUsageSseResponse(
  handlers: ReturnType<typeof createDaemonHandlers>,
  id: string,
  ...
  return createPushSseResponse<MeshAgentSessionUsage>({
    encoder,
    encode: (usage) => encodeSseFrame({ event: 'mesh.session_usage', data: usage }, encoder),
    signal,
    subscribe: (emit) => {
      const subscription = handlers.meshAgent.subscribeSessionUsage({
        id,
        transcriptTargetId,
        onUsage: (usage) => emit(meshAgentSessionUsageSchema.parse(usage))
      });
      return { dispose: subscription.dispose };
    }
  });
...
function createMeshAgentRawObservationSseResponse(
  ...
  const stream = createByteBoundedSseStream({
    start(ctrl) {
      ...
      const sink = createBoundedSseEncoderSink<MeshRawEvent>(ctrl, encode, dispose);
      try {
...
export function createMeshAgentController(handlers: ReturnType<typeof createDaemonHandlers>) {
  ...
  return new Elysia()
    ...
    .get(
      '/mesh/sessions/:id/usage',
      ({ params, query }) => handlers.meshAgent.sessionUsage({ id: params.id, ...query }),
      {
        params: meshSessionParams,
        query: meshAgentScopeQuery,
        response: { 200: meshAgentSessionUsageResponseSchema },
        detail: { summary: 'Read provider session usage' }
      }
    )
    .get(
      '/mesh/sessions/:id/usage/events',
      ({ params, query, request }) =>
        createMeshAgentSessionUsageSseResponse(handlers, params.id, query.transcriptTargetId, encoder, request.signal),
      {
        params: meshSessionParams,
        query: meshAgentScopeQuery,
        detail: { summary: 'Stream provider session usage updates', tags: ['http-only'] }
      }
    )
    .get(
    ...
    .get('/mesh/usage', () => handlers.meshAgent.usageOverview(), {
      response: { 200: meshUsageOverviewResponseSchema },
      detail: { summary: 'Refresh and read normalized Mesh usage snapshots' }
    })
    ...
    .get(
      ...
      ({ params, query, request }) =>
        createMeshAgentAuthEventsSseResponse(handlers, params.id, query.controlToken, encoder, request.signal),
      {
        params: meshAuthSessionParams,
        query: meshAgentAuthScopeQuery,
