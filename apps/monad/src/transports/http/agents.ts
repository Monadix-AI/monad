    ...
    new Elysia()
      ...
      .post(
        ...
        },
        {
          ...
          detail: {
            ...
            description: 'Creates a new agent, validating any Agent Credential grants before persistence.'
          }
        }
      )
      ...
      .patch('/agents/:id', async ({ params, body }) => handlers.agent.updateAgent({ agentId: params.id, ...body }), {
        ...
        detail: {
          ...
          description:
            'Partially updates an agent (metadata, model, Atoms, Sandbox, visibility, and validated Credential grants).'
        }
      })
      ...
      .put(
        '/agents/:id/prompt',
        async ({ params, body }) => handlers.agent.setAgentPrompt({ agentId: params.id, ...body }),
        {
      ...
      .post(
        '/agents/:id/mcp',
        async ({ params, body }) => handlers.agent.installAgentMcp({ agentId: params.id, ...body }),
        {
          params: contracts.mcpInstall.params,
          body: contracts.mcpInstall.body,
          response: contracts.mcpInstall.response,
          detail: {
            tags: ['http-only'],
            summary: 'Add an Agent MCP server',
            description: 'Writes a live MCP server owned by one Agent.'
          }
        }
      )
      .get(
