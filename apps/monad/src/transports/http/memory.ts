...
export function createMemoryController(handlers: ReturnType<typeof createDaemonHandlers>) {
  return new Elysia({ tags: ['http-only'] })
    ...
    .post(
      '/memory/backend/prepare',
      async ({ body }) => handlers.memory.prepareBackend(setMemoryBackendRequestSchema.parse(body)),
      {
        body: setMemoryBackendRequestSchema,
        response: { 200: okResponseSchema },
        detail: {
          summary: 'Prepare memory backend',
          description: 'Download a required local dependency without changing the active backend.'
        }
      }
    )
