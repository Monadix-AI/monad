...
const SESSION_ORIGIN = buildOperationSource({ transport: 'http', surface: 'api', client: 'openai-compat' });
    ...
    new Elysia({ tags: ['http-only'] })
        ...
        try {
          ...
          return jsonResponse({
            ...
            data: listed.map((a) => ({
              ...
              owned_by: 'monad',
              name: a.name,
              description: `Monad agent ${a.name}`
            }))
          });
        } catch (err) {
      ...
      .post('/v1/chat/completions', async ({ request }) => {
        ...
        if (sessionIdOverride) {
          ...
          // interactive (web/tui/editor/channel) session of the owner. The transport can't tell those
          // apart (web/tui/api all collapse to the `http` transport), so gate on origin.client.
