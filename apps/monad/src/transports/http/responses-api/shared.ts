...
export const SESSION_ORIGIN = buildOperationSource({ transport: 'http', surface: 'api', client: 'responses-api' });
...
export function internalErrorResponse(code = 'internal_error'): Response {
  return errorResponse('An internal error occurred.', 500, 'api_error', code);
}
...
export function handlerErrorToResponse(err: unknown): Response {
  if (err instanceof HandlerError) {
    ...
    if (httpStatus >= 500) return internalErrorResponse(err.code ?? err.kind);
