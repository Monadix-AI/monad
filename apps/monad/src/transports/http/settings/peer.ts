...
// back out — the token goes in via /credential and is stored beside its peer in mesh.json.
  ...
  return new Elysia({ tags: ['http-only'] })
    ...
    .put(
      ...
      async ({ params, body }) => handlers.peer.setPeerCredential({ id: params.id, ...body }),
      {
        ...
        response: { 200: okResponseSchema, 404: httpErrorSchema },
        detail: { summary: 'Set peer credential', description: "Stores the peer's bearer token in mesh.json." }
      }
    )
