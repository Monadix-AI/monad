...
const taskParams = serverParams.extend({ taskId: z.string() });
...
export function createMcpServerSettingsController(handlers: ReturnType<typeof createDaemonHandlers>) {
  return new Elysia({ tags: ['http-only'] })
    ...
    .get('/mcp-servers/:name/tasks/:taskId', async ({ params }) => handlers.mcpServer.getMcpTask(params), {
      params: taskParams,
      response: { 200: getMcpTaskResponseSchema, 404: httpErrorSchema },
      detail: { summary: 'Observe MCP Task', description: 'Refreshes and returns one durable MCP Task.' }
    })
    .post('/mcp-servers/:name/tasks/:taskId/cancel', async ({ params }) => handlers.mcpServer.cancelMcpTask(params), {
      params: taskParams,
      response: { 200: getMcpTaskResponseSchema, 404: httpErrorSchema },
      detail: { summary: 'Cancel MCP Task', description: 'Requests cooperative cancellation of one MCP Task.' }
    })
