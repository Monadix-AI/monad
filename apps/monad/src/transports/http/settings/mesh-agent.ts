...
import {
  ...
  httpErrorSchema,
  listInvitableMeshAgentsResponseSchema,
...
export function createMeshAgentSettingsController(handlers: ReturnType<typeof createDaemonHandlers>) {
  return new Elysia({ tags: ['http-only'] })
    ...
    .get('/mesh/invitable-agents', async () => handlers.meshAgentSettings.listInvitableMeshAgents(), {
      response: { 200: listInvitableMeshAgentsResponseSchema },
      detail: { summary: 'List invitable MeshAgents' }
    })
    ...
    .post('/mesh/agents/refresh', () => handlers.meshAgentSettings.refreshCatalog(), {
      response: { 200: okResponseSchema },
      detail: { summary: 'Refresh the MeshAgent catalog' }
    })
