...
export function systemRemoteGuardResponse(
  request: Request,
  peerAddress: string | null | undefined
): Response | undefined {
  if (isLoopbackPeer(peerAddress)) return;
...
export function createSystemController(handlers: ReturnType<typeof createDaemonHandlers>) {
  ...
  return new Elysia({ tags: ['http-only'] })
    .onBeforeHandle(({ request, server }) => {
      ...
      return systemRemoteGuardResponse(request, server?.requestIP(request)?.address);
    })
