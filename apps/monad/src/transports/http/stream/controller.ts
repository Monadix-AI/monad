...
import { projectDirectHttpErrorResponse } from '#/transports/public-error.ts';
...
export function streamBrowserGuardResponse(request: Request, remoteEnabled: boolean): Response | undefined {
  if (isBrowserRequestAllowed(request, { remoteEnabled })) return;
  return projectDirectHttpErrorResponse(request, 403, 'FORBIDDEN', 'forbidden', false);
}
...
export function createStreamController(
  ...
  return new Elysia().ws('/stream', {
    ...
    beforeHandle({ request }: { request: Request }) {
      ...
      return streamBrowserGuardResponse(request, enabled);
    },
    open(ws: unknown) {
