...
import { matchEnvRef, mcpTaskJournalPath } from '@monad/environment';
...
import { mcpTaskRuntimeSnapshot } from '#/capabilities/tools/registry/mcp/task-runtime.ts';
import { resolveEnvironmentMap } from '#/config/environment-refs.ts';
...
export const configMcpSource = (name: string): string => `config-mcp:${name}`;
...
/** Open ONE config/preset MCP connection over its transport, resolving environment references
...
async function connectOneMcp(
  ...
  paths: MonadPaths,
  config: Pick<ConfigAccess, 'get' | 'updateConfig'> | undefined,
  ...
  if (spec.transport !== 'http') {
    return connectMcpServer({
      ...
      args: spec.args,
      env: resolveEnvironmentMap(spec.env),
      cwd: spec.cwd,
      requestTimeoutMs: spec.requestTimeoutMs,
      taskJournal: new McpTaskJournal(mcpTaskJournalPath(paths, spec.name))
    });
  }
  const oauth =
...
export async function connectMcpServers(
  ...
  auth?: MonadAuth,
  config?: Pick<ConfigAccess, 'get' | 'updateConfig'>
...
export async function reconnectOneMcpServer(
  ...
  try {
    const conn = await connectOneMcp(spec, paths, config, auth, options.interactive ?? true);
    if (!registerMcpTools(conn, spec.trust, registry, name, configMcpSource(name))) {
      await conn.close();
      status.set(name, { state: 'failed', error: 'tool set refused by trust policy' });
...
function requireConfigAccess(
  config: Pick<ConfigAccess, 'get' | 'updateConfig'> | undefined
): Pick<ConfigAccess, 'get' | 'updateConfig'> {
...
export function collectMcpStatus(input: {
  ...
  const taskRuntime = mcpTaskRuntimeSnapshot();
  ...
  for (const spec of resolveConfigMcpSpecs(input.cfg)) {
    ...
    const live = conn && !conn.failure ? conn : undefined;
    out.push({
      name: spec.name,
      source,
      transport: spec.transport,
      state: live ? 'ready' : conn?.failure ? 'failed' : (status?.state ?? 'failed'),
      ...(conn?.failure ? { error: conn.failure } : status?.error ? { error: status.error } : {}),
      ...(!live && status?.retryAttempt !== undefined ? { retryAttempt: status.retryAttempt } : {}),
      ...(!live && status?.nextRetryAt ? { nextRetryAt: status.nextRetryAt } : {}),
      ...(live?.protocolVersion ? { protocolVersion: live.protocolVersion } : {}),
      ...(live?.protocolEra ? { protocolEra: live.protocolEra } : {}),
      toolCount: live?.tools.length ?? 0,
      tools: live?.tools.map((t) => t.name) ?? [],
      ...(live?.tasks ? { tasks: taskStatus(taskRuntime[spec.name]) } : {})
    });
  }
  for (const conn of input.file) {
    out.push({
      ...
      ...(conn.protocolEra ? { protocolEra: conn.protocolEra } : {}),
      toolCount: conn.tools.length,
      tools: conn.tools.map((t) => t.name),
      ...(conn.tasks ? { tasks: taskStatus(taskRuntime[conn.name]) } : {})
    });
  }
...
function taskStatus(stats: ReturnType<typeof mcpTaskRuntimeSnapshot>[string] | undefined) {
  const value = stats ?? {
    active: 0,
    cancelled: 0,
    completed: 0,
  ...
  const settled = value.cancelled + value.completed + value.failed;
  return {
    supported: true,
    active: value.active,
    queued: value.queued,
    ...
    averageDurationMs: settled ? value.totalDurationMs / settled : 0
  };
}
 ...
 * pack is untrusted: it must NOT be able to resolve environment references into the
 ...
 * SAME trust block as a config.json server (pin / autoApprove / hostEscape). Trusted standalone
 * files may resolve environment references; pack-provided files may not. HTTP servers are deduped by normalized url —
...
export async function connectFileMcpServers(
  ...
  const mcpFiles = await discoverFileMcpSources(paths);

  const seenHttp = new Set<string>(seedHttp ?? []);
  const connections: McpConnection[] = [];
  for (const { source, filePath, trusted, agentDir } of mcpFiles) {
      ...
      try {
        ...
        const conn = spec.url
          ? await connectMcpServer({
              ...
              transport: 'http',
              url: spec.url,
              headers: trusted ? spec.headers : resolvePackSafeMap(spec.headers, source, name)
            })
          : await connectMcpServer({
              name,
              command: spec.command as string,
              args: spec.args,
              env: trusted ? resolveEnvironmentMap(spec.env) : resolvePackSafeMap(spec.env, source, name)
            });
        const sourceName = agentDir ? `agent:${agentDir}:mcp:${name}` : name;
        if (!registerMcpTools(conn, fileTrust(spec), registry, name, 'file-mcp', sourceName)) {
          await conn.close();
          continue;
        }
...
export interface FileMcpSource {
  source: string;
  filePath: string;
  trusted: boolean;
  agentDir?: string;
...
export async function discoverFileMcpSources(paths: MonadPaths): Promise<FileMcpSource[]> {
  const mcpFiles: FileMcpSource[] = [];
  ...
  try {
    const agents = await readdir(paths.agents, { withFileTypes: true });
    for (const agent of agents.filter((entry) => entry.isDirectory()).sort((a, b) => a.name.localeCompare(b.name))) {
      const mcpDir = join(paths.agents, agent.name, 'mcp');
      try {
        ...
        for (const entry of entries
          ...
          mcpFiles.push({
            source: `${agent.name}/${entry.name.replace(/\.json$/, '')}`,
            filePath: join(mcpDir, entry.name),
            trusted: true,
            agentDir: agent.name
          });
        }
...
/**
 * Build the HTTP headers for a streamable-http MCP server from its directly stored auth config.
 * OAuth is filtered out before this is reached.
 */
...
export function mcpHttpHeaders(
  ...
  if (spec.auth.mode === 'bearer') return { ...base, authorization: `Bearer ${spec.auth.token}` };
  if (spec.auth.mode === 'headers') return { ...base, ...spec.auth.headers };
  return base; // 'none'
}
...
function registerMcpTools(
  ...
  if (!applyMcpTools(conn, trust, registry, label, source, sourceName)) return false;
  conn.onToolsChanged?.(() => {
    registry.clearToolsFrom(source);
    if (!applyMcpTools(conn, trust, registry, label, source, sourceName)) {
      conn.failure = 'tool set refused by trust policy';
      conn.tools = [];
...
function applyMcpTools(
  ...
  for (const t of tools) registry.registerTool(t, source, sourceName);
  return true;
}
