...
import type { McpServerStatus, McpTaskView } from '@monad/protocol';
...
export function createMcpControls(deps: {
  ...
  const taskConnection = (name: string) => {
    const connection =
      getConfigMcp().connections.get(name)?.conn ?? fileMcpConnections().find((candidate) => candidate.name === name);
    if (!connection?.tasks) throw new Error(`MCP server "${name}" does not expose controllable Tasks`);
    return connection.tasks;
  };
  const mcpTaskObserve = (name: string, taskId: string): Promise<McpTaskView> => taskConnection(name).observe(taskId);
  const mcpTaskCancel = (name: string, taskId: string): Promise<McpTaskView> => taskConnection(name).cancel(taskId);

  return { getMcpStatus, mcpAuthorize, mcpReconnect, mcpTaskObserve, mcpTaskCancel };
}
