...
// token/client persistence beside the owning MCP server. The device flow (RFC 8628, for headless daemons) is NOT
...
function loadToken(config: Pick<ConfigAccess, 'get'>, name: string): StoredOAuth | null {
  ...
  if (name === 'monadix') return cfg.monadix.oauth ?? null;
  const server = cfg.mcpServers.find((candidate) => candidate.name === name);
  return server?.transport === 'http' ? (server.oauth ?? null) : null;
}

async function saveToken(config: Pick<ConfigAccess, 'updateConfig'>, name: string, tokens: StoredOAuth): Promise<void> {
  await config.updateConfig((cfg) => {
    if (name === 'monadix') {
      cfg.monadix = { ...cfg.monadix, oauth: tokens };
      return;
    }
    const server = cfg.mcpServers.find((candidate) => candidate.name === name);
    if (server?.transport !== 'http') {
      throw new McpOAuthError(`MCP server "${name}" is missing — cannot persist OAuth tokens`);
    }
    server.oauth = tokens;
  });
}
...
async function refreshSilently(opts: McpOAuthOptions): Promise<boolean> {
  ...
  const clientRef: ClientRef = {
    id: opts.clientId ?? stored.clientId,
    issuer: stored.issuer,
    fixed: opts.clientId !== undefined
  };
 ...
 *  Kept in memory because the stored OAuth record is all-or-nothing — a client_id can't be persisted
 ...
 * loopback callback server, hand the SDK a provider that persists beside the MCP server, and let it do
...
async function authorizeInteractive(opts: McpOAuthOptions): Promise<void> {
  ...
  const clientRef: ClientRef = {
    id: opts.clientId ?? persisted?.clientId,
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    try {
      ...
      if (result === 'REDIRECT') {
        const callback = await loopback.callback; // resolves on the callback (state-validated)
        await auth(provider, {
 ...
 *  (with their client_id) beside the MCP server. The client_id lives in `clientRef` until a token ships;
...
function createProvider(
  ...
  return {
    ...
    clientInformation(ctx) {
      const issuer = ctx?.issuer ?? clientRef.issuer;
      if (!clientRef.id || !issuer || (!clientRef.fixed && clientRef.issuer !== issuer)) return undefined;
      return { client_id: clientRef.id, issuer };
    },
    saveClientInformation(info: StoredOAuthClientInformation, ctx) {
      clientRef.id = info.client_id;
      clientRef.issuer = ctx?.issuer ?? info.issuer;
    },
    async tokens(ctx) {
      const s = loadToken(opts.config, opts.serverName);
      if (!s?.accessToken || !s.issuer || (ctx && s.issuer !== ctx.issuer)) return undefined;
      return {
    ...
    async saveTokens(tokens: StoredOAuthTokens, ctx) {
      const issuer = tokens.issuer || ctx?.issuer;
      if (!issuer) throw new McpOAuthError('OAuth token response is missing its authorization server issuer');
...
function startLoopback(expectedState: string): Loopback {
  let resolveCallback!: (callback: AuthorizationCallback) => void;
  let rejectCallback!: (err: Error) => void;
  const callback = new Promise<AuthorizationCallback>((resolve, reject) => {
    resolveCallback = resolve;
    rejectCallback = reject;
  });
  const timer = setTimeout(() => rejectCallback(new McpOAuthError('authorization timed out')), AUTHORIZE_TIMEOUT_MS);
  ...
  const server = Bun.serve({
    ...
    fetch(req) {
      ...
      if (error) {
        clearTimeout(timer);
        rejectCallback(new McpOAuthError(`authorization denied: ${error}`));
