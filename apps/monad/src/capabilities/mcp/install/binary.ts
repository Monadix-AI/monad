...
export interface ReleaseAssetFetcherOptions {
  githubToken?: string;
  fetch?: DownloadFetch;
  resolveAssetName?: (platform: NodeJS.Platform, arch: string) => string | null;
  onDownloadProgress?: (progress: ReleaseAssetDownloadProgress) => void;
}

/** Real fetcher: resolve the release by tag, pick the platform/arch asset, download its bytes. */
export function createReleaseAssetFetcher(opts: ReleaseAssetFetcherOptions = {}): ReleaseAssetFetcher {
  ...
  return async (source, platform, arch) => {
    ...
    const relRes = await fetchImpl(relUrl, { headers });
    ...
    if (relRes.ok) {
      ...
      chosen = selectReleaseAsset(
        assets.map((asset) => asset.name),
        platform,
        arch
      );
      const asset = assets.find((candidate) => candidate.name === chosen);
      if (!chosen)
        throw new McpBinaryInstallError(
          `no release asset for ${platform}/${arch} in ${source.owner}/${source.repo}@${source.tag} (have: ${assets.map((asset) => asset.name).join(', ') || 'none'})`
        );
      if (!asset) throw new McpBinaryInstallError(`release asset ${chosen} not found`);
      downloadUrl = asset.browser_download_url;
    } else if ((relRes.status === 403 || relRes.status === 429) && opts.resolveAssetName) {
      chosen = opts.resolveAssetName(platform, arch);
      if (!chosen)
        throw new McpBinaryInstallError(
          `cannot resolve direct release asset for ${platform}/${arch} in ${source.owner}/${source.repo}@${source.tag} after GitHub API failed: ${relRes.status}`
        );
      downloadUrl = `https://github.com/${source.owner}/${source.repo}/releases/download/${encodeURIComponent(source.tag)}/${encodeURIComponent(chosen)}`;
    } else {
      throw new McpBinaryInstallError(
    ...
    const download = await downloadBytes(downloadUrl, {
      fetch: fetchImpl,
      headers: dlHeaders,
    ...
    if (sums) {
      const sumRes = await fetchImpl(sums.browser_download_url, { headers: dlHeaders }).catch(() => null);
