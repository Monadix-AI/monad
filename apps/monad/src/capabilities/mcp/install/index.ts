...
import type { InstalledMcpAtom, InstallMcpAtomRequest } from '@monad/protocol';
...
type InstallMcpServer = InstallMcpAtomRequest['server'];

function credentialHeaders(server: Extract<InstallMcpServer, { transport: 'http' }>): Record<string, string> {
  if (server.auth.mode === 'oauth') {
    throw new McpInstallError('OAuth is not supported for a hot atoms/mcp server');
  }
  const headers: Record<string, string> = {};
  if (server.auth.mode === 'bearer') {
    const token = server.auth.token;
    if (token && 'configured' in token) {
      throw new McpInstallError('A redacted bearer credential cannot be installed without a replacement value');
    }
    if (token?.action === 'replace') headers.authorization = `Bearer ${token.value}`;
  }
  if (server.auth.mode === 'headers') {
    for (const [name, update] of Object.entries(server.auth.headers)) {
      if ('configured' in update) {
        throw new McpInstallError('Redacted header credentials cannot be installed without replacement values');
      }
      if (update.action === 'replace') headers[name] = update.value;
    }
  }
  for (const [name, update] of Object.entries(server.headers ?? {})) {
    if ('configured' in update) {
      if (update.configured)
        throw new McpInstallError('Redacted headers cannot be installed without replacement values');
      continue;
    }
    if (update.action === 'replace') headers[name] = update.value;
  }
...
/** Map a write-only server request onto its owner-local file-MCP entry. */
function toFileEntry(server: InstallMcpServer): FileMcpEntry {
  ...
  const headers = credentialHeaders(server);
  return { url: server.url, headers: Object.keys(headers).length ? headers : undefined, trust };
}

export async function installMcpAtom(
  server: InstallMcpServer,
  deps: InstallMcpAtomDeps
): Promise<InstallMcpAtomOutcome> {
