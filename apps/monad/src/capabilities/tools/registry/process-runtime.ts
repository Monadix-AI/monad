...
import {
  ...
  sandboxedSpawn,
  ToolSecurityError,
  waitForProtectedExecutionTeardown
} from '@monad/sandbox';
...
export interface ProcessHandle {
  ...
  protectedExecutionTeardown: Promise<void>;
...
function pipeHandle(proc: Sub & { supervision?: SpawnSupervision }): ProcessHandle {
  return {
    ...
    protectedExecutionTeardown: waitForProtectedExecutionTeardown(proc),
    kill: (reason = 'manual') => {
    ...
    { cwd: dir, stdin: 'pipe', stdout: 'pipe', stderr: 'pipe', detached: true },
    {
      ...
      spawn: ((argv, spawnOptions) =>
        sandboxedSpawn(argv, spawnOptions, policy, {
          ...
          agentId: ctx.agentId,
          protectedExecution: options.protectedExecution
        })) as typeof Bun.spawn
    }
  );
