...
export const followSystemBackend: CodeExecBackend = {
  ...
  isAvailable: () => true,
  async execute(req) {
    const protectedExecution = req.protectedExecution ?? (await resolveProtectedExecutionForAgent(req.agentId));
...
const dockerCodeExecBackend: CodeExecBackend = {
  ...
  async execute(req) {
    if ((req.protectedExecution?.credentials.length ?? 0) > 0) throw new ProtectedExecutionError();
...
export const codeExecTool: Tool<z.infer<typeof codeExecInput>, CodeExecResult> = {
  ...
  run: async ({ language, code, timeoutMs, target }, ctx) => {
    ...
    if (protectedExecution.credentials.length > 0 && (target === 'host' || backend !== followSystemBackend)) {
