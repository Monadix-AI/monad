...
function getEntry(id: string, ctx: ToolContext): ProcEntry {
  ...
  const owned = entry?.ownerSessionId === ctx.sessionId && entry.ownerAgentId === ctx.agentId;
  if (entry && !owned) {
  ...
  if (!entry || !owned) throw new ToolSecurityError(`unknown process id "${id}"`);
  return entry;
}
...
export async function startBackgroundProcess(
  ...
  const startPty = (): ProcessHandle =>
    startPtyProcess(
      ...
      { cols, rows, maxRuntimeMs, signal: runCtx.signal, protectedExecution }
    );
  ...
  if (mode === 'pty') {
    try {
      ...
      if (err instanceof ProtectedExecutionError) throw err;
  ...
  entry = {
    ...
    credentialBearing: protectedExecution.credentials.length > 0,
    command,
    cwd: dir,
    mode,
    proc: started,
...
export async function terminateProcessesForSessionAgentChange(sessionId: string): Promise<void> {
  const entries = Array.from(registry.values()).filter((entry) => entry.ownerSessionId === sessionId);
  for (const entry of entries) {
    ...
    entry.proc.kill('shutdown');
    try {
      entry.proc.signal('SIGKILL');
    } catch {
      // The graceful stop may already have completed.
    }
  }
  await Promise.all(
    entries.map(async (entry) => {
      await entry.proc.exited.catch(() => {});
      await entry.proc.protectedExecutionTeardown;
    })
  );
}
...
function listProcesses(ctx: ToolContext, status?: ProcEntry['status']): ProcessListResult {
  ...
  return {
    processes: Array.from(registry.values())
      .filter((e) => e.ownerSessionId === ctx.sessionId && e.ownerAgentId === ctx.agentId)
