...
export function createToolCallTool(
  getTools: () => Tool[],
  filterForSession?: (sessionId: string, toolName: string) => boolean
  ...
  return {
    ...
    async run(input, ctx: ToolContext) {
      if (filterForSession && !filterForSession(ctx.sessionId, input.name)) {
        throw new Error(`Tool "${input.name}" is not available in this session.`);
      }
...
export const register: ToolModule<{
  getTools: () => Tool[];
  filterForSession?: (sessionId: string, toolName: string) => boolean;
}> = ({ getTools, filterForSession }) => [createToolCallTool(getTools, filterForSession)];
