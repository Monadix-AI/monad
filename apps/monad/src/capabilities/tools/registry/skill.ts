...
import type { ModelTier } from '@monad/protocol';
 ...
 * `modelTier` is the skill's declared capability tier (resolved to a model by the routing layer).
 */
...
function skillDisplayContent(skill: LoadedSkill, body: string, resource?: string) {
  return {
    ...
    ...(skill.modelTier ? { modelTier: skill.modelTier } : {}),
    ...(resource ? { resource } : {})
  };
}
  ...
  return {
    ...
    run: async ({ name, file }, ctx) => {
      ...
      if (skill.fork && runFork) {
        const result = await runFork(substituteSkillDir(skill.body, skill.dir), ctx, skill.modelTier, name);
