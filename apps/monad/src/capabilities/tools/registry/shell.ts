...
import {
  ...
  canSkipHighRiskApprovalInLocalSandbox,
...
function terminalBackend(ctx: ToolContext, protectedExecution: ProtectedExecutionContext) {
  if (protectedExecution.credentials.length > 0) {
    if (ctx.backends?.terminal.delegated) throw new ProtectedExecutionError();
  }
  return (
...
export const shellExecTool: Tool<z.infer<typeof shellExecInput>, ShellResult> = {
  ...
  run: async ({ command, cwd, timeoutMs, mode, terminalMode, cols, rows, idleTimeoutMs, maxRuntimeMs }, ctx) => {
    const protectedExecution = await resolveProtectedExecutionForAgent(ctx.agentId);
    if (protectedExecution.credentials.length > 0 && ctx.backends?.terminal.delegated) {
    ...
    return toolResult(normalizeForeground(await exec(terminalBackend(ctx, protectedExecution))));
  }
};
