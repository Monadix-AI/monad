...
import type { ProtectedExecutionContext } from '@monad/sandbox';
 ...
 */
function assertPathOutsideRoots(path: string, deniedRoots: readonly string[]): string {
  const candidate = resolve(path);
  for (const root of deniedRoots) {
    const rel = relative(resolve(root), candidate);
    if (rel === '' || (!rel.startsWith('..') && !isAbsolute(rel))) {
      throw new ToolSecurityError('sandbox path denied');
    }
  }
  return candidate;
}
...
export async function resolveReal(path: string, roots: string[] | undefined, mustExist = true): Promise<string> {
  ...
  const deniedRoots = sandboxReadDenyRoots();
  assertPathOutsideRoots(resolved, deniedRoots);
  const realRoots = roots ? await Promise.all(roots.map((r) => realpath(r).catch(() => r))) : undefined;
  const realDeniedRoots = await Promise.all(deniedRoots.map((root) => realpath(root).catch(() => resolve(root))));
  if (mustExist) {
    const real = await realpath(resolved);
    return assertPathOutsideRoots(assertPathWithinRoots(real, realRoots), realDeniedRoots);
  }
  const realDir = await realpath(dirname(resolved));
  return assertPathOutsideRoots(assertPathWithinRoots(join(realDir, basename(resolved)), realRoots), realDeniedRoots);
}

async function resolveCreatable(path: string, roots: string[] | undefined): Promise<string> {
  const resolved = assertPathWithinRoots(path, roots);
  const deniedRoots = sandboxReadDenyRoots();
  assertPathOutsideRoots(resolved, deniedRoots);
  const [realRoots, realDeniedRoots] = await Promise.all([
    roots ? Promise.all(roots.map((root) => realpath(root).catch(() => root))) : undefined,
    Promise.all(deniedRoots.map((root) => realpath(root).catch(() => resolve(root))))
  ]);
  ...
  while (true) {
    ...
    const candidate = join(realAncestor, ...suffix);
    return assertPathOutsideRoots(assertPathWithinRoots(candidate, realRoots), realDeniedRoots);
  }
}
...
async function sandboxExec(
  ...
  const proc = sandboxedSpawn(
    ...
    { sessionId, agentId, protectedExecution }
  );
...
export function createSandboxBackends(
  ...
  const { defaultCwd, sessionId, agentId, protectedExecution } = opts ?? {};
  return {
    fs: {
      delegated: false,
      ...
      async writeTextFile(path, content) {
        const creatable = await resolveCreatable(path, sandboxRoots);
        await mkdir(dirname(creatable), { recursive: true });
        ...
        try {
          leafIsSymlink = (await lstat(creatable)).isSymbolicLink();
        } catch {
      ...
      async moveFile(path, newPath) {
        const real = await resolveReal(path, sandboxRoots);
        const creatableNew = await resolveCreatable(newPath, sandboxRoots);
        await mkdir(dirname(creatableNew), { recursive: true });
    ...
    terminal: {
      delegated: false,
      exec: (opts) => sandboxExec(opts, sandboxRoots, defaultCwd, sessionId, agentId, protectedExecution)
    }
  };
}
