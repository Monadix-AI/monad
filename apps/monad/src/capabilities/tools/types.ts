...
import type { ResourceScope } from '@monad/protocol';
...
export interface ToolContext {
  ...
  /** Ask the session's user for input while a tool is running. */
  ask?(request: {
    requestId?: string;
    question: string;
    options?: string[];
    ...
    asker?: import('@monad/protocol').ClarifyAsker;
    urlElicitation?: import('@monad/protocol').UrlElicitation;
  }): Promise<{
    answer: string;
    status: 'answered' | 'timed-out' | 'cancelled';
  }>;
  /** Resolve a pending clarification from an out-of-band protocol notification. */
  resolveClarification?(requestId: string, action: 'complete' | 'cancel'): Promise<boolean>;
