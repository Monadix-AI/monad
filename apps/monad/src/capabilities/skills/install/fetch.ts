...
async function fetchGithubViaGit(
  source: GithubSource,
  executeGit: (args: string[]) => Promise<string>
): Promise<StagedSkillRepo> {
  ...
  try {
    if (source.path) {
      try {
        ...
        await executeGit(['git', '-C', checkoutDir, 'checkout', source.ref]);
      }
      await executeGit(['git', '-C', checkoutDir, 'sparse-checkout', 'set', source.path]);
    } else {
      try {
        await executeGit([
          'git',
        ...
        await executeGit(['git', 'clone', '--depth', '1', '--single-branch', repoUrl, checkoutDir]);
        await executeGit(['git', '-C', checkoutDir, 'checkout', source.ref]);
      }
    }
    const commit = await executeGit(['git', '-C', checkoutDir, 'rev-parse', 'HEAD']);
...
export function createSkillFetcher(opts: SkillFetcherOptions = {}): SkillFetcher {
  return async (source) => {
    try {
      ...
      return fetchGithubViaGit(source, opts.runGit ?? runGit);
    }
  };
}
