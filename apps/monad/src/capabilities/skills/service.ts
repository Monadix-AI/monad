...
export async function createSkillSubsystem(deps: {
  ...
  const discoverAgentSkillDirs = async (): Promise<string[]> => {
    try {
      const entries = await readdir(paths.agents, { withFileTypes: true });
      return entries
        .filter((entry) => entry.isDirectory())
        .sort((a, b) => a.name.localeCompare(b.name))
        .map((entry) => join(paths.agents, entry.name, 'skills'));
    } catch {
      return [];
    }
  };
  ...
  const skillSourceDirs = [
    ...new Set([...atomPackSkillDirs, paths.skills, workspaceSkillsDir, ...(await discoverAgentSkillDirs())])
  ];
  ...
  const mapSkill = (
    ...
    const listItem: SkillListItem = {
      ...
      ...(s.context === 'fork' && s.modelTier ? { modelTier: s.modelTier } : {}),
  ...
  const reloadSkills = async (): Promise<void> => {
    ...
    const res = await fresh.discoverMany([
      ...new Set([...freshAtomPackSkillDirs, paths.skills, workspaceSkillsDir, ...(await discoverAgentSkillDirs())])
    ]);
