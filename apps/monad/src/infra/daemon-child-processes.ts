...
export function daemonChildSupervisorArgv(options: DaemonChildSupervisorArgvOptions): string[] {
  ...
  const needsEntryPath = Boolean(entryPath && entryPath !== execPath);
  const executableName = basename(execPath).toLowerCase();
  const isBunRuntime = executableName === 'bun' || executableName === 'bun.exe';
  const argv = [isBunRuntime ? execPath : roleExecPath(execPath, 'watchdog')];
  if (needsEntryPath && entryPath) argv.push(entryPath);
