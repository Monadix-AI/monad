...
import { clarifyFormSchema, meshSessionIdSchema, sessionIdSchema } from '@monad/protocol';
...
function hitlItems(sqlite: Database, reads: Map<string, string>): InboxItem[] {
  ...
  for (const row of rows) {
    ...
    const form = clarifyFormSchema.safeParse(payload.form);
...
function allItems(sqlite: Database): InboxItem[] {
  ...
  const itemsByKey = new Map<string, InboxItem>();
  for (const item of [...mentionItems(sqlite, reads), ...approvalItems(sqlite, reads), ...hitlItems(sqlite, reads)]) {
    const current = itemsByKey.get(item.itemKey);
    const sameScope = Boolean(item.projectId) === Boolean(current?.projectId);
    if (!current || (item.projectId && !current.projectId) || (sameScope && item.createdAt > current.createdAt)) {
...
export function markOperatorInboxUnread(sqlite: Database, itemKeys: string[]): MarkInboxUnreadResponse {
  const keys = [...new Set(itemKeys)];
  const remove = sqlite.query('DELETE FROM inbox_item_reads WHERE item_key = ?');
  sqlite.transaction((values: string[]) => {
    for (const itemKey of values) remove.run(itemKey);
  })(keys);
  return { itemKeys: keys };
}

export function markAllOperatorInboxRead(
  sqlite: Database,
  readAt = new Date().toISOString()
): MarkAllInboxReadResponse {
  const itemKeys = allItems(sqlite)
    .filter((item) => !item.readAt)
    .map((item) => item.itemKey);
  if (itemKeys.length) markOperatorInboxRead(sqlite, itemKeys, readAt);
  return { readAt, count: itemKeys.length };
}
