...
import { chatMessageSchema, isReplyableMessage } from '@monad/protocol';
...
export type ReplyTargetErrorCode = 'reply_target_not_found' | 'invalid_reply_target';

export class ReplyTargetError extends Error {
  constructor(readonly code: ReplyTargetErrorCode) {
    super(code);
...
export function getSettledMessageMutationReplay(
  sqlite: Database,
  transcriptTargetId: TranscriptTargetId,
  idempotencyKey: IdempotencyKey
): ChatMessage | null {
  const row = sqlite
    .query(
      `SELECT command_fingerprint, result_message
       FROM message_mutations
       WHERE transcript_target_id = ? AND idempotency_key = ?`
    )
    .get(transcriptTargetId, idempotencyKey) as {
  ...
  if (!row?.command_fingerprint.startsWith('settle:')) return null;
  return chatMessageSchema.parse(JSON.parse(row.result_message));
}
...
export function assertReplyTarget(
  sqlite: Database,
  transcriptTargetId: TranscriptTargetId,
  replyToMessageId: MessageId,
  messageId?: MessageId
): void {
  if (replyToMessageId === messageId) throw new ReplyTargetError('invalid_reply_target');
  const row = sqlite
    .query('SELECT * FROM messages WHERE id = ? AND transcript_target_id = ?')
    .get(replyToMessageId, transcriptTargetId) as Record<string, unknown> | null;
  if (!row) throw new ReplyTargetError('reply_target_not_found');
  const target = decodeMessageRow(row);
  if (!target.active) throw new ReplyTargetError('reply_target_not_found');
  if (!isReplyableMessage(target)) throw new ReplyTargetError('invalid_reply_target');
}

function requireReplyTarget(sqlite: Database, message: ChatMessage): void {
  if (!message.replyToMessageId) return;
  assertReplyTarget(sqlite, message.sessionId, message.replyToMessageId, message.id);
}
...
export function createMessage(sqlite: Database, input: CreateMessageInput): MessageMutationResult {
  return sqlite.transaction(() => {
    ...
    requireReplyTarget(sqlite, message);
    sqlite
      .query(
