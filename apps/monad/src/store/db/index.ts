...
interface MessageAttachmentInsert {
  id: string;
  projectId: string;
  path: string;
  name: string;
...
function parseAttachmentIds(idsJson: string | null): string[] {
  if (!idsJson) return [];
  try {
...
export class Store {
    ...
    this.sqlite
      .query(
        `INSERT INTO native_agent_direct_messages
          (id, project_id, native_cli_session_id, from_agent, peer, text, attachment_ids, created_at)
         VALUES ($id, $projectId, $nativeCliSessionId, $fromAgent, $peer, $text, $attachmentIds, $createdAt)`
      )
      .run({
        $id: row.id,
  ...
  registerMessageAttachments(atts: readonly MessageAttachmentInsert[]): MessageAttachmentRef[] {
    ...
    return this.sqlite.transaction(() => atts.map((att) => this.registerMessageAttachment(att)))();
  }
  ...
  getMessageAttachment(id: string): (MessageAttachmentRef & { projectId: string; preview: string }) | null {
    const row = this.sqlite.query('SELECT * FROM message_attachments WHERE id = ?').get(id) as Record<
      string,
      unknown
    > | null;
    if (!row) return null;
  ...
  listNativeAgentDirectMessages(
    ...
    const refMap = this.getMessageAttachmentRefs([...new Set(rowIds.flat())]);
