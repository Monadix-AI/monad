...
export type MeshSessionUpsert = Omit<MeshSessionRow, 'projectMemberId'>;
...
function rowToMeshSession(r: Record<string, unknown>): MeshSessionRow {
  return {
    ...
    projectMemberId: (r.project_member_id as string | null) ?? null,
...
export function upsertMeshSession(sqlite: Database, row: MeshSessionUpsert): void {
  sqlite
    .query(
      `INSERT INTO mesh_sessions
         (id, transcript_target_id, agent_name, provider, working_path, state,
          runtime_role, agent_runtime_id, agent_runtime_token_hash,
          last_delivered_seq, last_visible_seq, pid,
       ...
       VALUES ($id, $transcriptTargetId, $agentName, $provider, $workingPath, $state,
               $runtimeRole, $agentRuntimeId, $agentRuntimeTokenHash,
               $lastDeliveredSeq, $lastVisibleSeq, $pid,
  ...
  return sqlite
    .query(
      `DELETE FROM mesh_sessions
       WHERE state IN ('exited','failed','stopped')
         AND exited_at IS NOT NULL
         AND exited_at < ?
         AND id NOT IN (
           SELECT current_native_runtime_session_id
           FROM session_bindings
           WHERE current_native_runtime_session_id IS NOT NULL
         )`
    )
    .run(cutoff).changes;
}
