  ...
  const tx = sqlite.transaction(() => {
    for (const message of sourceMessages) {
      ...
      const replyToMessageId =
        message.replyToMessageId === undefined ? undefined : clonedIds.get(message.replyToMessageId);
...
// A managed runtime has at most one active (pending/streaming) placeholder at a time, and meshSessionId
// uniquely identifies that runtime — so the locator keys on (target, meshSessionId, source, active) and
// deliberately does NOT match the member key. This is what lets an active placeholder emitted before the
// agentName->projectMemberId cutover still be found (by its meshSessionId) once completion/retire address
// it by projectMemberId, so they reuse the same row instead of orphaning it and emitting a duplicate reply.
export function findManagedMeshAgentStreamingMessage(
  ...
  const row = sqlite
    ...
    .get({ $target: transcriptTargetId, $meshSessionId: meshSessionId }) as {
    id: string;
  } | null;
  return row?.id ?? null;
}

// Guarded by messageId + meshSessionId (both strong keys); like the locator above it deliberately does not
// match the member key, so retiring an active placeholder emitted before the agentName->projectMemberId
// cutover succeeds when addressed by projectMemberId instead of the stored alias.
