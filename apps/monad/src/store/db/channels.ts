...
export function clearActiveConversation(sqlite: Database, channelId: string, conversationKey: string): void {
  sqlite
    .query('DELETE FROM channel_conversations WHERE channel_id = ? AND conversation_key = ?')
    .run(channelId, conversationKey);
}
