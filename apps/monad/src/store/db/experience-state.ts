...
export function compareAndDeleteExperienceState(
  db: Database,
  input: {
    atomPackId: string;
    projectId: string;
  ...
  return db.transaction(() => {
    ...
    const deleted = db
      .query(
        `DELETE FROM experience_state
         WHERE atom_pack_id = ? AND project_id = ? AND record_key = ? AND version = ?`
      )
      .run(input.atomPackId, input.projectId, input.key, input.expectedVersion).changes;
