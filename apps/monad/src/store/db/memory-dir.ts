...
// editable); there is no SQLite event table for L1 (deliberate divergence — see docs/internals/agent-runtime/memory.md).
//
...
export class MemoryDir {
  ...
  private atomicWrite(p: string, md: string): void {
    ...
    try {
      writeFileSync(tmp, md, { encoding: 'utf8', flag: 'wx', mode: 0o600 });
      renameSync(tmp, p);
    } catch (error) {
