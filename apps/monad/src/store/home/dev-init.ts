...
import { computeInitStatus, emptyAuth, loadAll, loadAuth, saveAgents, saveAuth, saveConfig } from '@monad/environment';
...
export async function ensureDevProvider(
  ...
  if (!cfg.model.providers.some((p) => p.id === provider.id)) {
    cfg.model.providers.push({ id: provider.id, label: provider.label, type: provider.type, credentials: [] });
  }
  ...
  const storedProvider = cfg.model.providers.find((candidate) => candidate.id === provider.id);
  if (!storedProvider) return { seeded: false, reason: 'no-config' };
  const pool = storedProvider.credentials;
  if (pool.length === 0) {
    pool.push({
      id: newId('cred'),
  ...
  if (telegramToken && !cfg.channels.some((c) => c.id === telegramChannelId)) {
    cfg.channels.push({
      ...
      credential: { token: telegramToken },
      rateLimitPerMin: 20
    });
  }
