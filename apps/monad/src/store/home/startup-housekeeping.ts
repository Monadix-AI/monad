...
type StartupHousekeepingTimer = { unref(): void };
type StartupHousekeepingScheduler = {
  setInterval(callback: () => void, delayMs: number): StartupHousekeepingTimer;
};

const defaultScheduler: StartupHousekeepingScheduler = {
...
export function startStartupHousekeeping(deps: {
  paths: MonadPaths;
  store: Store;
  logger: StartupLogger;
  scheduler?: StartupHousekeepingScheduler;
}): void {
  ...
  const delegatePruneTimer = scheduler.setInterval(
    () => {
  ...
  const meshAgentPruneTimer = scheduler.setInterval(
    () => {
