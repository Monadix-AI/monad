...
const CONTEXT_FILES = {
  agent: { names: ['AGENT.md'], template: 'AGENT.md' },
 ...
 * templates so AGENT/USER always have a default value even before customization. */
...
async function _loadWorkspaceContext(workspace: string): Promise<string> {
  ...
  return [slots.agent, slots.user].filter(Boolean).join('\n\n');
}
