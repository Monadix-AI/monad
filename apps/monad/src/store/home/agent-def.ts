...
// skill body / provider atom (see docs/internal/development/security-guidelines.md and skills.ts).
//
...
// is an MeshAgent subagent superset kept only so a subagent `.md` imports cleanly.

import type { AgentPromptSlots } from '@monad/protocol';
...
const USER_MD_FILENAME = 'USER.md';

/** AGENT.md YAML frontmatter — an MeshAgent subagent superset (`name`, `tools`,
...
/** Compose an AGENT.md from a config row + body: a `name` frontmatter fence then the body. */
export function composeAgentMd(meta: { name: string }, body: string): string {
...
async function loadPlainPrompt(agentsDir: string, dir: string, filename: string): Promise<string> {
  const file = Bun.file(join(agentDir(agentsDir, dir), filename));
  if (!(await file.exists())) return '';
  return (await file.text()).trim();
}

export async function loadAgentPromptSlots(agentsDir: string, dir: string): Promise<AgentPromptSlots> {
  const [agent, user] = await Promise.all([
    loadAgentBody(agentsDir, dir),
    loadPlainPrompt(agentsDir, dir, USER_MD_FILENAME)
  ]);
  return { agent: agent ?? '', user };
}

export async function writeAgentPromptSlots(
  agentsDir: string,
  dir: string,
  meta: { name: string },
  slots: AgentPromptSlots
): Promise<string> {
  const target = agentDir(agentsDir, dir);
  await mkdir(target, { recursive: true });
  await Promise.all([
    Bun.write(join(target, AGENT_MD_FILENAME), composeAgentMd(meta, slots.agent)),
    Bun.write(join(target, USER_MD_FILENAME), slots.user.trim() ? `${slots.user.trim()}\n` : '')
  ]);
  return target;
}
