...
export interface NativeCliSessionLauncherContext {
  ...
  observation: NativeCliObservationHub;
  appServerConnections: NativeCliAppServerConnectionManager;
  ...
  updateNativeCliPid(id: string, pid: number | null): void;
}

/** Builds a fresh native-CLI session: resolves the agent + launch spec, spawns the child process (or
 *  hands off to the cli-oneshot runner), wires up its output/app-server streams, and registers the
 *  live/exit/idle-resume bookkeeping shared with the rest of NativeCliHost. */
export class NativeCliSessionLauncher {
  constructor(private readonly ctx: NativeCliSessionLauncherContext) {}
  ...
  async start(args: {
    ...
    try {
      ...
      this.ctx.deps.store.upsertNativeCliSession({
        id,
      ...
      this.ctx.events.emit(args.transcriptTargetId, 'native_cli.exited', {
        nativeCliSessionId: id,
        exitCode: null,
        state: 'failed'
      });
    ...
    this.ctx.deps.store.upsertNativeCliSession(row);
    ...
    const attachExitHandler = (runtimeProc: NativeCliProcess): void => {
      void runtimeProc.exited.then((code) => {
        ...
        this.ctx.deps.store.closeNativeCliSession(id, exitedAt, code, state);
        this.ctx.events.emit(args.transcriptTargetId, 'native_cli.exited', {
          nativeCliSessionId: id,
          exitCode: code,
          state
    ...
    live.restartRuntime = launch.capabilities.includes('session-resume')
      ? async () => {
          ...
          this.ctx.updateNativeCliPid(id, nextProc.pid);
    ...
    this.ctx.events.emit(args.transcriptTargetId, 'native_cli.started', {
      nativeCliSessionId: id,
      agentName: runtimeAgentName,
      provider: agent.provider,
