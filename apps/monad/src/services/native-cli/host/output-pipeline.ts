...
export interface NativeCliOutputPipelineContext {
...
export class NativeCliOutputPipeline {
  ...
  private emitStructuredOutputEvent(
    ...
    if (event.type === 'session_ref') {
      ...
      if (providerSessionRef) {
        ...
        this.ctx.store.updateNativeCliSessionRef(id, providerSessionRef);
      }
      return;
    }
    ...
    if (event.type === 'connection_required') {
      const live = this.ctx.live.get(id);
      this.ctx.events.emit(transcriptTargetId, 'native_cli.connection_required', {
        nativeCliSessionId: id,
    ...
    if (event.type === 'provider_error') {
      ...
      this.ctx.events.emit(transcriptTargetId, 'native_cli.output', {
        nativeCliSessionId: id,
        stream: 'stderr',
        chunk: message,
        provider: adapter.provider,
    ...
    if (event.type === 'approval_requested') {
      ...
      this.ctx.events.emit(transcriptTargetId, 'native_cli.approval_requested', {
        nativeCliSessionId: id,
        provider: adapter.provider,
        requestId,
    ...
    if (event.type === 'approval_resolved') {
      ...
      this.ctx.events.emit(transcriptTargetId, 'native_cli.approval_resolved', {
        nativeCliSessionId: id,
        provider: adapter.provider,
        requestId,
  ...
  emitManagedProjectOutput(
    ...
    if (post && !this.ctx.store.hasUnconsumedNativeCliInbox(id)) return;
    ...
    if (cursor > 0) this.ctx.store.markNativeCliInboxConsumed(id, cursor);
