 ...
 * transport-neutral `NativeCliAppServerConnection`. Text frames are delivered to `onMessage`; pings
 * are answered; a server close or socket drop fires `onClose`.
 */
export async function connectAppServerUnix(opts: ConnectAppServerUnixOptions): Promise<NativeCliAppServerConnection> {
...
function handshake(opts: ConnectAppServerUnixOptions, deadline: number): Promise<NativeCliAppServerConnection> {
  return new Promise<NativeCliAppServerConnection>((resolve, reject) => {
