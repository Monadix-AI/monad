...
// monad-as-peer-client: expose `agent_peer_delegate` only for enabled peers with a credential.
// A peer missing its direct credential is skipped, not fatal. The peer runs the
...
export function createPeerDelegateTools(deps: {
  ...
  const { peers, gate, logger } = deps;
  const peerTargets: PeerDelegateTarget[] = [];
  for (const p of peers.filter((x) => x.enabled)) {
    try {
      const token = p.credential?.token;
      if (!token) throw new Error('peer credential is not configured');
