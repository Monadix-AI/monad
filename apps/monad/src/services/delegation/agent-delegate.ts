...
/** Build the `agent_delegate_to` tool. The target is re-resolved live at call time. */
export function createAgentDelegateTool(deps: AgentDelegateDeps): Tool<DelegateInput, { text: string }> {
  const roster = deps.agents();
  const lines = roster.map((agent) => `- ${agent.name}`).join('\n');
  return {
    ...
    run: async ({ agent, instruction }, ctx) => {
      ...
      // only those the target's atoms policy admits.
      const subTools = deps
        .tools()
