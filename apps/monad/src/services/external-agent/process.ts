import type { ExternalAgentAppServerTransport, ExternalAgentLaunchMode } from '@monad/protocol';

import { mkdir, readFile, stat, unlink, writeFile } from 'node:fs/promises';
import { dirname } from 'node:path';

export type ExternalAgentKillFn = (pid: number, signal: NodeJS.Signals) => void;
export type ExternalAgentTreeKillFn = (pid: number) => void;
...
export function pickPtyFallbackLaunchMode(
  supportedLaunchModes: ExternalAgentLaunchMode[],
  supportedAppServerTransports: ExternalAgentAppServerTransport[] = []
): ExternalAgentLaunchMode | undefined {
...
function killIfPresent(pid: number, signal: NodeJS.Signals, kill: ExternalAgentKillFn): void {
  try {
    kill(pid, signal);
  } catch (error) {
...
export function killExternalAgentProcess(
  pid: number | undefined,
  signal: NodeJS.Signals = 'SIGTERM',
  kill: ExternalAgentKillFn = process.kill,
  platform: NodeJS.Platform = process.platform,
  treeKill: ExternalAgentTreeKillFn = taskkillTree
): void {
...
/** Reconciliation registry of detached external agent pids, persisted so a crashed daemon can reap the
