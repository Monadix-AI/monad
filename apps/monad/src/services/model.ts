import type { MonadAuth, MonadConfig, ProviderCredential as StoredProviderCredential } from '@monad/environment';
...
export class ModelService {
  ...
  private persistConfig?: (cfg: MonadConfig) => Promise<void>;
  ...
  constructor(
    ...
    const deps: GatewayDeps = {
      ...
      credentialsFor: (providerId) =>
        (self.cfg.model.providers.find((provider) => provider.id === providerId)?.credentials ?? [])
          .slice()
  ...
  setConfigPersistence(persist: (cfg: MonadConfig) => Promise<void>): void {
  ...
  private reportCredential(
    ...
    const cred = this.cfg.model.providers
      .find((provider) => provider.id === providerId)
      ?.credentials.find((credential) => credential.id === credId);
    if (!cred) return;
  ...
  private scheduleSave(): void {
    ...
    this.saveTimer = setTimeout(() => {
      this.saveTimer = null;
      this.persistConfig?.(structuredClone(this.cfg)).catch(() => {
...
function credentialToHandle(cred: StoredProviderCredential): ProviderCredential {
  return {
    id: cred.id,
