...
// identity (AGENT.md/USER.md) is injected separately as the always-on core. There is no
...
import type { QdrantStatus } from '#/services/memory/qdrant.ts';
...
export interface MemoryServiceDeps {
  ...
  qdrantStatus?: () => QdrantStatus | undefined;
...
export interface MemoryService {
  ...
  consolidateScopes(scopes: readonly { kind: ScopeKind; id: string }[]): Promise<ConsolidateResult[]>;
...
export function createMemoryService(deps: MemoryServiceDeps): MemoryService {
  ...
  return {
    ...
    async consolidateScopes(scopes) {
      if (deps.backend() !== 'builtin') return [];
      const out: ConsolidateResult[] = [];
      for (const scope of scopes) out.push(await consolidateScope(scopeOf(scope.kind, scope.id)));
      return out;
    },
