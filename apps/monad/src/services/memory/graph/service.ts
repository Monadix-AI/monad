...
import type { AgentMemorySettings } from '@monad/protocol';
...
export function agentAutoConsolidationDue(memory: AgentMemorySettings, lastRunMs: number, nowMs: number): boolean {
  if (!memory.enabled || !memory.autoConsolidate) return false;
  const intervalMs = Math.max(1, memory.intervalMinutes) * 60_000;
