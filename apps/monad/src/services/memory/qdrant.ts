...
import type { QdrantPreparationStatus } from '@monad/protocol';
...
import {
  ...
  type ReleaseAssetDownloadProgress,
  type ReleaseSource
...
type QdrantAsset = { name: string; bytes: Uint8Array; checksums?: Map<string, string> };
type QdrantAssetFetcher = (
  source: ReleaseSource,
  platform: NodeJS.Platform,
  arch: string,
  onProgress: (progress: ReleaseAssetDownloadProgress) => void
) => Promise<QdrantAsset>;

export function resolveQdrantReleaseAssetName(platform: NodeJS.Platform, arch: string): string | null {
...
export interface QdrantStatus {
  ...
  error: string | null;
  installed: boolean;
  preparation: QdrantPreparationStatus;
}
...
export class QdrantManager {
  ...
  getStatus(): QdrantStatus {
    return { ...this._status, preparation: { ...this._status.preparation } };
  }
  ...
  private setPhase(phase: QdrantPhase, error: string | null = null): void {
    this._status = { ...this._status, phase, error };
  }
  ...
  private binaryPath(): string {
    return join(this.deps.binDir, binName());
  }
  ...
  prepare(): Promise<void> {
    if (existsSync(this.binaryPath())) {
      this._status = {
        ...this._status,
        phase: this._status.phase === 'downloading' ? 'idle' : this._status.phase,
        error: null,
        installed: true,
        preparation: {
          ...this._status.preparation,
          phase: 'ready',
          error: null
        }
      };
    ...
    this.preparing ??= this.downloadBinary()
      .then(() => {
        this._status = {
          ...this._status,
          phase: 'idle',
  ...
  /** Download and cache the platform binary without launching qdrant or changing memory configuration. */
  private async downloadBinary(): Promise<void> {
    ...
    const onProgress = (progress: ReleaseAssetDownloadProgress) => {
      this._status = {
        ...this._status,
        preparation: {
          phase: 'downloading',
    ...
    const asset = this.deps.fetch
      ? await this.deps.fetch({ ...REPO, tag: version }, process.platform, process.arch, onProgress)
      : await createReleaseAssetFetcher({
          onDownloadProgress: onProgress,
          resolveAssetName: resolveQdrantReleaseAssetName
        })({ ...REPO, tag: version }, process.platform, process.arch);
    ...
    await Bun.write(this.binaryPath(), bytes);
    if (process.platform !== 'win32') await chmod(this.binaryPath(), 0o755);
  }
