...
export interface MemoryHookOptions {
  extraContext?: (sessionId: string) => string | undefined;
  policyForSession: (sessionId: SessionId) => EffectiveAgentMemoryPolicy;
}
...
export function registerMemoryHooks(
  ...
  registry.registerHook({
    ...
    handler: async (input) => {
      ...
      const policy = opts.policyForSession(input.sessionId as SessionId);
      if (policy.effectiveLevel === 0) return undefined;
