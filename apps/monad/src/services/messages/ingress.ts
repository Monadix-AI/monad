...
import type {
  MessageIngress,
  MessageIngressDeps,
  MessageIngressMutationOutcome,
  MessageIngressPublishOptions
} from './types.ts';
...
function stableMessageMutationCommand(command: unknown): unknown {
  if (!command || typeof command !== 'object' || Array.isArray(command)) return command;
  const record = command as Record<string, unknown>;
  const data = record.data;
  if (!data || typeof data !== 'object' || Array.isArray(data)) return command;
  const attachments = (data as Record<string, unknown>).attachments;
  if (!Array.isArray(attachments)) return command;
  return {
    ...record,
    data: {
      ...data,
      attachments: attachments.map((attachment) => {
        ...
        const { id: _id, createdAt: _createdAt, ...stable } = attachment as Record<string, unknown>;
        return stable;
      })
    }
  };
...
function fingerprint(operation: string, command: unknown): string {
  return `${operation}:${JSON.stringify(stableValue(stableMessageMutationCommand(command)))}`;
}
...
export function createMessageIngress(deps: MessageIngressDeps): MessageIngress {
  ...
  async function postCommit(
    mutate: () => MessageMutationResult,
    type: EventType,
    buildPayload: (result: MessageMutationResult) => object,
    producer: MessageProducer,
    options?: MessageIngressPublishOptions
  ): Promise<MessageIngressMutationOutcome> {
    const { result, event } = deps.store.transaction<{ result: MessageMutationResult; event: Event | null }>(() => {
      const result = mutate();
      if (!result.changed) return { result, event: null };
      const event = makeEvent(result.message.sessionId, type, buildPayload(result), {
        actorAgentId: actorAgentId(producer)
      });
      if (eventDefinition(type).persistence === 'durable') deps.store.appendEvents([event]);
      return { result, event };
    });
    if (!event) return { message: result.message, changed: false };
  ...
  async function settleWithOutcome(
    input: SettleMessageCommand,
    options?: MessageIngressPublishOptions
  ): Promise<MessageIngressMutationOutcome> {
  ...
  return {
    ...
    async deliver(input, options) {
      return (await deliverWithOutcome(input, options)).message;
    },

    async begin(input, options) {
      const command = beginMessageCommandSchema.parse(input);
      return (await create(command, 'begin', options)).message;
    },
    ...
    async settle(input, options) {
      return (await settleWithOutcome(input, options)).message;
    },
    ...
    async fail(input, options) {
      ...
      const failed = await postCommit(
        () =>
          deps.store.failMessage({
            transcriptTargetId: command.transcriptTargetId,
            messageId: command.messageId,
            idempotencyKey: command.idempotencyKey,
    ...
    async remove(input, options) {
      ...
      await validate(command);
      const removed = await postCommit(
        () =>
          deps.store.removeMessage({
            transcriptTargetId: command.transcriptTargetId,
            messageId: command.messageId,
            idempotencyKey: command.idempotencyKey,
