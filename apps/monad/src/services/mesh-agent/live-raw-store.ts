...
export class LiveRawStore {
  ...
  async closeAndDelete(): Promise<void> {
    if (!this.closed) {
      ...
      this.oldestStatement.finalize();
      this.afterStatement.finalize();
      this.newestStatement.finalize();
      this.beforeStatement.finalize();
      this.database.close();
    }
    await Promise.all([
      removeLiveRawFile(this.path),
      removeLiveRawFile(`${this.path}-wal`),
      removeLiveRawFile(`${this.path}-shm`)
    ]);
  }
...
async function removeLiveRawFile(path: string): Promise<void> {
