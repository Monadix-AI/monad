...
import { tryResolveEnvironmentMap } from '#/config/environment-refs.ts';
...
function nodeToolchainBinDirs(): string[] {
  const home = homedir();
  const dirs: string[] = [];
  const nvmRoot = join(home, '.nvm', 'versions', 'node');
  try {
    for (const version of readdirSync(nvmRoot).sort((a, b) => b.localeCompare(a, undefined, { numeric: true }))) {
...
export function prependNodeToolchainPath(env: Record<string, string | undefined>): void {
  const seen = new Set<string>();
  env.PATH = [...nodeToolchainBinDirs(), ...(env.PATH ?? '').split(delimiter)]
    .filter((dir) => {
...
/**
 * Resolve `${env:NAME}` references in the agent env. Plain values pass through unchanged;
 * unresolvable or unsupported refs are dropped so a missing key doesn't block the spawn.
 */
export function resolveMeshAgentEnv(
  agentEnv: Record<string, string> | undefined,
  _auth: MonadAuth | undefined
): Record<string, string> | undefined {
  return tryResolveEnvironmentMap(agentEnv);
}
