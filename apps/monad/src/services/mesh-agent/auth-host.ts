...
import {
  ...
  MAX_OUTPUT_SNAPSHOT
} from '#/services/mesh-agent/constants.ts';
...
interface LiveMeshAgentAuthSession {
  id: MeshAgentAuthSessionId;
  controlToken: string;
  agentName: string;
  provider: MeshAgentView['provider'];
  proc?: MeshAgentProcess;
...
function authToView(session: LiveMeshAgentAuthSession): MeshAgentAuthSessionView {
  return {
    id: session.id,
    controlToken: session.controlToken,
    agentName: session.agentName,
    provider: session.provider,
    productIcon: getMeshAgentProviderAdapter(session.provider).productIcon,
...
export class MeshAgentAuthHost {
  private readonly log = createLogger('mesh-agent');
  private readonly liveAuth = new Map<string, LiveMeshAgentAuthSession>();
  private readonly authListeners = new Map<string, Set<MeshAgentAuthListener>>();
  ...
  constructor(private readonly deps: MeshAgentAuthHostDeps) {
  ...
  private trackAuthProcess(pid: number): Promise<void> {
    daemonChildProcesses.track(pid, 'mesh-agent-auth', () => killMeshAgentProcess(pid));
  ...
  private publishAuth(live: LiveMeshAgentAuthSession): void {
  ...
  async startAuth(agentName: string): Promise<MeshAgentAuthSessionView> {
    ...
    if (preflight?.state === 'authenticated') {
      ...
      const live: LiveMeshAgentAuthSession = {
        id,
    ...
    proc = supervisedSpawn(
      ...
      {
        ...daemonTrackedSpawnOptions({
          event: 'mesh.auth_spawn',
          log: this.log,
          context: { meshAgentAuthSessionId: id, agentName: agent.name, provider: agent.provider },
          kill: (child, signal) => killMeshAgentProcess(child.pid, signal),
          trackLabel: 'mesh-agent-auth',
          tracker: {
    ...
    const live: LiveMeshAgentAuthSession = {
      ...
      updatedAt: now,
      exitedAt: null,
      kill: (signal) => killMeshAgentProcess(proc.pid, signal)
    };
    this.liveAuth.set(id, live);
  ...
  private requireAuthSession(id: string, controlToken: string): LiveMeshAgentAuthSession {
    this.pruneAuthSessions();
    const live = this.liveAuth.get(id);
    if (!live) throw new Error(`MeshAgent auth session not found: ${id}`);
    if (live.controlToken !== controlToken) throw new Error(`MeshAgent auth session not found: ${id}`);
    return live;
  }
  ...
  inputAuth(id: string, controlToken: string, req: MeshAgentInputRequest): void {
    const live = this.requireAuthSession(id, controlToken);
    if (live.state !== 'running') throw new Error(`MeshAgent auth session is not running: ${id}`);
  ...
  async authStatus(agentName: string): Promise<MeshAgentAuthStatusResponse> {
    this.pruneAuthSessions();
    const agent = await this.requireAgent(agentName);
    const adapter = getMeshAgentProviderAdapter(agent.provider);
    const statusProbe = adapter.authStatus(agent);
    const launch = resolveMeshAgentLaunchCommand(adapter, statusProbe.launch);
    this.log.debug(
      {
        event: 'mesh.auth_status',
        agentName: agent.name,
        provider: agent.provider,
    ...
    if (result.timedOut) {
      this.log.warn(
        {
          event: 'mesh.auth_status_timeout',
          agentName: agent.name,
          provider: agent.provider,
          argv: redactedSpawnArgv(launch.argv),
          cwd: launch.cwd,
      ...
      throw new MeshAgentError('provider_timeout', `timed out checking MeshAgent auth status: ${agent.name}`);
    }
    const state = statusProbe.parse(result.output, result.code);
    this.log.debug(
      {
        event: 'mesh.auth_status_result',
        agentName: agent.name,
        provider: agent.provider,
  ...
  async usage(agentName: string): Promise<MeshAgentUsageResponse> {
    const agent = await this.requireAgent(agentName);
    const adapter = getMeshAgentProviderAdapter(agent.provider);
    const checkedAt = new Date().toISOString();
    const usageProbe = adapter.usage?.(agent);
    if (!usageProbe) {
      const snapshot: MeshAgentUsageResponse = {
        agentName: agent.name,
        provider: agent.provider,
        checkedAt,
        records: []
      };
      this.deps.store.replaceMeshAgentUsageSnapshot(snapshot);
      return snapshot;
    }
    const launch = resolveMeshAgentLaunchCommand(adapter, usageProbe.launch);
    this.log.debug(
      {
        event: 'mesh.usage',
        agentName: agent.name,
    ...
    if (result.timedOut) {
      ...
      throw new MeshAgentError('provider_timeout', `timed out checking MeshAgent usage: ${agent.name}`);
    }
    const records = usageProbe.parse(result.output, result.code);
    this.log.debug(
      {
        event: 'mesh.usage_result',
        agentName: agent.name,
        provider: agent.provider,
        exitCode: result.code,
  ...
  async preflight(agentName: string): Promise<MeshAgentStartPreflight> {
    const checkedAt = new Date().toISOString();
    const agent = await this.requireAgent(agentName);
    try {
