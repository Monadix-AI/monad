...
import type { MeshAgentLaunchMode, MeshAgentView, MeshSessionView } from '@monad/protocol';
import type { EventBus } from '#/services/event-bus.ts';
import type { LiveRawStore } from '#/services/mesh-agent/live-raw-store.ts';
import type { MeshAgentProcess, MeshAgentStdin, MeshAgentTerminal } from '#/services/mesh-agent/runtime-types.ts';
import type {
...
export interface LiveMeshSession {
  ...
  oneshotSpec?: MeshAgentLaunchSpec;
  ...
  oneshotTurnProc?: MeshAgentProcess;
  ...
  appServerRedial?: () => Promise<MeshAgentAppServerConnection>;
  ...
  initializeContext?: MeshAgentInitializeContext;
  providerSessionRef?: string | null;
  pendingApprovals: Map<string, Record<string, unknown>>;
  pendingEventPages: Map<
    string,
    {
  ...
  /** True while an `mesh.session.connection.opened` has fired for the current epoch without a
   *  matching `closed` — makes the open/close emit idempotent so a late teardown can't double-close. */
  connectionOpen?: boolean;
  providerEventCheckpoint?: string;
  providerEventIdentities?: Set<string>;
...
export interface MeshAgentHostDeps {
  ...
  /** Milliseconds before a resumable MeshAgent child is released while idle. <=0 disables it. */
  meshAgentIdleTimeoutMs?: number;
