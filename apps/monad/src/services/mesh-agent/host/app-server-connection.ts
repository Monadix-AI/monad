...
} from '#/services/mesh-agent/host/host-constants.ts';

export interface MeshAgentAppServerConnectionContext {
  ...
  rotateLiveCapture?(live: LiveMeshSession): void;
}
...
export class MeshAgentAppServerConnectionManager {
  constructor(private readonly ctx: MeshAgentAppServerConnectionContext) {}
   ...
   *  `--port` to (see `MeshAgentAppServerWsHints.port`) rather than parsing a self-announced one. Binds
  ...
  async raceAgainstExit<T>(connect: Promise<T>, exited: Promise<number>): Promise<T> {
    ...
    const exitGuard = exited.then((code) => {
      ...
      throw new Error(`MeshAgent process exited (code ${code}) before the app-server became ready`);
    });
    try {
  ...
  private emitReconnectRequired(id: string, live: LiveMeshSession): void {
    this.ctx.emit(live.transcriptTargetId, 'mesh.connection_required', {
      meshSessionId: id,
    ...
    setTimeout(() => {
      ...
      this.ctx.log.warn(
        {
          ...
          event: 'mesh.app_server_disconnected',
          meshSessionId: id,
      ...
      if (current.appServerRedial) {
        ...
        this.ctx.log.warn(
          {
            sessionId: current.transcriptTargetId,
            event: 'mesh.app_server_reconnect_churn_exceeded',
            meshSessionId: id,
            provider: current.provider,
      ...
      if (current.startup) {
        ...
        current.startup.reject(new Error(`MeshAgent app-server socket dropped before ready: ${id}`));
    ...
    for (let attempt = 1; attempt <= APP_SERVER_RECONNECT_ATTEMPTS; attempt++) {
      ...
      try {
        ...
        this.ctx.log.debug(
          {
            sessionId: current.transcriptTargetId,
            event: 'mesh.app_server_reconnected',
            meshSessionId: id
          },
