import type { Event, MeshAgentAuthStatusResponse, MeshAgentLoginRequirement, SessionId } from '@monad/protocol';
...
export class MeshAgentLoginNudge {
  ...
  private async onConnectionRequired(event: Event): Promise<void> {
    ...
    try {
      ...
      const sessionKey = `${event.sessionId}:${p.agentName}`;
      const existing = sessions.get(sessionKey);
      const observedAt = existing?.observedAt ?? new Date().toISOString();
      sessions.set(sessionKey, {
        sessionId: event.sessionId as SessionId,
        ...(p.meshSessionId ? { meshSessionId: meshSessionIdSchema.parse(p.meshSessionId) } : {}),
      ...
      this.deps.bus.publish(
        makeEvent(
          ...
          existing ? undefined : { at: observedAt }
        )
      );
    } catch (error) {
      this.log.warn(
  ...
  pendingLoginRequirements(sessionId: SessionId): MeshAgentLoginRequirement[] {
    const requirements: MeshAgentLoginRequirement[] = [];
    for (const sessions of this.pending.values()) {
      for (const card of sessions.values()) {
        if (card.sessionId !== sessionId) continue;
        requirements.push(
          meshAgentLoginRequirementSchema.parse({
            id: meshAgentLoginRequirementId(card.agentName, card.authAgentName),
