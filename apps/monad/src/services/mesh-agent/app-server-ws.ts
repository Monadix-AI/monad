 ...
 * `MeshAgentAppServerConnection` so the host drives a ws session exactly like a stdio one.
 ...
 */
export async function connectAppServerWs(opts: ConnectAppServerWsOptions): Promise<MeshAgentAppServerConnection> {
...
): Promise<MeshAgentAppServerConnection> {
 ...
 *  `MeshAgentLaunchSpec.appServerWs.port`), retrying until the gateway's HTTP/WS listener comes up or
