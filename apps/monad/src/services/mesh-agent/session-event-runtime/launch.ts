...
export function materializeTurnLaunch(
  ...
  if (
    input.delivery !== 'argv-tail' ||
    (input.separator !== undefined && input.separator !== '--') ||
    !Array.isArray(input.values)
  ) {
    invalid('argv turn input requires an optional literal -- separator');
  }
  ...
  return {
    ...launch,
    argv: [...launch.argv, ...(input.separator === '--' ? ['--'] : []), ...input.values]
  };
}
