...
export function validateSessionEventRuntimeDefinition(value: unknown): SessionEventRuntimeDefinition {
  ...
  if (plan.processModel === 'resident') {
    exactKeys(
      plan,
      ['processModel', 'launch', 'buildLaunch', 'channel', 'startup', 'reconnect', 'suspend'],
      'resident plan'
    );
    validateProcessLaunchPlan(plan.launch);
    if (plan.buildLaunch !== undefined && typeof plan.buildLaunch !== 'function') {
      fail('resident buildLaunch must be a function');
    }
    ...
    if (channel.kind === 'child-stdio') {
      ...
      exactKeys(channel, ['kind', 'endpoint', 'path', 'query', 'portArgument', 'connectDelayMs'], 'channel fields');
      if (channel.endpoint !== 'daemon-loopback') fail('websocket endpoint must be daemon-loopback');
      if (channel.connectDelayMs !== undefined) {
        positiveInteger(channel.connectDelayMs, 'websocket connect delay', 10_000);
      }
      if (
        channel.path !== undefined &&
        (typeof channel.path !== 'string' || !/^\/[A-Za-z0-9._~!$&'()*+,;=:@%/-]*$/.test(channel.path))
      ) {
        fail('websocket path must be a loopback URL path');
      }
      if (
        channel.portArgument !== undefined &&
        (typeof channel.portArgument !== 'string' || !/^--[a-z0-9-]+$/.test(channel.portArgument))
      ) {
        fail('websocket port argument must be a long option');
      }
      if (channel.query !== undefined) {
        const query = object(channel.query, 'websocket query');
        for (const [key, value] of Object.entries(query)) {
          if (!/^[A-Za-z0-9._~-]+$/.test(key) || typeof value !== 'string' || value.includes('\0')) {
            fail('websocket query must contain safe string entries');
          }
        }
      }
