  ...
  return new ChannelService(
    {
      session: {
        ...
        createProjectSession: (a) => {
          const gw = sessionGateway();
          if (!gw?.createProjectSession) throw new Error('channel project gateway used before session wiring');
          return gw.createProjectSession(a);
        },
        sendProjectMessage: (a) => {
          const gw = sessionGateway();
          if (!gw?.sendProjectMessage) throw new Error('channel project gateway used before session wiring');
          return gw.sendProjectMessage(a);
        },
        sendInline: (a, sink, runOpts) => {
          const gw = sessionGateway();
          if (!gw) throw new Error('channel gateway used before session wiring');
          return gw.sendInline(a, sink, runOpts);
        },
        reset: (a) => {
          const gw = sessionGateway();
