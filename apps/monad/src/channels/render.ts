...
import type { ChatMessage, Event, MessageProducer } from '@monad/protocol';
...
export function createRenderer({
  ...
  function onMessage(p: { text?: string; label?: string }): void {
    const body = channelTextRenderText(p.text ?? '');
    const text = body && p.label ? `${p.label}:\n${body}` : body;
    ...
    consume(event: Event) {
      switch (event.type) {
        case 'session.message.created': {
          const { message, producer } = parseEventPayload('session.message.created', event.payload);
          if (
            ...
            break;
          const label = meshAgentDisplayName(message, producer);
          if (message.type === 'error') sendNew(t('channel.error', { label: labeledText(message.text, label) }));
          else onMessage({ text: message.text, label });
          break;
        }
        ...
        case 'session.message.completed': {
          const { message, producer } = parseEventPayload('session.message.completed', event.payload);
          if (message.role !== 'assistant') break;
          onMessage({ text: message.text, label: meshAgentDisplayName(message, producer) });
          break;
        }
        case 'session.message.failed': {
          const { message, producer } = parseEventPayload('session.message.failed', event.payload);
          if (message.role !== 'assistant') break;
          sendNew(t('channel.error', { label: labeledText(message.text, meshAgentDisplayName(message, producer)) }));
          started = false;
          handle = undefined;
          buf = '';
          break;
...
function meshAgentDisplayName(message: ChatMessage, producer: MessageProducer): string | undefined {
  if (producer.kind !== 'mesh-agent') return undefined;
  const data = message.data;
  if (data && typeof data === 'object') {
    const displayName = (data as { agentDisplayName?: unknown }).agentDisplayName;
    if (typeof displayName === 'string' && displayName.trim()) return displayName.trim();
  }
  return producer.agentName;
}

function labeledText(text: string, label?: string): string {
