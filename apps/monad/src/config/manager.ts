...
export interface ConfigSource {
  ...
  saveSnapshot?(previous: ConfigSnapshot, next: ConfigSnapshot): Promise<void>;
...
export class ConfigManager {
  ...
  update(mutate: SnapshotMutation): Promise<ConfigSnapshot> {
    return this.enqueue(async () => {
      ...
      const saveSnapshot = this.source.saveSnapshot;
      if (saveSnapshot) await saveSnapshot(previous, next);
      else {
      ...
      try {
        ...
        if (saveSnapshot) await saveSnapshot(next, previous).catch(() => {});
        else {
          await Promise.all([
