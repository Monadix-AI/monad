...
export class RuntimeKernel<Snapshot = unknown> {
  ...
  async start(): Promise<void> {
    ...
    for (const layer of this.graph.layers) {
      for (const module of layer) {
        if (!this.hasOutput(module.id)) continue;
        module.afterReady?.(this.context.get(module.id), this.context, this.controller.signal);
      }
    }
  }
  ...
  private isDegraded(): boolean {
    const modules: RuntimeModuleState[] = Object.values(this.state.getState().modules);
    return modules.some((module) => ['blocked', 'degraded', 'failed'].includes(module.status));
  }
