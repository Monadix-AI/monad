...
type RuntimePhase = 'booting' | 'ready' | 'degraded' | 'reloading' | 'stopping' | 'failed';
type ModuleStatus = 'idle' | 'starting' | 'ready' | 'reloading' | 'degraded' | 'blocked' | 'failed' | 'stopped';
...
export interface RuntimeModule<Snapshot = unknown> {
  ...
  afterReady?(current: unknown, ctx: RuntimeContextReader, signal: AbortSignal): void;
