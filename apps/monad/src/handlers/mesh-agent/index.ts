...
export function createMeshAgentModule({ host, store, config }: MeshAgentDeps) {
  ...
  return {
    ...
    }): Promise<MeshAgentSessionUsageResponse> {
      requireMeshSessionScope(id, transcriptTargetId);
      try {
        return meshAgentSessionUsageResponseSchema.parse({ usage: await host.sessionUsage(id) });
      } catch (error) {
    ...
    subscribeSessionUsage({
      ...
      id: string;
      transcriptTargetId: MeshAgentTargetId;
      onUsage: (usage: MeshAgentSessionUsage) => void;
    }): { dispose: () => void } {
      requireMeshSessionScope(id, transcriptTargetId);
      return host.subscribeSessionUsage(id, onUsage);
    },
    ...
    async usageOverview(): Promise<MeshUsageOverviewResponse> {
      await requireConfig();
      return meshUsageOverviewResponseSchema.parse(await host.usageOverview());
    }
  };
}
