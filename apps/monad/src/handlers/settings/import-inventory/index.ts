...
import type {
  ...
  ImportInventoryRoot,
  ImportInventoryScope,
  ImportInventorySource,
  MeshAgentProvider
} from '@monad/protocol';
...
interface RootSpec {
  source: ImportInventorySource;
  sourceLabel: string;
  scope: ImportInventoryScope;
  kind: ImportInventoryRoot['kind'];
  path: string;
  shared?: boolean;
...
async function rootStatus(spec: RootSpec): Promise<ImportInventoryRoot> {
  try {
...
function rootMatchesRequest(spec: RootSpec, req: ImportInventoryOpenLocationRequest): boolean {
  return (
...
async function scanSkillRoot(spec: RootSpec): Promise<ImportInventoryItem[]> {
  const out: ImportInventoryItem[] = [];
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      out.push({
        ...
        path: skillDir,
        shared: spec.shared ?? false,
        ...(spec.ownerAgentDir ? { ownerAgentDir: spec.ownerAgentDir } : {}),
...
async function scanModelProviderConfigFile(spec: RootSpec, path: string): Promise<ImportInventoryItem[]> {
...
async function scanAgentConfigFile(spec: RootSpec, path: string): Promise<ImportInventoryItem[]> {
  const data = await readConfig(path);
  const out: ImportInventoryItem[] = [];
...
async function scanAgentMarkdownDirectory(spec: RootSpec, path: string): Promise<ImportInventoryItem[]> {
  let entries: Dirent<string>[];
  try {
...
async function scanMcpConfigFile(spec: RootSpec, path: string): Promise<ImportInventoryItem[]> {
  const data = await readConfig(path);
  const servers =
...
async function scanMcpRoot(spec: RootSpec): Promise<ImportInventoryItem[]> {
  try {
...
async function scanAgentRoot(spec: RootSpec): Promise<ImportInventoryItem[]> {
...
async function scanModelProviderRoot(spec: RootSpec): Promise<ImportInventoryItem[]> {
  if (!spec.provider) return [];
  try {
    const info = await stat(detectPathForRoot(spec));
    if (!info.isDirectory() && !info.isFile()) return [];
...
async function agentPrivateRoots(paths: MonadPaths): Promise<RootSpec[]> {
  try {
    const agents = await readdir(paths.agents, { withFileTypes: true, encoding: 'utf8' });
    return agents
      .filter((entry) => entry.isDirectory())
      .sort((a, b) => a.name.localeCompare(b.name))
      .flatMap((entry): RootSpec[] => [
        {
          source: 'monad',
          sourceLabel: `Monad agent ${entry.name}`,
          scope: 'user',
          kind: 'skills',
          path: join(paths.agents, entry.name, 'skills'),
          ownerAgentDir: entry.name
        },
        {
          source: 'monad',
          sourceLabel: `Monad agent ${entry.name}`,
          scope: 'user',
          kind: 'mcpServers',
          path: join(paths.agents, entry.name, 'mcp'),
          ownerAgentDir: entry.name
        }
      ]);
  } catch {
...
function sourceForAgentProvider(provider: MeshAgentProvider): ImportInventorySource {
  switch (provider) {
    case 'codex':
      return 'codex';
    case 'claude-code':
...
function agentScope(scope: string): ImportInventoryScope {
...
function providerConfigRoots(kind: Extract<ImportInventoryRoot['kind'], 'agents' | 'modelProviders'>): RootSpec[] {
  return uniqueSpecs(
...
export function createImportInventoryModule(paths: MonadPaths) {
  return {
    async list(): Promise<ImportInventoryResponse> {
      const home = homedir();
      const specs = [
        ...skillRoots(paths, home),
        ...mcpRoots(paths, home),
        ...(await agentPrivateRoots(paths)),
        ...agentRoots(),
        ...modelProviderRoots()
      ];
    ...
    async openLocation(req: ImportInventoryOpenLocationRequest): Promise<{ ok: true }> {
      const home = homedir();
      const specs = [
        ...skillRoots(paths, home),
        ...mcpRoots(paths, home),
