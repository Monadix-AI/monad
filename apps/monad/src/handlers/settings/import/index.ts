...
export function createSettingsImportModule({ paths, config, mcpReconnect }: SettingsImportDeps) {
  ...
  async function apply(req: ImportSettingsApplyRequest): Promise<ImportSettingsApplyResult> {
    ...
    for (const item of selected) {
      ...
      try {
        ...
        switch (payload.kind) {
          ...
          case 'credential':
            cfg.model.providers
              .find((provider) => provider.id === payload.providerId)
              ?.credentials.push({
                id: newId('cred'),
                label: payload.label,
                authType: payload.authType ?? 'api_key',
                priority:
                  cfg.model.providers.find((provider) => provider.id === payload.providerId)?.credentials.length ?? 0,
                source: 'import',
          ...
          case 'agent': {
            ...
            await writeAgentBody(paths.agents, stableDir, { name: agent.name }, payload.prompt);
    ...
    if (wroteAgents) await config.updateConfig(() => cfg);
