...
export function createSandboxModule(config: ConfigAccess, activationService: SandboxActivationService) {
  const knownDescriptors = new Map<string, ReturnType<typeof listSandboxBackendDescriptors>[number]['descriptor']>();

  function currentDescriptors() {
    const descriptors = listSandboxBackendDescriptors();
    for (const backend of descriptors) {
      knownDescriptors.set(serializeSandboxBackendRef(backend.ref), backend.descriptor);
    }
    return descriptors;
  }
  ...
  async function getSandboxSettings(): Promise<SandboxSettingsResponse> {
    ...
    const backendSettings = redactBackendSettings(cfg.sandbox.backendSettings, auth, knownDescriptors);
    const backends = descriptors.map((backend) => {
  ...
  async function setSandboxSettings(req: SetSandboxSettingsRequest): Promise<SandboxSettingsResponse> {
    ...
    if (req.backendSettings) {
      ...
      const backend = currentDescriptors().find((candidate) => serializeSandboxBackendRef(candidate.ref) === key);
