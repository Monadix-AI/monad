import type { Provider, ProviderCredential } from '@monad/environment';
...
export function createProvidersHandlers(ctx: ModelContext) {
  ...
  async function fetchAndCacheModels(provider: Provider, cred: ProviderCredential): Promise<ModelInfo[]> {
  ...
  return {
    ...
    async setProvider({ provider }: SetProviderRequest) {
      ...
      if (i >= 0) cfg.model.providers[i] = { ...next, credentials: previous?.credentials ?? [] };
    ...
    async patchProvider({ id, patch }: { id: string; patch: PatchProviderRequest }) {
      ...
      const next = { ...viewToProvider(mergedView), credentials: previous.credentials };
