import type { MonadConfig, Provider, ProviderCredential } from '@monad/environment';
...
export function credentialToView(c: ProviderCredential): CredentialView {
  return {
    id: c.id,
    label: c.label,
    authType: c.authType,
    ...
    configured: c.accessToken.length > 0
  };
}
...
export function credentialToHandle(c: ProviderCredential): {
  id: string;
  accessToken: string;
