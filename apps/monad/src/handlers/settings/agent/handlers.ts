...
import { installMcpAtom } from '#/capabilities/mcp/install/index.ts';
...
import {
  deleteAgentDir,
  loadAgentBody,
  loadAgentPromptSlots,
  toAgentDir,
  writeAgentPromptSlots
...
export function createAgentHandlers(ctx: AgentContext) {
  ...
  return {
    ...
    async createAgent(req: CreateAgentRequest) {
      const cfg = await ctx.read();
      const credentialIds = req.credentialIds ?? [];
      ctx.validateCredentialIds(credentialIds);
      ...
      let hasPrompt = false;
      const promptTransaction = req.prompt?.trim()
        ? await stageAgentCreatePrompt(ctx.paths.agents, {
            id,
            dir,
            name: req.name,
            prompt: req.prompt
      ...
      try {
        ...
        await ctx.commitCreatedAgent(
          cfg,
          credentialIds,
          promptTransaction
            ? async () => {
                await installAgentCreatePrompt(promptTransaction, { recoverOnFailure: false });
                promptInstalled = true;
              }
            : undefined
        );
      } catch (error) {
        if (promptTransaction && !promptInstalled) {
          await recoverAgentCreateTransaction(promptTransaction, ctx.configuredAgents());
        }
        throw error;
      }
      if (promptTransaction) {
        try {
          await completeAgentCreateTransaction(promptTransaction);
        } catch (error) {
          await recoverAgentCreateTransaction(promptTransaction, ctx.configuredAgents()).catch((recoveryError) => {
            throw new AggregateError([error, recoveryError], 'monad: agent create recovery failed');
          });
        }
        hasPrompt = true;
      }
    ...
    async updateAgent({ agentId, ...patch }: UpdateAgentRequest & { agentId: AgentId }) {
      ...
      const credentialIds = patch.credentialIds ?? a.credentialIds;
      ctx.validateCredentialIds(credentialIds);
      ...
      if (patch.credentialIds !== undefined) a.credentialIds = patch.credentialIds;
      if (patch.skills !== undefined) a.skills = patch.skills;
      ...
      if (patch.memory !== undefined) a.memory = patch.memory;
      await ctx.commit(cfg, credentialIds);
    ...
    async getAgentPrompt({ agentId }: { agentId: AgentId }) {
      ...
      const a = cfg.agent.agents.find((x) => x.id === agentId);
      if (!a) throw new HandlerError('not_found', `agent not found: ${agentId}`);
      const slots = await loadAgentPromptSlots(ctx.paths.agents, agentDirOf(a));
      return { prompt: slots.agent, slots };
    },

    async setAgentPrompt({ agentId, prompt, slots: patch }: SetAgentPromptRequest & { agentId: AgentId }) {
      const cfg = await ctx.read();
      const a = cfg.agent.agents.find((x) => x.id === agentId);
      if (!a) throw new HandlerError('not_found', `agent not found: ${agentId}`);
      ...
      const current = await loadAgentPromptSlots(ctx.paths.agents, a.dir);
      const slots: AgentPromptSlots = {
        ...current,
        ...patch,
        agent: patch?.agent ?? prompt ?? current.agent
      };
      await writeAgentPromptSlots(ctx.paths.agents, a.dir, { name: a.name }, slots);
      return { prompt: slots.agent, slots };
    },

    async installAgentMcp({ agentId, server, consent }: InstallMcpAtomRequest & { agentId: AgentId }) {
      const cfg = await ctx.read();
      const agent = cfg.agent.agents.find((item) => item.id === agentId);
      if (!agent) throw new HandlerError('not_found', `agent not found: ${agentId}`);
      const mcpDir = join(ctx.paths.agents, agentDirOf(agent), 'mcp');
      await mkdir(mcpDir, { recursive: true });
      try {
        return await installMcpAtom(server, {
          mcpDir,
          consent: () => consent
        });
      } catch (error) {
        throw new HandlerError('invalid', error instanceof Error ? error.message : String(error));
      }
    },
