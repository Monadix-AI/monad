...
import { type ResolvedSkillInstallTarget, resolveSkillInstallTarget, scopedSkillIds } from './skill-install-target.ts';
...
export function createSkillsModule(deps: AtomPacksDeps) {
  ...
  async function createSkillFromContent(
    { name, content }: Pick<CreateSkillRequest, 'content' | 'name'>,
    target: ResolvedSkillInstallTarget
  ): Promise<CreateSkillResponse> {
    try {
      const skillDir = await writeSkill(target.skillsDir, name, content);
      const enc = new TextEncoder();
      const warnings = scanSkillFiles(new Map([[`${name}/SKILL.md`, enc.encode(content)]]));
      return { id: `${target.scopePrefix}:${name}`, name, dir: skillDir, warnings };
    } catch (err) {
  ...
  async function installSkillsFromLocalPath({
    path,
    overwrite,
    target = resolveSkillInstallTarget(undefined, deps)
  }: InstallLocalSkillRequest & { target?: ResolvedSkillInstallTarget }): Promise<InstallSkillResponse> {
    ...
    for (const d of dirs) {
      ...
      try {
        skills.push(await installSkillFromDir(target.skillsDir, d, { overwrite }));
      } catch (err) {
        warnings.push(`Could not install ${basename(d)}: ${err instanceof Error ? err.message : String(err)}`);
      }
    }
    return { skills, skillIds: scopedSkillIds(target, skills), commit: '', warnings };
  }

  async function installSkillUpload(
    ...
    if (upload.extension === '.md') {
      ...
      if (!overwrite && (await Bun.file(join(target.skillsDir, frontmatter.name, 'SKILL.md')).exists())) {
        throw new Error(`skill "${frontmatter.name}" already exists`);
      }
      const created = await createSkillFromContent({ name: frontmatter.name, content }, target);
      return { skills: [created.name], skillIds: [created.id as string], commit: '', warnings: created.warnings };
    }
    ...
    try {
      return await installSkillsFromLocalPath({ path: unpacked.dir, overwrite, target });
    } finally {
  ...
  const skills = {
    ...
    async createSkill({ name, content, target: targetRequest }: CreateSkillRequest): Promise<CreateSkillResponse> {
      const target = resolveSkillInstallTarget(targetRequest, deps);
      const created = await createSkillFromContent({ name, content }, target);
      await deps.onChanged?.();
      return created;
    },
    ...
    async updateSkillContent({
      ...
      try {
        ...
        await deps.onChanged?.();
        return { id: target.id, name: target.name, dir: target.dir, warnings };
      } catch (err) {
        throw new HandlerError('invalid', err instanceof Error ? err.message : String(err));
      }
    },
    ...
    async uploadSkill({
      ...
      try {
        ...
        const targetRequest: SkillInstallTarget | undefined = agentId ? { kind: 'agent', agentId } : undefined;
        const target = resolveSkillInstallTarget(targetRequest, deps);
        return await reloadAfterSkillCommit(await installSkillUpload(upload, overwrite ?? false, target));
      } catch (err) {
        throw new HandlerError('invalid', err instanceof Error ? err.message : String(err));
      }
    },
    ...
    async checkSkillUpdates(): Promise<CheckSkillUpdatesResponse> {
      const skillsDir = deps.paths.skills;
      const token = deps.config?.get().cfg.atomRegistries.github?.token;
      let entries: Dirent[];
      try {
        entries = await readdir(skillsDir, { withFileTypes: true });
      } catch {
