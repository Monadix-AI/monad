...
import { resolveSkillInstallTarget, scopedSkillIds } from './skill-install-target.ts';
...
export function createSkillInstallers(
  ...
  return {
    async installSkill({
    ...
    }: InstallSkillRequest): Promise<InstallSkillResponse> {
      const target = resolveSkillInstallTarget(targetRequest, deps);
      const finalize = (out: InstallSkillResponse) =>
        reloadAfterSkillCommit({ ...out, skillIds: scopedSkillIds(target, out.skills) });
      if (source.startsWith('git+')) {
        const out = await installGitSkill(source, {
          skillsDir: target.skillsDir,
          skillsLock: target.skillsLock,
      ...
      const out = await installSkillFromGithub(source, {
        skillsDir: target.skillsDir,
        fetch: createSkillFetcher({ githubToken: registries?.github?.token }),
        consent: () => consent === true,
        review: reviewInstall,
        overwrite
      });
