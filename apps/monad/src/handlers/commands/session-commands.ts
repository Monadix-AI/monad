...
export interface CommandExecution {
  ...
  denyCommand?: (def: CommandDefinition, args: string) => CommandResult | null | undefined;
...
export interface LifecycleOps {
  create(args: { title: string; agentId?: AgentId; origin?: OperationSource }): Promise<{ sessionId: string }>;
...
export interface CommandBundle {
  ...
  consolidate(sessionId: SessionId, level?: number): Promise<ConsolidateSummary>;
  explainBelief(sessionId: SessionId, query: string): Promise<BeliefExplanation>;
  checkMemory(): Promise<ContradictionCheckSummary>;
  listCommands?(sessionId?: SessionId, filter?: CommandsListFilter): CommandItem[];
...
function genericExecution(runner: SessionCommandRunner, session: Session, busy: boolean): CommandExecution {
  ...
  return {
    ...
    isBusy: busy,
    denyCommand: commands.denyCommand ? (def, args) => commands.denyCommand?.(session.id, def, args) : undefined,
    ...
    services: {
      ...
      consolidate: (level?: number) => commands.consolidate(session.id, level),
      ...
      listCommands: async (): Promise<CommandItem[]> =>
        commands.listCommands?.(session.id) ?? commands.registry.list(commands.skills(), commands.t),
...
export async function tryRunSessionCommand(
  ...
  session: Session,
  text: string,
  opts: { sink?: (event: Event) => void; busy?: boolean; replyToMessageId?: MessageId } = {}
): Promise<boolean> {
  const result = await executeSessionCommand(runner, session, text, { busy: opts.busy });
  if (result === null) return false;
  await emitCommandTurn(runner.messageIngress, opts.sink, session.id, text, result, opts.replyToMessageId);
  return true;
}
