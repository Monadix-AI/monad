...
import type { AgentId, CommandItem, SessionId } from '@monad/protocol';
import type {
  BeliefExplanation,
  CommandDefinition,
  CommandResult,
...
import {
  applyMemoryCommandDiscovery,
  memoryCommandDecision,
  memoryCommandRequiredLevel
} from '#/handlers/commands/memory-policy.ts';
import { makeEvent } from '#/services/event-bus.ts';
import { resolveSessionMemoryPolicy } from '#/services/memory/policy.ts';
...
export function createCommandBundle(deps: CommandBundleDeps): CommandBundle {
  ...
  const policyFor = (sessionId: SessionId) => {
  ...
  return {
    ...
    skills,
    listCommands: (sessionId, filter = 'enabled'): CommandItem[] => {
      const commands = commandRegistry.list(skills(), i18n.t, { filter: 'all' });
      const scoped = sessionId ? applyMemoryCommandDiscovery(commands, policyFor(sessionId)) : commands;
      return scoped.filter(
        (command) => filter === 'all' || (filter === 'enabled' ? command.enabled : !command.enabled)
      );
    },
    denyCommand: (sessionId: SessionId, def: CommandDefinition, args: string): CommandResult | null => {
      const policy = policyFor(sessionId);
      const decision = memoryCommandDecision(def.name, args, policy);
      if (decision.allowed) return null;
      return {
        message: i18n.t(decision.messageKey, {
          level: String(memoryCommandRequiredLevel(def.name, args))
        })
      };
    },
    ...
    consolidate: (sessionId, level) => {
      const agentId = store.getSession(sessionId)?.agentIds[0] as AgentId | undefined;
      return runConsolidate({ agentId, level });
    },
