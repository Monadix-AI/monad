...
export function applyInteractionEvent(m: ProjectionMutations, event: Event): SessionUiEvent[] | undefined {
  switch (event.type) {
      ...
      return [
        {
          ...
          item: m.upsert({
            ...
            ...(p.form ? { form: p.form } : {}),
            ...(p.urlElicitation ? { urlElicitation: p.urlElicitation } : {}),
    ...
    case 'memory.suggestion': {
      ...
      return [m.setCustom({ id: event.id, name: event.type, data: p, status: 'streaming', seq: event.at })];
    }
    ...
    case 'task.created': {
      ...
      return [m.setCustom({ id: p.taskId, name: event.type, data: p, status: 'streaming', seq: event.at })];
    }
    case 'task.progress': {
    ...
    case 'task.completed': {
      ...
      return [m.setCustom({ id: p.taskId, name: event.type, data: p, status: 'done', seq: event.at })];
    }
    case 'task.failed': {
      const p = parseEventPayload('task.failed', event.payload);
      return [m.setCustom({ id: p.taskId, name: event.type, data: p, status: 'error', seq: event.at })];
    }
    case 'delegation.fs_request': {
      const p = parseEventPayload('delegation.fs_request', event.payload);
      return [m.setCustom({ id: p.requestId, name: event.type, data: p, status: 'streaming', seq: event.at })];
    }
