...
export function createSendProjectMessageHandler(ctx: SessionContext, deps: SendProjectMessageDeps) {
  ...
  async function sendProjectMessage({
    ...
    const isWorkplaceProject = session.projectId != null || isWorkplaceProjectTarget(session);
    const projectMembers = isWorkplaceProject ? workplaceProjectMembers(store, session.id) : [];
    const route = isWorkplaceProject
      ? ({ kind: 'send', text: routeSeedText, generate: false, direct: false, targetMention: undefined } as const)
      : routeChannelMessage({
          text: routeSeedText,
          acpAgentNames: acpAgents.map((agent: AcpAgentConfig) => agent.name),
          meshAgentNames: meshAgents.map((agent: MeshAgentConfig) => agent.name)
        });
    ...
    if (route.kind === 'send') {
      ...
      const routeGenerate = isWorkplaceProject ? false : route.generate;
      const result = await send({
        sessionId,
        text: route.text,
        attachments,
