...
import {
  ...
  processControlTool,
  terminateProcessesForSessionAgentChange
...
export function createLifecycleHandlers(ctx: SessionContext) {
  const {
    deps: { store, agent, oversight, delegation, sessionSandbox, hooks, hookCwd, agentSandboxRoots },
    aborts,
    requireSession,
    emitLifecycle,
    waitForRun
  ...
  const listVisibleSessions = (
    ...
    const visibleFilter =
      pendingSessionDeletes.size === 0 ? filter : { ...filter, excludeIds: [...pendingSessionDeletes.keys()] };
    const sessions = store
      .listSessions({ ...visibleFilter, limit, offset })
      .filter((session) => !isPendingSessionDelete(session.id));
    return {
      limit,
      offset,
      sessions,
      total: store.countSessions(visibleFilter)
    };
  };
  ...
  const teardownSessionRuntime = async (id: SessionId): Promise<boolean> => {
    ...
    const cacheDir = ctx.deps.paths?.cache;
    if (cacheDir) {
      await rm(join(cacheDir, 'message-attachments', id), { force: true, recursive: true }).catch((err: unknown) =>
        log.warn({ err, sessionId: id }, 'message attachment cache cleanup failed')
      );
    }
    const attachmentWorkspace = session.cwd ?? agentSandboxRoots?.(id)?.[0] ?? hookCwd;
    if (attachmentWorkspace) {
      await rm(join(attachmentWorkspace, '.monad-attachments', id), { force: true, recursive: true }).catch(
        (err: unknown) => log.warn({ err, sessionId: id }, 'message attachment workspace cleanup failed')
      );
    }
    return true;
  };
  ...
  const { listProjects, reorderProject, getProject, createProject, updateProject, deleteProject } =
    createProjectLifecycleHandlers(ctx, {
      resolveWorkspaceDir,
      teardownProjectSession: teardownSessionRuntime
    });
  ...
  return {
    ...
    async branch({
      ...
      atMessageId,
      origin
    }: { id: SessionId; origin?: OperationSource } & BranchSessionRequest) {
      const parent = requireSession(id);
      // The branching transport is carried on the server-stamped child `origin`; a parent owned by
      // another transport (or by automation) is not forkable across it.
      assertSessionWriteAuthority(parent);
