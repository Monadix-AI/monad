...
export function createOversightHandlers(oversight: OversightService) {
  return {
    ...
    async pending({ sessionId }: { sessionId?: string } = {}): Promise<ListPendingApprovalsResponse> {
      return {
        pending: oversight
          .listPendingRequests(sessionId)
          .map(({ id, sessionId: session, summary }) => ({ requestId: id, sessionId: session, tool: summary }))
      };
    },
    async list({
      sessionId,
...
export function createClarifyHandlers(clarify: InteractionService) {
  return {
    askStructured: clarify.askStructured,
    async respond({ requestId, answer, action }: ClarifyRespondRequest): Promise<ClarifyRespondResponse> {
      return clarify.respond(requestId, answer, action);
    }
  };
}
