...
export interface DaemonHandlerDeps extends SessionDeps, ModelDeps {
  ...
  /** Prepare a backend's local dependency without changing active memory configuration. */
  ...
  /** The one interaction authority: structured presenter interactions AND the agent→human clarify
   *  channel (the `clarify_ask` tool). Clarify handlers derive from it; there is no separate service. */
  ...
  mcpTaskObserve?: (name: string, taskId: string) => Promise<McpTaskView>;
  mcpTaskCancel?: (name: string, taskId: string) => Promise<McpTaskView>;
  ...
  /** Workplace experiences registered by atom packs during the last load sweep. */
  getWorkplaceExperiences?: () => import('#/handlers/atom-pack/atom-pack-registry.ts').RegisteredWorkplaceExperience[];
  /** Boot/rediscovery-built public workplace experience snapshot. */
  getWorkplaceExperienceSnapshot?: () => import('@monad/protocol').WorkplaceExperienceDefinition[] | undefined;
