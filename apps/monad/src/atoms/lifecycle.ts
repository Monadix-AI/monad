...
import { createWorkplaceExperienceSnapshot } from '#/handlers/atom-pack/atom-pack-content.ts';
...
export interface AtomDiscovery {
  ...
  /** The packs this load brought up, so a later rediscovery sweep can deactivate the ones it drops. */
  activeAtomPacks: Map<string, ManifestAtomPack>;
  refreshWorkplaceExperienceSnapshot: () => Promise<void>;
  getWorkplaceExperienceSnapshot: () =>
    | Awaited<ReturnType<typeof createWorkplaceExperienceSnapshot>>['experiences']
    | undefined;
}
...
export async function createAtomDiscovery(deps: {
  ...
  let workplaceExperienceSnapshot:
    | Awaited<ReturnType<typeof createWorkplaceExperienceSnapshot>>['experiences']
    | undefined;
  async function refreshWorkplaceExperienceSnapshot(): Promise<void> {
    const snapshot = await createWorkplaceExperienceSnapshot(paths.packs, [...registry.workplaceExperiences.values()]);
    workplaceExperienceSnapshot = snapshot.experiences;
    for (const warning of snapshot.warnings) {
      logger.warn(`monad: workplace experience "${warning.experienceId}" is not serviceable: ${warning.error}`);
    }
  }
  const channelRegistry = await createChannelRegistry(paths, {
    builtin: {
      ...
      onWorkplaceExperience: (experience, atomPackId, permissions) =>
        registry.registerWorkplaceExperience(experience, atomPackId, permissions),
    ...
    discovered: {
      ...
      onAgentAdapter: (a) => registerAgentAdapterImpl(a),
      // A discovered pack declaring the `sandbox` capability (e.g. a cloud launcher)
      ...
      onPackLoaded: (packId, atomPack) => activeAtomPacks.set(packId, atomPack),
      experienceReview: cfg.atomExperienceReview,
  ...
  void refreshWorkplaceExperienceSnapshot().catch((err) =>
    logger.warn(`monad: workplace experience warmup failed: ${err instanceof Error ? err.message : String(err)}`)
  );
  ...
  return {
    ...
    getWorkplaceExperienceSnapshot: () => workplaceExperienceSnapshot
  };
}
...
export interface AtomsLifecycleOptions {
  ...
  // The one interaction authority atom/Experience capabilities call `request` on. Required so this
  // module can never fall back to an isolated instance the transports/handlers don't observe.
  interactions: InteractionService;
}
