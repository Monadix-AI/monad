...
export const atomPackInstallRecordSchema = z.object({
  ...
  sourceKind: z.enum(['github', 'local', 'npm']).optional(),
...
export async function installAtomPack(spec: string, deps: InstallAtomPackDeps): Promise<InstallOutcome> {
  ...
  const revision = staged.revision ?? sha256Hex(staged.bundle);
  ...
  // Source-level trust warnings are surfaced for the consent decision. Canonical GitHub artifacts
  // always carry a transport checksum; manifest integrity additionally pins the executable bundle.
  if (source.kind === 'github' && !manifest.integrity) {
