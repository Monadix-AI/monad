...
export function generateMigrationAssets(): void {
  ...
  try {
    current = readFileSync(generatedModule, 'utf8');
  } catch {}
  if (current === content) return;
  const temporaryModule = `${generatedModule}.${randomUUID()}.tmp`;
  try {
    writeFileSync(temporaryModule, content, { encoding: 'utf8', flag: 'wx', mode: 0o600 });
    renameSync(temporaryModule, generatedModule);
  } finally {
    rmSync(temporaryModule, { force: true });
  }
}
