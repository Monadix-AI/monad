...
import { atomPackSourceSpec } from '../../src/components/OperationalScreens.tsx';
...
test('Atom Pack source input keeps GitHub repositories and marks local development folders', () => {
  expect({
    github: atomPackSourceSpec('github', ' github:owner/repo@main '),
    local: atomPackSourceSpec('local', ' /tmp/my-pack '),
    explicitLocal: atomPackSourceSpec('local', 'local:/tmp/my-pack')
  }).toEqual({
    github: 'github:owner/repo@main',
    local: 'local:/tmp/my-pack',
    explicitLocal: 'local:/tmp/my-pack'
  });
});
...
describe('wide chat panes', () => {
  ...
  test('shows the Plan pane purely off the user toggle, independent of mesh-agent presence', () => {
    expect(shouldShowPlan('wide', true, true, true)).toBe(true);
    expect(shouldShowPlan('wide', true, true, false)).toBe(false);
    expect(shouldShowPlan('medium', true, true, true)).toBe(false);
    expect(shouldShowPlan('wide', false, true, true)).toBe(false);
    expect(shouldShowPlan('wide', true, false, true)).toBe(false);
  });
...
describe('globalShortcut', () => {
  ...
  test('maps the globally discoverable shortcuts', () => {
    ...
    expect(globalShortcut('p', key({ ctrl: true }), false)).toBe('panel.plan');
