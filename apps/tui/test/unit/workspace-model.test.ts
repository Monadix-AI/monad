...
describe('Workplace Project Web model', () => {
  ...
  test('maps editable Project fields and rejects cwd changes after creation', () => {
    ...
    expect(projectUpdateRequest('cwd' as never, '/repo/other')).toBeNull();
  });
...
describe('Project member templates', () => {
  test('deduplicates ACP templates without replacing advanced settings', () => {
    const existing = [
  ...
  test('removes only the selected member template', () => {
    ...
    expect(removeProjectMemberTemplate(members, 'pmem_monad')).toEqual([
      { id: 'acp:codex', type: 'acp', name: 'codex' }
    ]);
  });
});
