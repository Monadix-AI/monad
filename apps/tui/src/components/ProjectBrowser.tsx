...
export function ProjectBrowser({
  ...
  const submitForm = async () => {
    try {
      ...
      if (mode === 'rename-project') {
        const request = projectUpdateRequest('title', draft);
        if (!request) return;
  ...
  const isForm = ['new-project-name', 'new-project-cwd', 'rename-project', 'new-session', 'rename-session'].includes(
    mode
  );
        ...
        <>
          ...
          <Text color={TUI_THEME.dim}>
            {mode === 'projects'
              ...
              : '↑↓ move · Enter open · n new · e rename · x delete · a archive · m members · Esc projects'}
          </Text>
        </>
      )}
