...
import { projectionPanelModel } from './projection-panel-model.ts';
import { TUI_THEME } from './theme.ts';

export function ProjectionPanel({ active, sessionId }: { active: boolean; sessionId: SessionId }) {
  const stateQuery = useStreamMeshAgentStateQuery(sessionId);
  const state = useMemo(
    () => (stateQuery.data ? foldMeshAgentExperienceState(stateQuery.data) : undefined),
    [stateQuery.data]
  );
  const sessions = state ? [...state.sessions.values()] : [];
  const observed = sessions.find((session) => session.lifecycle.state !== 'terminal') ?? sessions[0];
  const model = state && observed ? projectionPanelModel(state, observed) : undefined;
    ...
    >
      ...
      {observed && model ? (
        <Text color={TUI_THEME.dim}>
          {observed.agentName} · {observed.provider} · {model.runtime.label}
        </Text>
      ) : null}
      {model?.loginRequirements.map((login) => (
        <Text
          color={TUI_THEME.warning}
          key={login.id}
        >
          {t('cli.tui.projection.loginRequired', { agentName: login.agentName })}
        </Text>
      ))}
      {model?.approvals.map((approval) => (
        ...
        >
          {t('cli.tui.projection.approvalPending', { text: approval.text })}
        </Text>
      ))}
