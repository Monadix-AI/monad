...
import type { ProjectId, SessionId, UIClarificationItem } from '@monad/protocol';
...
export function Layout({
  ...
  const [respondClarify, clarifyResponse] = useClarifyRespondMutation();
  ...
  const meshStateQuery = useStreamMeshAgentStateQuery(currentSessionId ?? ('' as SessionId), {
  ...
  const contextStream = useStreamUiItemsQuery(currentSessionId ?? ('' as SessionId), {
    ...
    selectFromResult: ({ data }) => {
      ...
      return {
        ...
        urlClarification: projected.urlClarification
      };
    }
  });
  ...
  const memorySuggestion = chatOpen
    ...
    : undefined;
  const urlClarification = chatOpen ? contextStream.urlClarification : undefined;
  ...
  const meshAgentCount = Object.keys(meshStateQuery.data?.sessions ?? {}).length;
  const showProjection = shouldShowProjection(mode, chatOpen, currentSessionId !== null, meshAgentCount);
  const showPlan = shouldShowPlan(mode, chatOpen, currentSessionId !== null, planOpen);
  const chatWidths = chatPaneWidths(columns, navigationWidth, showProjection || showPlan);
  ...
  usePaste((text) => setEditor((state) => insertText(state, text)), {
    isActive: composerActive && !interactionPresenter.active && !memorySuggestion && !urlClarification
  });
  ...
  useInput((typed, key) => {
    if (urlClarification) {
      if (clarifyResponse.isLoading) return;
      const respond = (action: 'complete' | 'cancel') => {
        void respondClarify({ requestId: urlClarification.id, action })
          .unwrap()
          .catch((cause) => setStatus(safeErrorMessage(cause)));
      };
      if (key.escape || typed.toLowerCase() === 'c') respond('cancel');
      else if (key.return || typed.toLowerCase() === 'd') respond('complete');
      return;
    }
    ...
    if (key.tab) {
      ...
      registry.register({
        // 'projection' is the shared right-panel focus region — whichever of ProjectionPanel/
        // SessionPlanPanel is actually visible occupies this same slot, never both at once.
        active: showProjection || showPlan,
        height: rows,
        id: 'projection',
        order: 30,
        width: columns,
  ...
  const showInteraction = interactionPresenter.active !== null || urlClarification !== undefined;
  return (
    <Box
    ...
    >
      ...
      <Box flexGrow={1}>
        ...
        >
          {urlClarification ? (
            <UrlClarificationPrompt
              busy={clarifyResponse.isLoading}
              clarification={urlClarification}
            />
          ) : showInteraction ? (
...
function UrlClarificationPrompt({ busy, clarification }: { busy: boolean; clarification: UIClarificationItem }) {
  return (
    <Box
      flexDirection="column"
      paddingX={2}
      paddingY={1}
    >
      <Text color={TUI_THEME.dim}>{t('cli.tui.urlElicitation.title')}</Text>
      <Text
        bold
      ...
      <Text>{clarification.urlElicitation?.url}</Text>
      <Text color={TUI_THEME.dim}>
        {busy ? t('cli.tui.urlElicitation.submitting') : t('cli.tui.urlElicitation.actions')}
      </Text>
    </Box>
  );
}
...
function CapabilityContent({
  ...
  switch (capability.id) {
    ...
    case 'studio.atoms':
      return <AtomPacksScreen active={active} />;
