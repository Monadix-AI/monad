...
// The Plan pane is a durable per-session artifact, not tied to a mesh agent being present — unlike
// `shouldShowProjection`, it only needs a real session open in wide mode, gated by the user's own
// toggle (Ctrl+P) rather than session contents.
export function shouldShowPlan(
  ...
  return mode === 'wide' && chatOpen && hasSession && planOpen;
}
