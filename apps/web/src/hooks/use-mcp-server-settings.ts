import type { McpCatalogEntry, McpServerStatus, McpServerView, McpServerWrite } from '@monad/protocol';
...
export interface McpServerSettingsStore {
  ...
  refreshing: boolean;
  error?: string;
  saveServer: (s: McpServerWrite) => Promise<void>;
...
function preserveMcpSecrets(server: McpServerView): McpServerWrite {
  if (server.transport === 'stdio') return server;
  const auth =
    server.auth.mode === 'bearer'
      ? { mode: 'bearer' as const }
      : server.auth.mode === 'headers'
        ? { mode: 'headers' as const, headers: {} }
        : server.auth;
  return {
    name: server.name,
    transport: 'http',
...
export function useMcpServerSettings(): McpServerSettingsStore {
  ...
  const setEnabled = useCallback(
    async (s: McpServerView, enabled: boolean) => {
      await upsert({ ...preserveMcpSecrets(s), enabled }).unwrap();
    },
    [upsert]
  );
