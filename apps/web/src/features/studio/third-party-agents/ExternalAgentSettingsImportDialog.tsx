...
export function ExternalAgentSettingsImportDialog({
  ...
  const t = useT();
  const candidatesQ = useListExternalAgentSettingsImportCandidatesQuery(preset.id);
  const [path, setPath] = useState('');
  const [selectedCandidatePaths, setSelectedCandidatePaths] = useState<Set<string>>(new Set());
  const [replace, setReplace] = useState(false);
  const [preview, setPreview] = useState<ExternalAgentSettingsImportPreview | null>(null);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [result, setResult] = useState<{ applied: string[]; skipped: Array<{ id: string; reason: string }> } | null>(
    null
  );
  const [previewImport, previewState] = usePreviewExternalAgentSettingsImportMutation();
  const [applyImport, applyState] = useApplyExternalAgentSettingsImportMutation();
  ...
  function toggleItem(item: ExternalAgentSettingsImportItem) {
      ...
      <DialogHeader className="border-b bg-card/80 px-5 py-4 pr-12">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <span className="min-w-0">
            <DialogTitle className="truncate text-base">{t('web.externalAgent.importSettings')}</DialogTitle>
        ...
        <div className="flex flex-col gap-4 p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="external-agent-settings-import-path">{t('web.settings.import.path')}</Label>
              <Input
                disabled={busy}
                id="external-agent-settings-import-path"
          ...
          {candidates.length > 0 ? (
            ...
            <p className="text-muted-foreground text-xs">{t('web.externalAgent.importNoCandidates')}</p>
          )}
          {preview ? (
