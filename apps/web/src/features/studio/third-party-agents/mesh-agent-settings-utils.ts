...
import type { MeshAgentPresetView, MeshAgentProjectTemplate, MeshAgentView } from '@monad/protocol';
...
export const nextTemplateId = (templates: readonly MeshAgentProjectTemplate[]): string => {
...
export const normalizeProjectTemplates = (templates: readonly MeshAgentProjectTemplate[]): MeshAgentProjectTemplate[] =>
  templates
    .map((template) => ({
      id: template.id.trim(),
...
export type ProjectTemplateEditorRow = MeshAgentProjectTemplate & { rowKey: string };
...
export const BLANK_AGENT: MeshAgentView = {
  name: '',
  provider: 'codex',
  command: '',
  args: [],
