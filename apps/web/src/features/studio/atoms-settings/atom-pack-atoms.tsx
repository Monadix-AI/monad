...
import { ChannelBrandIcon } from '#/features/studio/channels-settings/ChannelBrandIcon';
...
function AtomKindBody({ atoms, form }: { atoms: AtomDescriptor[]; form: AtomForm }) {
  ...
  const isChannel = atoms[0]?.kind === 'channel';
  const isAgentAdapter = atoms[0]?.kind === 'agent-adapter';
        ...
        {atoms.map((atom) => (
          ...
          >
            <ChannelBrandIcon
              ...
              icon={atom.icon ?? atom.channel?.icon}
