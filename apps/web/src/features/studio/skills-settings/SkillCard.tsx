...
import { Badge, Button, Confirm, cn, Tooltip, TooltipContent, TooltipTrigger } from '@monad/ui';
    ...
    <article className={article}>
      ...
      <div className={controls}>
        <SwitchSetting
          ...
          className="min-h-10 px-4 py-2 transition-[background-color,color] duration-150 ease-out hover:bg-accent/50"
          disabled={disabledControl}
          onCheckedChange={onEnabledChange}
          switchAriaLabel={t('web.skills.skillEnabledToggle', { name: skill.name })}
          title={t('web.skills.skillEnabledTitle')}
          titleClassName="text-xs"
        />
        <SwitchSetting
          checked={autoload && skillAutoloadEnabled}
          className="min-h-10 border-t px-4 py-2 transition-[background-color,color] duration-150 ease-out hover:bg-accent/50 sm:border-t-0 sm:border-l"
          disabled={!autoload || !skillEnabled || autoloadDisabledControl}
          onCheckedChange={onAutoloadChange}
          switchAriaLabel={t('web.skills.skillAutoloadToggle', { name: skill.name })}
          title={t('web.skills.skillAutoloadTitle')}
          titleClassName="text-xs"
        />
      </div>
    </article>
  ...
  return (
    <>
      ...
      <Confirm
        ...
        confirmLabel={t('web.skills.remove')}
        confirmVariant="destructive"
        description={t('web.skills.confirmRemoveDescription', { name: skill.name })}
        error={removeFailed ? t('web.skills.removeFailed') : undefined}
        onConfirm={() => {
          setRemoveFailed(false);
          void remove({ name: skill.name })
            .unwrap()
        ...
        open={confirmRemove}
        pending={removing}
        pendingLabel={t('web.skills.removing')}
        title={t('web.skills.confirmRemove')}
      />
    </>
  );
}
