...
import { AddSkillMenu } from './AddSkillDialog';
...
function SkillsSettingsContent({ embedded = false }: { embedded?: boolean }) {
    ...
    return (
      <SwitchSetting
        ...
        descriptionClassName={cn(
          density === 'card' && 'max-w-[68ch] text-sm leading-5',
          density === 'row' && 'line-clamp-1'
        )}
        disabled={
          isAutoload
        ...
        icon={
          <HugeiconsIcon
            className={cn('size-4', isAutoload && 'text-muted-foreground')}
            icon={Icon}
          />
        }
  ...
  const renderInstalledStatus = () => (
    <>
      ...
      {skillInstances.length === 0 && !(isFetching || loadingSettings) ? (
