...
import { useCreateSkillMutation, useUpdateSkillContentMutation } from '@monad/client-rtk';
...
export function SkillEditorDialog({
  ...
  onClose: () => void;
  onSaved: (skill: { id: string; name: string }) => void;
}) {
  ...
  return (
    <SkillEditorForm
      ...
      key={editor.id ?? editor.name ?? editor.agentDir ?? 'new-skill'}
...
function SkillEditorForm({
  ...
  const save = async () => {
    ...
    if (editor.name) {
      ...
      const id = editor.id ?? `global:${name}`;
      const res = await updateSkillContent({ name, id, content })
        .unwrap()
        .catch(() => null);
      if (res) {
        onSaved({ id, name: res.name });
        return;
      }
    } else {
      const res = await createSkill({
        ...
        content,
        target: skillMutationTarget(editor.createTarget ?? { kind: 'workspace' })
      })
        .unwrap()
        .catch(() => null);
      if (res?.id) {
    ...
    >
      ...
      >
        ...
        <DialogBody className="flex flex-col gap-3">
          ...
          {files.length > 0 ? (
            <section className="flex max-h-[22vh] min-h-28 min-w-0 flex-col gap-2 overflow-hidden border-border/70 border-t pt-3">
