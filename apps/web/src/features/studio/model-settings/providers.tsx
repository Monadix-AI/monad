...
export function ProviderDialog({
  ...
  const handleNext = () => setStep((current) => providerDialogNextStep(mode, current));
  ...
  const sections = providerDialogSections(mode, step, Boolean(selectedMeta.needsUrl));
    ...
    >
      ...
      >
        ...
        <DialogBody className="flex flex-col gap-5">
          {sections.showProviderPicker ? (
            <div className="grid gap-2 sm:grid-cols-3">
              {PROVIDER_TYPES.map((p) => {
                const { logo: PrvLogo, color } = metaFor(p.value);
              ...
              {(mode === 'edit' || hasTestedProvider) && (
                <section className="flex flex-col gap-2">
                  <div className="flex items-center justify-between">
                    ...
                    {mode === 'edit' && (
                      <RefreshButton
                        label={t('web.model.refresh')}
                        loading={detail.isLoadingModels}
        ...
        <DialogFooter>
          <Button
            ...
            variant="outline"
          >
            {mode === 'edit' ? t('web.common.close') : t('web.common.cancel')}
          </Button>
          {sections.showProviderPicker ? (
            <Button
              onClick={handleNext}
          ...
          {mode === 'add' && !sections.showProviderPicker ? (
            <>
              <Button
                disabled={testing}
                onClick={() => void handleTestProvider()}
                size="sm"
                variant={hasTestedProvider ? 'outline' : 'default'}
              >
                {testing ? (
                  <>
                    <HugeiconsIcon
