...
export function RuntimeOverview() {
  ...
  const credentialCount = credentialsQuery.data?.credentials.length ?? 0;

  return (
    <PanelShell>
      <StudioBreadcrumbHeader title={t('web.studio.runtimeOverview')} />
      <PanelShellBody>
        <div className="mx-auto grid w-full max-w-6xl gap-5 p-4 pb-24 lg:grid-cols-[minmax(0,1fr)_19rem] lg:p-6 lg:pb-24">
          <main className="flex min-w-0 flex-col gap-5">
            ...
            <section className="flex flex-col gap-3">
              ...
              />
              <ReadinessRow
                actionHref={studioPath('credentials')}
                actionLabel={t('web.credentials.review')}
                body={t('web.credentials.overviewDescription', { count: credentialCount })}
                icon={Key01Icon}
                state={readinessState(credentialCount > 0, credentialsQuery.isLoading)}
