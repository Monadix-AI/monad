...
/**
 * Studio: the two-plane workbench. Monad Agent Runtime contains Monad-owned runtime policy and
 * capabilities; Monad Mesh coordinates provider-owned agents and Workplace projects.
 */
export function Studio({ onClose }: { onClose: () => void }) {
  const pathname = useShellPathname();
  const initStatus = useInitStatusQuery();
  const section: StudioSectionId = studioSectionFromPathname(pathname) ?? DEFAULT_STUDIO_SECTION;
