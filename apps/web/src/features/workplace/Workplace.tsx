import type { SessionId, WorkplaceExperienceProjectDialogRequest } from '@monad/protocol';
...
export const Workplace = memo(function Workplace({
  ...
  const requestProjectDialog = useCallback(
    (request: WorkplaceExperienceProjectDialogRequest) => {
    ...
    >
      ...
      >
        ...
        >
          {surface === 'workplace' ? (
            <ProjectHeader
              onAddMember={() => openSessionSettingsInStore(projectId)}
              project={project}
            />
          ) : null}
