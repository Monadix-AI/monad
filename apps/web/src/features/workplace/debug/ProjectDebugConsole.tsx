...
type DebugFilter = 'all' | 'http' | 'sse' | 'mesh-agent' | 'approval' | 'log' | 'error';

const FILTERS: DebugFilter[] = ['all', 'http', 'sse', 'mesh-agent', 'approval', 'log', 'error'];
...
export function filterDebugTraceEntries(
  ...
  if (filter === 'mesh-agent') {
    return entries.filter((entry) => entry.label.startsWith('mesh.'));
  }
