...
type MeshStep = 'cli' | 'project';

export function InitWizard({ homePath }: { homePath?: string }) {
  const t = useT();
  const { baseUrl: daemonBaseUrl, client: monadClient } = useMonadRuntime();
  const [step, setStep] = useState<Step>('choice');
  const [customHome, setCustomHome] = useState('');
  const [homeError, setHomeError] = useState('');
  const [restarting, setRestarting] = useState(false);
  const [meshStep, setMeshStep] = useState<MeshStep>('cli');
  ...
  async function handleSetHome(nextStep: Step) {
    ...
    if (customHome) {
      ...
      for (let i = 0; i < 30; i++) {
        ...
        try {
          const res = await fetch(daemonApiUrl(daemonBaseUrl, '/health'));
          if (res.ok) break;
        } catch {
          /* not yet */
        }
  ...
  const stepIndex =
    step === 'choice'
      ? 0
      : step === 'provider' || (step === 'mesh' && meshStep === 'cli')
        ? 1
        : step === 'model' || step === 'mesh'
          ? 2
          : 3;
