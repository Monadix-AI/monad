...
const PACKAGE_GROUP_LABELS = {
  monad: 'web.licenses.group.monad',
  cli: 'web.licenses.group.cli',
  web: 'web.licenses.group.web',
  tui: 'web.licenses.group.tui'
} as const;
...
export function LicensesSettings() {
  ...
  const matchesSearch = (pkg: (typeof packages)[number]) =>
    !normalizedSearch ||
    pkg.name.toLowerCase().includes(normalizedSearch) ||
    pkg.license.toLowerCase().includes(normalizedSearch) ||
    pkg.version.toLowerCase().includes(normalizedSearch);
  const visiblePackageCount = packages.filter(matchesSearch).length;
  const totalPackageCount = packages.length;
  const filteredGroups = packageGroups
    .map((group) => ({ ...group, packages: group.packages.filter(matchesSearch) }))
    ...
    isLoading ? (
      ...
      <div className="flex flex-col gap-5">
        {filteredGroups.map((group) => (
          <div
            className="licenses-group"
            key={group.id}
          >
            <div className="licenses-group-head">
              <h4 className="font-medium text-sm">{t(PACKAGE_GROUP_LABELS[group.id])}</h4>
              <span className="licenses-count">{t('web.licenses.packageCount', { count: group.packages.length })}</span>
            </div>
