...
export function ConnectionSettings() {
  ...
  const [confirmRemoteHttp, setConfirmRemoteHttp] = useState(false);
  const [confirmRotateRemoteToken, setConfirmRotateRemoteToken] = useState(false);
  const [rotateRemoteTokenError, setRotateRemoteTokenError] = useState<string>();
  ...
  async function applyHttpsToggle(enabled: boolean) {
  ...
  async function toggleHttps(enabled: boolean) {
    if (!enabled && network.settings?.remoteAccess.enabled) {
      setHttpsTransitionError(undefined);
      setConfirmRemoteHttp(true);
      return;
    }
    await applyHttpsToggle(enabled);
  }
  ...
  async function rotateRemoteToken() {
    ...
    try {
      ...
      setConfirmRotateRemoteToken(false);
    } catch (error) {
      setRotateRemoteTokenError(error instanceof Error ? error.message : t('web.conn.rotateTokenFailed'));
    }
  }
  ...
  return (
    <div className="flex min-w-0 flex-1 flex-col">
      ...
      <Confirm
        ...
        description={t('web.settings.system.remoteHttpConfirmDescription')}
        error={confirmRemoteHttp ? httpsTransitionError : undefined}
        onConfirm={() => void applyHttpsToggle(false)}
        onOpenChange={setConfirmRemoteHttp}
        open={confirmRemoteHttp}
        pending={networkBusy}
        pendingLabel={t('web.settings.system.switchingToHttp')}
        title={t('web.settings.system.remoteHttpConfirmTitle')}
      />
      <Confirm
        cancelLabel={t('web.common.cancel')}
        confirmLabel={t('web.conn.rotateToken')}
        description={t('web.conn.rotateTokenConfirmDescription')}
        error={rotateRemoteTokenError}
        onConfirm={() => void rotateRemoteToken()}
        onOpenChange={(open) => {
          setConfirmRotateRemoteToken(open);
          if (!open) setRotateRemoteTokenError(undefined);
        }}
        open={confirmRotateRemoteToken}
        pending={network.saving}
        pendingLabel={t('web.conn.rotatingToken')}
        title={t('web.conn.rotateTokenConfirmTitle')}
      />
    </div>
  );
}
