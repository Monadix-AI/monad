import type { ProjectId, Session, SessionAttentionState, SessionId } from '@monad/protocol';
...
function useSidebarSessionActions({
  ...
  const deleteProjectSession = useCallback(
    (projectId: string, sessionId: SessionId) => {
      ...
      queueSessionDelete({
        afterDelete: () => onCreateProjectSession(projectId),
        sessionId,
        title
      });
    },
    [onCreateProjectSession, projects, queueSessionDelete, t]
  );
  ...
  const deleteChatSession = useCallback(
    (sessionId: SessionId) => {
      ...
      queueSessionDelete({
        afterDelete: onCreateChatSession,
        sessionId,
        title
      });
    },
    [chatSessions, onCreateChatSession, queueSessionDelete, t]
  );
