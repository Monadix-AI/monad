...
function InboxSidebarBadge({
  ...
  count: number;
  label: string;
  tone: 'needs-response' | 'unread';
}) {
  const text = `${label} · ${count}`;
  return (
    <SidebarAttentionBadge
      data-sidebar-inbox-badge={tone}
      state={tone === 'needs-response' ? 'need-response' : 'unread'}
      title={text}
    >
      {text}
    </SidebarAttentionBadge>
...
const WorkspaceSectionList = memo(function WorkspaceSectionList() {
  ...
  const { data: inboxSummary } = useGetInboxSummaryQuery();
        ...
        <div className="mt-0.5 flex flex-col gap-0.5">
          ...
          >
            {inboxSummary?.needsResponseCount ? (
              <InboxSidebarBadge
                count={inboxSummary.needsResponseCount}
                label={meta.t('web.inbox.needResponse')}
                tone="needs-response"
              />
            ) : inboxSummary?.unreadCount ? (
              <InboxSidebarBadge
                count={inboxSummary.unreadCount}
                label={meta.t('web.inbox.unread')}
                tone="unread"
              />
            ) : null}
          </SidebarNavItem>
