...
function ChatSessionTreeItem({
  active,
  session
}: {
  active: boolean;
  session: Pick<Session, 'id' | 'title'> & {
    attentionState?: import('@monad/protocol').SessionAttentionState | null;
    generationState?: import('@monad/protocol').SessionGenerationState | null;
  };
}) {
  const { actions, meta } = useWorkspaceSidebar();
  const shortcutValue = useSidebarSessionShortcutValue(`chat:${session.id}`);
