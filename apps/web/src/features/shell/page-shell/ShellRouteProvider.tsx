...
import { useIsNarrowSidebarViewport } from '#/features/shell/use-sidebar-viewport';
...
export function ShellRouteProvider({ children }: { children: ReactNode }) {
  ...
  const allSessions = useMemo(() => {
    const byId = new Map<SessionId, Session>(sessions.map((session) => [session.id, session]));
  ...
  useEffect(() => {
    if (!narrowSidebarViewport) setNarrowSidebarOpen(false);
  }, [narrowSidebarViewport]);
  useEffect(() => {
  ...
  const reserveHeaderLeading = narrowSidebarViewport || sidebarCollapsed || sidebarAutoReveal;
  ...
  const sidebarProps = useMemo<ComponentProps<typeof SessionSidebar>>(
    () => ({
      ...
      responsive: {
        narrow: narrowSidebarViewport,
        onOpenChange: setNarrowSidebarOpen,
      ...
      <RightPanelProvider ownerId={rightPanelOwnerId}>
        <div className="app-shell relative flex h-screen overflow-hidden bg-background text-foreground">
          <AppShellSidebarReveal
            narrow={narrowSidebarViewport}
            narrowOpen={narrowSidebarOpen}
            onNarrowOpenChange={setNarrowSidebarOpen}
            onOpenWorkspace={sidebarProps.surfaces.onOpenWorkspace}
          />
