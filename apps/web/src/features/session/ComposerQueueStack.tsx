...
import { type SessionQueuedMessage, sessionMessagesCanSteer } from './session-route-contract';
...
export function ComposerQueueStack({
  ...
  const displayText = (item: SessionQueuedMessage): string => {
    const attachmentNames = item.attachments?.map((attachment) => attachment.name).join(', ') ?? '';
    return [item.text, attachmentNames].filter(Boolean).join('\n');
  };
  const stackedCards = queuedCardsForDisplay(items.map(displayText));
  const expandedCards = items.map((item, queueIndex) => ({ queueIndex, text: displayText(item) })).reverse();
    ...
    >
      <div className="absolute right-0 bottom-5 grid w-full bg-transparent transition-opacity group-hover:invisible group-hover:opacity-0">
        {stackedCards.map((card) => (
          <div
            className="popup-surface pointer-events-auto col-start-1 row-start-1 w-full self-end rounded-md px-3 py-2 pr-7 text-popover-foreground text-xs shadow-lg transition-transform"
          ...
          >
            <p className="line-clamp-5 min-w-0 whitespace-pre-line break-words leading-5">{card.text}</p>
      ...
      >
        {expandedCards.map((card) => (
          <div
            className="popup-surface relative w-full shrink-0 rounded-md px-3 py-2 pr-7 text-popover-foreground text-xs"
