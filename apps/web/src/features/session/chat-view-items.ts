...
function attachmentsFromParts(parts: UIPart[]): MessageAttachmentView[] {
  return parts.flatMap((part) => {
    if (part.type !== 'custom' || part.name !== 'attachment') return [];
    const parsed = messageAttachmentSchema.safeParse(part.data);
    if (!parsed.success) return [];
    const attachment: MessageAttachment = parsed.data;
    return [
      {
        ...attachment,
