import type { ComposerAttachmentItem, ComposerSendShortcut } from '@monad/ui';
...
export function composerCanSend({
  ...
  return (text.trim().length > 0 || attachmentCount > 0) && !disabled && !voiceActive;
}

export function selectedComposerFiles(files: FileList | null): File[] {
        ...
        attach: onAttachFiles ? (
          <>
            <input
              ...
              multiple
              onChange={(event) => {
                const files = selectedComposerFiles(event.currentTarget.files);
            ...
            />
            <ComposerIconButton
              ariaLabel={t('web.chat.attachFiles')}
              disabled={disabled}
              onClick={() => fileInputRef.current?.click()}
              title={t('web.chat.attachFiles')}
            >
              <HugeiconsIcon
                aria-hidden
          ...
          {attachments.length && onRemoveAttachment ? (
            <ComposerAttachmentStrip
              ...
              labels={{
                ...
                remove: (name) => t('web.chat.removeAttachment', { name })
              }}
...
function VoiceDebugPanel({
  ...
  return (
    <details className="mx-3 mb-3 rounded-md border border-border/70 bg-muted/30 px-3 py-2 text-[11px] text-muted-foreground">
      ...
      <dl className="mt-2 grid grid-cols-[88px_1fr] gap-x-3 gap-y-1 font-ui">
