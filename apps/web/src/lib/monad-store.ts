...
export function resolveConnection(
  viteDaemonWsBaseUrl = import.meta.env.VITE_MONAD_DAEMON_WS_BASE_URL
): MonadConnectionConfig {
  ...
  const useViteProxy = process.env.NODE_ENV === 'development' && isViteDevOrigin(window.location.origin);
  return {
    baseUrl: `${window.location.origin}${useViteProxy ? '/api' : ''}`,
    ...(useViteProxy && viteDaemonWsBaseUrl ? { wsBaseUrl: viteDaemonWsBaseUrl } : {})
  };
}
