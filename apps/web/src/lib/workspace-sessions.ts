...
type WorkspaceProjectSessionListItem = {
  id: Session['id'];
  title: string;
...
export const buildWorkspaceProjects = (
  ...
  const sessionProjectId = new Map((options.sessions ?? []).map((session) => [session.id, session.projectId]));
  const sessionsByProjectId = new Map<string, (WorkspaceProjectSessionListItem & { activityAt: string })[]>();
  for (const session of options.sessions ?? []) {
    ...
    sessions.push({
      ...
      ...(session.attentionState === undefined ? {} : { attentionState: session.attentionState }),
      ...(session.generationState === undefined ? {} : { generationState: session.generationState }),
      ...(session.unreadItemKeys === undefined ? {} : { unreadItemKeys: session.unreadItemKeys })
    });
