...
const networkConfigOverrideSchema = z.object({
...
const PLAYWRIGHT_REDUCED_MOTION_CSS = `
...
export function playwrightReducedMotionPlugin(enabled = process.env.VITE_PLAYWRIGHT_TEST === '1'): Plugin {
  return {
    name: 'monad-playwright-reduced-motion',
    transformIndexHtml() {
      if (!enabled) return;
      return [
        {
          tag: 'style',
          attrs: { 'data-playwright-reduced-motion': '' },
          children: PLAYWRIGHT_REDUCED_MOTION_CSS,
          injectTo: 'head'
        }
      ];
    }
...
export default defineConfig(({ command }) => ({
  ...
  define: {
    ...
    'import.meta.env.VITE_MONAD_DAEMON_WS_BASE_URL': JSON.stringify(command === 'serve' ? daemonProxyTarget() : ''),
        ...
        codeSplitting: {
          groups: [
            ...
            },
            {
              ...
              test: new RegExp(`${WORKSPACE_ROOT}atoms[/\\\\]src[/\\\\]workplace-experiences[/\\\\]chat-room[/\\\\]`)
            },
            {
              name: 'monad-experiences',
              test: new RegExp(`${WORKSPACE_ROOT}atoms[/\\\\]src[/\\\\]workplace-experiences[/\\\\]`)
            },
