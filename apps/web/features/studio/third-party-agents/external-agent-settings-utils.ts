...
export const normalizeProjectTemplates = (
  templates: readonly ExternalAgentProjectTemplate[]
): ExternalAgentProjectTemplate[] =>
  templates
    .map((template) => ({
      id: template.id.trim(),
...
export type ProjectTemplateEditorRow = ExternalAgentProjectTemplate & { rowKey: string };
...
export const BLANK_AGENT: ExternalAgentView = {
  name: '',
  provider: 'codex',
  command: '',
  args: [],
...
export const presetToView = (p: ExternalAgentPresetView): ExternalAgentView => ({
  name: p.id,
  provider: p.provider,
...
export const presetForAgent = (
  ...
  presets: readonly ExternalAgentPresetView[]
): ExternalAgentPresetView | undefined =>
...
export function presetHintKey(id: string): WebMessageIdWithoutParams {
  return `web.externalAgent.presetHint.${id}` as WebMessageIdWithoutParams;
}
