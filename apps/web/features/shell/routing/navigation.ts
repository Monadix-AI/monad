...
export function useAppShellNavigation({
  ...
  const handleOpenProjectSession = useCallback(
    (projectId: string, sessionId: SessionId) => {
      ...
      if (activeProjectSession?.projectId === projectId) {
        ...
        activeProjectSession.switchSession(sessionId);
        return;
      }
  ...
  useEffect(() => {
    if (!isSettingsRoute && settingsReturnPathState !== null) setSettingsReturnPathState(null);
  }, [setSettingsReturnPathState, settingsReturnPathState, isSettingsRoute]);
