...
async function mockModelSettingsApi(
  ...
  const credentials: Record<string, Credential[]> = {
    oai: [{ id: 'cred_main', label: 'main', configured: true, requestCount: 7, lastStatus: 'ok' }]
  };
  ...
  await page.route('**/api/v1/**', async (route) => {
          ...
          providers: [
            {
              type: 'openai',
              label: 'OpenAI',
              icon: { title: 'OpenAI', path: 'M4 4h16v16H4z' },
              strategy: 'native',
    ...
    const providerPath = path.match(/^\/v1\/settings\/model\/providers\/([^/]+)$/);
    if (method === 'PUT' && providerPath) {
    ...
    if (method === 'DELETE' && providerPath) {
      const id = decodeURIComponent(providerPath[1] ?? '');
    ...
    if (providerCredentials) {
      ...
      if (method === 'POST' && !credentialId) {
        ...
        const credential = {
          ...
          label: body.label,
          configured: body.accessToken.length > 0,
          requestCount: 0,
    ...
    if (method === 'GET' && providerModels) {
      ...
      return route.fulfill(json({ providerId, models: models[providerId] ?? [] }));
    }
...
async function addCompatibleProvider(page: Page, key = 'sk-added-9999') {
  ...
  await providerOption(page, 'OpenAI-compatible').click();
...
test.describe('Studio model settings', () => {
  ...
  test('focuses the model id input when opening a model picker', async ({ page }) => {
    ...
    await defaultCard.getByRole('button', { name: 'GPT 5.1' }).click();
  ...
  test('renders reasoning effort controls from model metadata only', async ({ page }) => {
    ...
    const effortSlider = page.locator('[data-slot="popover-content"]').getByRole('slider', { name: 'Effort' });
    await expect(effortSlider).toHaveAttribute('aria-valuemax', '2');
    await expect(effortSlider).toHaveAttribute('aria-valuetext', 'Xhigh');
    await effortSlider.press('Home');
    await expect(defaultCard.getByRole('button', { name: 'Effort Max' })).toBeVisible();

    await defaultCard.getByRole('button', { name: 'Effort Max' }).click();
    await page.locator('[data-slot="popover-content"]').getByRole('slider', { name: 'Effort' }).press('End');
    await expect(defaultCard.getByRole('button', { name: 'Effort High' })).toBeVisible();
    await page.reload();
    await expect(page.getByRole('heading', { name: /Studio\s*\/\s*Models/ })).toBeVisible();
    await expect(defaultCard.getByRole('button', { name: 'Effort High' })).toBeVisible();

    await defaultCard.getByRole('button', { name: 'GPT 5.1' }).hover();
  ...
  test('does not render reasoning controls when a reasoning model has no effort metadata', async ({ page }) => {
    ...
    await defaultCard.getByRole('button', { name: 'GPT 5.1 Mini' }).hover();
    const hoverCard = page.locator('[data-slot="hover-card-content"]');
  ...
  test('shows every provider output modality in the provider model list', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Edit provider' }).click({ force: true });
  ...
  test('shows media and embedding pricing without reasoning-only fields', async ({ page }) => {
    ...
    await defaultCard.getByRole('button', { name: 'Video Gen 1' }).hover();
    hoverCard = page.locator('[data-slot="hover-card-content"]');
    await expect(hoverCard.getByText('$0.08/seconds')).toBeVisible({ timeout: 15_000 });
    ...
    await defaultCard.getByRole('button', { name: 'Text Embedding 3 Large' }).hover();
    hoverCard = page.locator('[data-slot="hover-card-content"]');
  ...
  test('reloading the models page refetches model settings data', async ({ page }) => {
  ...
  test('adds a provider after base URL and key validation, then manages keys and model filtering', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Edit provider' }).nth(1).click({ force: true });
    await expect(page.getByRole('dialog').locator('.glass-foreground').filter({ hasText: 'key 1' }).first()).toHaveText(
      /key 1\s*••••\s*0 requests\s*Test/
    );
    ...
    await expect(
      page.getByRole('dialog').locator('.glass-foreground').filter({ hasText: 'backup' }).first()
    ).toHaveText(/backup\s*••••\s*0 requests\s*Test/);
  ...
  test('keeps provider add dialog state safe across back, cancel, failed test, and retry', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Next', exact: true }).click();
    await expect(page.getByPlaceholder('https://…')).toHaveValue('https://gateway.example/v1');
  ...
  test('tests and deletes existing provider credentials without removing provider configuration', async ({ page }) => {
    ...
    await page.getByRole('button', { name: 'Edit provider' }).first().click({ force: true });
    const dialog = page.getByRole('dialog');
    await dialog.getByRole('button', { name: 'Test' }).first().click();
    await expect(dialog.getByText('ok 42ms')).toBeVisible();

    await dialog.getByRole('button', { name: 'Delete key' }).click();
    await expect(dialog.getByText('No keys', { exact: true })).toBeVisible();
    await dialog.getByRole('button', { name: 'Key', exact: true }).click();
    await dialog.getByRole('button', { name: 'Test' }).first().click();
    await expect(dialog.getByText('Enter both a label and key.')).toBeVisible();
  });
  ...
  test('deletes an unused provider but keeps profile-backed providers protected', async ({ page }) => {
    ...
    await compatibleCard.getByRole('button', { name: 'Delete provider' }).click({ force: true });
  ...
  test('handles draft profile cancel, disabled save, custom model ids, and role clearing', async ({ page }) => {
    ...
    await expect(page.getByRole('button', { name: 'custom-chat-model' })).toBeVisible();
