...
async function installSkillsApiMock(
  ...
  await page.route(API_ROUTE_PATTERN, async (route) => {
    ...
    if (method === 'GET' && path === '/v1/sessions/attention') return json({ summaries: [] });
    ...
    if (method === 'POST' && path === '/v1/atoms/skills') {
      const payload = (await request.postDataJSON()) as { content?: string; name: string };
      if (payload.content?.includes('api-rejected-skill')) {
        return json({ error: 'save failed' }, 500);
      }
      return json({ id: `global:${payload.name}`, name: payload.name });
    }
...
test.describe('Studio skills settings', () => {
  test('removes a deleted global skill from the installed list', async ({ page }) => {
    ...
    await expect(page.getByText('CodeGraph Navigator', { exact: true })).toBeHidden();
  ...
  test('covers installed settings, card controls, updates, edit, remove, and add menu flows', async ({ page }) => {
    ...
    await uploadDialog.getByRole('button', { name: /Drop a file here or choose a file to upload/ }).click();
...
`);
    ...
    expect(
      requests.some(
        (request) =>
          request.method === 'POST' &&
          request.path === '/v1/atoms/skills' &&
          (request.body as { name?: string })?.name === 'editor-created-skill'
      )
    ).toBe(true);
  });
...
`);
    await newSkillDialog.getByRole('button', { name: 'Save' }).click();
    await expect(page.getByText('Cannot save the Skill. Check its content and try again.')).toBeVisible();
    await expect(newSkillDialog).toBeVisible();
...
`);
    ...
    await expect(page.getByText('Cannot upload the Skill. Check the file and try again.')).toBeVisible();
    await expect(uploadDialog).toBeVisible();
    expect(
      requests.some(
        (request) =>
  ...
  test('covers the empty installed state and its add action', async ({ page }) => {
    ...
    await page.getByRole('menuitem', { name: 'GitHub source' }).click();
