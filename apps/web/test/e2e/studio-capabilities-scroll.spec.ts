...
async function installCapabilitiesApiMock(page: Page) {
  await page.route(API_ROUTE_PATTERN, async (route) => {
    ...
    if (method === 'GET' && path === '/v1/workplace/projects') {
      return json({ projects: [], total: 0, limit: 50, offset: 0, orderRevision: 0 });
    }
    ...
    if (method === 'GET' && path === '/v1/settings/tool-backends') {
      return json({
        webSearch: { provider: 'auto', braveApiKey: { configured: false } },
        email: { backend: 'auto', resendApiKey: { configured: false } },
        codeExec: {
          backend: 'follow-system',
          e2bApiKey: { configured: false },
          dockerImage: null,
          availableBackends: ['follow-system', 'docker', 'e2b']
        }
      });
    }
    ...
    if (method === 'GET' && path === '/v1/settings/import/inventory') {
      return json({
        roots: [
          {
            source: 'shared',
