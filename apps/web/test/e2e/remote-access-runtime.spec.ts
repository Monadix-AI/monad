...
async function installRemoteAccessRuntimeMock(page: Page, options?: { abortNetworkPut?: boolean }) {
  ...
  await page.route(API_ROUTE_PATTERN, async (route) => {
    ...
    if (method === 'GET' && path === '/v1/sessions/attention') return route.fulfill(json({ summaries: [] }));
...
test('connection settings requires explicit confirmation before remote HTTP', async ({ page }) => {
  ...
  await page
    .getByRole('alertdialog', { name: 'Switch remote access to HTTP?' })
  ...
  await page
    .getByRole('alertdialog', { name: 'Switch remote access to HTTP?' })
    .getByRole('button', { name: 'Switch to HTTP' })
    .click();
