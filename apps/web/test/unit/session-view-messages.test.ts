...
test('server user echoes inherit matching local image previews without duplicating the message', () => {
  const server = message('msg_server_image', 'user', 'Use this image');
  server.parts.push({
    data: {
      bytes: 3,
  ...
  expect(items.map((item) => item.id)).toEqual(['msg_server_image']);
});
...
test('server assistant activity replaces the new-session pending agent shimmer', () => {
  const optimistic = [
    {
  ...
  const pending = buildViewMessages({
    commandPending: null,
    optimistic,
    transcriptMode: 'live',
    visibleHistory: [],
  ...
  const active = buildViewMessages({
    commandPending: null,
    optimistic,
    transcriptMode: 'live',
    visibleHistory: [],
    visibleLiveItems: [streamingMessage('a1', 'assistant', 'partial')]
  });

  expect(pending.map((item) => item.id)).toEqual(['local-home-turn-1:assistant']);
  expect(active.map((item) => item.id)).toEqual(['a1']);
});
