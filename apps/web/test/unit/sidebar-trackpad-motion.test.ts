...
test('sidebar paging is fully pager-driven with a JS edge rubber band', () => {
  ...
  const pagerSource = readFileSync(join(import.meta.dir, '../../features/shell/sidebar-trackpad-switch.ts'), 'utf8');
  ...
  expect(source).toContain('routeDrivenScrollRef.current = true');
  expect(source).toContain('if (routeDrivenScrollRef.current) return');
  ...
  expect(pagerSource).toContain('const TRACKPAD_PAGE_TURN_THRESHOLD_RATIO = 0.4');
  expect(pagerSource).toContain('const threshold = width * TRACKPAD_PAGE_TURN_THRESHOLD_RATIO');
