...
describe('canonical web route helpers', () => {
  test('gives every Inbox tab a canonical route with All last', () => {
    const paths = INBOX_FILTERS.map((filter) => inboxPath(filter));

    expect([...INBOX_FILTERS]).toEqual(['needs-response', 'unread', 'completed', 'all']);
    expect(paths).toEqual(['/inbox/needs-response', '/inbox/unread', '/inbox/completed', '/inbox/all']);
    expect(paths.map((path) => inboxFilterFromPathname(path))).toEqual([...INBOX_FILTERS]);
    expect(DEFAULT_INBOX_FILTER).toBe('needs-response');
    expect(inboxPath()).toBe('/inbox/needs-response');
    expect(isInboxPath('/inbox/completed')).toBe(true);
    expect(isInboxPath('/workspace/prj_ABCDEF123456')).toBe(false);
  });

  test('keeps Studio sections and their internal breadcrumbs under /studio', () => {
    expect(studioPath()).toBe('/studio/mesh');
    ...
    expect(studioSectionFromPathname('/studio/workplaceProjects')).toBeNull();
