...
test('a message from today shows only its time', () => {
  expect(formatMessageTimestamp('2026-08-07T09:05:00', 'en', NOW)).toBe(
...
test('a message from yesterday is labeled with the yesterday word', () => {
  ...
  expect(formatMessageTimestamp('2026-08-06T23:59:00', 'en', NOW)).toBe(`yesterday ${time}`);
});

test('relative day labels follow the requested locale', () => {
  const time = new Date('2026-08-06T23:59:00').toLocaleTimeString('zh', { hour: '2-digit', minute: '2-digit' });
  expect(formatMessageTimestamp('2026-08-06T23:59:00', 'zh', NOW)).toBe(`昨天 ${time}`);
});
...
test('an older same-year message carries month and day without the year', () => {
  ...
  expect(formatMessageTimestamp('2026-03-02T08:30:00', 'en', NOW)).toBe(`${date} ${time}`);
});
...
test('a previous-year message carries the year', () => {
  ...
  expect(formatMessageTimestamp('2025-12-31T10:00:00', 'en', NOW)).toBe(`${date} ${time}`);
});
...
test('a start-of-month boundary still resolves yesterday across months', () => {
  ...
  expect(formatMessageTimestamp('2026-07-31T22:00:00', 'en', now)).toBe(`yesterday ${time}`);
});

test('missing or unparseable timestamps yield null', () => {
  expect(formatMessageTimestamp(undefined, 'en', NOW)).toBeNull();
  expect(formatMessageTimestamp('not-a-date', 'en', NOW)).toBeNull();
});
...
test('repeated calls for the same locale and field shape reuse one Intl.DateTimeFormat instance', () => {
  ...
  try {
    for (let i = 0; i < 5; i++) {
      formatMessageTimestamp('2026-03-02T08:30:00', 'en', NOW);
    }
  } finally {
