---
title: "Setting Up Model Providers"
...
[the internals doc](/internals/infra/model-providers) for how it is defined. Every setup
surface below reads the same catalog from the daemon, so they always offer the same
providers.

## Three ways to set up

### 1. Guided terminal setup

```sh
monad init
```

The interactive wizard lists the provider catalog, prompts for the base URL when the
provider needs one, prompts for any provider-specific fields (for example the AWS region
for Amazon Bedrock), asks for the API key, tests the connection before saving anything,
and then lets you pick a default model from the provider's model list. If the test fails
you can retry with a different key or go back and pick another provider.
...
Run `monad` on a fresh install and the browser opens the same setup flow. Afterwards,
**Studio → Models and providers** (`/studio/models`) adds providers, manages each
provider's credentials, browses its models, and edits model profiles. See
...
- Providers, their credentials, and profiles live in `agents.json` under Monad's
  config directory. Provider credentials are native settings and do not use Agent
  Runtime Credentials or `${secret:...}` references.
...
- The user is responsible for protecting the native settings file. The daemon and
  settings APIs continue to redact secrets from responses and logs.
...
[the internals doc](/internals/infra/model-providers).

## Custom providers

A provider that is not in the catalog can still be added as a third-party atom pack:
drop a module that exports a `ModelProvider` into `~/.monad/atoms/providers/` and the daemon
picks it up without a restart. See
[model-providers.md](/internals/infra/model-providers) for the contract.
...
[CONTRIBUTING.md](https://github.com/Monadix-AI/monad/blob/main/CONTRIBUTING.md).
