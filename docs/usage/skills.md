---
title: "Skills"
description: "Install, author, discover, and govern portable Agent Skills in Monad."
keywords: ["Agent Skills", "SKILL.md", "agentskills.io", "skill installation", "skill discovery"]
---
...
1. **Personal** — `~/.monad/atoms/skills/` (applies everywhere)
2. **Workspace** — `~/.monad/agents/default/skills/` (travels with the workspace)
...
bundled skills alongside these scopes. See [atoms](/internals/infra/atoms).
...
| `modelTier` | no | `fast`/`smart`/`power` — capability tier the forked subagent runs at (only with `context: fork`); the routing layer picks a concrete model — see below |
...
Matched against `~/.monad/agents/default` (Monad's working area), evaluated at load and on every hot
...
```bash
monad skill list [--scope <runtime|global|atom-pack|agent>]   # what is installed, and from where
monad skill search <query>                                    # search the skill registry
monad skill install <path | owner/repo | github:owner/repo | git-url | registry-name>
monad skill update <name>
monad skill enable <name>
monad skill disable <name> [--autoload-only]
monad skill remove <name>                                     # personal-scope skills
monad skill new <name>                                        # scaffold a SKILL.md from a template
monad skill validate <path>                                   # parse-check offline, no daemon needed
```
...
```sh
monad skill new my-skill      # scaffold from the template
...
description: Research a topic thoroughly. Use for multi-step investigation.
context: fork
modelTier: power      # fast | smart | power
---
Research $ARGUMENTS thoroughly and report findings with sources.
```
...
Monad infrastructure: per-skill `model`/`effort` overrides (Monad uses `modelTier` instead) and
...
same trust boundary as a provider atom (see the [runtime security model](/internals/infra/runtime#security-model)).
