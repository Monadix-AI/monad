...
Both are **off by default**. Both flow through the same MCP client, approval gate, and tool-set pinning as any other MCP server. See [MCP servers](/usage/mcp) and the [runtime security model](/internals/infra/runtime#security-model).

Both presets are settings in `agents.json`, shown below. In the browser, **Studio →
...
megabyte of base64 never lands in the persisted or text result. This is what makes *both*
tracks usable.
...
- **Mutating** tools (click/type/drag/key/scroll) are tagged with the `host-control` gate key
  (`trust.hostEscape: true` on the server).
...
- A host-control allow **can never persist** beyond session scope — no permanent global/agent
  "always allow" — the daemon refuses to persist that grant. A **deny** at any
