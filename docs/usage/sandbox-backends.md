...
The available field types are `string`, `number`, `boolean`, `select`, and `secret`. Studio renders this schema generically. A launcher may also use Host Interaction for a setup flow that needs an immediate user response; see [host-interactions.md](/internals/infra/host-interactions).
...
- plaintext is stored directly in the owning sandbox backend setting;
...
## VM guest and host binaries

The VM backend needs four helper binaries of its own — the guest vsock exec agent, the seccomp
observer, gvforwarder, and the Windows `winvm-helper`. They are not shipped inside the release:
...
themselves — see
[`packages/sandbox-vm`](https://github.com/Monadix-AI/monad/tree/main/packages/sandbox-vm).
