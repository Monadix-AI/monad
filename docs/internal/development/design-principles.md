...
description: "Higher-altitude rules for how features are designed in Monad, before any code is written. Two principles govern every feature: it must behave the same on."
---
...
Concrete, code-level rules live in the focused docs ([security guidelines](security-guidelines.md),
[code conventions](conventions.md), [CLI design](cli-design.md)); this document is the design intent
those rules serve.
...
- **Verify on every platform.** Per `AGENTS.md`, exercise each
...
[security guidelines](security-guidelines.md). Read it before building anything
