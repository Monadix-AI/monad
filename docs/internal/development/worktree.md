...
description: "Develop Monad safely in isolated Git worktrees with deterministic ports, setup, verification, merge, and cleanup."
---
...
The repo pins Bun in `package.json` under `"packageManager"`. `mise.toml` tells mise
to read that version, load the current worktree's `.env.local`, and add `.dev/bin`
to `PATH`. Entering the worktree prints the active Bun version.

Full setup instructions with bash, fish, and PowerShell variants:
[CONTRIBUTING.md § Install and activate mise](https://github.com/Monadix-AI/monad/blob/main/CONTRIBUTING.md#install-and-activate-mise).

Short version for zsh. Copy and paste this block once:
...
Run `mise trust && mise install` once in each clone. To upgrade Bun, bump
...
```sh
bun scripts/bun-test.ts packages/protocol/ --only-failures
...
```sh
mise run quality:check  # complete commit-time gate, no tracked-file changes
...
|---|---|---|
| Security | [Security guidelines](security-guidelines.md) | Injections, credential leaks, socket/CSRF exposure, untrusted input paths |
| Performance | [Performance guidelines](performance-guidelines.md) | Latency budget, memory, bundle size, hot-path allocations |
| Code style & typing | [Code conventions](conventions.md) | Single source of truth for types, no re-declarations, comment policy |
| Functional architecture | [Repository architecture](../../engineering/architecture.md) | Package boundaries, allowed dependency directions, no cross-layer leaks |
| UI design | [UI guidelines](design/ui-guidelines.md) | Token usage, component patterns, accessibility, dark-mode |
| UX design | [UX guidelines](design/ux-guidelines.md) | Interaction model, copy tone, loading/error/empty states |
...
| UX writing | [UX writing guidelines](design/ux-writing-guidelines.md) | Terminology, tone, capitalization, consequences, recovery steps, and destructive-action clarity |
...
3. Turbo generates stale inputs first, then runs `@monad/monad`, `@monad/web`,
   Storybook, and devtools as persistent tasks.
4. `mise activate` loads `.env.local` and `.dev/bin` from the current worktree, then
   restores the previous environment when you leave it.
...
The optional Phoenix observability service is shared explicitly with
`mise run dev:services`; dependency installation never starts it.
...
- **To recover from an active port collision**, stop this worktree's running services,
  run `mise run dev:ports -- --rotate`, then start `bun run dev` again. Rotation scans for
  the next complete seven-port set and atomically updates `.env.local`; mise loads it
...
See [runtime architecture](../../internals/infra/runtime.md) for the full transport and binding model.
