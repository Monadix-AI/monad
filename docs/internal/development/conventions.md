...
description: "Code-level and typing habits that apply everywhere in this repo. Rationale lives here; the short version is in AGENTS.md."
---
...
| DB rows | `apps/monad/src/store` | drizzle `$inferSelect` rows, mapped **back to** protocol types |
...
- Reference implementations: `packages/protocol/src/domain.ts`,
  `packages/environment/src/config/config.ts`.
...
- **"`apps/web` should use framework server routes"** — false. The documented
  [web router](https://github.com/Monadix-AI/monad/blob/main/apps/web/docs/router.md) is a Vite-built TanStack Router SPA. Daemon
