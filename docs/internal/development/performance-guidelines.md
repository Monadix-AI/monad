...
description: "Performance is a feature, but it is the one feature you must measure before you change. The rules below split along the two runtimes in this repo:"
---
...
- **Backend** — the Bun daemon (`apps/monad`, including its `src/store` SQLite/KV layer
  and `src/agent` loop) and its packages (`@monad/protocol`, `@monad/environment`,
  `@monad/sandbox`, …). Long-lived process,
  serving REST + a multiplexed WebSocket over TCP/UDS. See [runtime](../../internals/infra/runtime.md),
  [daemon architecture](../../internals/infra/daemon-architecture.md), and
  [repository architecture](../../engineering/architecture.md).
- **Frontend** — the web client (`apps/web`): Vite, TanStack Router, React 19,
  Redux, Tailwind 4. See [UI guidelines](design/ui-guidelines.md) and
  [apps/web/docs/router.md](https://github.com/Monadix-AI/monad/blob/main/apps/web/docs/router.md).
...
> a budget (see [code conventions](conventions.md)).
...
- **`parse` at the edge, not in the loop.** Zod validation belongs at the
  HTTP/WS/disk boundary (per [code conventions](conventions.md)). Re-parsing
  the same payload deeper in the call stack is wasted CPU on every request.
- **SQLite: prepare once, reuse.** `apps/monad/src/store` (rows) and `store/kv` run on `bun:sqlite`.
...
### CLI cold start ≈ 165 ms, dominated by `@monad/protocol` module init

Measured on macOS/arm64 with `bun apps/cli/src/main.ts version` and a per-import breakdown:

| Import | Cost |
|---|---|
| `@monad/protocol` | 81 ms |
| `apps/cli/src/commands/index.ts` (all 40 command modules, deps warm) | 49 ms |
| `@monad/environment` | 22 ms |
| `initCliI18n()` (config read + locale pack scan) | 13 ms |
| `@monad/client`, `cac` | 12 ms |

`@monad/protocol` is the dominant cost and is paid by every invocation: it builds the whole zod
schema graph (method table, event table, HTTP contract) at module init, and both `main.ts` and every
command need it. That is a protocol-package concern, not a CLI one — do not chase it from the CLI.

**Accepted, with one exception carved out.** Lazy-loading the command modules would recover at most
the 49 ms row, and only by splitting each command's metadata (name, aliases, synopsis, group,
subcommands) out of the module that implements it — a single-source-of-truth violation that rots the
moment someone adds a verb. Not worth 49 ms. The one place the latency is actually felt is shell
completion, where `monad __complete` runs on every TAB press; that path skips `initCliI18n()`
(see `main.ts`) because it emits raw ids and no human-readable text.

Re-open this if `@monad/protocol`'s init cost is reduced at the source, or if a measurement shows
completion latency is still the thing users notice.
