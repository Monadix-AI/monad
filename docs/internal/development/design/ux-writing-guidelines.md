...
description: "Write clear Monad interface text that explains system state, supports decisions, and helps people recover from errors."
---
...
The rules below follow common platform writing conventions, adapted to Monad's
surfaces. They are the product-specific source of truth for all UI copy.
...
Two styles, split by surface:

| Surface | Style |
| --- | --- |
| Documentation titles: page `title`, `sidebarTitle`, and documentation navigation labels | Title Case |
| Everything else — product UI, CLI, TUI, daemon, channel, and notification text, including documentation body copy and section headings | Sentence case |

### Sentence case (product surfaces)
...
- Menu items
- Tabs
- Navigation items inside the product
- Dialog titles
- Form labels
- Field placeholders
- Tooltips
...
- Table headers
- Status labels
- CLI, TUI, and daemon headings, prompts, and console output
- Documentation body text and section headings
...
| `Run tests` | `Run Tests` |
| daemon log: `Waiting for the daemon` | `Waiting For The Daemon` |

### Title Case (documentation titles)

Capitalize the first word, the last word, and every other word except articles
(`a`, `an`, `the`), coordinating conjunctions (`and`, `but`, `or`, `nor`, `for`,
`so`, `yet`), and prepositions of four letters or fewer (`in`, `on`, `at`, `to`,
`of`, `by`, `from`, `with`, `into`, `over`).

This applies only to a documentation page's `title` and `sidebarTitle` frontmatter
and to the group and tab labels in `docs/docs.json`. Section headings inside a page
stay sentence case.
...
| Use | Do not use |
| --- | --- |
| `Sessions, Agents, and Approvals` | `Sessions, agents, and approvals` |
| `Install or Remove Monad` | `Install Or Remove Monad` |
| `Choose an Agent Runtime for a Mesh Member` | `Choose An Agent Runtime For A Mesh Member` |
| `Get Started with Monad` | `Get Started With Monad` |

Exceptions (both styles):
...
starts in another locale. See [UX guidelines](ux-guidelines.md) for what must
...
- All interaction elements, CLI, TUI, and daemon text use sentence case; documentation page titles, sidebar titles, and navigation labels use Title Case.
