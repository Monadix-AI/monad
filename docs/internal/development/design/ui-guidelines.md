...
description: "Build consistent Monad web surfaces with the shared component library, design tokens, responsive layout, and accessibility rules."
---
...
specs — lives in the public [design system](../../../design/design-system.md); treat that file as the single
source of truth for tokens and this file as the rules for applying them. Interaction and
state conventions are in [UX guidelines](ux-guidelines.md); all user-facing copy
follows [UX writing guidelines](ux-writing-guidelines.md).
...
- **Use tokens, never one-off values.** Components use the raw RGB tokens
  ...
  when matching the base primitives, and the semantic aliases (`--background`, `--card`,
...
- Icon-only buttons need an accessible name and normally a tooltip carrying the same
  ...
  [UX writing guidelines](ux-writing-guidelines.md).

## Accessibility

WCAG 2.1 AA is the minimum. The copy side (accessible names, alt text, live status
text) is specified in [UX writing guidelines](ux-writing-guidelines.md); this is the
...
- Exact easing and duration tokens are in the public [design system](../../../design/design-system.md).
...
- [ ] Hover-revealed controls have a touch fallback and copy follows
      [UX writing guidelines](ux-writing-guidelines.md) — see
