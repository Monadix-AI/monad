---
title: "Parallel Agent Development"
audience: "internal-agent"
description: "This guide is for engineers or orchestrator agents driving multiple coding agents simultaneously toward a shared feature set. For single-agent worktree."
---
...
[worktree development](../development/worktree.md); this document focuses on the extra problems parallelism
...
```text
Task A: apps/monad/src/store/db/schema.ts, apps/monad/drizzle/
Task B: packages/protocol/src/domain.ts, packages/protocol/src/rpc/method-table.ts
Task C: apps/cli/src/commands/session/
```
...
It never conflicts with other worktrees. See [worktree development Part 2](../development/worktree.md#part-2--environment-reference).
...
3. Interface contracts with other agents (e.g. "do not modify the `Session*` types in
   `packages/protocol/src/domain.ts` — those are owned by agent-b").
...
[worktree development §5](../development/worktree.md#5-squash-and-merge-into-main). Even when multiple agents
