...
description: "Ideas discussed and worth remembering, but with no current commitment to implement. Each entry records the reasoning behind the idea and why it was deferred."
tag: "Draft"
noindex: "true"
---
...
- Core changes: `packages/environment/src/config/` (load/save functions) and `packages/environment/src/paths.ts` (filename constants)
...
- `packages/protocol/src/rpc/method-table.ts` is the single schema source of truth from which
...
- Memory now has multiple write paths and lifecycles (built-in L1, mem0 observation,
  L2/L3 consolidation, session-scope cleanup). Incognito semantics must cover all of
  them consistently rather than guard only the `memory` tool.
...
- The product chooses whether incognito means write suppression only or zero recall
  and zero writes.

**Minimum-cost path if revisited:** add an `incognito` mode to the session model and
store row; add a built-in `/incognito` command; guard `memoryTool` / `observeTurn` /
`recallContext` and consolidation; add a session badge in web/TUI. Estimated 1–2 days.
...
from parallel read-only fan-out. The route is validated by prior art in the wider agent
ecosystem: orchestrators that embed complexity rules in the system prompt (1 agent for
lookups, 2–4 for comparisons, more for broad research) report large gains, and explicit
complexity routers work best when the routing decision stays observable rather than
hidden. The known failure modes shape the design defensively — keep multi-agent to
...
- Monad already has all the primitives (`AgentLoop`, `runSubagent()` in
  `apps/monad/src/capabilities/tools/registry/delegate.ts`, both delegate tools, per-role model resolution), so
...
**Minimum-cost path if revisited:** P0 (MVP): add `orchestration` to
`monadProfileSchema` + a `router` model role
(`packages/environment/src/config/agents.ts`, `packages/protocol/src/domain.ts`); new
