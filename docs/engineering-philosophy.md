...
**In documentation:** `AGENTS.md` is the single source of truth for agent instructions. `docs/conventions.md` owns code style rules. Topic-specific docs own their domain. When guidance is needed in multiple places, one file is the authority and the others reference it — they do not restate it.
