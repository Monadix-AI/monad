...
- Every new or modified test case must prove observable behavior: returned contracts, state transitions, emitted events, transport responses, user interactions, side effects, or errors. Do not use `toBeDefined()`, `toBeTruthy()`, `toBeFalsy()`, or equivalent presence checks to prove that a logic branch, entity, registry entry, DOM node, or mock exists; exercise it and assert its exact result instead.
