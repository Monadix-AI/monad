...
description: "This document records the codebase boundaries that should stay stable as Monad grows. For the daemon startup graph and runtime lifecycle, see."
---
...
[daemon-architecture.md](/internals/infra/daemon-architecture). For transport and security
posture, see [runtime.md](/internals/infra/runtime).
...
Arrows point at what a layer is allowed to depend on. Anything not drawn is not allowed.

```mermaid
flowchart TB
  subgraph Apps["Executable apps"]
    CLIA["apps/cli"]
    WebA["apps/web"]
    TUIA["apps/tui"]
    Daemon["apps/monad (daemon)"]
  end

  subgraph Client["Client layers"]
    RTK["@monad/client-rtk"]
    Cl["@monad/client"]
  end

  subgraph UI["Presentation"]
    UIP["@monad/ui"]
    XP["@monad/sdk-experience"]
  end

  subgraph Base["Contracts and host"]
    Proto["@monad/protocol"]
    Env["@monad/environment"]
    SDK["@monad/sdk-atom"]
  end
...
- `services/log-maintenance/lifecycle.ts` owns persistent-log cleanup scheduling and shutdown.
...
- **`@monad/sdk-experience` (root)** — the React-free **contract**: the published
  ...
  (`WORKPLACE_EXPERIENCE_API_VERSION`, `defineWorkplaceExperience`,
  `isWorkplaceExperienceApiCompatible`, `bindWorkplaceExperience`, the DOM event bridge).
...
- **`@monad/sdk-experience/react`** — a curated re-export subset of `@monad/client-rtk`'s RTK
  ...
  host app's Redux `<Provider>` (see `apps/web/src/lib/monad-store.ts` / `monad-runtime-provider.tsx`).
