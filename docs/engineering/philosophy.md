...
description: "> AI changes the bottleneck of software engineering from code production to code verification. A good engineering system should therefore make correct code."
---
...
**In documentation:** `AGENTS.md` is the single source of truth for agent instructions. `docs/internal/development/conventions.md` owns code style rules. Topic-specific docs own their domain. When guidance is needed in multiple places, one file is the authority and the others reference it — they do not restate it.
