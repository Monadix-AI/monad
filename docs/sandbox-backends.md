...
The available field types are `string`, `number`, `boolean`, `select`, and `secret`. Studio renders this schema generically. A launcher may also use Host Interaction for a setup flow that needs an immediate user response; see [host-interactions.md](host-interactions.md).
