---
title: "Peer Federation — Daemon-to-Daemon Task Delegation"
description: "One Monad daemon can delegate a self-contained task to a peer Monad daemon's agent (compute federation). The caller's agent invokes the agentpeerdelegate."
---
...
This is the inverse of [ACP delegation](/internals/agent-team-runtime/acp): there Monad drives an external ACP agent over stdio
...
see [runtime.md](/internals/infra/runtime)) as the transport. The delegating daemon (A) is just another OpenAI
client pointed at the peer (B); B drives a normal agent session and streams tokens back.

```mermaid
sequenceDiagram
  autonumber
  participant LA as Daemon A · agent loop
  participant TL as agent_peer_delegate (high-risk)
  participant B as Daemon B · /openai/v1/chat/completions
  participant SB as Daemon B · agent loop

  LA->>TL: delegate(peerName, instruction)
  Note over TL: the model supplies a peer NAME,<br/>never a URL or token
  TL->>B: HTTP + SSE, bearer token from mesh.json
  B->>B: inbound approval gate
  B->>SB: session.create + sendInline (surface: api)
  SB->>SB: runs on B's OWN tools, files, credentials
  SB-->>B: message deltas … completed
  B-->>TL: streamed answer
  TL-->>LA: tool result
```
...
- `apps/monad/src/services/delegation/peer-delegate.ts` — the `agent_peer_delegate` tool (high-risk; streams
...
- `packages/environment/src/config/mesh.ts` — peers and their direct credentials in
  `mesh.json`.
- `apps/monad/src/handlers/settings/peer/` + `transports/http/settings/peer.ts` — settings
...
A peer is infra/security config (a delegation target + its credential), so it lives in
`mesh.json`, alongside ACP agents and managed native agents. The daemon watches that file, so an
...
"peers": [
  {
    ...
    "credential": { "token": "peer-token-value" },
    "enabled": true
  }
]
```
...
monad peer token <id> <token>                              # store token beside the peer + enable
monad peer list                                            # configured peers (no secrets)
monad peer enable|disable <id>
monad peer remove <id>                                     # removes the peer and its credential
```
...
- **Native secret ownership** — the token lives in `mesh.json`; settings responses
  never echo it back, and the user is responsible for the file's security.
...
See the [runtime security model](/internals/infra/runtime#security-model) for the general agent-containment boundary.
...
- `apps/monad/test/e2e/peer-settings.test.ts` — settings CRUD over a real temporary
  home (secret stays in `mesh.json` and is redacted from responses).
...
- `packages/environment/test/unit/peer-secret.test.ts` — direct peer credential
  validation and removed-reference rejection.
