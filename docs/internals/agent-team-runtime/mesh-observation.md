---
title: "Third-Party Agent Observation"
description: "Understand the durable raw-event and convenience-projection contract for observing provider-native third-party agent activity."
---
Status: **accepted**

Third-party agent observation has two views over provider activity. The raw view preserves
...
agent, are observed through the session's own UI projection instead — history plus a
standing stream of neutral `SessionUiEvent` frames:

```text
GET /v1/sessions/:id/ui-items
GET /v1/sessions/:id/ui-stream
```

There is no member-scoped observation endpoint; the projection is session-scoped and each
frame carries its own producer attribution. See
[realtime-channels.md](/internals/infra/realtime-channels) for the stream's resume semantics.
