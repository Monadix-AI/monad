---
title: "ACP — Editor Integration (Agent Client Protocol)"
description: "Use Monad as an ACP agent for editors or as an ACP client for delegated agent work."
keywords: ["ACP", "Agent Client Protocol", "editor integration", "agent delegation"]
---
Monad speaks [ACP](https://agentclientprotocol.com) **both ways**:

- **As an agent** — editors (Zed, and others) drive Monad as their built-in agent: streamed
...
Built on the official SDK `@agentclientprotocol/sdk@^1.3.0`.
...
Key files (`apps/monad/src/transports/acp/`): `agent.ts` (the `MonadAcpAgent` adapter),
`connection.ts` (`AgentSideConnection` wiring + the `AcpHandlers` port), `bridge.ts` (the
`AcpHandlers` RPC proxy to the daemon), `launch.ts` (discover/auto-spawn + bridge),
`documents.ts` (open-document tracking), `translate.ts` (pure Event↔ACP), `backends.ts`
(editor-facing fs/terminal backends). The daemon side:
`services/delegation/delegation.ts` (reverse fs/terminal) and the per-session runtime
registry behind `sessions.configureRuntime` (`handlers/session/`).
...
self-contained subtask, returning its answer. Register agents in `mesh.json`:
...
turn's stream. `env` is operator-vetted infrastructure config → it lives in `mesh.json`, alongside
the third-party agent and peer definitions.
...
`apps/monad/test/e2e/acp-stdio.test.ts` (with its `acp-stdio.helper.ts`) spawns a real
`monad --acp --mock-model` and drives it with the SDK client — initialize → new → prompt → fork.
Fastest inner loop:
`bun scripts/bun-test.ts apps/monad/test/e2e/acp-{transport,bridge,delegate,stdio}.test.ts --only-failures`.
