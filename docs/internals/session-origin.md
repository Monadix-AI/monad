...
- `writableBy` — which transports may **send into** the session. Enforced by
  `assertWriteAllowed` in [messaging/index.ts](../../apps/monad/src/handlers/session/handlers/messaging/index.ts)
  on `send`/`generate` (`http`) and `sendInline` (`runOpts.transport`).
- `branchableBy` — which transports may **fork** the session. Enforced by
  `assertBranchAllowed` in [lifecycle/index.ts](../../apps/monad/src/handlers/session/handlers/lifecycle/index.ts),
...
`DEFAULT_BRANCHABLE_BY` in [origin.ts](../../apps/monad/src/handlers/session/origin.ts)) **then
...
[security-guidelines.md §7](../engineering/security-guidelines.md): never fed to the model, never logged,
...
Tests: [session-write-policy.test.ts](../../apps/monad/test/unit/sessions/session-write-policy.test.ts).
