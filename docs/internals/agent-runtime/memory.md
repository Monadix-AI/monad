---
title: "Memory"
description: "Understand Monad's shipped memory layers, retrieval, persistence, knowledge graph, laws, and backend boundaries."
---
How the shipped memory subsystem works. This document and the code are the source of
truth; the rejected alternatives are recorded in the deliberate-divergences section.
...
Shipped in `apps/monad/src/services/memory/graph/`. The storage, extraction,
reconciliation, and query contracts below are authoritative.
...
- `/consolidate [level]` (or `/memory consolidate`) — manual, with optional depth
  override.
- **Background catch-up** — opt-in per Agent via
  `agent.agents[].memory.autoConsolidate`; each Agent has an independent
  `intervalMinutes` schedule (default 30). A 60s tick runs the pipeline at that
  Agent's effective memory level when due. Runs are isolated per Agent, and
  hot-reloaded settings apply without recreating the timer.
...
- **Control API + web:** memory status (backend, mem0 models, qdrant phase),
  per-Agent memory and consolidation settings, per-scope fact
  browse/add/edit/forget, core-file editor, laws
...
- **Config:** `memory.backend`, `memory.decay.*`, and `memory.mem0.*` are global;
  `agent.agents[].memory.*` controls each Agent's memory behavior. All are
  hot-reloaded.

## Deliberate design choices

Kept here so superseded alternatives are not re-litigated piecemeal:
