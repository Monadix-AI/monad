---
title: "Tools"
description: "Understand Monad's built-in Agent tools, registration contract, risk tiers, approvals, and execution boundaries."
---
...
apps/monad/src/capabilities/tools/
  ...
  configure-backends.ts  resolves the configured backends from settings
  approval/              path-escalation gating, approval policy, resource approvals
  registry/              the tool implementations (one file or folder per tool)
    ...
    fs.ts shell.ts …
    prompts/             prompt templates owned by a tool
...
The call-time guards themselves (`assertPathWithinRoots`, `assertUrlAllowed`, `isBlockedIp`,
...
- **Imports:** the `Tool` type comes from `@/capabilities/tools/types.ts`; the only SDK import a tool needs is
  ...
  `@monad/{monad,environment,client}`.

Repository contributors should also follow the [security rules](https://github.com/Monadix-AI/monad/blob/main/docs/internal/development/security-guidelines.md) and [performance rules](https://github.com/Monadix-AI/monad/blob/main/docs/internal/development/performance-guidelines.md). Related public documentation covers [skills](/usage/skills) and [hooks](/internals/infra/hooks).
