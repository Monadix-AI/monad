---
title: "Runtime: Transport, Configuration & Security Model"
description: "Understand the daemon's network binding, physical transports, configuration, remote access, and resulting security model."
---
...
reload, and extension boundaries — is [daemon-architecture.md](/internals/infra/daemon-architecture)'s
territory. These are the deep technical details kept out of the
[README](/index). Repository contributors should also follow the [code-level security rules](https://github.com/Monadix-AI/monad/blob/main/docs/internal/development/security-guidelines.md).
...
[daemon-architecture.md](/internals/infra/daemon-architecture).
...
>   in `config.json`.
> - **Semantic transport** (see
>   [operation-source.md](/internals/agent-runtime/operation-source)) — *which ingress class
>   produced an operation*: `http` (local control transports — CLI, TUI, web UI),
>   `acp` (editor agents), or `channel` (chat tools). This is the server-stamped
>   `SessionTransport` field in immutable `OperationSource` provenance. Both `tcp`
>   and `uds` connections are classified as `http` in this sense. A narrow temporary
>   containment gate also compares this field on selected session mutations; it is
>   not a client-configurable access policy.
>
...
flowchart LR
  subgraph Local["Local clients"]
    ...
    Editor["Editor (ACP bridge)"]
  end

  subgraph Daemon["Daemon listeners"]
    T["TCP 127.0.0.1:port<br/>REST · SSE · WebSocket"]
    U["Unix socket ~/.monad/runtime/monad.sock (0600)<br/>REST · SSE · JSON-RPC"]
  end

  CLI -->|"network.transport = uds (default)"| U
  CLI -.->|"fallback when the socket can't be dialled"| T
  Browser -->|"TCP only — a page cannot reach AF_UNIX"| T
  Editor --> U
  R["Remote clients"] -.->|"only when network.remoteAccess.enabled<br/>bearer token required"| T
```

The daemon always serves its HTTP API over **two** local channels at once: TCP
loopback (`127.0.0.1:<port>`) and a Unix-domain socket (`~/.monad/runtime/monad.sock`).
...
> adapter, not in feature code. This is the canonical example of the repository's [thin platform adapter rule](https://github.com/Monadix-AI/monad/blob/main/docs/internal/development/design-principles.md).
...
interface only** (`127.0.0.1`) plus the Unix socket under `~/.monad/runtime/` — neither
...
- **Agent Runtime Credentials** live in `auth.json`, written `0600` (owner-only), and
  are available only through protected agent execution. Native feature credentials
  live directly beside their owning settings; see
  [Agent Runtime Credentials](/usage/agent-runtime-credentials).
- **No-port mode.** `monad --stdio` / `MONAD_STDIO=true` talks JSON-RPC over stdin/stdout with no port and no socket.

> This is an evolving posture, not a hardened one. The loopback-trust model includes malicious local web pages in its threat scope. A loopback IP check proves only that a request came from the machine, not that the caller is authorized. Hardening includes Origin and Host validation, WebSocket origin checks, locked permissions on the socket and `config.json`, and tool-argument sandboxing. Contributors must follow the [repository security rules](https://github.com/Monadix-AI/monad/blob/main/docs/internal/development/security-guidelines.md) before changing a network boundary, filesystem path, credential, or tool dispatch.
