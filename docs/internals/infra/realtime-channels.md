---
title: "Realtime Channels: Control, Message Generation, and UI Projection"
description: "Understand how Monad separates session generation SSE from control-plane WebSocket events and reconciles durable state."
---
Status: **accepted**
...
For physical TCP/Unix transport details, see [runtime.md](/internals/infra/runtime). The wire contracts live in [`@monad/protocol`](https://github.com/Monadix-AI/monad/tree/main/packages/protocol), the transport client in [`@monad/client`](https://github.com/Monadix-AI/monad/tree/main/packages/client), and shared caches in [`@monad/client-rtk`](https://github.com/Monadix-AI/monad/tree/main/packages/client-rtk).
...
Third-party agent diagnostics use their own raw and convenience observation streams. They are not chat-generation channels; see [mesh-agents.md](/usage/mesh-agents).
...
Provider-raw output never enters this stream. Third-party agent raw/convenience observation has separate authorization, event pagination, cursor, and connection-epoch semantics. See [Use third-party agents and build adapters](/usage/mesh-agents).
