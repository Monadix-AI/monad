...
description: "Monad is a local, daemon-first Agent Team Runtime. The daemon is the only long-lived process that owns team state, identity, policy, credentials,."
---
Monad is a local, daemon-first Agent Team Runtime. The daemon is the only long-lived process that owns team state, identity, policy, credentials, collaboration, extension loading, and network transports. CLI, Web, TUI, Agent Client Protocol, and channel clients enter through public daemon contracts.

Monad Mesh is the agent team runtime the daemon exposes. Monad Agent Runtime supplies its first-party model and tool loop; other agent runtimes join the same mesh through adapters while the daemon retains team authority.
...
**Scope: inside the process** includes the startup graph, lifecycle modules, hot
reload, and extension boundaries. The daemon's outside surface includes binding,
physical transports, configuration, environment variables, and the security model. It is
[runtime.md](/internals/infra/runtime)'s territory. For agent-reachable tool rules, see
[tools.md](/internals/agent-runtime/tools).
...
1. Runs preflight: resolves flags, paths, singleton/process mode, logging, and
   `MONAD_HOME`.
2. Builds the core runtime: opens the store, loads the four settings files
   (`config.json`, `agents.json`, `mesh.json`, `auth.json`), then starts lifecycle
...
5. Launches transports: HTTP REST/SSE over TCP and Unix socket, WebSocket push,
   stdio mode, ACP mode, channels, and background monitors (binding and
   fallback semantics: [runtime.md](/internals/infra/runtime)).

The user-visible result: the daemon becomes reachable, clients can send
...
flowchart TD
  ...
  Handlers --> Transports["Transports listen: REST/SSE, WebSocket, stdio, ACP"]
...
| `services.log-maintenance` | `services/log-maintenance/lifecycle.ts` | Persistent-log retention scheduling, accepted-policy application, and shutdown |
...
5. RuntimeKernel starts lifecycle layers:
   store
   agent.model / platform.sandbox / services.log-maintenance
   capabilities
   atoms
...
9. launchDaemonTransports() starts TCP, Unix socket, WebSocket, stdio/ACP, and channels.
