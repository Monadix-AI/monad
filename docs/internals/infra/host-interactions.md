...
description: "Host interactions let built-in features and atom packs request bounded user input without shipping UI code. The requester declares semantic content; Monad."
---
...
- Non-interactive CLI emits `interaction_required` with the interaction ID. A user can resume with `monad approval answer <id>`.
