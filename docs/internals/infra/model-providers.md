...
description: "Monad's gateway is a \"gateway of gateways\": every provider is one place a request can be sent. The set of providers Monad offers out of the box is defined."
---
...
The provider catalog in [`packages/atoms/src/providers/catalog.ts`](https://github.com/Monadix-AI/monad/blob/main/packages/atoms/src/providers/catalog.ts)
...
atoms `providers.test.ts`) assert the catalog covers exactly `KNOWN_PROVIDER_TYPES`, so the
enum and the catalog can't drift.

```mermaid
flowchart LR
  Turn["Agent turn<br/>needs a model for a role"] --> Router["Model router<br/>role → profile → (provider, modelId)"]
  Router --> Cat["PROVIDER_CATALOG<br/>single source of truth"]
  Cat --> Native["strategy: native<br/>dedicated AI SDK package"]
  Cat --> Compat["strategy: openai-compatible<br/>preset over the bundled adapter"]
  Native --> Out["HTTPS to the provider"]
  Compat --> Out
  Cfg["config.json / agents.json<br/>profiles + credentials"] -. hot reload .-> Router
```
...
- **`native`** — a dedicated AI SDK package, one provider module in
  [`packages/atoms/src/providers/`](https://github.com/Monadix-AI/monad/tree/main/packages/atoms/src/providers).
...
- OpenAI-compatible endpoint → add one `ModelProviderType` enum member + one
  ...
  [`apps/web/src/lib/ProviderMeta.tsx`](https://github.com/Monadix-AI/monad/blob/main/apps/web/src/lib/ProviderMeta.tsx).
...
Provider credentials are native Monad settings stored directly on their provider in
`agents.json`. They are redacted from settings responses and never use
`auth.json`, `${secret:...}`, or the Agent Runtime Credentials registry.
...
`~/.monad/atoms/providers/`. The daemon discovers and watches that directory
(`registry.discover(dir)` + `watchProviders()` in
[`apps/monad/src/services/model.ts`](https://github.com/Monadix-AI/monad/blob/main/apps/monad/src/services/model.ts)), so the atom pack
registers without a restart. The atom pack contract is documented in
[`provider.ts`](https://github.com/Monadix-AI/monad/blob/main/apps/monad/src/agent/model/provider.ts).
