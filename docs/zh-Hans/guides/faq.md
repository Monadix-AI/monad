...
四种情况下 Monad 是错的工具。它不是托管平台：如果你要把 Agent 产品交付给你的客户，你需要的是带自动扩缩和多租户的服务化平台，而不是单机守护进程。它不是编写框架：如果你要的是在自己的应用里把 Agent 逻辑写成图，那就用框架，之后需要时再通过 ACP 挂到 Monad 上。它不做跨所有者协作：对等联邦只在**你自己拥有的**守护进程之间委派，涉及不同所有者、需要独立信任与计费的协作属于 Monadix。它也不做机群调度：没有分布式调度器，也没有统一任务模式，集群规模的负载不在范围内。

还有两条限制值得先知道。Monad Mesh 处于 alpha、Monad Agent Runtime 仍是实验阶段，接口可能在版本之间变化。以及 `net: 'filtered'` 的出站控制在所有平台上都是应用层实现：直接打开裸 socket 的子进程可以绕过域名白名单，只有 macOS 例外。各平台实际强制了什么，见[沙箱后端](/usage/sandbox-backends)（英文）。
